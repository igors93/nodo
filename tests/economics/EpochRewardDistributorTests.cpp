...
work(const std::string &validator, std::uint64_t epoch, std::uint32_t weight,
     const std::string &evidenceHash,
     ValidationWorkResult result = ValidationWorkResult::ACCEPTED) {
  return ValidationWorkRecord(
      validator, epoch, ValidationWorkType::VALIDATE_BLOCK, result,
      "target-" + evidenceHash, evidenceHash, weight, kTimestamp);
}

ValidatorScoreRecord score(const std::string &validator, std::uint64_t epoch,
                           std::int32_t previousScore, std::int32_t newScore) {
  return ValidatorScoreRecord(validator, epoch, previousScore, newScore,
                              ValidatorScoreReason::CONSISTENT_VALIDATION,
                              "score-evidence-" + validator, kTimestamp);
}
...
void testDistributorCreatesGenesisRewardsFromAcceptedWork() {
  ...
  const std::vector<ValidationWorkRecord> workRecords = {
      ...
      work("nodo1validatorRejected", 1, 999, "evidence-rejected",
           ValidationWorkResult::REJECTED)};

  const std::vector<ValidatorScoreRecord> scoreRecords = {
      score("nodo1validatorA", 1, 50, 100), score("nodo1validatorB", 1, 50, 50),
      score("nodo1validatorIgnored", 2, 50, 100)};

  const EpochRewardDistribution distribution =
      EpochRewardDistributor::distribute(
          1, 10, 20, Amount::fromNodo(36500), Amount::fromNodo(10), 100, policy,
          workRecords, scoreRecords, "accepted-block-hash", kTimestamp);

  requireCondition(distribution.isValid(),
                   "Epoch reward distribution should be valid.");

  requireCondition(distribution.protectionEpoch().workDemandBasisPoints() ==
                       10000,
                   "Work demand should be fully satisfied.");

  requireCondition(distribution.protectionEpoch().securityEmission() ==
                       Amount::fromNodo(4),
                   "Security emission should be capped by policy.");

  requireCondition(distribution.protectionEpoch().rewardPool() ==
                       Amount::fromNodo(14),
                   "Reward pool should include fees plus security emission.");

  requireCondition(distribution.distributedSecurityEmission() ==
                       Amount::fromNodo(4),
                   "Distributor must mint only security emission, not fees.");

  requireCondition(distribution.validatorRewards().size() == 2U,
                   "Two validators should receive rewards.");

  requireCondition(
      distribution.validatorRewards()[0].validatorAddress() ==
          "nodo1validatorA",
  ...
  requireCondition(rewards.size() == 2U,
                   "Distribution should expose two GenesisReward records.");

  requireCondition(rewards[0].policyVersion() == policy.policyVersion(),
                   "GenesisReward policy version mismatch.");

  requireCondition(rewards[0].acceptedBlockHash() == "accepted-block-hash",
                   "GenesisReward accepted block hash mismatch.");

  requireCondition(rewards[0].isValid(),
...
void testDistributorUsesBootstrapCapWhenSupplyIsZero() {
  ...
  const EpochRewardDistribution distribution =
      EpochRewardDistributor::distribute(1, 0, 5, Amount::fromRawUnits(0),
                                         Amount::fromRawUnits(0), 100, policy,
                                         workRecords, scoreRecords,
                                         "bootstrap-block-hash", kTimestamp);

  requireCondition(distribution.protectionEpoch().emissionCap() ==
                       Amount::fromNodo(100),
  ...
  requireCondition(
      distribution.validatorRewards().front().rewardAmount() ==
          Amount::fromNodo(50),
...
void testDistributorRejectsDuplicateAcceptedEvidence() {
  ...
  try {
    (void)EpochRewardDistributor::distribute(
        1, 0, 10, Amount::fromNodo(36500), Amount::fromRawUnits(0), 100, policy,
        workRecords, scoreRecords, "accepted-block-hash", kTimestamp);
  } catch (const std::exception &) {
    rejected = true;
  }

  requireCondition(rejected, "Duplicate accepted evidence should be rejected.");
}
...
void testDistributorDoesNotRewardValidatorsWithoutScore() {
  ...
  const EpochRewardDistribution distribution =
      EpochRewardDistributor::distribute(
          1, 0, 10, Amount::fromNodo(36500), Amount::fromRawUnits(0), 100,
          policy, workRecords, scoreRecords, "accepted-block-hash", kTimestamp);

  requireCondition(distribution.validatorRewards().empty(),
                   "Validator without score should not receive minted reward.");

  requireCondition(distribution.distributedSecurityEmission().isZero(),
...
void testDistributorRejectsInvalidInputs() {
  ...
  try {
    (void)EpochRewardDistributor::distribute(
        1, 10, 9, Amount::fromNodo(36500), Amount::fromRawUnits(0), 100, policy,
        workRecords, scoreRecords, "accepted-block-hash", kTimestamp);
  } catch (const std::exception &) {
    rejected = true;
  }
  ...
  try {
    (void)EpochRewardDistributor::distribute(
        1, 0, 10, Amount::fromNodo(36500), Amount::fromRawUnits(0), 0, policy,
        workRecords, scoreRecords, "accepted-block-hash", kTimestamp);
  } catch (const std::exception &) {
    rejected = true;
  }

  requireCondition(rejected, "Zero target work should be rejected.");
}

void testDistributorCalculatesProportionalRewardsForParticipation() {
  const EpochEmissionPolicy policy =
      EpochEmissionPolicy::developmentDefaultPolicy();

  // 100 target work: A does 100 (100%), B does 50 (50%).
  const std::vector<ValidationWorkRecord> workRecords = {
      work("nodo1validatorA", 1, 100, "evidence-a"),
      work("nodo1validatorB", 1, 50, "evidence-b")};

  const std::vector<ValidatorScoreRecord> scoreRecords = {
      score("nodo1validatorA", 1, 100, 100),
      score("nodo1validatorB", 1, 100, 100)};

  const EpochRewardDistribution distribution =
      EpochRewardDistributor::distribute(
          1, 10, 20, Amount::fromNodo(36500), Amount::fromRawUnits(0),
          150, // total target work
          policy, workRecords, scoreRecords, "accepted-block-hash", kTimestamp);

  requireCondition(distribution.isValid(), "Distribution should be valid.");

  requireCondition(distribution.validatorRewards().size() == 2U,
                   "Two validators should receive rewards.");

  const Amount rewardA = distribution.validatorRewards()[0].rewardAmount();
  const Amount rewardB = distribution.validatorRewards()[1].rewardAmount();

  requireCondition(
      rewardA.rawUnits() == rewardB.rawUnits() * 2 ||
          rewardA.rawUnits() == rewardB.rawUnits() * 2 + 1,
      "Validator A (100% participation) should receive twice the reward of "
      "Validator B (50% participation) (allowing 1 unit precision rounding).");
}
...
int main() {
  try {
    ...
    testDistributorCalculatesProportionalRewardsForParticipation();

    std::cout << "Nodo epoch reward distributor tests passed.\n";
    return 0;
  } catch (const std::exception &error) {
    std::cerr << "Nodo epoch reward distributor tests failed: " << error.what()
              << "\n";
    return 1;
  }
}
