...
inline config::GenesisAccountConfig
fundedConsensusTestAccount(const crypto::KeyPair &userKey) {
  return config::GenesisAccountConfig(
      userKey.address().value(),
      utils::Amount::fromRawUnits(2'000'000'000'000LL), 0);
}

inline void admitConsensusTestTransfer(node::NodeRuntime &runtime,
  ...
  const core::Transaction transaction =
      core::TransactionBuilder::buildSignedTransfer(
          core::TransactionBuildRequest(
              "consensus-phase-recipient", utils::Amount::fromRawUnits(1'000),
              utils::Amount::fromRawUnits(100), nonce, transactionTimestamp),
  ...
  const auto admission = runtime.mutableMempool().admitTransaction(
      ...
      crypto::SecurityContext::USER_TRANSACTION, transactionTimestamp + 1);

  if (!admission.accepted()) {
    throw std::runtime_error("Consensus test transaction admission failed: " +
                             admission.reason());
  }
}
