...
std::string tamperFirstHexCharacter(const std::string &value) {
  if (value.empty()) {
    throw std::invalid_argument("Cannot tamper empty string.");
  }

  const std::string replacementPrefix = value.front() == '0' ? "1" : "0";
...
std::string readTextFile(const std::filesystem::path &filePath) {
  ...
  requireCondition(input.is_open(), "Failed to open file for test reading: " +
                                        filePath.string());

  std::ostringstream buffer;
  buffer << input.rdbuf();

  requireCondition(input.good() || input.eof(),
                   "Failed while reading test file: " + filePath.string());

  return buffer.str();
}

std::filesystem::path storageRootPath(const std::string &name) {
  return std::filesystem::path("build") / "tests" / "storage" / name;
}
...
Blockchain buildReferenceBlockchain() {
  ...
  MintRecord genesisMint("mint_storage_integration_igor_001",
                         "auth_storage_integration_001", "igor",
                         Amount::fromNodo(1000), MintReason::GENESIS_ALLOCATION,
                         0, 0, "GENESIS", kBaseTimestamp);

  SignatureBundle genesisSignatureBundle = SignatureBundle::createSignature(
      genesisMint.serialize(), mintKeyPair.publicKey(),
      mintKeyPair.privateKeyForSigningOnly(), kBaseTimestamp + 1, mintProvider,
      SigningDomain::MINT_AUTHORIZATION);

  requireCondition(genesisSignatureBundle.isValidForPolicy(
                       cryptoPolicy, SecurityContext::MINT_OPERATION),
                   "Genesis signature policy check failed.");

  LedgerRecord genesisLedgerRecord =
      LedgerRecord::fromMintRecord(genesisMint, kBaseTimestamp + 2);

  Block genesisBlock = Block::createGenesisBlock(
      std::vector<LedgerRecord>{genesisLedgerRecord}, kBaseTimestamp + 3);

  blockchain.addGenesisBlock(genesisBlock);

  Transaction transferTransaction(
      TransactionType::TRANSFER, "igor", "ana", Amount::fromNodo(25),
      Amount::fromRawUnits(100000), 1, kBaseTimestamp + 4);

  SignatureBundle transferSignatureBundle = SignatureBundle::createSignature(
      transferTransaction.signingPayload(), igorKeyPair.publicKey(),
      igorKeyPair.privateKeyForSigningOnly(), kBaseTimestamp + 5, userProvider,
      SigningDomain::USER_TRANSACTION);

  transferTransaction.attachSignatureBundle(transferSignatureBundle);

  requireCondition(transferTransaction.isStructurallyValid(
                       cryptoPolicy, SecurityContext::USER_TRANSACTION),
                   "Transfer transaction validation failed.");

  LedgerRecord transferLedgerRecord = LedgerRecord::fromTransaction(
      transferTransaction, cryptoPolicy, SecurityContext::USER_TRANSACTION,
      kBaseTimestamp + 6);

  Block transferBlock(1, blockchain.latestBlock().hash(),
                      std::vector<LedgerRecord>{transferLedgerRecord},
                      kBaseTimestamp + 7);

  blockchain.addBlock(transferBlock);

  requireCondition(blockchain.isValid(false),
                   "Reference Blockchain is invalid.");

  return blockchain;
}

void writeCompleteStorage(const Blockchain &blockchain,
  ...
  requireCondition(blockFileStore.storedBlockFileCount() == blockchain.size(),
                   "Stored block file count does not match Blockchain size.");

  for (const auto &block : blockchain.blocks()) {
    requireCondition(blockFileStore.verifyStoredBlock(block),
                     "Stored block snapshot verification failed.");
  }

  ChainManifest manifest =
      ChainManifest::fromBlockchain(blockchain, kBaseTimestamp + 20);
  ...
  requireCondition(loadedManifest.matchesBlockchain(blockchain),
                   "Stored ChainManifest does not match Blockchain.");

  ChainManifest codecRebuiltManifest =
      ChainManifestCodec::deserialize(loadedManifest.serialize());
  ...
  const std::string tamperedManifestHash =
      tamperFirstHexCharacter(loadedManifest.manifestHash());

  tamperedManifest.replace(tamperedManifest.find(loadedManifest.manifestHash()),
  ...
  requireCondition(manifestTamperRejected,
                   "ChainManifestCodec accepted a tampered manifest hash.");

  BlockStorageIndex index = BlockStorageIndex::fromBlockchainAndManifest(
      blockchain, loadedManifest, kBaseTimestamp + 21);
  ...
  requireCondition(
      loadedIndex.matchesBlockchainAndManifest(blockchain, loadedManifest),
      "Stored BlockStorageIndex does not match Blockchain and ChainManifest.");

  BlockStorageIndex codecRebuiltIndex =
      BlockStorageIndexCodec::deserialize(loadedIndex.serialize());
  ...
  BlockIndexEntry codecRebuiltEntry = BlockStorageIndexCodec::deserializeEntry(
      loadedIndex.entries().front().serialize());

  requireCondition(codecRebuiltEntry.serialize() ==
                       loadedIndex.entries().front().serialize(),
                   "BlockStorageIndexCodec round-trip changed first index "
                   "entry serialization.");

  std::string tamperedIndex = loadedIndex.serialize();

  const std::string tamperedIndexHash =
      tamperFirstHexCharacter(loadedIndex.indexHash());

  tamperedIndex.replace(tamperedIndex.find(loadedIndex.indexHash()),
                        loadedIndex.indexHash().size(), tamperedIndexHash);
  ...
  requireCondition(indexTamperRejected,
  ...
  tamperedEntry.replace(tamperedEntry.find(originalEntryFileName),
  ...
  const std::filesystem::path firstBlockSnapshotPath =
      rootPath / "blocks" / loadedIndex.entries().front().fileName();

  const std::string firstSerializedBlock = readTextFile(firstBlockSnapshotPath);

  const BlockSnapshotHeader codecRebuiltHeader =
      BlockSnapshotHeaderCodec::deserializeFromSerializedBlock(
          firstSerializedBlock);

  requireCondition(codecRebuiltHeader.isValid(),
                   "BlockSnapshotHeaderCodec produced an invalid header.");

  requireCondition(
      codecRebuiltHeader.blockIndex() ==
          loadedIndex.entries().front().blockIndex(),
      "BlockSnapshotHeaderCodec produced an unexpected block index.");

  requireCondition(
      codecRebuiltHeader.blockHash() ==
          loadedIndex.entries().front().blockHash(),
  ...
  requireCondition(recordCountPosition != std::string::npos,
                   "Could not find recordCount marker for header tamper test.");

  tamperedSerializedBlock.replace(recordCountPosition, recordCountMarker.size(),
                                  "recordCount=999");
...
void assertLoadedBlockchainMatchesReference(
  ...
  requireCondition(storageReadReport.success(),
  ...
  requireCondition(loadReport.success(), "BlockchainLoader audit failed: " +
                                             loadReport.failureReason());

  Blockchain loadedBlockchain =
      BlockchainLoader::loadFromStorageRoot(rootPath.string());

  requireCondition(loadedBlockchain.isValid(false),
  ...
  requireCondition(loadedPublicState.isSupplyAuditable(),
                   "Loaded public State failed supply audit.");

  requireCondition(loadedPublicState.balanceOf("igor").toString() ==
                       "974.99900000 NODO",
                   "Loaded Igor balance is incorrect.");

  requireCondition(loadedPublicState.balanceOf("ana").toString() ==
                       "25.00000000 NODO",
...
void assertTamperedStorageIsRejected(const Blockchain &referenceBlockchain,
                                     const std::filesystem::path &rootPath) {
  ...
  requireCondition(!index.entries().empty(),
                   "BlockStorageIndex has no entries for tamper test.");

  const std::filesystem::path firstBlockPath =
      rootPath / "blocks" / index.entries().front().fileName();

  {
    std::ofstream output(firstBlockPath,
                         std::ios::out | std::ios::binary | std::ios::app);

    requireCondition(output.is_open(),
  ...
  requireCondition(!tamperedReport.success(),
                   "Tampered storage was not rejected.");
}

void testBlockchainStorageIntegration() {
  const Blockchain referenceBlockchain = buildReferenceBlockchain();

  const std::filesystem::path validStorageRoot = storageRootPath("valid_chain");

  writeCompleteStorage(referenceBlockchain, validStorageRoot);

  assertLoadedBlockchainMatchesReference(referenceBlockchain, validStorageRoot);

  const std::filesystem::path tamperedStorageRoot =
      storageRootPath("tampered_chain");

  assertTamperedStorageIsRejected(referenceBlockchain, tamperedStorageRoot);
