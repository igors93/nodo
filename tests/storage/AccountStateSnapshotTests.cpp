...
std::filesystem::path tempDir() {
  const std::filesystem::path dir =
      std::filesystem::temp_directory_path() / "nodo_gap6_snapshot_test";
...
void testSnapshotRoundTrip() {
  ...
  const AccountStateSnapshot snap("genesis-id-abc", 100, "block-hash-100",
                                  view);
  ...
  require(loaded->height() == 100, "Loaded height must match saved height.");
...
void testNegativeBalanceSnapshotIsRejectedWithoutThrowing() {
  const std::filesystem::path dir = tempDir();
  AccountStateSnapshotStore store(dir);
  storage::AtomicFile::writeTextFile(store.snapshotPath(),
...
void testPartialReplayMatchesFullReplay() {
  const config::GenesisConfig genesis(
      ...
      {config::BootstrapValidatorConfig(
          crypto::KeyPair::createDeterministicBls12381KeyPair("snap-val")
              .publicKey(),
          1, 1, "snap-val-meta")},
      {config::GenesisAccountConfig("gap6-account",
                                    utils::Amount::fromRawUnits(5000), 0)},
  ...
  const core::AccountStateView fullReplay =
      node::RuntimeAccountStateBuilder::accountStateViewAtTip(genesis,
                                                              blockchain, 0);
...
int main() {
  try {
    ...
    std::cerr << "Nodo Gap6 account-state-snapshot tests failed: " << e.what()
              << "\n";
    return 1;
  }
}
