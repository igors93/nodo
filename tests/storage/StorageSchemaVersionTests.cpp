...
void testWritesAndLoadsCurrentNodeDataDirectorySchema() {
  const std::filesystem::path path = tempPath("current");
...
void testRejectsMissingSchemaVersionFile() {
  ...
  requireCondition(!validation.accepted() &&
                       validation.status() ==
...
void testRejectsUnknownFutureSchemaVersion() {
  const std::filesystem::path path = tempPath("future");

  clean(path);
  std::filesystem::create_directories(path);

  writeFile(StorageSchemaVersionFile::pathForRoot(path),
...
int main() {
  try {
    ...
    std::cerr << "Nodo storage schema version tests failed: " << error.what()
              << "\n";
    return 1;
  }
}
