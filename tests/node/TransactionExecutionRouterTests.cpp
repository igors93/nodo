...
public:
  ...
  RecordingDomainExecutor() {
    m_domains = {{"burns", "burns"},       {"governance", "governance"},
...
private:
  ...
  core::TransactionDomainExecutionResult
  record(const std::string &handler, const core::AccountStateView &accounts) {
    lastHandler = handler;
    return reject ? core::TransactionDomainExecutionResult::rejected(
                        "REJECTED", "domain rejected")
                  : core::TransactionDomainExecutionResult::accepted(accounts,
                                                                     m_domains);
  }
};
...
core::Transaction transactionFor(core::TransactionType type) {
  ...
  const utils::Amount amount =
      policy.amountRule == core::TransactionAmountRule::ZERO
          ? utils::Amount()
...
void testEveryRegisteredTypeRoutesAndProducesTypedReceipt() {
  require(core::TransactionTypePolicyRegistry::policies().size() ==
  ...
  for (const auto type : core::TransactionTypePolicyRegistry::allTypes()) {
    ...
    const core::TransactionExecutionResult result =
        core::TransactionExecutionRouter::execute(
            tx, core::TransactionExecutionContext(before, 100, 1900000100, true,
                                                  false, true, "", nullptr,
                                                  &domain));
    require(result.success(),
            "Registered transaction type must route successfully: " +
...
void testDebitAndCreditRulesAreCentralized() {
  ...
  const auto transfer = core::TransactionExecutionRouter::execute(
      ...
      core::TransactionExecutionContext(before, 1, 1900000000, true, false,
                                        true, "", nullptr, &domain));
  require(
      transfer.accounts().accountOrDefault("sender-1").balance().rawUnits() ==
          1'000'000'000 - 105,
      "Transfer must debit amount plus fee.");

  const auto withdraw = core::TransactionExecutionRouter::execute(
      transactionFor(core::TransactionType::STAKE_WITHDRAW),
      core::TransactionExecutionContext(before, 100, 1900000000, true, false,
                                        true, "", nullptr, &domain));
  require(
      withdraw.accounts().accountOrDefault("sender-1").balance().rawUnits() ==
          1'000'000'000 + 95,
      "Stake withdrawal must credit amount and debit only fee.");
}
...
void testDomainRejectionIsAtomic() {
  ...
  require(before.accountOrDefault("sender-1").balance().rawUnits() ==
                  1'000'000'000 &&
...
void testProtocolOnlyConceptsAreNotTransactionTypes() {
  for (const std::string oldType : {"MINT_REWARD", "PENALTY", "VALIDATOR_VOTE",
    ...
    bool rejected = false;
    try {
      (void)core::transactionTypeFromString(oldType);
...
void testEveryAdmittedTypeHasABlockPreviewRoute() {
  ...
  for (const auto type : core::TransactionTypePolicyRegistry::allTypes()) {
    ...
    transaction.attachSignatureBundle(crypto::SignatureBundle::createSignature(
        transaction.signingPayload(), key.publicKey(),
        key.privateKeyForSigningOnly(), 1900000000, provider,
  ...
  require(preview.accepted() &&
          ...
          "Every admitted user type must have a deterministic preview route: " +
              preview.reason());
}
