...
KeyPair userKeyPair() {
  return KeyPair::createDeterministicEd25519KeyPair("protocol-replay-user");
}
...
GenesisConfig genesisConfig() {
  return GenesisConfig(
      ...
      {BootstrapValidatorConfig(validatorKeyPair().publicKey(), 1, 1,
                                "protocol-replay-validator")},
      {GenesisAccountConfig(userKeyPair().address().value(),
                            Amount::fromRawUnits(1'000'000'000'000LL), 0)},
      "protocol-replay-genesis");
}

PeerInfo localPeer() {
  return PeerInfo("protocol-replay-peer", "127.0.0.1:9903", "nodo/0.1", 0,
                  kTimestamp);
}
...
std::int64_t minimumFeeRawUnits(const GenesisConfig &genesis) {
  const std::uint64_t raw = genesis.networkParameters().minimumFeeRawUnits();
  require(raw <= static_cast<std::uint64_t>(
...
void testReplayAdvancesAccountsAndDomainsTogether() {
  ...
  const Transaction tx = TransactionBuilder::buildSignedStakeDeposit(
      TransactionBuildRequest(validatorAddress, Amount::fromRawUnits(kStake),
                              Amount::fromRawUnits(kFee), 1, kTimestamp + 10),
      userSigner(), kChainId);

  require(runtime.mutableMempool()
              .admitTransaction(tx, CryptoPolicy::developmentPolicy(),
  ...
  require(
      replay.execution.validators.serialize() ==
          runtime.validatorRegistry().serialize(),
      "Unified replay validator registry must match the finalized runtime.");
  require(replay.execution.supply == runtime.supplyState().latestSupply(),
          "Unified replay supply must match the finalized runtime supply.");
  require(RuntimeStateVerifier::calculateLatestStateRoot(
              genesis, runtime.blockchain()) == replay.stateRoot,
          "RuntimeStateVerifier must verify the full protocol state root, not "
          "an account-only root.");

  const std::string accountOnlyRoot =
      nodo::core::StateRootCalculator::calculateAccountStateRoot(
          replay.accounts);
  require(
...
void testInitialReplayCommitsGenesisDomains() {
  ...
  require(replay.execution.validators.size() == 1,
          "Genesis replay must seed the bootstrap validator domain.");
  require(replay.execution.staking
                  .accountOrDefault(validatorKeyPair().address().value())
                  .bondedAmount()
                  .rawUnits() == kGenesisStake,
