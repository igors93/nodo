...
void testReceivePathDropsInvalidMessages() {
  ...
  auto sendEnvelope = [&](const p2p::NetworkEnvelope &env) {
    const std::string serialized =
        env.serialize(); // wait, serialization is done via ProtocolMessageCodec
  ...
  // Let's just use orchestrator.gossipInjectLoopback to test oversize, because
  // we don't have the full serialization stack handy here. PING max size is
  // 1024. Send 2048 bytes.
  ...
  requireCondition(pingMsgs.empty(),
                   "Oversize PING message reached handler (spy check failed).");

  // Test oversize block sync request (limit 4096)
  orchestrator.gossipInjectLoopback(p2p::NetworkMessageType::BLOCK_SYNC_REQUEST,
                                    std::string(5000, 'y'), now);
  orchestrator.tick(now);
