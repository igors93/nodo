...
std::array<NodeSpec, 3> makeNodeSpecs() {
  return {NodeSpec{"e2e-node-a", "127.0.0.1:32101",
                   crypto::KeyPair::createDeterministicBls12381KeyPair(
                       "three-node-e2e-validator-a")},
          NodeSpec{"e2e-node-b", "127.0.0.1:32102",
                   crypto::KeyPair::createDeterministicBls12381KeyPair(
                       "three-node-e2e-validator-b")},
          NodeSpec{"e2e-node-c", "127.0.0.1:32103",
                   crypto::KeyPair::createDeterministicBls12381KeyPair(
...
config::GenesisConfig makeGenesis(const std::array<NodeSpec, 3> &specs) {
  ...
  return config::GenesisConfig(
      config::NetworkParameters::developmentLocal(), kGenesisTimestamp,
      validators,
...
p2p::PeerInfo peerInfo(const NodeSpec &spec) {
  return p2p::PeerInfo(spec.nodeId, spec.endpoint, kProtocolVersion, 0,
                       kGenesisTimestamp);
}
...
class TestNode {
public:
  TestNode(const NodeSpec &spec, const config::GenesisConfig &genesis,
           const std::shared_ptr<p2p::LoopbackTransportBus> &bus,
           std::size_t index)
      : spec(spec), directory(testRoot() / ("node-" + std::to_string(index))),
        runtime(startRuntime(genesis, spec)), transport(bus),
...
void connectNodes(TestNode &left, std::size_t leftIndex, TestNode &right,
                  std::size_t rightIndex) {
  require(left.mesh.registerPeer(peerMetadata(right, rightIndex)).success(),
          "Unable to register " + right.spec.nodeId + " on " +
              left.spec.nodeId);
  require(right.mesh.registerPeer(peerMetadata(left, leftIndex)).success(),
          "Unable to register " + left.spec.nodeId + " on " +
              right.spec.nodeId);
  require(left.mesh.connectPeer(right.spec.nodeId).success(),
          "Unable to connect " + left.spec.nodeId + " to " + right.spec.nodeId);
  require(right.mesh.connectPeer(left.spec.nodeId).success(),
...
core::Transaction makeTransaction(const config::GenesisConfig &genesis) {
  ...
  return core::TransactionBuilder::buildSignedTransfer(
      core::TransactionBuildRequest(
          "three-node-e2e-recipient", utils::Amount::fromRawUnits(1'000),
          utils::Amount::fromRawUnits(100), 1, kTransactionTimestamp),
      crypto::Signer(userKey(), provider),
      genesis.networkParameters().chainId());
}

void submitAndPropagateTransaction(TestNode &submitter, TestNode &receiver,
  ...
  const auto admission = submitter.runtime.mutableMempool().admitTransaction(
      transaction, policy, crypto::SecurityContext::USER_TRANSACTION,
      kTransactionTimestamp + 1);
  require(admission.accepted(),
          "Submitting node rejected the test transaction.");

  const std::string payload = node::PersistentMempoolStore::serializeForGossip(
      transaction, userKey().publicKey(), kTransactionTimestamp + 1);
  require(!payload.empty(), "Transaction gossip serialization failed.");
  require(submitter.mesh
                  .broadcast(p2p::NetworkMessageType::TRANSACTION_GOSSIP,
                             payload, kTransactionTimestamp + 2)
                  .acceptedCount() == 1,
  ...
  require(messages.size() == 1,
          "Online peer did not receive transaction gossip.");
  const auto decoded = node::PersistentMempoolStore::deserializeGossip(
      messages.front().payload(), policy,
      crypto::SecurityContext::USER_TRANSACTION,
      genesis.networkParameters().chainId());
  require(decoded.has_value() &&
              receiver.runtime.mutableMempool()
                  .admitTransaction(decoded->transaction, policy,
                                    crypto::SecurityContext::USER_TRANSACTION,
                                    decoded->acceptedAt)
                  .success(),
...
void configureConsensus(TestNode &node,
...
void finalizeWithTwoOfThreeValidators(TestNode &first, TestNode &second) {
  for (std::int64_t step = 0; step < 40; ++step) {
    ...
    require(!firstResult.hasError(), first.spec.nodeId + " consensus failed: " +
                                         firstResult.errorMessage);
    require(!secondResult.hasError(),
            second.spec.nodeId +
                " consensus failed: " + secondResult.errorMessage);
...
std::vector<unsigned char> unwrapCanonical(const std::string &payload) {
  ...
  require(payload.rfind(prefix, 0) == 0,
          "Sync message does not use canonical protocol framing.");
  const std::string hex = payload.substr(prefix.size());
  require(crypto::isHexString(hex),
          "Sync message contains invalid hex framing.");
  return crypto::hexDecode(hex);
}

node::PersistentBlockSyncBatch
buildSyncBatch(const TestNode &source,
               const node::NetworkBlockSyncRequest &request, std::int64_t now) {
  require(request.isValid(), "Source received an invalid sync request.");
  require(request.locator().fromHeight() == 1,
          "The lagging node must request the first finalized height.");
  require(std::find(request.locator().knownAncestorHashes().begin(),
  ...
  const node::FinalizedBlockArtifact artifact =
      node::FinalizedBlockArtifactCodec::readBlockArtifactFile(
          node::FinalizedBlockStore::blockFilePath(source.directory,
                                                   block.index()));
  require(artifact.block().hash() == block.hash(),
  ...
  const node::PersistentBlockSyncItem item(
      ...
      block.stateRoot(), now, artifact.finalizedRecord().serialize());
  return node::PersistentBlockSyncBatch(source.spec.nodeId, block.index(),
                                        block.index(), {item}, now);
}

void synchronizeLaggingNode(TestNode &restartedSource, TestNode &lagging,
                            const config::GenesisConfig &genesis,
                            std::int64_t now) {
  ...
  const node::PersistentSyncCheckpoint checkpoint =
      node::PersistentSyncCheckpoint::genesis(laggingTip.hash(),
                                              "genesis-state-root", now);
  const node::ChainStatusMessage remoteStatus(
      genesis.networkParameters().networkName(),
      genesis.networkParameters().chainId(), kProtocolVersion,
      sourceTip.index(), sourceTip.hash(), sourceTip.index(), sourceTip.hash());
  const node::PersistentSyncPlan plan =
      node::PersistentBlockStateSyncPlanner::planFromRemoteStatus(
          checkpoint, remoteStatus, lagging.spec.nodeId,
          restartedSource.spec.nodeId,
          node::NODO_PERSISTENT_SYNC_MAX_BLOCK_BATCH, now);
  require(plan.requestBlocks(),
          "Lagging node did not plan a block sync request.");

  require(lagging.mesh
                  .sendTo(restartedSource.spec.nodeId,
                          p2p::NetworkMessageType::BLOCK_SYNC_REQUEST,
                          wrapCanonical(serialization::ProtocolMessageCodec::
                                            encodeNetworkBlockSyncRequest(
  ...
  require(requests.size() == 1,
          "Restarted source did not receive the sync request.");
  ...
  require(decodedRequest.requesterNodeId() == requests.front().senderNodeId(),
  ...
  require(
      restartedSource.mesh
              .sendTo(
                  lagging.spec.nodeId,
                  p2p::NetworkMessageType::BLOCK_SYNC_RESPONSE,
                  wrapCanonical(
                      node::PersistentBlockStateSyncCodec::encodeBlockSyncBatch(
                          response)),
                  now + 2)
              .acceptedCount() == 1,
  ...
  const std::vector<p2p::NetworkEnvelope> responses =
      lagging.mesh.drainInbox(p2p::NetworkMessageType::BLOCK_SYNC_RESPONSE);
  require(responses.size() == 1,
          "Lagging node did not receive the sync response.");
  ...
  require(decodedResponse.sourcePeerId() == responses.front().senderNodeId(),
  ...
  const node::PersistentSyncApplyResult applied =
      node::PersistentBlockStateSyncApplier::importFinalizedBatch(
          checkpoint, decodedResponse, lagging.runtime, lagging.directory,
          &cpStore, now + 4);
  require(applied.applied(), "Lagging node rejected sync: " + applied.reason());
  require(applied.checkpoint().has_value() &&
...
std::size_t proposerIndex(const std::array<std::unique_ptr<TestNode>, 3> &nodes,
                          const config::GenesisConfig &genesis) {
  ...
  throw std::runtime_error(
      "Scheduled proposer is absent from the three-node fixture.");
}
...
void testThreeNodeProtocolJourney() {
  ...
  for (std::size_t index = 0; index < nodes.size(); ++index) {
    ...
    const node::NodeDataDirectoryInitResult initialized =
        node::NodeDataDirectory::initialize(
            directory, genesis, peerInfo(specs[index]), kGenesisTimestamp + 1);
    require(initialized.initialized(),
            "Data directory initialization failed for " + specs[index].nodeId +
                ": " + initialized.reason());
    nodes[index] =
        std::make_unique<TestNode>(specs[index], genesis, bus, index);
  }
  ...
  require(lagging.runtime.mempool().empty(),
  ...
  // Manually propose the block for the producer since NodeDaemon isn't used
  const std::uint64_t height = 1;
  const std::uint64_t round = 1;
  const std::int64_t now = kTransactionTimestamp + 15;
  ...
  require(candidate.produced(),
          "Test producer failed to produce a candidate block.");

  const consensus::BlockProposalResult proposal =
      consensus::BlockProposalPhase::propose(
          candidate.block(), producer.validatorSigner->address(), round, now,
          *producer.validatorSigner, producer.mesh, validatorProvider);
  require(proposal.proposed(),
          "Test producer failed to broadcast its proposal.");

  // Loopback for the producer to vote on its own proposal
  ...
  const node::RuntimeStateLoadResult restarted =
      node::RuntimeStateLoader::loadFromDataDirectory(
          producer.directory, genesis, peerInfo(producer.spec));
  require(restarted.loaded(),
          "Finalized node failed to restart: " + restarted.reason());
  producer.runtime = restarted.runtime();
  require(producer.runtime.blockchain().size() == 2 &&
  ...
  synchronizeLaggingNode(producer, lagging, genesis,
                         kTransactionTimestamp + 100);

  require(lagging.runtime.blockchain().latestBlock().hash() ==
              producer.runtime.blockchain().latestBlock().hash(),
          "Lagging node did not converge to the restarted source tip.");
  require(lagging.runtime.finalizationRegistry().hasFinalizedHeight(1),
          "Lagging node did not import the block finalization proof.");
  require(lagging.runtime.supplyState().serialize() ==
  ...
  const core::AccountStateView producerAccounts =
      node::RuntimeAccountStateBuilder::accountStateViewAtTip(
          genesis, producer.runtime.blockchain(), minimumFee);
  const core::AccountStateView laggingAccounts =
      node::RuntimeAccountStateBuilder::accountStateViewAtTip(
          genesis, lagging.runtime.blockchain(), minimumFee);
  require(laggingAccounts.serialize() == producerAccounts.serialize(),
          "Lagging node account balances or nonces differ after sync.");
  require(
      laggingAccounts.accountOrDefault(userKey().address().value()).nonce() ==
          1,
  ...
  const node::RuntimeStateLoadResult laggingRestart =
      node::RuntimeStateLoader::loadFromDataDirectory(
          lagging.directory, genesis, peerInfo(lagging.spec));
  require(laggingRestart.loaded(),
          "Synchronized node failed to restart: " + laggingRestart.reason());
  require(laggingRestart.runtime().blockchain().latestBlock().hash() ==
              producer.runtime.blockchain().latestBlock().hash(),
          "Synchronized tip was not durable across restart.");
  require(laggingRestart.runtime().supplyState().serialize() ==
              producer.runtime.supplyState().serialize(),
...
int main() {
  try {
    ...
    std::cerr << "Three-node protocol end-to-end test FAILED: " << error.what()
              << '\n';
    return 1;
  }
}
