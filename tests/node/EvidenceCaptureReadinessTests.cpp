...
// Minimal test infrastructure — we only need to exercise the withEvidenceCaptureHealth() path.
...
void testReadinessWarnsWhenCaptureUnhealthy() {
    ...
    ReadinessContextBuilder builder(testDirConfig(), localnetParams(), emptyKeyMetadata());
