...
int main() {
  ...
  PersistentSyncCheckpoint checkpoint = PersistentSyncCheckpoint::genesis(
      "genesis-hash", "genesis-state-root", now);

  ChainStatusMessage sameHeight("localnet", "chain-a", "proto-v1", 0,
                                "genesis-hash", 0, "genesis-hash");

  auto noSync = PersistentBlockStateSyncPlanner::planFromRemoteStatus(
      checkpoint, sameHeight, "node-a", "node-b", 32, now + 1);

  assert(noSync.notRequired());

  ChainStatusMessage unfinalizedAhead("localnet", "chain-a", "proto-v1", 10,
                                      "block-10", 0, "genesis-hash");
  assert(PersistentBlockStateSyncPlanner::planFromRemoteStatus(
             checkpoint, unfinalizedAhead, "node-a", "node-b", 32, now + 1)
             .notRequired());

  ChainStatusMessage ahead("localnet", "chain-a", "proto-v1", 10, "block-10",
                           10, "block-10");

  auto blockPlan = PersistentBlockStateSyncPlanner::planFromRemoteStatus(
      checkpoint, ahead, "node-a", "node-b", 4, now + 2);
  ...
  assert(blockPlan.blockRequest()->locator().knownAncestorHashes().front() ==
         "genesis-hash");
  ...
  ChainStatusMessage nearThreshold(
      "localnet", "chain-a", "proto-v1",
      PersistentSyncPlan::SNAPSHOT_GAP_THRESHOLD - 1, "block-499",
      PersistentSyncPlan::SNAPSHOT_GAP_THRESHOLD - 1, "block-499");

  auto nearThresholdPlan =
      PersistentBlockStateSyncPlanner::planFromRemoteStatus(
          checkpoint, nearThreshold, "node-a", "node-b", 512, now + 3);

  assert(nearThresholdPlan.requestBlocks());
  assert(!nearThresholdPlan.requestSnapshot());

  // Large gaps also use REQUEST_BLOCKS; batch size is capped by
  // maxBlocksPerRequest.
  ChainStatusMessage farAhead("localnet", "chain-a", "proto-v1", 10000,
                              "block-10000", 10000, "block-10000");

  auto farAheadPlan = PersistentBlockStateSyncPlanner::planFromRemoteStatus(
      checkpoint, farAhead, "node-a", "node-b", 512, now + 4);
  ...
  const PersistentSnapshotSyncManifest original(
      "node-origin", 43200, "epoch1-block-hash", "epoch1-state-root",
      "epoch1-snapshot-digest", now + 10);
