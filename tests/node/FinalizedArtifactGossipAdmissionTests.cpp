...
p2p::PeerInfo localPeer() {
  return p2p::PeerInfo("artifact-receiver", "127.0.0.1:39101", kProtocolVersion,
                       0, kGenesisTimestamp);
}
...
RuntimeFixture runtimeWithCanonicalBlock(const config::GenesisConfig &genesis) {
  ...
  const economics::ValidationWorkRecord work(
      validatorKey().address().value(), 1,
      economics::ValidationWorkType::VALIDATE_BLOCK,
      economics::ValidationWorkResult::ACCEPTED,
      "artifact-gossip-canonical-target", "artifact-gossip-canonical-evidence",
      1, kBlockTimestamp - 1);
  core::Block canonicalBlock(
      1, runtime.blockchain().latestBlock().hash(),
      {core::LedgerRecord::fromValidationWorkRecord(work, kBlockTimestamp - 1)},
      kBlockTimestamp);
  runtime.mutableBlockchain().addBlock(canonicalBlock);
  return RuntimeFixture{std::move(runtime), std::move(canonicalBlock)};
}

consensus::FinalizedBlockRecord
finalizedRecordFor(const core::Block &block,
  ...
  const consensus::ValidatorVoteRecord vote =
      consensus::ValidatorVoteRecord::createVote(
          key.address().value(), key.publicKey(),
          key.privateKeyForSigningOnly(), block.index(), block.hash(),
          block.previousHash(), 1, consensus::ValidatorVoteDecision::PRECOMMIT,
          "finalized-artifact-gossip-test", kFinalizedAt - 1, provider);
  const consensus::QuorumCertificateBuildResult qc =
      consensus::QuorumCertificateBuilder::buildFromVotes(
          block.index(), block.hash(), block.previousHash(), 1, {vote},
          validators, crypto::CryptoPolicy::developmentPolicy(), provider);
  if (!qc.certified()) {
    throw std::runtime_error(qc.reason());
  }
  return consensus::FinalizedBlockRecord(block.index(), block.hash(),
                                         block.previousHash(), 1, kFinalizedAt,
                                         qc.certificate());
}

p2p::GossipMeshConfig meshConfig(const std::string &nodeId,
                                 const config::GenesisConfig &genesis) {
  return p2p::GossipMeshConfig(
      nodeId, genesis.networkParameters().networkName(),
      genesis.networkParameters().chainId(), kProtocolVersion,
      genesis.deterministicId(), 60, 4, 100, 50);
}
...
p2p::PeerMetadata peer(const std::string &nodeId, std::uint16_t port) {
  return p2p::PeerMetadata(nodeId, p2p::PeerEndpoint("127.0.0.1", port),
                           "fingerprint-" + nodeId, kGenesisTimestamp,
                           kGenesisTimestamp, 0, false);
}

p2p::NetworkEnvelope deliverThroughGossip(p2p::GossipMesh &sender,
                                          p2p::GossipMesh &receiver,
                                          const std::string &payload,
                                          std::int64_t now) {
  assert(sender
             .broadcast(p2p::NetworkMessageType::FINALIZED_BLOCK_ARTIFACT,
                        payload, now)
             .acceptedCount() == 1);
  assert(sender.flushOutbound(now).acceptedCount() == 1);
  assert(receiver.receiveAvailable(now).acceptedCount() == 1);

  const std::vector<p2p::NetworkEnvelope> inbox =
      receiver.drainInbox(p2p::NetworkMessageType::FINALIZED_BLOCK_ARTIFACT);
  assert(inbox.size() == 1);
  return inbox.front();
}
...
std::string tamperSignature(std::string payload) {
  ...
  payload[signaturePosition] = payload[signaturePosition] == '0' ? '1' : '0';
  return payload;
}
...
void testMalformedAndTamperedArtifactsAreRejectedThroughGossipInbox() {
  ...
  const p2p::NetworkEnvelope malformed = deliverThroughGossip(
      sender, receiver, "not-a-finalized-block-record", kFinalizedAt);
  const auto malformedResult = node::FinalizedArtifactGossipAdmission::admit(
      malformed, runtime, policy, provider, store);
  assert(malformedResult.status() ==
  ...
  const p2p::NetworkEnvelope mismatched = deliverThroughGossip(
      sender, receiver, differentRecord.serialize(), kFinalizedAt + 1);
  const auto mismatchResult = node::FinalizedArtifactGossipAdmission::admit(
      mismatched, runtime, policy, provider, store);
  assert(mismatchResult.status() ==
         node::FinalizedArtifactGossipAdmissionStatus::BLOCK_MISMATCH);
  assert(runtime.finalizationRegistry().size() == 0);
  assert(!store.load(canonicalBlock.index()).has_value());

  const p2p::NetworkEnvelope tampered = deliverThroughGossip(
      sender, receiver, tamperSignature(validRecord.serialize()),
      kFinalizedAt + 2);
  const auto tamperedResult = node::FinalizedArtifactGossipAdmission::admit(
      tampered, runtime, policy, provider, store);
  assert(
  ...
  const auto validResult = node::FinalizedArtifactGossipAdmission::admit(
      valid, runtime, policy, provider, store);
  ...
  const p2p::NetworkEnvelope duplicate = deliverThroughGossip(
      sender, receiver, validRecord.serialize(), kFinalizedAt + 4);
  const auto duplicateResult = node::FinalizedArtifactGossipAdmission::admit(
