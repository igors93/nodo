...
evidenceFor(const crypto::KeyPair &key,
  ...
  const auto makeVote = [&](const std::string &blockHash) {
    return consensus::ValidatorVoteRecord::createVote(
        ...
        consensus::ValidatorVoteDecision::PRECOMMIT, "audit-test",
...
void registerValidator(const crypto::KeyPair &key,
                       core::ValidatorRegistry &validators,
                       node::StakingRegistry &staking) {
  require(validators
              .registerValidator(core::ValidatorRegistrationRecord(
                  key.address().value(), key.publicKey(), 1,
                  "finalized-slashing-audit-validator", kTimestamp))
              .success(),
...
void testFinalizedEvidenceMustHavePenaltyAndDomainEffects() {
  ...
  const core::LedgerRecord record =
      node::CanonicalSlashingTransition::buildEvidenceRecord(evidence,
                                                             kTimestamp + 3);
  const core::Block block(2, "previous-finalized-block", {record},
  ...
  node::CanonicalSlashingTransition::applyEvidenceRecords(
      {record}, block.index(), block.timestamp(), history,
  ...
  const node::FinalizedSlashingEvidenceAuditResult audit =
      node::FinalizedSlashingEvidenceAudit::auditBlockEffects(
          block, ledger, validators, staking);
  require(audit.passed() && audit.evidenceCount() == 1,
          "Finalized evidence must audit after penalty, registry and staking "
          "effects.");
}
...
void testFinalizedEvidenceWithoutPenaltyIsRejected() {
  ...
  const node::FinalizedSlashingEvidenceAuditResult audit =
      node::FinalizedSlashingEvidenceAudit::auditBlockEffects(
          block, emptyLedger, validators, staking);
  require(!audit.passed(), "A finalized block with evidence but no penalty "
