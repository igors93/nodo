...
void testBuildsFeeEconomicBalance() {
    ...
    requireCondition(
        balance.active() &&
        balance.totalFee().rawUnits() == 100 &&
        balance.validatorRewardAmount().rawUnits() == 50 &&
        balance.treasuryAmount().rawUnits() == 30 &&
        balance.burnAmount().rawUnits() == 20,
        "Fee economic balance should split fees into validator reward, treasury and burn."
    );
}
...
void testBuildsBurnAndTreasuryRecords() {
    ...
    requireCondition(
        ...
        treasury.treasuryAddress() == nodo::node::ProtectionTreasury::TREASURY_ADDRESS,
        "Treasury fee record should allocate the treasury share to the protocol treasury."
    );
}
...
void testZeroFeesRemainValid() {
    ...
    requireCondition(
        balance.active() &&
        balance.validatorRewardAmount().rawUnits() == 0 &&
...
int main() {
    try {
        ...
        std::cerr << "Nodo fee economics tests failed: "
                  << error.what()
                  << "\\n";
        return 1;
    }
