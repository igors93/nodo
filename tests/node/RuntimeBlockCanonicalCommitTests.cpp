...
config::GenesisConfig genesisConfig() {
  return config::GenesisConfig(
      ...
      {config::BootstrapValidatorConfig(validatorKey().publicKey(), 1, 1,
                                        "canonical-validator")},
      {test::fundedConsensusTestAccount(userKey())}, "canonical-commit-test");
}
...
node::NodeRuntime startRuntime() {
  const node::NodeRuntimeStartResult started =
      node::NodeRuntimeFactory::startFromGenesis(node::NodeRuntimeConfig(
          genesisConfig(),
          p2p::PeerInfo("canonical-peer", "127.0.0.1:29998", "nodo/test", 0,
                        kTimestamp),
          16));
...
utils::Amount balanceAt(node::NodeRuntime &runtime,
...
struct TestGossipMesh {
  ...
  TestGossipMesh()
      : bus(std::make_shared<p2p::LoopbackTransportBus>()), transport(bus),
        mesh(p2p::GossipMeshConfig(
                 "canonical-peer", "localnet", "chain-localnet", "1",
                 genesisConfig().deterministicId(), 60, 2, 100, 50),
             transport) {}
};

consensus::BlockFinalizationPhaseResult finalizeThroughConsensus(
    node::NodeRuntime &runtime, const core::Block &block,
  ...
  require(consensus::BlockVotingPhase::castPrevote(
              runtime, block, 1, kTimestamp + 3, signer, gossip.mesh)
              .cast(),
          "Prevote must be accepted.");
  require(consensus::BlockVotingPhase::castPrecommit(
  ...
  return consensus::BlockFinalizationPhase::tryFinalize(
      runtime, block, block.index(), block.hash(), block.previousHash(), 1,
      cryptoContext.policy(), cryptoContext.signatureProvider(), kTimestamp + 5,
      directoryConfig);
}
...
void testLocalAndDistributedPipelinesProduceEquivalentState() {
  ...
  test::admitConsensusTestTransfer(localRuntime, userKey(), 1, kTimestamp + 1);
  test::admitConsensusTestTransfer(distributedRuntime, userKey(), 1,
                                   kTimestamp + 1);
  ...
  require(candidate.produced(),
          "Distributed pipeline must produce a candidate.");
...
void testGovernanceProposalIsCommittedWithoutPrematureExecution() {
  ...
  const core::Transaction proposal =
      core::TransactionBuilder::buildSignedGovernanceProposal(
          core::GovernanceProposalPayload::parameterChange(
              "Minimum fee", "Set minimum fee after governance approval",
              "MINIMUM_FEE_RAW", "250", 1)
              .serialize(),
          utils::Amount::fromRawUnits(100), 1, kTimestamp + 1,
  ...
  const mempool::MempoolAdmissionResult admission =
      runtime.mutableMempool().admitTransaction(
          proposal, crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, kTimestamp + 1);
  require(admission.accepted(), "Governance proposal must enter the mempool.");

  const node::RuntimeBlockPipelineResult result =
      node::RuntimeBlockPipeline::produceAndFinalizeLocalnetBlock(
          runtime, node::RuntimeBlockPipelineConfig(10, 1, 1, kTimestamp + 2),
          validatorSigner());

  require(result.finalized(),
          "Governance block must finalize: " + result.reason());
  require(runtime.governanceExecutor().hasProposal(proposal.id()) &&
              runtime.effectiveMinimumFeeRawUnits() ==
                  genesisConfig().networkParameters().minimumFeeRawUnits(),
          "Finalization must record the proposal without applying it before a "
          "vote.");
  require(runtime.blockchain().latestBlock().stateRoot() ==
  ...
  test::admitConsensusTestTransfer(runtime, userKey(), 2, kTimestamp + 3);
  const node::RuntimeBlockPipelineResult nextBlock =
      node::RuntimeBlockPipeline::produceAndFinalizeLocalnetBlock(
          runtime, node::RuntimeBlockPipelineConfig(10, 1, 1, kTimestamp + 4),
          validatorSigner());
  require(nextBlock.finalized(), "Unapproved governance proposals must not "
                                 "change the effective minimum fee: " +
                                     nextBlock.reason());
}
...
void testPersistenceFailureDoesNotPartiallyCommitRuntime() {
  ...
  require(runtime.blockchain().size() == chainSizeBefore,
          "Persistence failure must not append the block.");
  require(runtime.mempool().size() == mempoolSizeBefore,
          "Persistence failure must not remove mempool transactions.");
  require(runtime.supplyState().latestSupply() == supplyBefore,
          "Persistence failure must not change supply.");
  require(runtime.consensusRoundManager().currentState().height() ==
              consensusHeightBefore,
          "Persistence failure must not advance consensus height.");
  require(!runtime.finalizationRegistry().hasFinalizedHeight(
...
int main() {
  try {
    ...
    std::cerr << "Runtime canonical block commit tests FAILED: " << error.what()
              << "\n";
    return 1;
  }
}
