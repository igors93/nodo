...
core::Transaction tx(core::TransactionType type, const std::string &target,
                     std::int64_t amount, std::uint64_t nonce,
                     std::string data = {}) {
  return data.empty()
             ? core::Transaction(
                   type, kOwner, target, utils::Amount::fromRawUnits(amount),
                   utils::Amount::fromRawUnits(10), nonce, kTimestamp + nonce)
             : core::Transaction(type, kOwner, target,
                                 utils::Amount::fromRawUnits(amount),
                                 utils::Amount::fromRawUnits(10), nonce,
                                 kTimestamp + nonce, std::move(data));
}

core::TransactionExecutionResult
execute(const core::Transaction &transaction,
        const core::AccountStateView &current,
        core::TransactionDomainExecutor &executor, std::uint64_t height) {
  return core::TransactionExecutionRouter::execute(
      transaction,
      core::TransactionExecutionContext(
          current, height, kTimestamp + static_cast<std::int64_t>(height), true,
          false, true, "", nullptr, &executor));
}
...
void testCanonicalDomainHandlersAndDeterministicReplay() {
  ...
  auto run = [&](std::shared_ptr<node::ProtocolExecutionState> tracker) {
    ...
    const std::vector<core::Transaction> transactions = {
        ...
        tx(core::TransactionType::STAKE_DEPOSIT, base.validatorAddress, 1000,
           2),
    ...
    for (const auto &transaction : transactions) {
      ...
      const auto result =
          execute(transaction, view, *executor, executionHeight);
      require(result.success(),
              "protocol handler rejected valid transaction: " +
                  result.reason());
      require(result.receipt().isValid(),
              "protocol handler generated invalid receipt");
    ...
    return std::make_pair(view.serialize(),
                          tracker->governance.serialize() +
                              tracker->staking.serialize() +
                              tracker->validators.serialize() +
                              std::to_string(tracker->supply.rawUnits()) +
                              receiptHashes.front() + receiptHashes.back());
  };

  auto firstTracker =
      std::make_shared<node::ProtocolExecutionState>(base.state);
  auto secondTracker =
      std::make_shared<node::ProtocolExecutionState>(base.state);
...
void testValidatorRegistrationExitAndUnjail() {
  ...
  const std::string newAddress =
      crypto::AddressDerivation::deriveFromPublicKey(newValidator.publicKey())
          .value();
  const core::ValidatorRegistrationPayload registration(
      newValidator.publicKey(), "new-validator-meta");
  const auto registered =
      execute(tx(core::TransactionType::VALIDATOR_REGISTER, newAddress,
                 core::ValidatorRegistry::MIN_VALIDATOR_STAKE_RAW_UNITS, 1,
                 registration.serialize()),
              view, *executor, 100);
  require(registered.success(),
          "validator registration must execute: " + registered.reason());
  require(tracker->validators.entryForAddress(newAddress) != nullptr &&
              tracker->validators.entryForAddress(newAddress)->ownerAddress() ==
                  kOwner,
          "validator registration must commit identity, owner and pending "
          "lifecycle state");

  const auto exit = execute(tx(core::TransactionType::VALIDATOR_EXIT_REQUEST,
                               base.validatorAddress, 0, 2),
                            registered.accounts(), *executor, 100);
  require(exit.success() &&
              tracker->validators.entryForAddress(base.validatorAddress)
                      ->status() ==
  ...
  require(jailed.state.validators
              .jailValidator(jailed.validatorAddress, 1, kTimestamp + 2)
              .success(),
  ...
  const auto unjailed =
      execute(tx(core::TransactionType::VALIDATOR_UNJAIL_REQUEST,
                 jailed.validatorAddress, 0, 1),
              accounts(), *jailedExecutor, 100);
  require(
      unjailed.success() &&
          !jailedTracker->validators.entryForAddress(jailed.validatorAddress)
               ->jailed() &&
          !jailedTracker->staking.accountOrDefault(jailed.validatorAddress)
               .jailed(),
      "authorized unjail must update validator registry and staking state "
      "together");
}
