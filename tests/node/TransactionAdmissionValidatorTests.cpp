...
crypto::KeyStoreLoadResult createAndLoadKey(const std::filesystem::path &path,
  ...
  const crypto::KeyStoreCreateResult created = crypto::KeyStore::createLocalKey(
      path, keyId, crypto::KeyStoreKeyType::USER, seed, kTimestamp);

  if (!created.success()) {
    throw std::runtime_error("Key creation failed in admission test: " +
                             created.reason());
  }

  const crypto::KeyStoreLoadResult loaded =
      crypto::KeyStore::loadKey(path, keyId);

  if (!loaded.loaded()) {
    throw std::runtime_error("Key load failed in admission test: " +
                             loaded.reason());
  }
...
core::Transaction buildTransaction(const crypto::KeyPair &keyPair,
                                   const crypto::SignatureProvider &provider,
                                   std::int64_t feeRawUnits,
                                   std::uint64_t nonce = 1,
                                   std::int64_t timestamp = kTimestamp + 10) {
  const crypto::Signer signer(keyPair, provider);

  return core::TransactionBuilder::buildSignedTransfer(
      core::TransactionBuildRequest(
          "nodo-localnet-recipient", utils::Amount::fromRawUnits(1000),
          utils::Amount::fromRawUnits(feeRawUnits), nonce, timestamp),
      signer, "nodo-localnet-admission-test");
}

core::AccountStateView accountStateFor(const std::string &address,
  ...
  requireCondition(
      view.putAccount(core::AccountState(
          address, utils::Amount::fromRawUnits(balanceRawUnits), nonce)),
...
core::Transaction
supportedTransaction(core::TransactionType type, const crypto::KeyPair &keyPair,
                     const crypto::SignatureProvider &provider) {
  const auto &typePolicy = core::TransactionTypePolicyRegistry::policyFor(type);
  const utils::Amount amount =
      typePolicy.amountRule == core::TransactionAmountRule::ZERO
          ? utils::Amount()
          : utils::Amount::fromRawUnits(
                type == core::TransactionType::VALIDATOR_REGISTER ? 1'000'000
                                                                  : 1000);
  std::string target = "nodo-target";
  std::string data;
  if (type == core::TransactionType::TRANSFER)
    target = "nodo-recipient";
  if (type == core::TransactionType::BURN)
    target = "nodo_burn";
  if (type == core::TransactionType::GOVERNANCE_PROPOSE) {
    target = "nodo_governance";
    data = core::GovernanceProposalPayload::parameterChange(
               "Minimum fee", "Set minimum fee through governance",
               "MINIMUM_FEE_RAW", "10", 10)
               .serialize();
  }
  if (type == core::TransactionType::GOVERNANCE_VOTE) {
    target = "proposal-id";
    data = core::GovernanceVotePayload("proposal-id", "validator-address",
  ...
  if (type == core::TransactionType::VALIDATOR_REGISTER) {
    const crypto::KeyPair validator =
        crypto::KeyPair::createDeterministicBls12381KeyPair(
            "admission-validator");
    target =
        crypto::AddressDerivation::deriveFromPublicKey(validator.publicKey())
            .value();
    data = core::ValidatorRegistrationPayload(validator.publicKey(),
  ...
  core::Transaction tx =
      data.empty() ? core::Transaction(type, keyPair.address().value(), target,
                                       ...
                                       1, kTimestamp + 10)
                   : core::Transaction(type, keyPair.address().value(), target,
                                       amount, utils::Amount::fromRawUnits(100),
                                       1, kTimestamp + 10, data);
...
void testLocalAndNetworkAdmissionShareCompleteTypeMatrix() {
  ...
  const crypto::KeyStoreLoadResult key =
      createAndLoadKey(path, "matrix-user", "admission-matrix-seed");
  ...
  for (const auto type : core::TransactionTypePolicyRegistry::allTypes()) {
    const core::Transaction tx =
        supportedTransaction(type, key.keyPair(), provider);
    const auto local =
        node::TransactionAdmissionValidator::validateLocalSubmission(
            tx, key.metadata(), network,
            crypto::CryptoPolicy::developmentPolicy(),
            crypto::SecurityContext::USER_TRANSACTION, provider);
    requireCondition(local.accepted(), "Local admission matrix rejected " +
                                           core::transactionTypeToString(type) +
                                           ": " + local.reason());

    const auto networkResult =
        node::TransactionAdmissionValidator::validateNetworkSubmission(
            tx, network,
            accountStateFor(key.metadata().address(), 2'000'000, 0),
            mempool::Mempool(), crypto::CryptoPolicy::developmentPolicy(),
            crypto::SecurityContext::USER_TRANSACTION, provider);
    requireCondition(networkResult.accepted(),
                     "Network admission matrix rejected " +
                         core::transactionTypeToString(type) + ": " +
...
void testAcceptsValidLocalSubmission() {
  ...
  const crypto::KeyStoreLoadResult key =
      createAndLoadKey(path, "local-user", "admission-test-seed");

  const crypto::Ed25519SignatureProvider provider;
  const core::Transaction transaction =
      buildTransaction(key.keyPair(), provider, 100);

  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateLocalSubmission(
          transaction, key.metadata(), localnetWithMinimumFee(100),
          crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);
...
void testRejectsTransactionBelowMinimumFee() {
  ...
  const crypto::Ed25519SignatureProvider provider;
  const core::Transaction transaction =
      buildTransaction(key.keyPair(), provider, 99);
  ...
  requireCondition(!result.accepted() &&
                       result.status() ==
                           node::TransactionAdmissionStatus::BELOW_MINIMUM_FEE,
...
void testRejectsTransactionSignedByDifferentKey() {
  ...
  const crypto::KeyStoreLoadResult differentKey =
      createAndLoadKey(path, "other-user", "admission-other-seed");

  const crypto::Ed25519SignatureProvider provider;
  const core::Transaction transaction =
      buildTransaction(signerKey.keyPair(), provider, 100);

  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateLocalSubmission(
          transaction, differentKey.metadata(), localnetWithMinimumFee(100),
          crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);

  requireCondition(!result.accepted() &&
                       result.status() ==
                           node::TransactionAdmissionStatus::INVALID_KEY,
                   "Transaction signed by one key must not be admitted with "
                   "another key metadata.");
...
void testAcceptsRuntimeSubmissionWithSufficientBalance() {
  ...
  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateRuntimeSubmission(
          transaction, key.metadata(), localnetWithMinimumFee(100),
          accountStateFor(key.metadata().address(), 1100, 0),
          mempool::Mempool(), crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);

  requireCondition(result.accepted(),
                   "Runtime admission should accept transaction when balance "
...
void testRejectsRuntimeSubmissionWithInsufficientBalance() {
  ...
  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateRuntimeSubmission(
          transaction, key.metadata(), localnetWithMinimumFee(100),
          accountStateFor(key.metadata().address(), 1099, 0),
          mempool::Mempool(), crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);
  ...
  requireCondition(
      !result.accepted() &&
          result.status() ==
              node::TransactionAdmissionStatus::INSUFFICIENT_BALANCE,
      "Runtime admission should reject transaction when balance cannot cover "
      "amount plus fee.");
...
void testAcceptsRuntimeQueuedNonceWhenReservedBalanceIsSufficient() {
  ...
  const core::Transaction pending =
      buildTransaction(key.keyPair(), provider, 100, 1, kTimestamp + 20);
  const core::Transaction queued =
      buildTransaction(key.keyPair(), provider, 100, 2, kTimestamp + 21);

  mempool::Mempool mempool;
  requireCondition(
      mempool
          .admitTransaction(pending, crypto::CryptoPolicy::developmentPolicy(),
  ...
  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateRuntimeSubmission(
          queued, key.metadata(), localnetWithMinimumFee(100),
          accountStateFor(key.metadata().address(), 2200, 0), mempool,
          crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);

  requireCondition(result.accepted(), "Runtime admission should accept queued "
                                      "nonce when balance covers the queue.");
...
void testAcceptsRuntimeReplacementWhenHigherFeeIsAllowed() {
  ...
  const core::Transaction replacement =
      buildTransaction(key.keyPair(), provider, 200, 1, kTimestamp + 21);
  ...
  const node::TransactionAdmissionResult result =
      node::TransactionAdmissionValidator::validateRuntimeSubmission(
          replacement, key.metadata(), localnetWithMinimumFee(100),
          accountStateFor(key.metadata().address(), 1200, 0), mempool,
          crypto::CryptoPolicy::developmentPolicy(),
          crypto::SecurityContext::USER_TRANSACTION, provider);

  requireCondition(result.accepted(),
                   "Runtime admission should allow higher-fee replacement when "
                   "mempool policy allows it.");
...
void testRejectsRuntimeReplacementWhenPolicyDisallowsIt() {
  ...
  mempool::Mempool mempool(mempool::MempoolConfig(5000, 0, false, 60 * 60));
  requireCondition(
      mempool
          .admitTransaction(pending, crypto::CryptoPolicy::developmentPolicy(),
                            crypto::SecurityContext::USER_TRANSACTION,
  ...
  requireCondition(!result.accepted() &&
                       result.status() ==
                           node::TransactionAdmissionStatus::CONFLICTING_NONCE,
                   "Runtime admission should reject same-nonce replacement "
                   "when mempool policy disallows it.");
