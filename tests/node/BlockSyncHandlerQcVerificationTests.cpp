...
static std::string getQcSenderAddress() {
  const crypto::KeyPair kp =
      crypto::KeyPair::createDeterministicEd25519KeyPair("qc-test-key");
...
core::Transaction testTx(std::uint64_t nonce) {
  ...
  core::Transaction tx(core::TransactionType::TRANSFER, senderAddress,
                       "qc-recipient", utils::Amount::fromRawUnits(100),
...
core::AccountStateView senderView() {
  ...
  view.putAccount(core::AccountState(getQcSenderAddress(),
...
core::StateTransitionPreviewContext buildContext(const core::Blockchain &) {
  return core::StateTransitionPreviewContext(
      10, senderView(), false, true, "", 0, "test-chain", "localnet", {}, {},
...
core::Block validBlockFor(const core::Blockchain &blockchain,
                          std::uint64_t nonce) {
  const core::Transaction tx = testTx(nonce);
  const core::Block draft(blockchain.latestBlock().index() + 1,
                          blockchain.latestBlock().hash(), {txRecord(tx)},
                          kTimestamp + 1, "", "");
  ...
  if (!preview.accepted()) {
    throw std::runtime_error("validBlockFor: preview failed: " +
                             preview.reason());
  }
  return core::Block(blockchain.latestBlock().index() + 1,
                     blockchain.latestBlock().hash(), {txRecord(tx)},
...
std::string registerValidator(core::ValidatorRegistry &registry,
  ...
  core::ValidatorRegistrationRecord rec(address, kp.publicKey(),
                                        /*activationEpoch=*/1, "meta-" + seed,
                                        kTimestamp);
...
// Build a structurally valid + cryptographically valid FinalizedBlockRecord for
// `block`. Uses `validatorKp` to cast a PRECOMMIT vote, then assembles a QC and
// wraps it.
...
struct GossipPair {
  ...
  GossipPair()
      : bus(std::make_shared<p2p::LoopbackTransportBus>()), transportA(bus),
        transportB(bus),
        meshA(p2p::GossipMeshConfig("node-a", "localnet", "qc-test", "1",
                                    "genesis-id-qc", 60, 5, 100, 50),
              transportA),
        meshB(p2p::GossipMeshConfig("node-b", "localnet", "qc-test", "1",
                                    "genesis-id-qc", 60, 5, 100, 50),
              transportB) {
    meshA.registerPeer(
        p2p::PeerMetadata("node-b", p2p::PeerEndpoint("127.0.0.1", 19200),
                          "fp-b", kTimestamp, kTimestamp, 0, false));
    meshB.registerPeer(
        p2p::PeerMetadata("node-a", p2p::PeerEndpoint("127.0.0.1", 19201),
                          "fp-a", kTimestamp, kTimestamp, 0, false));
  ...
  void deliverBlockResponse(const std::vector<core::Block> &blocks) {
    const std::string payload =
        node::BlockSyncHandler::serializeBlockList(blocks);
    meshA.broadcast(p2p::NetworkMessageType::BLOCK_RESPONSE, payload,
                    kTimestamp);
...
// ── STATE_ROOT_ONLY: accepts block without QC in registry ─────────────────
...
void testStateRootOnlyAcceptsWithoutQc() {
  ...
  const std::size_t applied = node::BlockSyncHandler::applyResponses(
      gp.meshB, blockchain, buildContext, emptyRegistry,
      node::BlockSyncQcMode::STATE_ROOT_ONLY, kTimestamp + 2);

  requireCondition(
      applied == 1,
      "STATE_ROOT_ONLY must accept a block without a QC in registry.");
  requireCondition(blockchain.size() == 2U, "Blockchain must grow by 1.");
}

// ── QC_REQUIRED: rejects block not in finalization registry ───────────────
...
void testQcRequiredRejectsBlockWithoutFinalizationRecord() {
  ...
  const std::size_t applied = node::BlockSyncHandler::applyResponses(
      gp.meshB, blockchain, buildContext, emptyRegistry,
      node::BlockSyncQcMode::QC_REQUIRED, kTimestamp + 2);

  requireCondition(
      applied == 0,
      "QC_REQUIRED must reject a block not in the finalization registry.");
  requireCondition(blockchain.size() == 1U,
                   "Blockchain must not grow when QC is missing.");
}

// ── QC_REQUIRED: accepts block present in finalization registry
// ────────────
...
void testQcRequiredAcceptsBlockWithFinalizationRecord() {
  ...
  const std::size_t applied = node::BlockSyncHandler::applyResponses(
      gp.meshB, blockchain, buildContext, finalizationRegistry,
      node::BlockSyncQcMode::QC_REQUIRED, kTimestamp + 2);

  requireCondition(applied == 1,
                   "QC_REQUIRED must accept block when QC record is present.");
  requireCondition(blockchain.size() == 2U, "Blockchain must grow by 1.");
}

// ── QC_REQUIRED: stops at first block missing QC in a batch ───────────────
...
void testQcRequiredStopsAtFirstBlockMissingQc() {
  ...
  requireCondition(
      applied == 1,
      "QC_REQUIRED must stop after the first block whose QC is missing.");
  requireCondition(blockchain.size() == 2U,
                   "Only the finalized block must be applied.");
}
