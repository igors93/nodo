...
void testArchiveModeNeverPrunesAnyHeight() {
    ...
    requireCondition(
        config.mode() == NodePruningMode::ARCHIVE,
        "archiveMode() must return ARCHIVE mode."
    );
    requireCondition(
        config.isValid(),
        "ARCHIVE mode must be valid."
    );

    // ARCHIVE never prunes, regardless of heights
    requireCondition(!config.shouldPruneBlockAtHeight(0, 10000),   "ARCHIVE: height 0 should not be pruned.");
    requireCondition(!config.shouldPruneBlockAtHeight(100, 10000), "ARCHIVE: height 100 should not be pruned.");
    requireCondition(!config.shouldPruneBlockAtHeight(9999, 10000),"ARCHIVE: height 9999 should not be pruned.");
    requireCondition(!config.shouldPruneStateAtHeight(0, 10000),   "ARCHIVE: state height 0 should not be pruned.");

    requireCondition(
        config.retainFromHeight(10000) == 0,
...
void testFullModeWith2EpochsAt3000PrunesOldButNotRecent() {
    ...
    requireCondition(
        config.mode() == NodePruningMode::FULL,
    ...
    requireCondition(
        ...
        "FULL: height 2500 at currentHeight=3000, 2 epochs should not be pruned."
    );
    requireCondition(
        !config.shouldPruneBlockAtHeight(1000, currentHeight, epochBlocks),
        "FULL: retainFromHeight=1000 — exactly at boundary should not be pruned."
    );
    requireCondition(
    ...
    requireCondition(
        config.retainFromHeight(currentHeight, epochBlocks) == 1000,
        "FULL retainFromHeight must be currentHeight - retainEpochs*epochBlocks = 1000."
    );
}
...
void testLightModePrunesEverythingExceptCurrentTip() {
    ...
    requireCondition(
        config.mode() == NodePruningMode::LIGHT,
        "lightMode() must return LIGHT mode."
    );
    requireCondition(
        config.isValid(),
        "LIGHT mode must be valid."
    );

    // Everything below currentHeight should be pruned
    requireCondition(
        config.shouldPruneBlockAtHeight(0, currentHeight),
        "LIGHT: height 0 should be pruned."
    );
    requireCondition(
        config.shouldPruneBlockAtHeight(4999, currentHeight),
    ...
    requireCondition(
        config.retainFromHeight(currentHeight) == currentHeight,
...
void testSerializeDeserializeRoundTripArchive() {
    ...
    requireCondition(
        serialized.find("ARCHIVE") != std::string::npos,
    ...
    requireCondition(
        restored.mode() == NodePruningMode::ARCHIVE,
...
void testSerializeDeserializeRoundTripFull() {
    ...
    requireCondition(
        restored.mode() == NodePruningMode::FULL,
        "Deserialized FULL mode must be FULL."
    );
    requireCondition(
        restored.retainEpochs() == 5,
...
void testSerializeDeserializeRoundTripLight() {
    ...
    requireCondition(
        restored.mode() == NodePruningMode::LIGHT,
...
void testFullModeRetainFromHeightAtBoundary() {
    // If currentHeight < retainEpochs * epochBlocks, retainFrom = 0 (nothing to prune)
    ...
    requireCondition(
        !config.shouldPruneBlockAtHeight(0, 500, 1000),
        "FULL: when chain is shorter than retention period, no block should be pruned."
    );
}
...
int main() {
    try {
        ...
        std::cerr << "Nodo NodePruningConfig tests failed: "
                  << error.what()
                  << "\n";
        return 1;
    }
