...
Transaction signedTransfer(std::uint64_t nonce) {
  ...
  Transaction tx(TransactionType::TRANSFER, kp.address().value(),
                 "gossip-recipient", Amount::fromRawUnits(1000),
                 Amount::fromRawUnits(100), nonce,
  ...
  tx.attachSignatureBundle(SignatureBundle::createSignature(
      tx.signingPayload(), kp.publicKey(), kp.privateKeyForSigningOnly(),
      tx.timestamp(), provider, SigningDomain::USER_TRANSACTION));
...
void testSerializeForGossipRoundTrip() {
  ...
  requireCondition(!payload.empty(),
                   "serializeForGossip should produce non-empty payload.");

  const CryptoPolicy policy = CryptoPolicy::developmentPolicy();
  Mempool mempool;

  const auto decoded = PersistentMempoolStore::deserializeGossip(
      payload, policy, SecurityContext::USER_TRANSACTION, "nodo-localnet-1");

  const bool admitted = decoded.has_value() &&
                        mempool
  ...
  requireCondition(
      admitted,
      "deserializeGossip should decode a valid signed tx for admission.");
  requireCondition(!mempool.empty(),
                   "Mempool should have one transaction after admission.");
}
...
void testGossipDeduplicationViaSeenCache() {
  ...
  requireCondition(!secondSeen,
                   "Second call for same tx should return false (duplicate).");
}
...
void testInvalidGossipPayloadRejected() {
  ...
  const auto decoded = PersistentMempoolStore::deserializeGossip(
      "invalid-gossip-payload", policy, SecurityContext::USER_TRANSACTION,
      "nodo-localnet-1");

  requireCondition(!decoded.has_value(),
                   "Invalid gossip payload should be rejected.");
  requireCondition(mempool.empty(),
                   "Mempool should remain empty after rejection.");
}

void testUnsignedTransactionGossipIsEmpty() {
  Transaction tx(TransactionType::TRANSFER, "gossip-sender", "gossip-recipient",
                 Amount::fromRawUnits(500), Amount::fromRawUnits(50), 99,
                 kTimestamp);
  ...
  requireCondition(payload.empty(),
                   "serializeForGossip with unsigned tx should return empty.");
}
