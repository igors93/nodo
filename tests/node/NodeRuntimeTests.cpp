...
vote(const std::string &validatorAddress, std::uint64_t height,
     std::uint64_t round,
     const std::string &blockHash = "node-runtime-block-hash-a") {
  const nodo::crypto::PublicKey votePublicKey(
      nodo::crypto::CryptoAlgorithm::BLS12_381, std::string(96, 'a'));
  ...
  bundle.addSignature(nodo::crypto::Signature(
      ...
      nodo::crypto::CryptoAlgorithm::BLS12_381, votePublicKey,
      std::string(192, 'b'), kTimestamp + 1));

  return nodo::consensus::ValidatorVoteRecord(
      validatorAddress, votePublicKey, height, blockHash,
      "node-runtime-previous-hash", round,
      nodo::consensus::ValidatorVoteDecision::PRECOMMIT,
      "node-runtime-reason-hash", kTimestamp + 1, bundle);
}

void testNodeRuntimeStartsFromGenesis() {
  const NodeRuntimeConfig config(genesisConfig(), peer("local-node", 0), 8);

  const auto result = NodeRuntimeFactory::startFromGenesis(config);

  requireCondition(result.started(),
                   "Node runtime should start from valid genesis config.");

  requireCondition(result.runtime().blockchain().size() == 1U,
                   "Runtime blockchain should start at genesis.");

  requireCondition(result.runtime().validatorRegistry().activeCount() == 2U,
                   "Runtime should have bootstrap validator registry.");

  requireCondition(result.runtime().chainSummary().isValid(),
                   "Runtime chain summary should be valid.");
}

void testRuntimeInitializesConsensusRoundFromProposerSchedule() {
  const auto start = NodeRuntimeFactory::startFromGenesis(
      NodeRuntimeConfig(genesisConfig(), peer("local-node", 0), 8));

  requireCondition(start.started(), "Runtime should start.");

  const auto &state = start.runtime().consensusRoundManager().currentState();

  const std::string expectedProposer =
      nodo::consensus::ProposerSchedule::selectProposer(
          start.runtime().validatorRegistry(),
          genesisConfig().networkParameters().chainId(), 1, 1);

  requireCondition(
      state.height() == 1 && state.round() == 1 &&
...
void testRuntimeConsensusVoteAdmissionChecksCurrentRoundReplayAndConflicts() {
  ...
  const auto accepted =
      runtime.submitConsensusVote(vote("validator-network-a", 1, 1));

  const auto replay =
      runtime.submitConsensusVote(vote("validator-network-a", 1, 1));

  const auto conflicting = runtime.submitConsensusVote(
      vote("validator-network-a", 1, 1, "node-runtime-block-hash-b"));

  runtime.mutableConsensusRoundManager().advanceRound(2, "validator-b",
                                                      kTimestamp + 2);

  const auto stale =
      runtime.submitConsensusVote(vote("validator-network-b", 1, 1));

  requireCondition(accepted.accepted(),
                   "Current-round vote should be accepted.");

  requireCondition(replay.status() == VoteCollectStatus::REJECTED_REPLAY,
                   "Duplicate vote should be rejected as replay.");

  requireCondition(conflicting.status() ==
...
void testPeerManagerAddUpdateCapacityAndBestPeer() {
  ...
  requireCondition(manager.addOrUpdatePeer(peer("peer-a", 1)).success(),
                   "Peer A should be accepted.");

  requireCondition(manager.addOrUpdatePeer(peer("peer-b", 3)).success(),
                   "Peer B should be accepted.");

  requireCondition(manager.addOrUpdatePeer(peer("peer-c", 5)).status() ==
                       PeerUpdateStatus::CAPACITY_REACHED,
                   "Third peer should be rejected by capacity.");

  const PeerInfo *best = manager.bestSyncPeer(1);

  requireCondition(best != nullptr && best->peerId() == "peer-b",
  ...
  requireCondition(best != nullptr && best->peerId() == "peer-a",
...
void testRuntimeCreatesChainSummaryMessage() {
  ...
  const auto message =
      start.runtime().localChainSummaryMessage("remote-node", kTimestamp + 10);

  requireCondition(message.isValid(),
...
void testRuntimeEvaluatesBetterPeerForSync() {
  ...
  const ChainForkSummary remoteSummary(3, 2, "remote-latest-hash");

  const auto plan = start.runtime().evaluatePeerForSync(
      peer("remote-node", 2), remoteSummary, kTimestamp + 20);

  requireCondition(plan.decision() == LocalSyncDecision::REQUEST_BLOCKS &&
                       plan.shouldRequestBlocks(),
                   "Better remote peer should trigger sync request.");
}

void testRuntimeRejectsInvalidConfig() {
  const NodeRuntimeConfig invalidConfig(GenesisConfig(), peer("local-node", 0),
                                        8);

  const auto result = NodeRuntimeFactory::startFromGenesis(invalidConfig);
...
void testRuntimeHalt() {
  ...
  requireCondition(runtime.isRunning(),
                   "Runtime should start in RUNNING state.");

  requireCondition(!runtime.isHalted(),
                   "Runtime should not be halted initially.");

  requireCondition(runtime.isValid(), "Runtime should be valid initially.");
...
int main() {
  try {
    ...
    std::cerr << "Nodo node runtime tests failed: " << error.what() << "\n";
    return 1;
  }
}
