...
void testSingleValidatorDistributionLocksTenPercent() {
    ...
    requireCondition(
        ...
        rewards[0].reason() == RewardDistributionCalculator::BLOCK_FINALIZATION_FEE_REASON &&
        rewards[0].isValid(),
        "Single validator reward should split into 90 liquid and 10 locked raw units."
    );
}
...
void testRemainderIsDistributedDeterministically() {
    const std::vector<RewardDistribution> rewards =
        RewardDistributionCalculator::buildForVoters(
            ...
            {"validator-c", "validator-a", "validator-b"},
            7
        );
    ...
    requireCondition(
        rewards[0].totalReward().rawUnits() == 34 &&
...
int main() {
    try {
        ...
        std::cerr << "Nodo reward distribution tests failed: "
                  << error.what()
                  << "\n";
        return 1;
    }
