...
ProtectionRewardSettlement validSettlement(
    const std::string& addr = "validator-001",
    std::uint64_t block = 10,
    std::int64_t planned = 1000,
    std::int64_t earned = 800,
    std::int64_t deferred = 200,
    const std::string& workDigest = "work-digest-abc"
) {
    return ProtectionRewardSettlement(
        addr, block,
        Amount::fromRawUnits(planned),
        Amount::fromRawUnits(earned),
        Amount::fromRawUnits(deferred),
        500, 700,
        ProtectionRewards::PROTECTION_SETTLEMENT_REASON,
        "grant-digest-001",
        workDigest
    );
}
...
void testSettlementWithoutWorkDigestFailsAudit() {
    const ProtectionRewardSettlement noEvidence =
...
void testCategoryProtectionWhenEarnedOnly() {
    const ProtectionRewardSettlement s = validSettlement(
        "v1", 10, 1000, 1000, 0
    );
    assert(ProtectionRewards::categoryForSettlement(s) == RewardCategory::PROTECTION);
}
...
void testCategoryDeferredWhenDeferredPositive() {
    ...
    assert(ProtectionRewards::categoryForSettlement(s) == RewardCategory::DEFERRED_PROTECTION);
}
