...
void testProtectionRequiresWorkEvidence() {
    ...
    assert(rewardCategoryRequiresWorkEvidence(RewardCategory::DEFERRED_PROTECTION));
...
void testExtraordinaryBlockedInDefenseMode() {
    ...
    assert(!rewardCategoryBlockedInDefenseMode(RewardCategory::DEFERRED_PROTECTION));
    assert(!rewardCategoryBlockedInDefenseMode(RewardCategory::REJECTED));
}

void testToString() {
    assert(rewardCategoryToString(RewardCategory::NORMAL_BLOCK) == "NORMAL_BLOCK");
    assert(rewardCategoryToString(RewardCategory::PROTECTION) == "PROTECTION");
    assert(rewardCategoryToString(RewardCategory::DEFERRED_PROTECTION) == "DEFERRED_PROTECTION");
    assert(rewardCategoryToString(RewardCategory::EXTRAORDINARY) == "EXTRAORDINARY");
