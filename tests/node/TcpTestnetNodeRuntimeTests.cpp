...
int main() {
  const std::filesystem::path root = std::filesystem::temp_directory_path() /
                                     "nodo_tcp_testnet_node_runtime_test";

  std::filesystem::remove_all(root);

  const std::string genesisId = "nodo-localnet-genesis";

  node::TcpTestnetNodeRuntime nodeA(node::TcpTestnetNodeRuntimeConfig(
      "node-a", "127.0.0.1", 0, "nodo-localnet", "localnet-chain", "1.0.0",
      genesisId, root / "node-a", 30, 3, 100, 50, 1000));

  node::TcpTestnetNodeRuntime nodeB(node::TcpTestnetNodeRuntimeConfig(
      "node-b", "127.0.0.1", 0, "nodo-localnet", "localnet-chain", "1.0.0",
      genesisId, root / "node-b", 30, 3, 100, 50, 1000));
  ...
  assert(
      nodeA.connectUnverifiedPeer("node-b", nodeB.transport().localEndpoint())
          .success());
  assert(!nodeA.gossipMesh().peerRegistry().contains("node-b"));

  const crypto::KeyPair identityA =
      crypto::KeyPair::createDeterministicEd25519KeyPair("tcp-runtime-node-a");
  const crypto::KeyPair identityB =
      crypto::KeyPair::createDeterministicEd25519KeyPair("tcp-runtime-node-b");

  p2p::PeerMetadata peerA("node-a", nodeA.transport().localEndpoint(),
                          identityA.publicKey().fingerprint(), 1000, 1000, 0,
                          false);

  p2p::PeerMetadata peerB("node-b", nodeB.transport().localEndpoint(),
                          identityB.publicKey().fingerprint(), 1000, 1000, 0,
                          false);

  node::ChainStatusMessage status(
      "nodo-localnet", "localnet-chain", "1.0.0", 10,
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", 10,
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");

  assert(status.isValid());

  assert(node::PeerHandshakeAutoRegistrar::initiateHandshake(nodeA.gossipMesh(),
                                                             "node-b", 1000)
             .allAccepted());

  for (std::int64_t attempt = 0;
       attempt < 20 && (!nodeA.gossipMesh().peerRegistry().contains("node-b") ||
    ...
    (void)node::PeerHandshakeAutoRegistrar::processInbox(
        ...
        nodeB.gossipMesh().drainInbox(p2p::NetworkMessageType::PEER_CHALLENGE),
        peerB, status, identityB, now);
    (void)nodeA.tick(now);
    (void)node::PeerHandshakeAutoRegistrar::processInbox(
        nodeA.gossipMesh(),
        nodeA.gossipMesh().drainInbox(p2p::NetworkMessageType::PEER_HELLO),
        nodeA.gossipMesh().drainInbox(p2p::NetworkMessageType::PEER_CHALLENGE),
  ...
  for (int attempt = 0;
       attempt < 1000 && nodeB.gossipMesh().inbox().countForType(
                             p2p::NetworkMessageType::CHAIN_STATUS) == 0;
       ++attempt) {
    (void)nodeB.tick(3000);
  }

  const auto manualStatuses =
      nodeB.gossipMesh().drainInbox(p2p::NetworkMessageType::CHAIN_STATUS);
  assert(!manualStatuses.empty());
  for (const auto &envelope : manualStatuses) {
    assert(
        node::ChainStatusGossipCodec::decode(envelope.payload()).has_value());
  }

  const p2p::NetworkEnvelope invalidSyncMessage(
      "nodo-localnet", "localnet-chain", "1.0.0",
      p2p::NetworkMessageType::SLASHING_EVIDENCE_REQUEST, "node-b", 3001, 30,
      "malformed-sync-request");
  for (std::int64_t offset = 0; offset < 3; ++offset) {
    nodeA.gossipMesh().reportPeerMisbehavior(
        invalidSyncMessage, p2p::PeerMisbehaviorType::INVALID_MESSAGE,
        "Slashing evidence request payload is malformed.", 3001 + offset);
  }
  ...
  node::TcpTestnetNodeRuntime reloadedNodeA(node::TcpTestnetNodeRuntimeConfig(
      "node-a", "127.0.0.1", 0, "nodo-localnet", "localnet-chain", "1.0.0",
      genesisId, root / "node-a", 30, 3, 100, 50, 1000));
  ...
  p2p::PeerMetadata rotatedIdentity(
      "node-b-rotated", nodeB.transport().localEndpoint(),
      identityB.publicKey().fingerprint(), 2000, 2000, 0, false);
  assert(!reloadedNodeA.addPeer(rotatedIdentity).success());
  assert(!reloadedNodeA.gossipMesh().peerRegistry().contains("node-b-rotated"));
  assert(!reloadedNodeA.transport().hasPeerEndpoint("node-b-rotated"));
  assert(reloadedNodeA.gossipMesh()
             .peerRegistry()
             .peerByIdentityKey(restored->identityKey())
             ->quarantined());
