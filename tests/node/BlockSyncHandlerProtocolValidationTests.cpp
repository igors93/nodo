...
static std::string getSyncSenderAddress() {
  const crypto::KeyPair kp =
      crypto::KeyPair::createDeterministicEd25519KeyPair("sync-key");
...
core::Transaction testTx(std::uint64_t nonce) {
  ...
  core::Transaction tx(core::TransactionType::TRANSFER, senderAddress,
                       "sync-recipient", utils::Amount::fromRawUnits(100),
...
core::StateTransitionPreviewContext senderContext() {
  ...
  view.putAccount(core::AccountState(getSyncSenderAddress(),
...
struct GossipPair {
  ...
  GossipPair()
      : bus(std::make_shared<p2p::LoopbackTransportBus>()), transportA(bus),
        transportB(bus),
        meshA(p2p::GossipMeshConfig("node-a", "localnet", "chain-sync-test",
                                    "1", "genesis-id-sync", 60, 5, 100, 50),
              transportA),
        meshB(p2p::GossipMeshConfig("node-b", "localnet", "chain-sync-test",
                                    "1", "genesis-id-sync", 60, 5, 100, 50),
              transportB) {
    meshA.registerPeer(
        p2p::PeerMetadata("node-b", p2p::PeerEndpoint("127.0.0.1", 19100),
                          "fp-b", kTimestamp, kTimestamp, 0, false));
    meshB.registerPeer(
        p2p::PeerMetadata("node-a", p2p::PeerEndpoint("127.0.0.1", 19101),
...
void testApplyResponsesRejectsBlockWithWrongStateRoot() {
  ...
  const std::size_t applied = node::BlockSyncHandler::applyResponses(
      gp.meshB, blockchain, buildContext, kEmptyRegistry,
      node::BlockSyncQcMode::STATE_ROOT_ONLY, kTimestamp + 2);

  requireCondition(applied == 0,
...
void testApplyResponsesRejectsBlockWithWrongReceiptsRoot() {
  ...
  const core::Block badBlock(1, blockchain.latestBlock().hash(), {record(tx)},
                             kTimestamp + 1, preview.stateRoot(),
                             kWrongReceiptsRoot);
...
void testApplyResponsesAcceptsBlockWithCorrectRoots() {
  ...
  requireCondition(applied == 1, "Block with correct roots must be applied.");
  requireCondition(
      blockchain.size() == 2U,
...
void testApplyResponsesStopsAtFirstInvalidBlock() {
  ...
  const std::size_t applied = node::BlockSyncHandler::applyResponses(
      gp.meshB, blockchain, buildContext, kEmptyRegistry,
      node::BlockSyncQcMode::STATE_ROOT_ONLY, kTimestamp + 3);

  // First block in the batch has wrong roots → stops at 0 applied.
  requireCondition(applied == 0, "Sync must stop at the first invalid block "
                                 "without applying later blocks.");
}
