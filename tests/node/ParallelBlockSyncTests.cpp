...
Block makeBlock(std::uint64_t index, const std::string& prevHash, const std::string& suffix) {
    return Block(
        index,
        prevHash,
        {
...
void testBeginSyncReturnsDownloadingHeaders() {
    ...
    requireCondition(
        progress.phase == SyncPhase::DOWNLOADING_HEADERS,
        "beginSync should return DOWNLOADING_HEADERS phase."
    );

    requireCondition(
        progress.localHeight == 0 && progress.targetHeight == 10,
...
void testOnBodyReceivedOnlyAcceptsVerifiedHeaders() {
    ...
    Block genesis = Block::createGenesisBlock(
        { LedgerRecord::fromValidationWorkRecord(makeWork("gen-body"), kTimestamp) },
        kTimestamp
    );
    ...
    requireCondition(
        !rejected,
        "onBodyReceived should reject body when no verified header exists for that height."
    );

    // Now add the header first
    const bool headerAccepted = sync.onHeadersReceived(
        { genesis.serialize() },
        0,
        "peer-1"
    );
    ...
    const bool bodyAccepted = sync.onBodyReceived(genesis.serialize(), 0, "peer-1");
    requireCondition(
        bodyAccepted,
...
void testDrainReadyBlocksReturnsOrderedBlocks() {
    ...
    Block block1(
        1,
        block0.hash(),
        { LedgerRecord::fromValidationWorkRecord(makeWork("drain-1"), kTimestamp + 1) },
        kTimestamp + 1
    );
...
int main() {
    try {
        ...
        std::cerr << "Nodo ParallelBlockSync tests failed: "
                  << error.what()
                  << "\n";
        return 1;
    }
