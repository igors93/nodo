...
int main() {
    ...
    assert(!meshA.handshakeReplayGuard().outstandingChallenge(
        "node-b", 999
    ).has_value());
    ...
    assert(meshB.inbox().countForType(
    ...
    assert(transportB.send(p2p::TransportMessage(
        "node-b", "node-a", replayWrappedHello, 1003
    )).success());
