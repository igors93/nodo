...
void testStakingBondAndSelection() {
  ...
  requireCondition(manager.getValidatorStake("val1") == 1000,
                   "Val1 stake should be 1000");
  requireCondition(manager.getValidatorStake("val2") == 2000,
                   "Val2 stake should be 2000");

  auto activeSet = manager.getActiveValidatorSet(2);
  requireCondition(activeSet.size() == 2, "Active set size should be 2");
  requireCondition(activeSet[0] == "val2",
                   "Val2 should be first (highest stake)");
...
void testStakingUnbonding() {
  ...
  requireCondition(manager.initiateUnbond("val1", 400, 100),
                   "Initiate unbond of 400 at block 100");
  requireCondition(manager.getValidatorStake("val1") == 600,
                   "Val1 stake should reduce to 600");
...
void testJailingAndSlashing() {
  ...
  requireCondition(manager.isJailed("val1", 120),
                   "Val1 should be jailed at block 120");
  requireCondition(!manager.isJailed("val1", 160),
                   "Val1 should not be jailed at block 160");
  ...
  requireCondition(activeAfterJail.size() == 2,
                   "Both validators should be active after jail expiry");
  requireCondition(activeAfterJail[0] == "val1",
                   "Val1 should return as highest-stake validator");
  requireCondition(activeAfterJail[1] == "val2",
                   "Val2 should remain in active set");

  // Slash val2 by 50%
  manager.slashValidator("val2", 0.5, 100);
  requireCondition(manager.getValidatorStake("val2") == 250,
                   "Val2 stake should be slashed to 250");
  requireCondition(manager.isJailed("val2", 150),
                   "Slashed val2 should be jailed");
}
