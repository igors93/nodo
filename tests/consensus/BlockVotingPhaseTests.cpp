...
config::GenesisConfig minimalGenesis() {
  return config::GenesisConfig(
      config::NetworkParameters::developmentLocal(), kTs,
      {config::BootstrapValidatorConfig(
          crypto::KeyPair::createDeterministicBls12381KeyPair("vote-val")
              .publicKey(),
          1, 1, "vote-val-meta")},
      {test::fundedConsensusTestAccount(userKey())}, "block-voting-phase-test");
}
...
struct TestGossipMesh {
  ...
  TestGossipMesh(const std::string &nodeId, const std::string &genesisId)
      : bus(std::make_shared<p2p::LoopbackTransportBus>()), transport(bus),
        mesh(p2p::GossipMeshConfig(nodeId, "localnet", "chain-localnet", "1",
...
core::Block produceCandidate(node::NodeRuntime &runtime) {
  ...
  if (!candidate.produced()) {
    throw std::runtime_error("produceCandidate: production failed: " +
                             candidate.reason());
  }
...
void testPrevoteIsAcceptedAndRecordedInVotePool() {
  ...
  const consensus::VoteCastResult result =
      consensus::BlockVotingPhase::castPrevote(runtime, block, 1, kTs + 3,
                                               signer, tgm.mesh);
...
void testPrecommitIsAcceptedAndRecordedInVotePool() {
  ...
  // First cast prevote.
  consensus::BlockVotingPhase::castPrevote(runtime, block, 1, kTs + 3, signer,
                                           tgm.mesh);

  // Then cast precommit.
  const consensus::VoteCastResult result =
      consensus::BlockVotingPhase::castPrecommit(runtime, block, 1, kTs + 4,
                                                 signer, tgm.mesh);
  ...
  require(foundPrecommit,
          "VotePool must contain a PRECOMMIT after castPrecommit.");
}
