...
config::GenesisConfig genesisConfig() {
  return config::GenesisConfig(
      ...
      {config::BootstrapValidatorConfig(
           validatorKey("slash-validator-a").publicKey(), 1, 1,
           "slash-validator-a"),
       config::BootstrapValidatorConfig(
           validatorKey("slash-validator-b").publicKey(), 1, 1,
           "slash-validator-b")},
      {test::fundedConsensusTestAccount(userKey())}, "canonical-slashing-test");
}

node::NodeRuntime startRuntime() {
  const auto started = node::NodeRuntimeFactory::startFromGenesis(
      node::NodeRuntimeConfig(genesisConfig(),
                              p2p::PeerInfo("slashing-peer", "127.0.0.1:29991",
  ...
  TestGossipMesh()
      : bus(std::make_shared<p2p::LoopbackTransportBus>()), transport(bus),
        mesh(p2p::GossipMeshConfig(
                 "slashing-peer", "localnet", "chain-localnet", "1",
                 genesisConfig().deterministicId(), 60, 4, 100, 50),
             transport) {}
};

consensus::BlockFinalizationPhaseResult finalize(node::NodeRuntime &runtime,
                                                 const core::Block &block,
                                                 std::int64_t timestamp) {
  TestGossipMesh gossip;
  const std::vector<std::string> validatorSeeds = {"slash-validator-a",
                                                   "slash-validator-b"};
  for (const std::string &seed : validatorSeeds) {
    ...
    require(consensus::BlockVotingPhase::castPrevote(
                runtime, block, 1, timestamp, validatorSigner, gossip.mesh)
                .cast(),
            "Historical validator prevote must be accepted.");
    require(consensus::BlockVotingPhase::castPrecommit(
                runtime, block, 1, timestamp + 1, validatorSigner, gossip.mesh)
                .cast(),
  ...
  return consensus::BlockFinalizationPhase::tryFinalize(
      ...
      context.policy(), context.signatureProvider(), timestamp + 2);
}
...
consensus::DoubleVoteEvidence doubleVoteEvidence() {
  ...
  const auto makeVote = [&](const std::string &blockHash) {
    return consensus::ValidatorVoteRecord::createVote(
        ...
        1, blockHash, "genesis-parent-hash", 1,
        consensus::ValidatorVoteDecision::PRECOMMIT,
        "canonical-slashing-reason", kTimestamp + 10, provider);
  };

  return consensus::DoubleVoteEvidence(makeVote("conflicting-block-a"),
...
void testEvidenceIsFinalizedByTheNextBlock() {
  ...
  require(finalize(runtime, firstCandidate.block(), kTimestamp + 3).finalized(),
  ...
  require(restored.hash() == evidenceCandidate.block().hash(),
          "The evidence block must survive canonical serialization.");

  require(
      finalize(runtime, evidenceCandidate.block(), kTimestamp + 21).finalized(),
      "The evidence block must finalize.");

  const std::string offender =
      validatorKey("slash-validator-a").address().value();
  require(
      runtime.validatorPenaltyLedger().containsEvidence(evidence.evidenceId()),
  ...
  require(offenderEntry != nullptr && offenderEntry->jailed(),
  ...
  require(runtime.stakingRegistry().activeStakeFor(offender).rawUnits() <
          ...
          "The offending validator active stake must drop below the consensus "
          "minimum.");
  require(runtime.validatorRegistry().activeCount() == 1,
  ...
  require(rebuiltRoot == runtime.blockchain().latestBlock().stateRoot(),
          "Slashing state must be committed by the finalized state root.");

  const auto duplicate = consensus::BlockProductionPhase::produce(
      runtime, node::RuntimeBlockPipelineConfig(10, 0, 1, kTimestamp + 30),
      {evidence});
  require(!duplicate.produced(),
...
int main() {
  try {
    ...
    std::cerr << "Canonical slashing pipeline tests FAILED: " << error.what()
              << "\n";
    return 1;
  }
}
