...
void testNetworkParametersAreValidAndDeterministic() {
    ...
    requireCondition(
        !params.deterministicId().empty(),
        "Valid network parameters should have deterministic id."
    );

    const NetworkParameters invalid(
        "bad chain id with spaces",
        "nodo",
        "nodo/0.1",
        60,
        1,
    ...
    requireCondition(
        !invalid.isValid(),
        "Unsafe chain id should be invalid."
    );
}
...
void testGenesisConfigBuildsBlockchainAndValidatorRegistry() {
    ...
    requireCondition(
        config.isValid(),
        "Valid genesis config should pass audit."
    );

    requireCondition(
        !config.deterministicId().empty(),
        "Valid genesis config should have deterministic id."
    );

    const auto result =
        GenesisBuilder::build(config);
    ...
    requireCondition(
        result.blockchain().size() == 1U,
        "Genesis blockchain should have exactly one block."
    );

    requireCondition(
        result.validatorRegistry().activeCount() == 2U,
...
void testGenesisRejectsTooFewValidators() {
    ...
    requireCondition(
        GenesisBuilder::build(config).status() == GenesisBuildStatus::INVALID_CONFIG,
...
int main() {
    try {
        ...
        std::cerr << "Nodo genesis config tests failed: "
                  << error.what()
                  << "\n";
        return 1;
    }
