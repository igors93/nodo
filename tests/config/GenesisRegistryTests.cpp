...
void testRegisteredGenesisId() {
    const std::string localnetId = GenesisRegistry::registeredGenesisId("localnet");
    assert(!localnetId.empty());

    const std::string testnetId = GenesisRegistry::registeredGenesisId("testnet-candidate");
...
void testGenesisIsDeterministic() {
    ...
    assert(first.genesis().deterministicId() == second.genesis().deterministicId());
}
