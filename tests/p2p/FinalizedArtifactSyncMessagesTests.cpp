...
void testAnnouncementValidFields() {
  FinalizedArtifactAnnouncement ann(
      "node-a", "nodo-localnet", "nodo-localnet-1", "genesis-id-v1", 42,
      "block-hash-42", "block-hash-41", "digest-abc123", 1000);
  assert(ann.isValid());
...
void testAnnouncementInvalidIfMissingGenesisId() {
  FinalizedArtifactAnnouncement ann("node-a", "nodo-localnet",
                                    "nodo-localnet-1", "", 1, "hash",
                                    "prev-hash", "digest", 1000);
  assert(!ann.isValid());
}

void testRequestValidFields() {
  FinalizedArtifactRequest req("node-requester", "nodo-localnet",
                               "nodo-localnet-1", "genesis-id-v1", 42,
                               "block-hash-42", "artifact-digest-42", 2000);
  assert(req.isValid());
...
void testRequestInvalidIfMissingGenesisId() {
  FinalizedArtifactRequest req("node-a", "net", "chain", "", 1, "hash",
                               "digest", 1000);
  assert(!req.isValid());
}

void testResponseWithArtifact() {
  FinalizedArtifactResponse resp = FinalizedArtifactResponse::withArtifact(
      "node-responder", "nodo-localnet", "nodo-localnet-1", "genesis-id-v1", 42,
      "block-hash-42", "artifact-digest-42", "RAW_ARTIFACT_DATA", 3000);
...
void testResponseRejected() {
  FinalizedArtifactResponse resp = FinalizedArtifactResponse::rejected(
      "node-responder", FinalizedArtifactResponseStatus::GENESIS_MISMATCH,
      "Genesis id does not match local genesis.", 3000);
  assert(!resp.hasArtifact());
...
void testStatusToString() {
  ...
  assert(finalizedArtifactResponseStatusToString(
         ...
         "GENESIS_MISMATCH");
  assert(finalizedArtifactResponseStatusToString(
             FinalizedArtifactResponseStatus::NETWORK_MISMATCH) ==
         "NETWORK_MISMATCH");
}

void testAnnouncementSerializeContainsGenesisId() {
  FinalizedArtifactAnnouncement ann("node", "net", "chain", "my-genesis-id", 1,
                                    "hash", "prev", "digest", 100);
  const std::string s = ann.serialize();
  assert(s.find("my-genesis-id") != std::string::npos);
}

void testRequestSerializeContainsGenesisId() {
  FinalizedArtifactRequest req("req", "net", "chain", "my-genesis-id", 1,
                               "hash", "digest", 100);
