...
void testParseFromLines() {
  const std::vector<std::string> lines = {"# comment line", "127.0.0.1:9000",
                                          "192.168.1.2:9001", "invalid-no-port",
                                          ""};
...
void testParseInvalidPortIgnored() {
  const auto peers =
      nodo::p2p::BootstrapPeerList::parseFromLines({"host:notaport"});
  assert(peers.empty());
}

void testParsePortWithTrailingTextIgnored() {
  const auto peers =
      nodo::p2p::BootstrapPeerList::parseFromLines({"host:9000abc"});
  assert(peers.empty());
}
