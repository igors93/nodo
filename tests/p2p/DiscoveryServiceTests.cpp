...
void testUdpDiscovery() {
    ...
    nodeB.registerPeerDiscoveredCallback([&](const std::string& peerId, const std::string& host, std::uint16_t tcpPort) {
    ...
    std::vector<std::pair<std::string, std::pair<std::string, std::uint16_t>>> seedPeers = {
        {"node-b", {"127.0.0.1", nodeB.localUdpPort()}}
    };
