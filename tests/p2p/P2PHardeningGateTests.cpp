...
PeerMetadata makePeer(const std::string &nodeId, const std::string &host,
                      std::uint16_t port) {
  return PeerMetadata(nodeId, PeerEndpoint(host, port), "fingerprint-" + nodeId,
                      1000, 1000, 0, false);
}
...
GossipMeshConfig hardenedConfig(const std::string &nodeId) {
  ...
  return GossipMeshConfig(nodeId, "localnet", "chain-localnet", "1",
                          "test-genesis-v1", 60, 2, true, true, eclipseConfig,
                          100, 50);
}
...
void testNonHandshakeMessageRequiresAuthenticatedSession() {
  ...
  const GossipDeliveryReport queued =
      meshA.sendTo("node-b", NetworkMessageType::CHAIN_STATUS, "status", 1000);
  assert(queued.acceptedCount() == 0);
  assert(queued.rejectedCount() == 1);

  const NetworkEnvelope envelope("localnet", "chain-localnet", "1",
                                 NetworkMessageType::CHAIN_STATUS, "node-a",
                                 1001, 60, "status");
  assert(transportA.send(TransportMessage("node-a", "node-b", envelope, 1001))
             .sent());
