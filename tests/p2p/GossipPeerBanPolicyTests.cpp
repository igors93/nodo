...
PeerMetadata makePeer(const std::string &nodeId) {
  return PeerMetadata(nodeId, PeerEndpoint("127.0.0.1", 30333),
...
GossipMeshConfig config(const std::string &nodeId) {
  return GossipMeshConfig(nodeId, "localnet", "chain-localnet", "1",
                          "genesis-v1", 60, 2, false, false,
                          EclipseGuardConfig::defaults(), 100, 50, 60);
}
...
int main() {
  ...
  NetworkEnvelope invalid("wrongnet", "chain-localnet", "1",
                          NetworkMessageType::CHAIN_STATUS, "node-b", 1000, 60,
                          "bad-status");
  mesh.reportPeerMisbehavior(invalid, PeerMisbehaviorType::INVALID_MESSAGE,
                             "test invalid message one", 1000);
  assert(!mesh.peerBannedAt("node-b", 1001));
  mesh.reportPeerMisbehavior(invalid, PeerMisbehaviorType::INVALID_MESSAGE,
                             "test invalid message two", 1002);
