...
PeerSubnetInfo makePeer(const std::string &id, const std::string &ip) {
  PeerSubnetInfo peer;
  peer.peerId = id;
  peer.ipAddress = ip;
...
void testAllowsPeerFromNewSubnet() {
  ...
  const std::vector<PeerSubnetInfo> current = {
      makePeer("peer-1", "192.168.1.10"), makePeer("peer-2", "10.0.0.5")};
  ...
  requireCondition(result.outcome() == EclipseCheckOutcome::ALLOWED,
...
void testRejectsWhenSubnetSaturated() {
  ...
  const std::vector<PeerSubnetInfo> current = {
      makePeer("peer-a", "192.168.1.10"), makePeer("peer-b", "192.168.1.20"),
      makePeer("peer-c", "10.0.0.5"),     makePeer("peer-d", "10.0.1.5"),
      makePeer("peer-e", "10.0.2.5"),     makePeer("peer-f", "10.0.3.5"),
      makePeer("peer-g", "10.0.4.5"),     makePeer("peer-h", "10.0.5.5"),
  ...
  requireCondition(!result.isAllowed(),
                   "Peer should be rejected when its subnet already has "
                   "maxPeersPerSubnet peers.");

  requireCondition(result.outcome() ==
...
void testDiversityScoreForAllDistinctSubnets() {
  ...
  requireCondition(score == 1.0, "diversityScore should return 1.0 when all "
                                 "peers are from distinct subnets.");
}
...
void testRecommendEvictionsFromOverRepresentedSubnet() {
  ...
  const std::vector<PeerSubnetInfo> peers = {
      makePeer("over-1", "192.168.1.10"), makePeer("over-2", "192.168.1.20"),
      makePeer("over-3", "192.168.1.30"), makePeer("other-1", "10.0.0.1"),
...
void testExtractSubnetPrefixParsesIPv4() {
  ...
  requireCondition(PeerSubnetInfo::extractSubnetPrefix("255.255.255.0") ==
                       "255.255.255",
                   "extractSubnetPrefix should handle 255.255.255.0.");

  requireCondition(PeerSubnetInfo::extractSubnetPrefix("").empty(),
...
int main() {
  try {
    ...
    std::cerr << "Nodo EclipseGuard tests failed: " << error.what() << "\n";
    return 1;
  }
}
