...
int main() {
    EncryptedPeerSession sender(
        "node-a",
        "node-b",
        "local-testnet-shared-secret",
        100
    );

    EncryptedPeerSession receiver(
        "node-b",
        "node-a",
        "local-testnet-shared-secret",
        100
    ...
    EncryptedPeerChannelFrame frame =
        sender.sealEnvelope(envelope, 121);
    ...
    EncryptedPeerSession freshReceiver(
        "node-b",
        "node-a",
        "local-testnet-shared-secret",
        100
    ...
    assert(tamperedResult.status() == EncryptedPeerChannelStatus::AUTHENTICATION_FAILED);

    const std::string encoded = EncryptedPeerChannelCodec::encodeFrameToString(frame);
    EncryptedPeerChannelFrame decoded = EncryptedPeerChannelCodec::decodeFrameFromString(encoded);
