...
void testValidAnnouncementAccepted() {
    nodo::p2p::FinalizedArtifactAnnouncement ann(
        kSenderNode, kNetworkId, kChainId, kGenesisId,
        5, "hash-5", "hash-4", "digest-5", kTs
    );
    assert(ann.isValid());

    std::string reason;
    const bool ok = nodo::p2p::FinalizedArtifactPayloadParser::validateAnnouncement(
...
void testMalformedAnnouncementRejected() {
    ...
    nodo::p2p::FinalizedArtifactAnnouncement ann(
        "", kNetworkId, kChainId, kGenesisId,
        5, "hash-5", "hash-4", "digest-5", kTs
    );
    assert(!ann.isValid());
...
void testAnnouncementNetworkMismatchRejected() {
    nodo::p2p::FinalizedArtifactAnnouncement ann(
        kSenderNode, "wrong-network", kChainId, kGenesisId,
        5, "hash-5", "hash-4", "digest-5", kTs
    );
...
void testAnnouncementGenesisMismatchRejected() {
    nodo::p2p::FinalizedArtifactAnnouncement ann(
        kSenderNode, kNetworkId, kChainId, "wrong-genesis",
        5, "hash-5", "hash-4", "digest-5", kTs
    );
...
void testValidRequestAccepted() {
    nodo::p2p::FinalizedArtifactRequest req(
        "node-requester", kNetworkId, kChainId, kGenesisId,
...
void testRequestChainMismatchRejected() {
    nodo::p2p::FinalizedArtifactRequest req(
        "node-requester", kNetworkId, "wrong-chain", kGenesisId,
        5, "hash-5", "digest-5", kTs
    );
...
void testResponseGenesisMismatchRejected() {
    nodo::p2p::FinalizedArtifactResponse resp =
        nodo::p2p::FinalizedArtifactResponse::withArtifact(
            kSenderNode, kNetworkId, kChainId, "wrong-genesis",
            5, "hash-5", "digest-5", "raw-content-xyz", kTs
        );
    assert(resp.isValid());
