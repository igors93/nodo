...
int main() {
  ...
  GossipMeshConfig configA("node-a", "localnet", "chain-localnet", "1",
  ...
  assert(meshB.inbox().countForType(NetworkMessageType::TRANSACTION_ANNOUNCE) ==
         1);
  assert(meshB.inbox()
             .messagesForType(NetworkMessageType::TRANSACTION_ANNOUNCE)[0]
             .payload() == "tx-1");

  std::size_t queuedFlood = 0;
  for (int index = 0; index < 100; ++index) {
    const GossipDeliveryReport queuedMessage =
        meshA.broadcast(NetworkMessageType::TRANSACTION_ANNOUNCE,
                        "tx-flood-" + std::to_string(index), 1010);
    queuedFlood += queuedMessage.acceptedCount();
  }

  assert(queuedFlood == 100);

  const GossipDeliveryReport sentFlood = meshA.flushOutbound(1010);

  assert(sentFlood.acceptedCount() == 100);

  const GossipDeliveryReport receivedFlood = meshB.receiveAvailable(1010);
