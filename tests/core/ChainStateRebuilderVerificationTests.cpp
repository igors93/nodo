...
crypto::KeyPair keyPairFor(const std::string &accountName) {
  return crypto::KeyPair::createDeterministicEd25519KeyPair("rebuilder-test-" +
                                                            accountName);
}
...
core::Transaction makeTransfer(const std::string &fromAccountName,
                               const std::string &to, std::uint64_t nonce,
                               std::int64_t feeRaw = 1000) {
  const crypto::KeyPair kp = keyPairFor(fromAccountName);
  const crypto::Ed25519SignatureProvider provider;

  core::Transaction tx(core::TransactionType::TRANSFER,
                       addressFor(fromAccountName), to,
                       utils::Amount::fromRawUnits(50000),
                       utils::Amount::fromRawUnits(feeRaw), nonce, kTimestamp);
...
authoritativeContextFor(const std::string &senderAccountName,
                        std::uint64_t nonce = 0, std::int64_t minFee = 1000) {
  core::AccountStateView view;
  view.putAccount(core::AccountState(addressFor(senderAccountName),
                                     utils::Amount::fromRawUnits(100000000),
                                     nonce));
  return core::StateTransitionPreviewContext(
      minFee, view, false, true, "", 0, kChainId, kNetworkName, {}, {},
      []() { return std::make_unique<NoopProtocolDomainExecutor>(); }, true);
}

// Build a block with correct stateRoot/receiptsRoot using
// StateTransitionEngine.
core::Block blockWithRealRoots(const core::Blockchain &chain,
                               const core::Transaction &tx,
                               const core::StateTransitionPreviewContext &ctx,
                               std::uint64_t height, std::int64_t ts) {
  const core::LedgerRecord ledgerRecord = makeRecord(tx);
  const core::Block draft(height, chain.latestBlock().hash(), {ledgerRecord},
                          ts, "", "");
  ...
  if (!preview.accepted()) {
    throw std::runtime_error("blockWithRealRoots: engine failed: " +
                             preview.reason());
  }

  return core::Block(height, chain.latestBlock().hash(), {ledgerRecord}, ts,
                     preview.stateRoot(), preview.receiptsRoot());
}

// Build a minimal valid genesis LedgerRecord (blocks require at least one
// record).
...
void testGenesisOnlyChainPassesVerification() {
  ...
  const StateRebuildReport report =
      ChainStateRebuilder::rebuildAndVerifyViaEngine(
          chain, [](const core::Blockchain &) {
            return authoritativeContextFor("nobody");
          });

  requireCondition(report.success(),
                   "Genesis-only chain must pass: " + report.failureReason());
  requireCondition(report.commitmentVerificationPassed(),
                   "Genesis-only chain commitment verification must pass.");
  requireCondition(report.blockCount() == 1,
...
void testTwoBlockChainPassesVerification() {
  ...
  requireCondition(report.success(),
                   "Two-block chain with valid commitments must pass: " +
                       report.failureReason());
  requireCondition(report.commitmentVerificationPassed(),
                   "Two-block chain commitment verification must pass.");
  requireCondition(report.blockCount() == 2,
...
void testBlockWithTamperedStateRootFails() {
  ...
  const core::Block tamperedBlock1(
      block1.index(), block1.previousHash(), block1.records(),
      block1.timestamp(),
  ...
  const StateRebuildReport report =
      ChainStateRebuilder::rebuildAndVerifyViaEngine(
          tamperedChain, [](const core::Blockchain &) {
            return authoritativeContextFor("alice", 0, 1000);
          });
  ...
  requireCondition(!report.commitmentFailureReason().empty(),
...
void testBlockWithTamperedReceiptsRootFails() {
  ...
  const core::Block tamperedBlock1(
      block1.index(), block1.previousHash(), block1.records(),
      block1.timestamp(), block1.stateRoot(),
      "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef");
  ...
  requireCondition(report.firstFailedCommitmentHeight() == 1,
...
void testReportSerializesNewFields() {
  ...
  requireCondition(serialized.find("commitmentVerificationPassed=true") !=
                       std::string::npos,
                   "Serialized report must contain "
                   "commitmentVerificationPassed=true for genesis chain.");
  requireCondition(
...
void testAuditBlockchainDoesNotSetCommitmentFields() {
  ...
  // auditBlockchain does NOT call the engine — commitment fields should be
  // defaults.
  ...
  requireCondition(!report.commitmentVerificationPassed(),
                   "auditBlockchain must NOT set commitmentVerificationPassed "
                   "(no engine call).");
}
...
int main() {
  try {
    ...
    std::cout << "PASS testAuditBlockchainDoesNotSetCommitmentFields"
              << std::endl;

    std::cout << "All ChainStateRebuilderVerificationTests passed."
              << std::endl;
    return 0;
