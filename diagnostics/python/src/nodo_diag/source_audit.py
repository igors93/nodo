...
def extract_persisted_fields(finalized_store_text: str) -> set[str]:
    ...
    for match in re.finditer(
        r'fields\.emplace_back\(\s*prefix\s*\+\s*"([^"]+)"', finalized_store_text
    ):
        fields.add("*." + match.group(1))
...
def extract_allowed_fields(runtime_loader_text: str) -> set[str]:
    ...
    for match in re.finditer(
        r'allowedFields\.insert\(\s*prefix\s*\+\s*"([^"]+)"', runtime_loader_text
    ):
        fields.add("*." + match.group(1))
...
def extract_canonical_fields(runtime_loader_text: str) -> set[str]:
    ...
    for match in re.finditer(
        r'canonicalFields\.emplace_back\(\s*prefix\s*\+\s*"([^"]+)"',
        runtime_loader_text,
    ):
        fields.add("*." + match.group(1))
    ...
    for match in re.finditer(
        r'fields\.emplace_back\(\s*prefix\s*\+\s*"([^"]+)"', runtime_loader_text
    ):
        fields.add("*." + match.group(1))
