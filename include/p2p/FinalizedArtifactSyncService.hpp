...
public:
  ...
  static FinalizedArtifactSyncResult
  accepted(node::FinalizedArtifactImportResult importResult);

  static FinalizedArtifactSyncResult
  rejected(FinalizedArtifactSyncRejectionReason reason, std::string detail);
...
public:
  ...
  static bool validateAnnouncement(
      ...
      const std::string &localNetworkId, const std::string &localChainId,
      const std::string &localGenesisId, std::string &outRejectionReason);

  // Returns false if the request is structurally invalid.
  static bool validateRequest(const FinalizedArtifactRequest &request,
  ...
  static bool validateResponse(const FinalizedArtifactResponse &response,
                               const std::string &localNetworkId,
                               const std::string &localChainId,
                               const std::string &localGenesisId,
 ...
 * Rejected sync input is recorded via the rejection reason without mutating
 * state.
 */
...
public:
  ...
  static FinalizedArtifactSyncResult processResponse(
      ...
      const std::string &localNetworkId, const std::string &localChainId,
      const std::string &localGenesisId, const std::string &expectedDigest,
      const node::NodeDataDirectoryConfig &targetDir,
      node::NodeRuntime &runtime, const config::GenesisConfig &genesisConfig,
