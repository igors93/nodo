...
public:
  explicit PeerReputation(std::int32_t threshold = 50,
  ...
  void reportBehavior(const std::string &nodeId, const std::string &ipAddress,
                      std::int32_t delta);

  void reportBehavior(const std::string &nodeId, const std::string &ipAddress,
                      std::int32_t delta, std::int64_t now,
