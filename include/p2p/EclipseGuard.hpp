...
struct EclipseGuardConfig {
    ...
    double maxSingleSubnetFraction;         // max fraction from one subnet (default: 0.25)
...
public:
    ...
    static EclipseCheckResult rejected(EclipseCheckOutcome reason, std::string detail);
...
private:
    ...
    EclipseCheckResult(EclipseCheckOutcome outcome, std::string detail, bool allowed);
};
...
class EclipseGuard {
public:
    explicit EclipseGuard(EclipseGuardConfig config = EclipseGuardConfig::defaults());

    // Check if a new peer can be admitted given the current peer set.
    EclipseCheckResult checkAdmission(
    ...
    std::vector<std::string> recommendEvictions(
        const std::vector<PeerSubnetInfo>& currentPeers,
    ...
    std::map<std::string, std::size_t> subnetCounts(
