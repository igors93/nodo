 ...
 *
 * Important: this class deliberately lives above Transport and below
 * GossipMesh. Consensus code should never know whether the underlying delivery
 * is loopback, TCP, encrypted TCP, QUIC or something else.
 */
class EncryptedPeerChannelFrame {
public:
  EncryptedPeerChannelFrame();

  EncryptedPeerChannelFrame(std::string sessionId, std::string fromNodeId,
                            std::string toNodeId, std::uint64_t sequence,
                            std::int64_t createdAt, std::string nonceHex,
...
std::string
encryptedPeerChannelStatusToString(EncryptedPeerChannelStatus status);

class EncryptedPeerOpenResult {
public:
  EncryptedPeerOpenResult();

  EncryptedPeerOpenResult(EncryptedPeerChannelStatus status, std::string reason,
...
public:
  ...
  EncryptedPeerSession(std::string localNodeId, std::string remoteNodeId,
                       std::string sharedSecret, std::int64_t establishedAt);
  ...
  EncryptedPeerChannelFrame sealEnvelope(const NetworkEnvelope &envelope,
                                         std::int64_t now);

  EncryptedPeerOpenResult openFrame(const EncryptedPeerChannelFrame &frame);

  std::string serialize() const;

  static std::string deriveSessionId(const std::string &leftNodeId,
...
public:
  static constexpr std::size_t MAX_ENCRYPTED_FRAME_BYTES =
      core::ProtocolLimits::MAX_TRANSPORT_FRAME_BYTES;

  static std::vector<unsigned char>
  encodeFrame(const EncryptedPeerChannelFrame &frame);

  static EncryptedPeerChannelFrame
  decodeFrame(const std::vector<unsigned char> &bytes);

  static std::string
  encodeFrameToString(const EncryptedPeerChannelFrame &frame);

  static EncryptedPeerChannelFrame
  decodeFrameFromString(const std::string &bytes);

  static bool isValidFrameBytes(const std::vector<unsigned char> &bytes);
};
