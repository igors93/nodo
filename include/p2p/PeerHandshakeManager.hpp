...
public:
  ...
  PeerChallengeMessage(std::string challengerNodeId,
                       std::string challengedNodeId, std::string nonce,
                       std::string ephemeralPublicKeyHex,
                       std::int64_t createdAt, std::uint32_t ttlSeconds);
...
public:
  ...
  PeerHelloMessage(PeerMetadata peer, std::string networkId,
                   std::string chainId, std::string protocolVersion,
                   std::string genesisId, node::ChainStatusMessage chainStatus,
...
enum class PeerHandshakeStatus { ACCEPTED, REJECTED };
...
public:
  static NetworkEnvelope
  createChallengeEnvelope(const GossipMeshConfig &config,
                          const std::string &challengedNodeId,
                          const std::string &nonce, std::int64_t now);

  static NetworkEnvelope createChallengeEnvelope(
      const GossipMeshConfig &config, const std::string &challengedNodeId,
      const std::string &nonce, const std::string &ephemeralPublicKeyHex,
      std::int64_t now);

  static std::optional<PeerChallengeMessage>
  challengeFromEnvelope(const GossipMeshConfig &config,
                        const NetworkEnvelope &envelope, std::int64_t now);

  static NetworkEnvelope
  createHelloEnvelope(const GossipMeshConfig &config,
  ...
  static PeerHandshakeResult
  validateHello(const GossipMeshConfig &config, const NetworkEnvelope &envelope,
                const std::string &expectedChallengeNonce, std::int64_t now);
  ...
  static std::optional<PeerMetadata>
  peerMetadataFromHello(const NetworkEnvelope &envelope, std::int64_t now);

  static std::string challengeNonceFromHello(const NetworkEnvelope &envelope);

  static std::string
  ephemeralPublicKeyFromHello(const NetworkEnvelope &envelope);
};
