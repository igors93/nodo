...
public:
  ...
  TcpIpRateLimitPolicy(std::size_t bucketCapacity, std::size_t refillTokens,
...
enum class TcpConnectionDirection { INBOUND, OUTBOUND };
...
public:
  ...
  TcpConnectionSlotPolicy(std::size_t maxTotal, std::size_t maxInbound,
                          std::size_t maxOutbound, std::size_t maxPerIp,
...
public:
  ...
  TcpCandidatePolicy();
  TcpCandidatePolicy(std::size_t maxTotal, std::size_t maxPerIp,
...
public:
  ...
  TransportResult bind(const std::string &localNodeId,
                       const PeerEndpoint &endpoint);

  TransportResult bind(const std::string &localNodeId, const std::string &host,
                       std::uint16_t port);
  ...
  void registerPeerEndpoint(const std::string &remoteNodeId,
                            const PeerEndpoint &endpoint);

  bool hasPeerEndpoint(const std::string &remoteNodeId) const;
  ...
  std::size_t pendingCandidateCountForIp(const std::string &remoteIp) const;
  std::size_t
  pendingCandidateCountForSubnet(const std::string &subnetPrefix) const;
  ...
  ipBackoffRemaining(const std::string &remoteIp) const;
  std::chrono::milliseconds
  subnetBackoffRemaining(const std::string &subnetPrefix) const;
  ...
  bool authenticateConnection(TransportConnectionId connectionId,
                              const std::string &remoteNodeId) override;

  bool rejectConnection(TransportConnectionId connectionId) override;

  bool
  isConnectionAuthenticated(TransportConnectionId connectionId,
                            const std::string &remoteNodeId) const override;

  void setIpQuarantineCheck(std::function<bool(const std::string &)> check);
...
private:
  ...
  PollFdResult pollFd(SocketHandle fd, bool unidentified);

  void rememberConnection(const std::string &remoteNodeId, SocketHandle fd,
                          bool authenticated, TcpConnectionDirection direction,
                          std::string remoteIp);

  TransportConnectionId nextConnectionId();
  SocketHandle socketForSend(const TransportMessage &message) const;
  void closeCandidateConnection(TransportConnectionId connectionId);
  void
  pruneExpiredCandidateConnections(std::chrono::steady_clock::time_point now);
  void decrementCandidateIpCount(const std::string &remoteIp);
  void decrementCandidateSubnetCount(const std::string &subnetPrefix);
  bool consumeIpAdmissionToken(const std::string &remoteIp,
                               std::chrono::steady_clock::time_point now);
  bool consumeSubnetAdmissionToken(const std::string &subnetPrefix,
                                   std::chrono::steady_clock::time_point now);
  ...
  void pruneIdleIpAdmissionStates(std::chrono::steady_clock::time_point now);
  std::chrono::milliseconds backoffForLimitHits(std::size_t limitHits) const;

  bool ensureConnectionSlotFor(const std::string &remoteNodeId,
                               const std::string &remoteIp,
                               TcpConnectionDirection direction,
                               std::chrono::steady_clock::time_point now);
  std::optional<std::string>
  evictionCandidateFor(const std::string &remoteNodeId,
                       const std::string &remoteIp,
                       TcpConnectionDirection direction) const;
  std::size_t connectedDirectionCount(TcpConnectionDirection direction) const;
