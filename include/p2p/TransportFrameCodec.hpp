...
public:
  ...
  encodeFrame(const NetworkEnvelope &envelope);

  static NetworkEnvelope decodeFrame(const std::vector<unsigned char> &frame);

  static bool isValidFrame(const std::vector<unsigned char> &frame);
};
