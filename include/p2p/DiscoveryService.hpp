 ...
 * It manages peer routing tables using XOR distances and answers dynamic peer
 * searches.
 */
class DiscoveryService {
public:
  DiscoveryService(const std::string &localNodeId, std::uint16_t udpPort,
                   std::uint16_t tcpPort);
  ~DiscoveryService();
  ...
  void addPeer(const std::string &peerId, const std::string &host,
               std::uint16_t tcpPort, std::uint16_t udpPort);

  std::vector<DiscoveryPeerInfo> findClosestPeers(const std::string &targetId,
                                                  std::size_t count);

  void bootstrap(const std::vector<
                 std::pair<std::string, std::pair<std::string, std::uint16_t>>>
                     &seedPeers);
...
private:
  ...
  void sendFindNode(const asio::ip::udp::endpoint &endpoint,
                    const std::string &targetId);
  void sendNeighbors(const asio::ip::udp::endpoint &endpoint,
                     const std::vector<DiscoveryPeerInfo> &neighbors);
  ...
  xorDist(const std::array<unsigned char, 32> &a,
          const std::array<unsigned char, 32> &b);
  static std::size_t
  prefixLength(const std::array<unsigned char, 32> &distance);
};
