...
public:
  ...
  TransportResult deliver(const TransportMessage &message);

  std::optional<TransportMessage> poll(const std::string &localNodeId);

  std::size_t queuedMessageCount(const std::string &nodeId) const;
...
private:
  ...
  static std::string connectionKey(const std::string &localNodeId,
                                   const std::string &remoteNodeId);
...
public:
  ...
  explicit LoopbackTransport(std::shared_ptr<LoopbackTransportBus> bus);
