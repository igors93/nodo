...
public:
   ...
   */
  using BlockProducerCallback = std::function<std::optional<core::Block>(
  ...
  void
  setDataDirectoryConfig(const node::NodeDataDirectoryConfig *directoryConfig);
...
private:
  ...
  void broadcastSlashingEvidence(const DoubleVoteEvidence &evidence,
                                 std::int64_t now);

  void broadcastSlashingEvidence(const ProposerEquivocationEvidence &evidence,
                                 std::int64_t now);
  ...
  bool persistedVoteMatchesCandidate(const ValidatorVoteRecord &vote,
                                     const core::Block &candidate,
                                     std::uint64_t round,
                                     ValidatorVoteDecision decision) const;

  void rebroadcastPersistedVotesForCandidate(const core::Block &candidate,
                                             std::uint64_t round,
                                             std::int64_t now,
                                             ConsensusTickResult &result);
