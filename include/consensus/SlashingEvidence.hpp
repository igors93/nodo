...
enum class SlashingEvidenceSeverity { UNKNOWN, WARNING, SLASHABLE };

std::string slashingEvidenceSeverityToString(SlashingEvidenceSeverity severity);
SlashingEvidenceSeverity
slashingEvidenceSeverityFromString(const std::string &value);

enum class SlashingEvidenceValidationStatus { ACCEPTED, DUPLICATE, REJECTED };
...
public:
  ...
  SlashingEvidenceRecord(std::string evidenceId, SlashingEvidenceType type,
                         std::string validatorAddress, std::string payloadHash,
...
public:
  ...
  DoubleVoteEvidence(ValidatorVoteRecord firstVote,
                     ValidatorVoteRecord secondVote, std::int64_t detectedAt);
...
public:
  ...
  ProposerEquivocationEvidence(std::string firstProposal,
  ...
  static ProposerEquivocationEvidence
...
public:
  ...
  SlashingEvidenceValidationResult(SlashingEvidenceValidationStatus status,
                                   std::string reason,
...
public:
  static SlashingEvidenceValidationResult
  verifyDoubleVoteEvidence(const DoubleVoteEvidence &evidence,
                           const crypto::CryptoPolicy &policy,
                           const crypto::SignatureProvider &provider);

  static SlashingEvidenceValidationResult
  validateDoubleVoteStructure(const DoubleVoteEvidence &evidence);
  ...
  static SlashingEvidenceValidationResult verifyDoubleVoteEvidenceForHistory(
      const DoubleVoteEvidence &evidence, std::uint64_t maximumOffenseHeight,
      const core::ValidatorSetHistory &validatorSetHistory,
      const crypto::CryptoPolicy &policy,
      const crypto::SignatureProvider &provider);

private:
  static bool bothVotesVerify(const DoubleVoteEvidence &evidence,
                              const crypto::CryptoPolicy &policy,
                              const crypto::SignatureProvider &provider);
};
