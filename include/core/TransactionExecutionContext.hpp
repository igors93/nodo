...
public:
  static TransactionDomainExecutionResult
  accepted(AccountStateView accounts,
           std::map<std::string, std::string> domains);
  static TransactionDomainExecutionResult rejected(std::string code,
                                                   std::string reason);
...
public:
  ...
  virtual TransactionDomainExecutionResult applyBurn(const Transaction &,
                                                     const AccountStateView &,
                                                     std::uint64_t,
                                                     std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyStakeDeposit(const Transaction &, const AccountStateView &,
                    std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyStakeUnlock(const Transaction &, const AccountStateView &, std::uint64_t,
                   std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyStakeWithdraw(const Transaction &, const AccountStateView &,
                     std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyStakeTopUp(const Transaction &, const AccountStateView &, std::uint64_t,
                  std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyValidatorRegister(const Transaction &, const AccountStateView &,
                         std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyValidatorExitRequest(const Transaction &, const AccountStateView &,
                            std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  ...
  applyGovernanceProposal(const Transaction &, const AccountStateView &,
                          std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyGovernanceVote(const Transaction &, const AccountStateView &,
                      std::uint64_t, std::int64_t) = 0;
  virtual TransactionDomainExecutionResult
  applyGovernanceExecute(const Transaction &, const AccountStateView &,
                         std::uint64_t, std::int64_t) = 0;

  virtual TransactionDomainExecutionResult
  finalizeBlock(const AccountStateView &accounts, utils::Amount totalFee,
                const std::vector<LedgerRecord> &protocolRecords,
                std::uint64_t blockHeight, std::int64_t blockTimestamp) = 0;
...
using TransactionDomainExecutorFactory =
    std::function<std::unique_ptr<TransactionDomainExecutor>()>;
