...
public:
  ...
  ValidatorRegistrationPayload(crypto::PublicKey validatorPublicKey,
  ...
  static ValidatorRegistrationPayload
...
class ValidatorKeyRotationPayload {
public:
  ValidatorKeyRotationPayload();
  ValidatorKeyRotationPayload(std::string oldValidatorAddress,
                              crypto::PublicKey newValidatorPublicKey,
                              std::string metadataHash,
                              std::uint64_t activationEpoch,
                              std::string reasonHash);

  const std::string &oldValidatorAddress() const;
  const crypto::PublicKey &newValidatorPublicKey() const;
  const std::string &metadataHash() const;
  std::uint64_t activationEpoch() const;
  const std::string &reasonHash() const;

  std::string newValidatorAddress() const;
  bool isValid() const;
  std::string serialize() const;
  static ValidatorKeyRotationPayload deserialize(const std::string &serialized);

private:
  std::string m_oldValidatorAddress;
  crypto::PublicKey m_newValidatorPublicKey;
  std::string m_metadataHash;
  std::uint64_t m_activationEpoch;
  std::string m_reasonHash;
};

enum class GovernanceProposalType { PARAMETER_CHANGE, TREASURY_SPEND, TEXT };
...
public:
  ...
  static GovernanceProposalPayload parameterChange(
      ...
      std::uint64_t votingPeriodBlocks = 3, std::uint64_t quorumNumerator = 1,
      std::uint64_t quorumDenominator = 2, std::uint64_t approvalNumerator = 2,
  ...
  static GovernanceProposalPayload treasurySpend(
      ...
      std::int64_t amountRaw, std::uint64_t votingStartDelayBlocks = 1,
      std::uint64_t votingPeriodBlocks = 3, std::uint64_t quorumNumerator = 1,
      std::uint64_t quorumDenominator = 2, std::uint64_t approvalNumerator = 2,
      std::uint64_t approvalDenominator = 3);

  static GovernanceProposalPayload
  text(std::string title, std::string description,
       std::uint64_t votingStartDelayBlocks = 1,
       std::uint64_t votingPeriodBlocks = 3, std::uint64_t quorumNumerator = 1,
       std::uint64_t quorumDenominator = 2, std::uint64_t approvalNumerator = 2,
       std::uint64_t approvalDenominator = 3);
...
enum class GovernanceVoteChoice { YES, NO, ABSTAIN };
...
public:
  ...
  GovernanceVotePayload(std::string proposalId, std::string validatorAddress,
