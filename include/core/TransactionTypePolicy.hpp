...
enum class TransactionAmountRule { ZERO, POSITIVE };

enum class TransactionDebitRule { FEE_ONLY, AMOUNT_PLUS_FEE };

enum class TransactionCreditRule { NONE, TARGET_AMOUNT, SENDER_AMOUNT };
...
public:
  ...
  static bool validateShape(const Transaction &transaction,
                            std::string &reason);
