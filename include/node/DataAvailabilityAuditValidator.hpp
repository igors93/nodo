...
public:
    ...
    static DataAvailabilityAuditResult failed(
...
public:
    ...
    static DataAvailabilityAuditResult validateResponse(
    ...
    static DataAvailabilityAuditResult validateFailureEvidence(
        const DataAvailabilityChallenge& challenge,
        const DataAvailabilityFailureEvidence& evidence
    );

    // Validate that an artifact's recomputed digest matches the given expected digest.
    // Used during reload to detect corrupted artifacts.
    static DataAvailabilityAuditResult validateArtifactDigest(
