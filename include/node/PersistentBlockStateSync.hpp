 ...
 * safe and testable sync plan/apply/persist primitives that the runtime can
 * call after existing block/state validators accept real block data.
 */
enum class PersistentSyncStatus { UNKNOWN, IDLE, SYNCING, COMPLETE, FAILED };
...
public:
  ...
  static PersistentSyncCheckpoint genesis(const std::string &genesisBlockHash,
                                          const std::string &genesisStateRoot,
                                          std::int64_t now);

  static PersistentSyncCheckpoint deserialize(const std::string &serialized);
...
public:
  static PersistentSyncCheckpointReadResult
  loaded(PersistentSyncCheckpoint checkpoint);
...
public:
  ...
  static PersistentSyncCheckpointWriteResult
  invalidCheckpoint(std::string reason);
...
public:
  explicit PersistentSyncCheckpointStore(std::filesystem::path dataDirectory);
  ...
  PersistentSyncCheckpointWriteResult
  save(const PersistentSyncCheckpoint &checkpoint) const;
...
public:
  ...
  PersistentBlockSyncItem(std::uint64_t height, std::string blockHash,
...
public:
  ...
  PersistentBlockSyncBatch(std::string sourcePeerId, std::uint64_t fromHeight,
...
public:
  ...
  PersistentSnapshotSyncManifest(std::string sourcePeerId,
  ...
  static PersistentSnapshotSyncManifest
...
public:
  // Minimum height gap that triggers a snapshot request instead of
  // block-by-block sync.
  static constexpr std::uint64_t SNAPSHOT_GAP_THRESHOLD = 500;

  PersistentSyncPlan();

  PersistentSyncPlan(PersistentSyncPlanStatus status, std::string reason,
                     std::optional<NetworkBlockSyncRequest> blockRequest,
                     std::optional<PersistentSnapshotSyncManifest>
                         snapshotRequest = std::nullopt);
...
public:
  static PersistentSyncPlan
  planFromRemoteStatus(const PersistentSyncCheckpoint &localCheckpoint,
                       const ChainStatusMessage &remoteStatus,
                       const std::string &localNodeId,
                       const std::string &sourcePeerId,
                       std::uint64_t maxBlocksPerRequest, std::int64_t now);
};

enum class PersistentSyncApplyStatus {
  APPLIED,
  REJECTED,
  // The batch only covers blocks this node already finalized (the local tip
  // advanced between the sync request and this response). Nothing was
  // imported, but the peer agrees with the local chain — callers must not
  // count this as a sync failure.
  STALE
};
...
public:
  ...
  PersistentSyncApplyResult(PersistentSyncApplyStatus status,
                            std::string reason,
...
public:
  ...
  static PersistentSyncApplyResult applyValidatedBatch(
      ...
      std::function<
          core::StateTransitionPreviewContext(const core::Blockchain &)>
          contextBuilder,
      std::int64_t now,
...
public:
  ...
  encodeCheckpoint(const PersistentSyncCheckpoint &checkpoint);

  static PersistentSyncCheckpoint
  decodeCheckpoint(const std::vector<unsigned char> &bytes);

  static std::string hashCheckpoint(const PersistentSyncCheckpoint &checkpoint);

  static std::vector<unsigned char>
  encodeBlockSyncItem(const PersistentBlockSyncItem &item);

  static PersistentBlockSyncItem
  decodeBlockSyncItem(const std::vector<unsigned char> &bytes);

  static std::vector<unsigned char>
  encodeBlockSyncBatch(const PersistentBlockSyncBatch &batch);

  static PersistentBlockSyncBatch
  decodeBlockSyncBatch(const std::vector<unsigned char> &bytes);

  static std::string hashBlockSyncBatch(const PersistentBlockSyncBatch &batch);

  static std::vector<unsigned char>
  encodeSnapshotManifest(const PersistentSnapshotSyncManifest &manifest);

  static PersistentSnapshotSyncManifest
  decodeSnapshotManifest(const std::vector<unsigned char> &bytes);

  static std::string
  hashSnapshotManifest(const PersistentSnapshotSyncManifest &manifest);
};
