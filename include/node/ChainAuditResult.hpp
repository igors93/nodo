...
enum class ChainAuditStatus { PASSED, FAILED };

std::string chainAuditStatusToString(ChainAuditStatus status);
...
public:
  ...
  passed(std::string networkName, std::string cryptoProfile,
         std::uint64_t latestHeight, std::string latestHash,
         std::string latestStateRoot, std::size_t loadedBlockCount,
         std::size_t loadedMempoolTransactionCount, std::size_t validatorCount);

  static ChainAuditResult failed(std::string reason);
