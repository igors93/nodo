...
enum class BlockSyncQcMode { STATE_ROOT_ONLY, QC_REQUIRED };

/*
 * BlockSyncHandler responds to BLOCK_REQUEST messages and applies
 * BLOCK_RESPONSE.
 *
 * Protocol:
 *  Requester: sends BLOCK_REQUEST with BlockLocator{fromHeight, maxBlocks}
 *  Responder: reads its blockchain, serializes blocks
 * fromHeight..min(fromHeight+maxBlocks, tip) and sends back a BLOCK_RESPONSE
...
public:
   ...
   */
  static void serveRequests(p2p::GossipMesh &gossip,
   ...
   */
  static p2p::GossipDeliveryReport requestBlocks(p2p::GossipMesh &gossip,
  ...
  static std::size_t applyResponses(
      p2p::GossipMesh &gossip, core::Blockchain &blockchain,
      std::function<
          core::StateTransitionPreviewContext(const core::Blockchain &)>
          contextBuilder,
      const consensus::BlockFinalizationRegistry &finalizationRegistry,
      BlockSyncQcMode qcMode, std::int64_t now);

  // Serialize a list of blocks into a response payload.
  static std::string serializeBlockList(const std::vector<core::Block> &blocks);

  // Deserialize a response payload into a list of blocks.
  // Returns empty vector on failure.
  static std::vector<core::Block>
  deserializeBlockList(const std::string &payload);
};
