...
class FeeEconomics {
public:
    static constexpr const char* FEE_SPLIT_POLICY_REASON =
        "FEE_SPLIT_POLICY_V1";

    static constexpr const char* FEE_BALANCE_REASON =
        "FEE_ECONOMIC_BALANCE";

    static constexpr const char* FEE_BURN_REASON =
        "PARTIAL_FEE_BURN";

    static constexpr const char* TREASURY_FEE_REASON =
        "TREASURY_FEE_ALLOCATION";

    static constexpr const char* NOT_EVALUATED_REASON =
        "FEE_ECONOMICS_NOT_EVALUATED";

    static FeeEconomicBalance buildFeeEconomicBalance(
        std::uint64_t blockHeight,
        utils::Amount totalFee
    );

    static FeeBurnRecord buildFeeBurnRecord(
        const FeeEconomicBalance& feeBalance,
        utils::Amount supplyBefore
    );

    static TreasuryFeeRecord buildTreasuryFeeRecord(
        const FeeEconomicBalance& feeBalance
    );

    static bool sameBalance(
        const FeeEconomicBalance& left,
        const FeeEconomicBalance& right
    );

    static bool sameBurn(
        const FeeBurnRecord& left,
        const FeeBurnRecord& right
    );

    static bool sameTreasuryFee(
