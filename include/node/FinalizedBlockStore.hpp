...
std::string finalizedBlockStoreStatusToString(FinalizedBlockStoreStatus status);

class FinalizedBlockStoreResult {
public:
  FinalizedBlockStoreResult();

  static FinalizedBlockStoreResult stored(NodeRuntimeManifest manifest,
                                          std::filesystem::path blockPath);

  static FinalizedBlockStoreResult
  alreadyStored(NodeRuntimeManifest manifest, std::filesystem::path blockPath);

  static FinalizedBlockStoreResult rejected(FinalizedBlockStoreStatus status,
                                            std::string reason);
...
public:
  ...
  static void
  recoverInterruptedCommit(const NodeDataDirectoryConfig &directoryConfig);

  static FinalizedBlockStoreResult
  persist(const NodeDataDirectoryConfig &directoryConfig,
          const NodeRuntime &runtime,
          const RuntimeBlockPipelineResult &pipelineResult,
          std::int64_t updatedAt);

  static FinalizedBlockStoreResult
  persistBatch(const NodeDataDirectoryConfig &directoryConfig,
               const NodeRuntime &finalRuntime,
               const std::vector<RuntimeBlockPipelineResult> &results,
               std::int64_t updatedAt);

  static std::filesystem::path
  blockFilePath(const NodeDataDirectoryConfig &directoryConfig,
                std::uint64_t blockIndex);

  static std::string
  finalizedBlockFileContents(const RuntimeBlockPipelineResult &pipelineResult);

  static std::filesystem::path
  commitJournalPath(const NodeDataDirectoryConfig &directoryConfig);
