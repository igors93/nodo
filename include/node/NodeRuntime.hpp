...
public:
  ...
  static PeerUpdateResult accepted(p2p::PeerInfo peer);

  static PeerUpdateResult updated(p2p::PeerInfo peer);

  static PeerUpdateResult duplicate(p2p::PeerInfo peer);

  static PeerUpdateResult rejected(PeerUpdateStatus status, std::string reason);
...
public:
  explicit LocalPeerManager(std::size_t maxPeers = 128);

  PeerUpdateResult addOrUpdatePeer(const p2p::PeerInfo &peer);

  bool removePeer(const std::string &peerId);

  bool hasPeer(const std::string &peerId) const;

  const p2p::PeerInfo *peer(const std::string &peerId) const;

  const p2p::PeerInfo *bestSyncPeer(std::uint64_t localHeight) const;
...
enum class NodeRuntimeStatus { STOPPED, RUNNING, HALTED };
...
public:
  ...
  NodeRuntimeConfig(config::GenesisConfig genesisConfig,
                    p2p::PeerInfo localPeer, std::size_t maxPeers);
...
public:
  ...
  NodeRuntime(NodeRuntimeConfig config, core::Blockchain blockchain,
  ...
  const std::vector<economics::BurnRecord> &burnRecords() const;
  std::vector<economics::BurnRecord> &mutableBurnRecords();
  ...
  cachedAccountStateAtTip(std::int64_t minimumFeeRawUnits) const;
  ...
  // Directly set the cached account state for the current chain tip.
  // This allows maintaining the cache without replaying from genesis.
  void setCachedAccountStateAtTip(core::AccountStateView view);
  ...
  submitConsensusVote(const consensus::ValidatorVoteRecord &vote);

  bool advanceConsensusRoundIfTimedOut(std::int64_t now);

  p2p::PeerMessage localChainSummaryMessage(const std::string &toPeerId,
                                            std::int64_t createdAt) const;

  p2p::LocalSyncPlan
  evaluatePeerForSync(const p2p::PeerInfo &remotePeer,
...
private:
  ...
  std::vector<economics::BurnRecord> m_burns;
...
public:
  ...
  static NodeRuntimeStartResult started(NodeRuntime runtime);

  static NodeRuntimeStartResult rejected(NodeRuntimeStartStatus status,
                                         std::string reason);
...
public:
  static NodeRuntimeStartResult
  startFromGenesis(const NodeRuntimeConfig &config);
};
