...
public:
  FinalizedArtifactValidationContext(
      const config::GenesisConfig &genesisConfig, NodeRuntime &runtime,
      const crypto::ProtocolCryptoContext &cryptoContext,
      std::filesystem::path blockPath, std::uint64_t requiredVoteCount,
  ...
  void setStatePreview(core::StateTransitionPreviewContext previewContext,
  ...
  void setExpectedFeeEconomicBalance(FeeEconomicBalance balance);

  bool hasExpectedFeeEconomicBalance() const;
  const FeeEconomicBalance &expectedFeeEconomicBalance() const;

  void setExpectedRewards(std::vector<RewardDistribution> rewards);

  const std::vector<RewardDistribution> &expectedRewards() const;

  void
  setExpectedLockedStakePositions(std::vector<LockedStakePosition> positions);

  const std::vector<LockedStakePosition> &expectedLockedStakePositions() const;

  void
  setExpectedSecurityScoreRecords(std::vector<SecurityScoreRecord> records);
  ...
  expectedSecurityCheckpoints() const;

  void
  setExpectedRiskAssessments(std::vector<ValidatorRiskAssessment> assessments);
  ...
  expectedContainmentDecisions() const;

  void setExpectedNetworkPolicies(std::vector<ValidatorNetworkPolicy> policies);
...
private:
  ...
  std::optional<std::vector<LockedStakePosition>>
      m_expectedLockedStakePositions;
  std::optional<std::vector<SecurityScoreRecord>>
      m_expectedSecurityScoreRecords;
  std::optional<std::vector<ValidatorSecurityCheckpoint>>
      m_expectedSecurityCheckpoints;
  std::optional<std::vector<ValidatorRiskAssessment>> m_expectedRiskAssessments;
  std::optional<std::vector<ValidatorContainmentDecision>>
      m_expectedContainmentDecisions;
