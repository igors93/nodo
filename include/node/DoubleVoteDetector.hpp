 ...
 * same height+round, different block hashes) and submits them to the
 * EvidencePool.
 *
...
public:
  /*
   * Scan the VotePool for double-votes and submit any found to the
   * EvidencePool. For each conflicting vote, pairs it with the original vote it
   * conflicts with by matching validator+height+round. Returns a result
   * describing what was submitted and what was skipped as duplicates.
   */
  static DoubleVoteDetectionResult detect(const consensus::VotePool &votePool,
...
private:
   ...
   */
  static bool isDoubleVotePair(const consensus::ValidatorVoteRecord &a,
