...
public:
  ...
  static DefenseModeTransitionApplyResult
  rejected(DefenseModeTransitionApplyStatus status, std::string reason);
 ...
 * leaves the prior state intact. The caller receives a deterministic result
 * that describes exactly what happened — there is no silent partial update.
 */
...
public:
  ...
  static DefenseModeTransitionApplyResult
  applyActivation(const economics::DefenseModeTransitionRecord &transition,
  ...
  static DefenseModeTransitionApplyResult
  applyExit(const economics::DefenseModeTransitionRecord &transition,
            const RuntimeSafetyState &currentState, bool auditRequiredByPolicy,
            std::uint64_t minimumAuditHeight,
            const std::filesystem::path &persistPath, std::int64_t updatedAt);
};
