...
public:
  ...
  InflationEpochSnapshot(std::string status, std::uint64_t blockHeight,
...
public:
  ...
  MintAuthorizationRecord(std::string status, std::uint64_t blockHeight,
                          ...
                          std::string governanceDigest, std::string reason,
                          std::string sourceEpochDigest);

  static MintAuthorizationRecord none(const InflationEpochSnapshot &epoch);
...
public:
  ...
  SupplyExpansionRecord(std::string status, std::uint64_t blockHeight,
                        utils::Amount mintedAmount,
                        std::string recipientAddress,
                        std::string authorizationId, std::string policyId,
                        std::string reason,
                        std::string sourceAuthorizationDigest);

  static SupplyExpansionRecord
  none(const MintAuthorizationRecord &authorization,
...
public:
  ...
  static constexpr const char *NO_RECIPIENT_ADDRESS = "NO_MINT_RECIPIENT";

  static constexpr std::uint32_t REQUIRED_APPROVAL_BASIS_POINTS = 8000;

  static InflationEpochSnapshot
  buildInflationEpochSnapshot(const config::GenesisConfig &genesisConfig,
                              std::uint64_t blockHeight,
                              utils::Amount mintedThisEpoch);

  static MintAuthorizationRecord
  buildNoMintAuthorization(const InflationEpochSnapshot &epoch);

  static SupplyExpansionRecord
  buildNoSupplyExpansion(const MintAuthorizationRecord &authorization,
                         const InflationEpochSnapshot &epoch);

  static MintAuthorizationRecord
  buildEpochRewardAuthorization(const InflationEpochSnapshot &epoch,
                                utils::Amount authorizedAmount,
                                const std::string &authorizationId,
                                const std::string &rewardEvidenceDigest);

  static SupplyExpansionRecord
  buildEpochRewardExpansion(const MintAuthorizationRecord &authorization,
                            const InflationEpochSnapshot &epoch);

  static bool sameEpoch(const InflationEpochSnapshot &left,
                        const InflationEpochSnapshot &right);

  static bool sameAuthorization(const MintAuthorizationRecord &left,
                                const MintAuthorizationRecord &right);

  static bool sameExpansion(const SupplyExpansionRecord &left,
