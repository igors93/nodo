...
public:
  ...
  TcpTestnetNodeRuntimeConfig(std::string nodeId, std::string host,
                              std::uint16_t port, std::string networkId,
...
public:
  ...
  TcpTestnetPeerFileEntry(std::string nodeId, p2p::PeerEndpoint endpoint,
                          std::string publicKeyFingerprint);

  TcpTestnetPeerFileEntry(std::string nodeId, p2p::PeerEndpoint endpoint,
                          std::string publicKeyFingerprint,
                          std::int64_t firstSeenAt, std::int64_t lastSeenAt,
                          std::int32_t score, bool quarantined,
  ...
  TcpTestnetPeerFileEntry(std::string nodeId, p2p::PeerEndpoint endpoint,
                          ...
                          std::int32_t score, bool quarantined,
                          std::size_t invalidMessageCount,
                          std::int64_t bannedUntil, std::string banReason,
  ...
  static TcpTestnetPeerFileEntry parseLine(const std::string &line);
...
private:
  ...
  std::size_t m_escalationCount;
};

class TcpTestnetPeerStore {
public:
  static std::vector<TcpTestnetPeerFileEntry>
  load(const std::filesystem::path &peersFile);

  static void save(const std::filesystem::path &peersFile,
                   const std::vector<TcpTestnetPeerFileEntry> &peers);
};

class TcpTestnetNodeRuntime {
public:
  explicit TcpTestnetNodeRuntime(TcpTestnetNodeRuntimeConfig config);
  ...
  bool hasAuthenticatedSession(const std::string &remoteNodeId) const;

  p2p::TransportResult start();
  void stop();

  p2p::PeerRegistryResult addPeer(const p2p::PeerMetadata &peer);

  std::size_t loadPeersFromDisk(std::int64_t now);

  void savePeersToDisk() const;

  p2p::TransportResult connectPeer(const std::string &remoteNodeId);

  p2p::TransportResult connectUnverifiedPeer(const std::string &remoteNodeId,
                                             const p2p::PeerEndpoint &endpoint,
                                             std::int64_t now = 0);

  p2p::GossipDeliveryReport broadcast(p2p::NetworkMessageType type,
                                      const std::string &payload,
                                      std::int64_t now);

  p2p::GossipDeliveryReport
  broadcastChainStatus(const ChainStatusMessage &chainStatus, std::int64_t now);

  p2p::GossipDeliveryReport tick(std::int64_t now);
