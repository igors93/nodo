...
public:
  ...
  ChainStatusMessage(std::string networkId, std::string chainId,
                     std::string protocolVersion, std::uint64_t latestHeight,
                     std::string latestBlockHash, std::uint64_t finalizedHeight,
...
public:
  ...
  BlockLocator(std::uint64_t fromHeight, std::uint64_t maxBlocks,
...
public:
  ...
  NetworkBlockSyncRequest(std::string requesterNodeId, BlockLocator locator,
...
public:
  ...
  RoundAdvanceMessage(std::uint64_t height, std::uint64_t round,
                      std::string proposerAddress, std::int64_t createdAt);
