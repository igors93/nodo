...
public:
  ...
  EpochRewardSettlement(economics::EpochRewardDistribution distribution,
...
public:
  ...
  static EpochRewardSettlement
  buildForCandidate(const NodeRuntime &runtime, std::uint64_t candidateHeight,
                    std::int64_t candidateTimestamp);

  static bool candidateRecordsMatch(const NodeRuntime &runtime,
                                    const core::Block &candidate,
                                    std::string &rejectionReason);

  static bool finalizedRecordsMatch(const NodeRuntime &runtime,
   ...
   */
  static EpochRewardSettlement
  settleCanonicalRecords(std::uint64_t blockHeight, std::int64_t blockTimestamp,
                         const std::vector<core::LedgerRecord> &records,
                         utils::Amount supplyBefore,
                         const core::AccountStateView &currentAccounts);

  static core::AccountStateView
  applyDistributions(const std::vector<RewardDistribution> &distributions,
                     const core::AccountStateView &accounts);

private:
  static std::vector<core::LedgerRecord>
  epochRecords(const std::vector<core::LedgerRecord> &records);

  static EpochRewardSettlement buildFromFinalizedHistory(
      const NodeRuntime &runtime, std::uint64_t settlementHeight,
      std::int64_t settlementTimestamp, utils::Amount supplyBefore);
};
