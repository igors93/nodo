...
public:
   ...
   */
  static std::vector<HandshakeRegistrationResult>
  processInbox(p2p::GossipMesh &gossip,
               const std::vector<p2p::NetworkEnvelope> &hellos,
               const std::vector<p2p::NetworkEnvelope> &challenges,
   ...
   */
  static p2p::GossipDeliveryReport
  initiateHandshake(p2p::GossipMesh &gossip, const std::string &targetNodeId,
                    std::int64_t now,
