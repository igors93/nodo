...
MempoolConfig::MempoolConfig()
    : m_maxTransactions(5000), m_minimumFeeRaw(0), m_replaceByHigherFee(true),
      m_maxTransactionAgeSeconds(3600), m_maxMempoolSizeBytes(50 * 1024 * 1024),
      m_maxTransactionSizeBytes(131072) {}

MempoolConfig::MempoolConfig(std::size_t maxTransactions,
    ...
    : m_maxTransactions(maxTransactions), m_minimumFeeRaw(minimumFeeRaw),
...
std::size_t MempoolConfig::maxTransactions() const { return m_maxTransactions; }

std::int64_t MempoolConfig::minimumFeeRaw() const { return m_minimumFeeRaw; }

bool MempoolConfig::replaceByHigherFee() const { return m_replaceByHigherFee; }
...
bool MempoolConfig::isValid() const {
  return m_maxTransactions > 0 && m_minimumFeeRaw >= 0 &&
         m_maxTransactionAgeSeconds > 0 && m_maxMempoolSizeBytes > 0 &&
         m_maxTransactionSizeBytes > 0;
}

MempoolEntry::MempoolEntry(core::Transaction transaction,
                           std::int64_t acceptedAt)
    : m_transaction(std::move(transaction)), m_acceptedAt(acceptedAt) {}

const core::Transaction &MempoolEntry::transaction() const {
  return m_transaction;
}

std::int64_t MempoolEntry::acceptedAt() const { return m_acceptedAt; }
...
bool MempoolEntry::isValid(const crypto::CryptoPolicy &policy,
                           crypto::SecurityContext context) const {
  if (m_acceptedAt <= 0) {
    return false;
  }

  return m_transaction.isStructurallyValid(policy, context);
}
...
std::string MempoolEntry::serialize() const {
  ...
  oss << "MempoolEntry{"
      << "acceptedAt=" << m_acceptedAt << ";priorityFeeRaw=" << priorityFeeRaw()
      << ";transaction=" << m_transaction.serialize() << "}";

  return oss.str();
}

std::string mempoolAdmissionStatusToString(MempoolAdmissionStatus status) {
  switch (status) {
...
MempoolAdmissionResult::MempoolAdmissionResult()
    : m_status(MempoolAdmissionStatus::INVALID_TRANSACTION),
      m_reason("Uninitialized mempool admission result."), m_transactionId(""),
      m_replacedTransactionId("") {}

MempoolAdmissionResult
MempoolAdmissionResult::accepted(std::string transactionId) {
...
MempoolAdmissionResult
MempoolAdmissionResult::duplicate(std::string transactionId) {
  MempoolAdmissionResult result;
...
MempoolAdmissionResult
MempoolAdmissionResult::replaced(std::string transactionId,
...
MempoolAdmissionResult
MempoolAdmissionResult::rejected(MempoolAdmissionStatus status,
                                 std::string reason) {
  MempoolAdmissionResult result;
...
const std::string &MempoolAdmissionResult::reason() const { return m_reason; }
...
std::string MempoolAdmissionResult::serialize() const {
  ...
  oss << "MempoolAdmissionResult{"
      ...
      << ";reason=" << m_reason << ";transactionId=" << m_transactionId
...
Mempool::Mempool(MempoolConfig config)
    : m_config(config), m_entriesById(), m_transactionIdBySenderNonce(),
...
MempoolAdmissionResult Mempool::admitTransaction(
    const core::Transaction &transaction, const crypto::CryptoPolicy &policy,
    crypto::SecurityContext context, std::int64_t acceptedAt) {
  if (!m_config.isValid()) {
  ...
  if (contains(transaction.id())) {
    return MempoolAdmissionResult::duplicate(transaction.id());
  }
  ...
  const std::string senderNonce = senderNonceKey(transaction);

  const auto existingByNonce = m_transactionIdBySenderNonce.find(senderNonce);
  ...
  if (existingByNonce != m_transactionIdBySenderNonce.end()) {
    ...
    if (!m_config.replaceByHigherFee()) {
      return MempoolAdmissionResult::rejected(
          ...
          "Another transaction with the same sender nonce is already in "
          "mempool.");
    }
    ...
    if (transaction.fee().rawUnits() < minimumReplacementFee) {
      return MempoolAdmissionResult::rejected(
          MempoolAdmissionStatus::CONFLICTING_NONCE,
          "Replacement transaction must pay at least 10% more than the "
          "existing fee.");
    }
  ...
  if (!replacesExistingTransaction &&
    ...
    return MempoolAdmissionResult::rejected(
        MempoolAdmissionStatus::CAPACITY_REACHED, "Mempool capacity reached.");
  }
  ...
  while (projectedSize + incomingSize > m_config.maxMempoolSizeBytes()) {
    ...
    if (transaction.fee().rawUnits() <= lowestFee) {
      return MempoolAdmissionResult::rejected(
          MempoolAdmissionStatus::FEE_TOO_LOW,
          "Mempool is full and incoming transaction fee is too low to trigger "
          "eviction.");
    }
  ...
  MempoolEntry entry(transaction, acceptedAt);

  m_transactionIdBySenderNonce.emplace(entry.senderNonceKey(),
                                       transaction.id());

  m_entriesById.emplace(transaction.id(), entry);
  m_currentMempoolSizeBytes += incomingSize;

  if (replacesExistingTransaction) {
    return MempoolAdmissionResult::replaced(transaction.id(),
...
bool Mempool::contains(const std::string &transactionId) const {
  return m_entriesById.find(transactionId) != m_entriesById.end();
}

const MempoolEntry *Mempool::entry(const std::string &transactionId) const {
  const auto found = m_entriesById.find(transactionId);
...
bool Mempool::removeTransaction(const std::string &transactionId) {
  auto found = m_entriesById.find(transactionId);
...
std::size_t Mempool::pruneExpired(std::int64_t currentTime) {
  if (currentTime <= 0) {
    return 0;
  }
  ...
  for (const auto &[transactionId, entry] : m_entriesById) {
    if (currentTime > entry.acceptedAt() &&
        currentTime - entry.acceptedAt() >
            m_config.maxTransactionAgeSeconds()) {
...
std::size_t Mempool::removeConflicting(const std::string &sender,
...
Mempool::transactionsForBlock(std::size_t maxCount) const {
...
std::vector<core::Transaction> Mempool::transactionsForBlock(
    std::size_t maxCount,
    const core::AccountStateView &accountStateView) const {
  ...
  while (selectedInPass && transactions.size() < maxCount) {
    ...
    for (const auto &entry : entries) {
      ...
      if (nextNonce == nextNonceBySender.end()) {
        ...
        nextNonce = nextNonceBySender
                        .emplace(transaction.fromAddress(), account.nonce() + 1)
                        .first;
      }
...
MempoolStats Mempool::stats() const {
  ...
  result.averageFee = utils::Amount::fromRawUnits(static_cast<std::int64_t>(
      sum / static_cast<__int128>(m_entriesById.size())));

  return result;
}

std::size_t Mempool::size() const { return m_entriesById.size(); }

bool Mempool::empty() const { return m_entriesById.empty(); }

bool Mempool::replaceByHigherFee() const {
  return m_config.replaceByHigherFee();
}

bool Mempool::isValid(const crypto::CryptoPolicy &policy,
                      crypto::SecurityContext context) const {
  if (!m_config.isValid()) {
    return false;
  }
  ...
  for (const auto &[transactionId, entry] : m_entriesById) {
    ...
    if (!entry.isValid(policy, context)) {
      return false;
    }
...
std::string Mempool::serialize() const {
  ...
  oss << "Mempool{"
      << "size=" << m_entriesById.size() << ";entries=[";
...
std::string Mempool::senderNonceKey(const core::Transaction &transaction) {
  return transaction.fromAddress() + "#" + std::to_string(transaction.nonce());
}

void Mempool::removeIndexesForEntry(const MempoolEntry &entry) {
  m_transactionIdBySenderNonce.erase(entry.senderNonceKey());
}
