...
utils::Amount FeeMarket::computeNextBaseFee(
    ...
    if (targetBlockCapacity == 0) {
        ...
        const std::int64_t floored =
            std::max(current, MINIMUM_BASE_FEE_RAW);
    ...
    if (lastBlockTxCount > targetBlockCapacity) {
        ...
        // delta = current * (lastBlockTxCount - targetBlockCapacity) / targetBlockCapacity
        // but capped at maxDelta
...
bool FeeMarket::isFeeSufficient(
    utils::Amount txFee,
...
utils::Amount FeeMarket::estimateFee(
    FeeUrgency urgency,
    utils::Amount baseFee
) {
...
FeeMarketState FeeMarket::initialState(
    std::uint64_t targetBlockCapacity
) {
...
FeeMarketState FeeMarket::advanceState(
    ...
    next.baseFee             = computeNextBaseFee(
        current.baseFee,
        actualTxCount,
