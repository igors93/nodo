...
TreasuryExecutionValidationResult::accepted() {
...
TreasuryExecutionValidationResult TreasuryExecutionValidationResult::rejected(
    TreasuryExecutionValidationStatus status, std::string reason) {
  TreasuryExecutionValidationResult r;
  r.m_status = status;
...
TreasuryExecutionValidationStatus
TreasuryExecutionValidationResult::status() const {
  return m_status;
}
const std::string &TreasuryExecutionValidationResult::reason() const {
  return m_reason;
}
...
TreasuryExecutionValidationResult TreasuryExecutionValidator::validateEvidence(
  ...
  const TreasurySpendValidationResult spendResult =
      TreasurySpendValidator::validateSpend(
          DefenseModeState::INACTIVE, DefenseModePolicy::defaultPolicy(),
          evidence.treasuryAccountBefore(), evidence.policy(),
          evidence.proposal(), evidence.approval(),
          evidence.currentBlockHeight(), evidence.epochSpentSoFar());

  if (!spendResult.accepted()) { // TreasurySpendValidationResult::accepted() is
                                 // not renamed
    return TreasuryExecutionValidationResult::rejected(
        TreasuryExecutionValidationStatus::SPEND_VALIDATOR_REJECTED,
        "TreasuryExecutionValidator: TreasurySpendValidator rejected the "
        "spend: " +
            spendResult.reason());
  }
  ...
  if (govResult.status() ==
    ...
    return TreasuryExecutionValidationResult::rejected(
        ...
        "TreasuryExecutionValidator: evidence is missing governance approval "
        "context. "
        "Production treasury evidence must be produced via "
        "GovernanceApprovalBridge: " +
            govResult.reason());
  }
