...
std::string treasuryDebitStatusToString(TreasuryDebitStatus status) {
  switch (status) {
  case TreasuryDebitStatus::ACCEPTED:
    return "ACCEPTED";
  case TreasuryDebitStatus::LOCKED:
    return "LOCKED";
  case TreasuryDebitStatus::INSUFFICIENT_BALANCE:
    return "INSUFFICIENT_BALANCE";
  case TreasuryDebitStatus::NEGATIVE_AMOUNT:
    return "NEGATIVE_AMOUNT";
  case TreasuryDebitStatus::INVALID_ACCOUNT:
...
TreasuryDebitResult
TreasuryDebitResult::insufficientBalance(utils::Amount available,
                                         utils::Amount requested) {
  TreasuryDebitResult r;
...
TreasuryDebitStatus TreasuryDebitResult::status() const { return m_status; }

const std::string &TreasuryDebitResult::reason() const { return m_reason; }
...
TreasuryAccount::TreasuryAccount()
    : m_balance(utils::Amount::fromRawUnits(0)), m_epoch(0), m_locked(false),
      m_valid(false),
      m_rejectionReason("TreasuryAccount: default-constructed.") {}

TreasuryAccount::TreasuryAccount(std::string treasuryId,
                                 std::string accountAddress,
                                 utils::Amount balance, std::uint64_t epoch,
                                 bool locked, std::string reason)
    : m_treasuryId(std::move(treasuryId)),
      m_accountAddress(std::move(accountAddress)), m_balance(balance),
      m_epoch(epoch), m_locked(locked), m_lockReason(std::move(reason)),
...
const std::string &TreasuryAccount::accountAddress() const {
...
const std::string &TreasuryAccount::rejectionReason() const {
...
std::string TreasuryAccount::serialize() const {
  ...
  oss << "TreasuryAccount{"
      << "id=" << m_treasuryId << ";address=" << m_accountAddress
      << ";balanceRaw=" << m_balance.rawUnits() << ";epoch=" << m_epoch
      << ";locked=" << (m_locked ? "1" : "0")
      << ";valid=" << (m_valid ? "1" : "0") << "}";
  return oss.str();
}
