...
SupplyDelta
SupplyDeltaBuilder::build(std::uint64_t blockHeight,
  ...
  return SupplyDelta(blockHeight, blockHash, epoch, supplyBefore,
                     utils::Amount::fromRawUnits(mintSum),
                     utils::Amount::fromRawUnits(burnSum),
                     utils::Amount::fromRawUnits(supplyAfterRaw), mintRecords,
                     burnRecords);
}
