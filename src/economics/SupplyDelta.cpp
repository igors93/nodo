...
SupplyDelta::SupplyDelta()
    : m_blockHeight(0), m_blockHash(""), m_epoch(0),
...
SupplyDelta::SupplyDelta(std::uint64_t blockHeight, std::string blockHash,
                         std::uint64_t epoch, utils::Amount supplyBefore,
                         utils::Amount mintedAmount, utils::Amount burnedAmount,
    ...
    : m_blockHeight(blockHeight), m_blockHash(std::move(blockHash)),
      m_epoch(epoch), m_supplyBefore(supplyBefore),
      m_mintedAmount(mintedAmount), m_burnedAmount(burnedAmount),
      m_supplyAfter(supplyAfter), m_mintRecords(std::move(mintRecords)),
      m_burnRecords(std::move(burnRecords)) {}

SupplyDelta SupplyDelta::noOp(std::uint64_t blockHeight,
                              const std::string &blockHash, std::uint64_t epoch,
                              utils::Amount currentSupply) {
  return SupplyDelta(blockHeight, blockHash, epoch, currentSupply,
                     utils::Amount::fromRawUnits(0),
                     utils::Amount::fromRawUnits(0), currentSupply, {}, {});
}
...
const std::vector<MintRecord> &SupplyDelta::mintRecords() const {
  return m_mintRecords;
}
const std::vector<BurnRecord> &SupplyDelta::burnRecords() const {
  return m_burnRecords;
}

bool SupplyDelta::isValid() const { return rejectionReason().empty(); }
...
std::string SupplyDelta::rejectionReason() const {
  ...
  for (const auto &mint : m_mintRecords) {
    ...
    // Consistency: every mint must belong to the same block and epoch as this
    // delta.
    if (mint.epoch() != m_epoch) {
      return "SupplyDelta rejected: MintRecord id='" + mint.id() + "' epoch " +
             std::to_string(mint.epoch()) + " does not match delta epoch " +
    ...
    if (mint.sourceBlockIndex() != m_blockHeight) {
      return "SupplyDelta rejected: MintRecord id='" + mint.id() +
             ...
             " does not match delta blockHeight " +
             std::to_string(m_blockHeight) + ".";
    }
  ...
  if (mintSum != m_mintedAmount.rawUnits()) {
    return "SupplyDelta rejected: mintedAmount (" +
           ...
           ") does not equal sum of mint records (" + std::to_string(mintSum) +
           ").";
  }
  ...
  for (const auto &burn : m_burnRecords) {
    ...
    // Consistency: every burn must belong to the same block and epoch as this
    // delta.
  ...
  if (burnSum != m_burnedAmount.rawUnits()) {
    return "SupplyDelta rejected: burnedAmount (" +
           ...
           ") does not equal sum of burn records (" + std::to_string(burnSum) +
           ").";
  }
  ...
  if (after != expected) {
    return "SupplyDelta rejected: supply equation mismatch: "
           ...
           ") - "
           "burned(" +
           std::to_string(burned) + ") = " + std::to_string(expected) +
           " but supplyAfter = " + std::to_string(after) + ".";
  }
...
std::string SupplyDelta::serialize() const {
  ...
  oss << "SupplyDelta{"
      << "blockHeight=" << m_blockHeight << ";blockHash=" << m_blockHash
      << ";epoch=" << m_epoch
