...
ValidatorPenaltyRecord::ValidatorPenaltyRecord()
    : m_validatorAddress(""),
      ...
      m_previousScore(0),
      m_newScore(0),
      m_reason(ValidatorPenaltyReason::UNKNOWN),
      m_action(ValidatorPenaltyAction::UNKNOWN),
      m_evidenceHash(""),
      m_firstBlockHash(""),
      m_conflictingBlockHash(""),
      m_firstSignatureDigest(""),
      m_conflictingSignatureDigest(""),
      m_timestamp(0) {}
...
std::uint64_t ValidatorPenaltyRecord::epoch() const {
    return m_epoch;
}
...
std::int32_t ValidatorPenaltyRecord::newScore() const {
    return m_newScore;
}
...
std::int64_t ValidatorPenaltyRecord::timestamp() const {
    return m_timestamp;
}
...
bool ValidatorPenaltyRecord::isValid() const {
    ...
    if (!isSafeText(m_evidenceHash) ||
        !isSafeText(m_firstBlockHash) ||
...
std::string ValidatorPenaltyRecord::deterministicId() const {
    if (!isValid()) {
        throw std::logic_error("Invalid ValidatorPenaltyRecord has no deterministic id.");
    }

    return hashString(
        "NODO_VALIDATOR_PENALTY_RECORD_V1|" + serialize()
    );
}

ValidatorScoreRecord ValidatorPenaltyRecord::createScoreRecord() const {
    if (!isValid()) {
        throw std::logic_error("Invalid ValidatorPenaltyRecord cannot create score record.");
    }
    ...
    if (!scoreRecord.isValid()) {
        throw std::logic_error("ValidatorPenaltyRecord created invalid ValidatorScoreRecord.");
    }
...
ValidatorPenaltyRecord ValidatorPenaltyRecord::deserialize(
    const std::string& serialized
) {
    if (serialized.rfind("ValidatorPenaltyRecord{", 0) != 0) {
        throw std::invalid_argument("Serialized data is not a ValidatorPenaltyRecord.");
    }

    ValidatorPenaltyRecord record(
        serialization::FieldCodec::extractField(serialized, "validator"),
        static_cast<std::uint64_t>(
            std::stoull(serialization::FieldCodec::extractField(serialized, "epoch"))
        ),
        static_cast<std::uint64_t>(
            std::stoull(serialization::FieldCodec::extractField(serialized, "blockIndex"))
        ),
        static_cast<std::int32_t>(
            std::stoi(serialization::FieldCodec::extractField(serialized, "previousScore"))
        ),
        static_cast<std::int32_t>(
            std::stoi(serialization::FieldCodec::extractField(serialized, "newScore"))
        ),
        ...
        serialization::FieldCodec::extractField(serialized, "conflictingBlockHash"),
        serialization::FieldCodec::extractField(serialized, "firstSignatureDigest"),
        serialization::FieldCodec::extractField(serialized, "conflictingSignatureDigest"),
        std::stoll(serialization::FieldCodec::extractField(serialized, "timestamp"))
    );

    if (!record.isValid()) {
        throw std::invalid_argument("Deserialized ValidatorPenaltyRecord is invalid.");
    }

    if (record.serialize() != serialized) {
        throw std::logic_error("ValidatorPenaltyRecord round-trip serialization mismatch.");
    }

    return record;
}

bool ValidatorPenaltyRecord::isScoreInRange(
    std::int32_t score
) {
...
std::int32_t ValidatorPenaltyPolicy::applyDoubleSignPenalty(
    ...
    return std::max(
        ValidatorScoreRecord::MIN_SCORE,
...
ValidatorPenaltyRecord ValidatorPenaltyPolicy::createDoubleSignPenaltyRecord(
    ...
    if (!evidence.isValid()) {
        throw std::invalid_argument("Invalid double-sign evidence rejected by penalty policy.");
    }
    ...
    const std::int32_t newScore =
        applyDoubleSignPenalty(previousScore);
    ...
    ValidatorPenaltyRecord record(
        ...
        evidence.conflictingProposal().signatureDigest(),
        timestamp
    );

    if (!record.isValid()) {
        throw std::logic_error("Penalty policy produced invalid ValidatorPenaltyRecord.");
    }
