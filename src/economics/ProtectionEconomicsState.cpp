...
ProtectionEconomicsState::ProtectionEconomicsState()
    : m_acceptedWorkByValidator(),
      m_latestScoreByValidator(),
      m_rewardCoinLots(),
      m_totalAcceptedWorkWeight(0),
      m_workRecordCount(0),
      m_scoreRecordCount(0),
      m_protectionEpochCount(0),
      m_genesisRewardCount(0),
...
void ProtectionEconomicsState::applyProtectionEpochTotals(
    ...
    if (rewardPool < securityEmission) {
        throw std::invalid_argument("Reward pool cannot be lower than security emission.");
    }
...
bool ProtectionEconomicsState::hasValidator(
    ...
    return m_acceptedWorkByValidator.find(validatorAddress) != m_acceptedWorkByValidator.end() ||
           m_latestScoreByValidator.find(validatorAddress) != m_latestScoreByValidator.end();
}
...
const std::map<std::string, std::uint64_t>& ProtectionEconomicsState::acceptedWorkByValidator() const {
    return m_acceptedWorkByValidator;
}

const std::map<std::string, std::int32_t>& ProtectionEconomicsState::latestScoreByValidator() const {
    return m_latestScoreByValidator;
}

const std::vector<core::CoinLot>& ProtectionEconomicsState::rewardCoinLots() const {
