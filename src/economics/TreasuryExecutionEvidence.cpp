...
TreasuryExecutionEvidence::TreasuryExecutionEvidence()
    : m_evidenceId(""), m_proposal(), m_approval(), m_policy(),
      m_treasuryAccountBefore(), m_currentBlockHeight(0),
      m_epochSpentSoFar(utils::Amount::fromRawUnits(0)), m_spendRecord(),
      m_createdAt(0), m_hasGovernanceContext(false), m_governanceContext(),
      m_validated(false), m_valid(false),
      m_rejectionReason(
          "TreasuryExecutionEvidence: default-constructed, not valid.") {}

TreasuryExecutionEvidence::TreasuryExecutionEvidence(
    std::string evidenceId, TreasuryProposal proposal,
    TreasuryApproval approval, TreasuryPolicy policy,
    TreasuryAccount treasuryAccountBefore, std::uint64_t currentBlockHeight,
    utils::Amount epochSpentSoFar, TreasurySpendRecord spendRecord,
    std::int64_t createdAt)
    : m_evidenceId(std::move(evidenceId)), m_proposal(std::move(proposal)),
      m_approval(std::move(approval)), m_policy(std::move(policy)),
      m_treasuryAccountBefore(std::move(treasuryAccountBefore)),
      m_currentBlockHeight(currentBlockHeight),
      m_epochSpentSoFar(epochSpentSoFar), m_spendRecord(std::move(spendRecord)),
      m_createdAt(createdAt), m_hasGovernanceContext(false),
      m_governanceContext(), m_validated(false), m_valid(false),
      m_rejectionReason("") {}
...
TreasuryExecutionEvidence::TreasuryExecutionEvidence(
    ...
    TreasuryAccount treasuryAccountBefore, std::uint64_t currentBlockHeight,
    utils::Amount epochSpentSoFar, TreasurySpendRecord spendRecord,
    std::int64_t createdAt, GovernanceApprovalContext governanceContext)
    : m_evidenceId(std::move(evidenceId)), m_proposal(std::move(proposal)),
      m_approval(std::move(approval)), m_policy(std::move(policy)),
      m_treasuryAccountBefore(std::move(treasuryAccountBefore)),
      m_currentBlockHeight(currentBlockHeight),
      m_epochSpentSoFar(epochSpentSoFar), m_spendRecord(std::move(spendRecord)),
      m_createdAt(createdAt), m_hasGovernanceContext(true),
      m_governanceContext(std::move(governanceContext)), m_validated(false),
      m_valid(false), m_rejectionReason("") {}

const std::string &TreasuryExecutionEvidence::evidenceId() const {
  return m_evidenceId;
}
const TreasuryProposal &TreasuryExecutionEvidence::proposal() const {
  return m_proposal;
}
const TreasuryApproval &TreasuryExecutionEvidence::approval() const {
  return m_approval;
}
const TreasuryPolicy &TreasuryExecutionEvidence::policy() const {
  return m_policy;
}
const TreasuryAccount &
TreasuryExecutionEvidence::treasuryAccountBefore() const {
  return m_treasuryAccountBefore;
}
std::uint64_t TreasuryExecutionEvidence::currentBlockHeight() const {
  return m_currentBlockHeight;
}
utils::Amount TreasuryExecutionEvidence::epochSpentSoFar() const {
  return m_epochSpentSoFar;
}
const TreasurySpendRecord &TreasuryExecutionEvidence::spendRecord() const {
  return m_spendRecord;
}
std::int64_t TreasuryExecutionEvidence::createdAt() const {
  return m_createdAt;
}
...
const GovernanceApprovalContext &
TreasuryExecutionEvidence::governanceContext() const {
...
void TreasuryExecutionEvidence::validate() const {
  ...
  if (m_createdAt <= 0) {
    m_valid = false;
    m_rejectionReason =
        "TreasuryExecutionEvidence: createdAt must be positive.";
    return;
  }
  ...
  if (!m_treasuryAccountBefore.isValid()) {
    m_valid = false;
    m_rejectionReason =
        "TreasuryExecutionEvidence: treasuryAccountBefore is invalid: " +
  ...
  if (m_spendRecord.recipientAddress() != m_proposal.recipientAddress()) {
    m_valid = false;
    m_rejectionReason =
        "TreasuryExecutionEvidence: spendRecord.recipientAddress='" +
  ...
  if (m_spendRecord.executedAtBlock() != m_currentBlockHeight) {
    m_valid = false;
    m_rejectionReason =
        "TreasuryExecutionEvidence: spendRecord.executedAtBlock=" +
  ...
  if (m_hasGovernanceContext &&
      m_governanceContext.governanceLifecycle.proposalEnvelope()
              .treasuryProposal()
              .proposalId() != m_proposal.proposalId()) {
    m_valid = false;
    m_rejectionReason =
  ...
  if (m_spendRecord.treasuryBalanceBefore() !=
      m_treasuryAccountBefore.balance()) {
    m_valid = false;
    m_rejectionReason =
        "TreasuryExecutionEvidence: spendRecord.treasuryBalanceBefore=" +
