...
TreasuryProposal::TreasuryProposal()
    : m_amount(utils::Amount::fromRawUnits(0)), m_createdAtBlock(0),
      m_requestedEpoch(0), m_valid(false),
      m_rejectionReason("TreasuryProposal: default-constructed.") {}

TreasuryProposal::TreasuryProposal(std::string proposalId,
                                   std::string recipientAddress,
                                   utils::Amount amount, std::string purpose,
    ...
    : m_proposalId(std::move(proposalId)),
      ...
      m_purpose(std::move(purpose)), m_createdAtBlock(createdAtBlock),
      m_requestedEpoch(requestedEpoch), m_proposer(std::move(proposer)),
...
const std::string &TreasuryProposal::recipientAddress() const {
...
std::uint64_t TreasuryProposal::createdAtBlock() const {
  return m_createdAtBlock;
}
std::uint64_t TreasuryProposal::requestedEpoch() const {
  return m_requestedEpoch;
}
const std::string &TreasuryProposal::proposer() const { return m_proposer; }
bool TreasuryProposal::isValid() const { return m_valid; }
const std::string &TreasuryProposal::rejectionReason() const {
...
std::string TreasuryProposal::serialize() const {
  ...
  oss << "TreasuryProposal{"
      << "id=" << m_proposalId << ";recipient=" << m_recipientAddress
      << ";amountRaw=" << m_amount.rawUnits() << ";purpose=" << m_purpose
      << ";createdAtBlock=" << m_createdAtBlock << ";epoch=" << m_requestedEpoch
      << ";proposer=" << m_proposer << ";valid=" << (m_valid ? "1" : "0")
      << "}";
  return oss.str();
}
