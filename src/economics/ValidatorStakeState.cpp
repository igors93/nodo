...
std::string bondingStatusToString(BondingStatus status) {
    switch (status) {
        case BondingStatus::BONDED:    return "BONDED";
        case BondingStatus::UNBONDING: return "UNBONDING";
        case BondingStatus::JAILED:    return "JAILED";
        case BondingStatus::TOMBSTONED: return "TOMBSTONED";
...
bool ValidatorStakeState::hasAppliedEvidence(const std::string& evidenceId) const {
    return std::find(m_appliedEvidenceIds.begin(), m_appliedEvidenceIds.end(), evidenceId)
           != m_appliedEvidenceIds.end();
}
...
bool ValidatorStakeState::isValid() const {
