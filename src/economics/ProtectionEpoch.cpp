...
ProtectionEpoch::ProtectionEpoch()
    : m_epochId(0), m_startBlock(0), m_endBlock(0),
      m_feesCollected(utils::Amount::fromRawUnits(0)),
      m_emissionCap(utils::Amount::fromRawUnits(0)), m_workDemandBasisPoints(0),
      m_targetWorkWeight(0), m_acceptedWorkWeight(0), m_policyVersion(""),
      m_evidenceBlockHash("") {}

ProtectionEpoch::ProtectionEpoch(std::uint64_t epochId,
    ...
    : m_epochId(epochId), m_startBlock(startBlock), m_endBlock(endBlock),
      m_feesCollected(feesCollected), m_emissionCap(emissionCap),
      m_workDemandBasisPoints(workDemandBasisPoints), m_targetWorkWeight(0),
      m_acceptedWorkWeight(0), m_policyVersion(""), m_evidenceBlockHash("") {}

ProtectionEpoch::ProtectionEpoch(
    std::uint64_t epochId, std::uint64_t startBlock, std::uint64_t endBlock,
    utils::Amount feesCollected, utils::Amount emissionCap,
    std::uint32_t workDemandBasisPoints, std::uint64_t targetWorkWeight,
    std::uint64_t acceptedWorkWeight, std::string policyVersion,
...
std::uint64_t ProtectionEpoch::epochId() const { return m_epochId; }

std::uint64_t ProtectionEpoch::startBlock() const { return m_startBlock; }

std::uint64_t ProtectionEpoch::endBlock() const { return m_endBlock; }

utils::Amount ProtectionEpoch::feesCollected() const { return m_feesCollected; }

utils::Amount ProtectionEpoch::emissionCap() const { return m_emissionCap; }

std::uint32_t ProtectionEpoch::workDemandBasisPoints() const {
  return m_workDemandBasisPoints;
}

std::uint64_t ProtectionEpoch::targetWorkWeight() const {
  return m_targetWorkWeight;
}
std::uint64_t ProtectionEpoch::acceptedWorkWeight() const {
  return m_acceptedWorkWeight;
}
const std::string &ProtectionEpoch::policyVersion() const {
  return m_policyVersion;
}
const std::string &ProtectionEpoch::evidenceBlockHash() const {
  return m_evidenceBlockHash;
}

bool ProtectionEpoch::hasCanonicalSettlementMetadata() const {
  return m_targetWorkWeight > 0 && m_acceptedWorkWeight <= m_targetWorkWeight &&
...
bool ProtectionEpoch::isValid() const {
  ...
  if (hasCanonicalSettlementMetadata()) {
    ...
    if (bounded != m_workDemandBasisPoints)
      return false;
...
utils::Amount ProtectionEpoch::securityEmission() const {
  ...
  return utils::Amount::fromRawUnits(
      multiplyDivideRawUnits(m_emissionCap.rawUnits(), m_workDemandBasisPoints,
                             BASIS_POINTS_DENOMINATOR));
}
...
std::string ProtectionEpoch::serialize() const {
  ...
  oss << "ProtectionEpoch{"
      << "epochId=" << m_epochId << ";startBlock=" << m_startBlock
      << ";endBlock=" << m_endBlock
      ...
      << ";evidenceBlockHash=" << m_evidenceBlockHash << ";securityEmissionRaw="
      << (isValid() ? securityEmission().rawUnits() : 0)
      << ";rewardPoolRaw=" << (isValid() ? rewardPool().rawUnits() : 0) << "}";

  return oss.str();
}

ProtectionEpoch ProtectionEpoch::deserialize(const std::string &serialized) {
  ...
  ProtectionEpoch epoch(
      ...
      field("policyVersion"), field("evidenceBlockHash"));
...
std::int64_t ProtectionEpoch::multiplyDivideRawUnits(
    std::int64_t rawUnits, std::uint64_t numerator, std::uint64_t denominator) {
  if (rawUnits < 0) {
  ...
  const std::uint64_t raw = static_cast<std::uint64_t>(rawUnits);

  const std::uint64_t quotient = raw / denominator;

  const std::uint64_t remainder = raw % denominator;
  ...
  if (numerator != 0 &&
    ...
    throw std::overflow_error(
        "Protection epoch emission calculation overflow.");
  }
