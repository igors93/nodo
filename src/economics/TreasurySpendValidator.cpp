...
TreasurySpendValidationResult::TreasurySpendValidationResult()
    : m_status(TreasurySpendStatus::INVALID_TREASURY),
      m_reason("Uninitialized."), m_spendRecord() {}

TreasurySpendValidationResult
TreasurySpendValidationResult::accepted(TreasurySpendRecord spendRecord) {
...
TreasurySpendValidationResult
TreasurySpendValidationResult::rejected(TreasurySpendStatus status,
                                        std::string reason) {
  TreasurySpendValidationResult r;
  r.m_status = status;
...
TreasurySpendValidationResult TreasurySpendValidator::validateSpend(
    ...
    const DefenseModePolicy &defenseModePolicy, const TreasuryAccount &treasury,
    const TreasuryPolicy &policy, const TreasuryProposal &proposal,
    const TreasuryApproval &approval, std::uint64_t currentBlockHeight,
  ...
  if (policy.requireApproval()) {
    if (!approval.isValid()) {
      return TreasurySpendValidationResult::rejected(
          ...
          "approval required but approval is invalid: " +
  ...
  // Timelock: currentBlockHeight >= proposal.createdAtBlock +
  ...
  if (currentBlockHeight < unlockHeight) {
    return TreasurySpendValidationResult::rejected(
        ...
        "timelock not satisfied: currentBlock=" +
            std::to_string(currentBlockHeight) +
            " unlockAt=" + std::to_string(unlockHeight));
  }

  // Treasury balance check (before limit checks so balance is the first
  // monetary gate).
  ...
  // proposal cannot produce an identical spend record under different
  // conditions.
  const std::string spendId = "spend:" + proposal.proposalId() + ":" +
                              std::to_string(currentBlockHeight) + ":" +
  ...
  const TreasurySpendRecord spendRecord(
      spendId, proposal.proposalId(), proposal.recipientAddress(),
      proposal.amount(), proposal.purpose(), currentBlockHeight,
      proposal.requestedEpoch(), balanceBefore, balanceAfter);
