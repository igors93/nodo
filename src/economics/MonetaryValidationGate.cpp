...
std::string
monetaryValidationGateStatusToString(MonetaryValidationGateStatus status) {
  switch (status) {
...
MonetaryValidationGateResult::MonetaryValidationGateResult()
    : m_accepted(false),
      ...
      m_firewallStatus(MonetaryFirewallStatus::INVALID_POLICY), m_reason("") {}
...
MonetaryValidationGateResult::rejected(MonetaryFirewallStatus firewallStatus,
                                       std::string reason) {
  MonetaryValidationGateResult r;
  r.m_accepted = false;
...
MonetaryValidationGateStatus MonetaryValidationGateResult::status() const {
  return m_status;
}
MonetaryFirewallStatus MonetaryValidationGateResult::firewallStatus() const {
  return m_firewallStatus;
}
const std::string &MonetaryValidationGateResult::reason() const {
  return m_reason;
}
...
MonetaryValidationGateResult MonetaryValidationGate::validate(
    const MonetaryPolicy &policy, const SupplyDelta &delta,
  ...
  if (!firewallResult.isAccepted()) {
    return MonetaryValidationGateResult::rejected(firewallResult.status(),
