...
std::string slashResultToString(SlashResult result) {
  switch (result) {
  case SlashResult::APPLIED:
    return "APPLIED";
  case SlashResult::ALREADY_APPLIED:
    return "ALREADY_APPLIED";
  case SlashResult::INVALID_EVIDENCE:
    return "INVALID_EVIDENCE";
  case SlashResult::WOULD_EXCEED_STAKE:
    return "WOULD_EXCEED_STAKE";
  case SlashResult::VALIDATOR_TOMBSTONED:
    return "VALIDATOR_TOMBSTONED";
  default:
    return "UNKNOWN";
  }
}

SlashResult StakeSlashApplication::apply(ValidatorStakeState &state,
...
SlashResult StakeSlashApplication::applyPenaltyDecision(
  ...
  if (!decision.isValid() || decision.evidenceId().empty() ||
  ...
  if (decision.slashable()) {
    const SlashResult slashResult =
        apply(state, decision.evidenceId(),
