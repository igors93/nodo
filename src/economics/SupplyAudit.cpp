...
SupplyAuditResult::SupplyAuditResult() : m_valid(false), m_reason("") {}

SupplyAuditResult SupplyAuditResult::valid(utils::Amount genesisSupply,
...
utils::Amount SupplyAuditResult::genesisSupply() const {
  return m_genesisSupply;
}
...
utils::Amount SupplyAuditResult::totalRewardsMinted() const {
  return m_totalRewardsMinted;
}
utils::Amount SupplyAuditResult::totalFeesBurned() const {
  return m_totalFeesBurned;
}
utils::Amount SupplyAuditResult::freeCirculation() const {
  return m_freeCirculation;
}
...
SupplyAuditResult SupplyAudit::audit(
    utils::Amount genesisSupply, const std::vector<ValidatorStakeState> &stakes,
    utils::Amount treasury, utils::Amount totalRewardsMinted,
    utils::Amount totalFeesBurned, utils::Amount freeCirculation) {
  ...
  for (const auto &stake : stakes) {
    if (!stake.isValid()) {
      return SupplyAuditResult::invalid("Invalid stake state for validator: " +
  ...
  const utils::Amount lhs =
      (genesisSupply + totalRewardsMinted) - totalFeesBurned;
  const utils::Amount rhs =
      totalBonded + freeCirculation + treasury + totalSlashed;
  ...
  if (lhs != rhs) {
    ...
    oss << "Supply invariant violated: "
        << "lhs=" << lhs.rawUnits() << " != rhs=" << rhs.rawUnits()
  ...
  return SupplyAuditResult::valid(genesisSupply, totalBonded, totalSlashed,
                                  treasury, totalRewardsMinted, totalFeesBurned,
                                  freeCirculation);
}

std::string supplyAuditStatusToString(SupplyAuditStatus status) {
  switch (status) {
  case SupplyAuditStatus::VALID:
    return "VALID";
  case SupplyAuditStatus::INVALID_POLICY:
    return "INVALID_POLICY";
  case SupplyAuditStatus::INVALID_DELTA:
    return "INVALID_DELTA";
  case SupplyAuditStatus::SUPPLY_CONTINUITY_FAILURE:
    return "SUPPLY_CONTINUITY_FAILURE";
  case SupplyAuditStatus::SUPPLY_OVERFLOW:
    return "SUPPLY_OVERFLOW";
  case SupplyAuditStatus::SUPPLY_UNDERFLOW:
    return "SUPPLY_UNDERFLOW";
  default:
    return "UNKNOWN";
  }
}
...
SupplySequenceAuditResult::SupplySequenceAuditResult()
    : m_valid(false), m_status(SupplyAuditStatus::INVALID_POLICY), m_reason(""),
      m_finalSupply(utils::Amount::fromRawUnits(0)), m_deltaCount(0),
      m_failedBlockHeight(0) {}

SupplySequenceAuditResult
SupplySequenceAuditResult::valid(utils::Amount finalSupply,
...
SupplySequenceAuditResult
SupplySequenceAuditResult::invalid(SupplyAuditStatus status, std::string reason,
...
const std::string &SupplySequenceAuditResult::reason() const {
  return m_reason;
}
utils::Amount SupplySequenceAuditResult::finalSupply() const {
  return m_finalSupply;
}
std::size_t SupplySequenceAuditResult::deltaCount() const {
  return m_deltaCount;
}
std::uint64_t SupplySequenceAuditResult::failedBlockHeight() const {
...
std::string SupplySequenceAuditResult::serialize() const {
  ...
  oss << "SupplySequenceAuditResult{"
      ...
      << ";failedBlockHeight=" << m_failedBlockHeight << ";reason=" << m_reason
      << "}";
  return oss.str();
}

SupplySequenceAuditResult
SupplyAudit::auditDeltaSequence(const MonetaryPolicy &policy,
                                const std::vector<SupplyDelta> &deltas) {
  if (!policy.isValid()) {
    return SupplySequenceAuditResult::invalid(
        SupplyAuditStatus::INVALID_POLICY,
        "MonetaryPolicy is invalid: " + policy.rejectionReason(), 0);
  }
  ...
  for (const auto &delta : deltas) {
    ...
    if (delta.supplyBefore() != expectedSupply) {
      return SupplySequenceAuditResult::invalid(
          ...
          "Supply continuity failure at block " +
              std::to_string(delta.blockHeight()) + ": expected supplyBefore=" +
              std::to_string(expectedSupply.rawUnits()) + " but got " +
    ...
    if (delta.supplyAfter().isNegative()) {
      return SupplySequenceAuditResult::invalid(
          ...
          "Supply underflow at block " + std::to_string(delta.blockHeight()) +
              ".",
          delta.blockHeight());
    }
