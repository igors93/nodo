...
StakeAccount::StakeAccount()
    : m_validatorAddress(""), m_bondedAmount(utils::Amount::fromRawUnits(0)),
      m_slashedAmount(utils::Amount::fromRawUnits(0)), m_jailed(false),
      m_tombstoned(false) {}

StakeAccount::StakeAccount(std::string validatorAddress,
...
StakeAccount::StakeAccount(std::string validatorAddress,
                           utils::Amount bondedAmount,
                           utils::Amount slashedAmount, bool jailed,
                           bool tombstoned)
    : m_validatorAddress(std::move(validatorAddress)),
      m_bondedAmount(bondedAmount), m_slashedAmount(slashedAmount),
      m_jailed(jailed), m_tombstoned(tombstoned) {
  if (!isValid())
    throw std::invalid_argument("Invalid complete StakeAccount state.");
}

const std::string &StakeAccount::validatorAddress() const {
...
void StakeAccount::applySlash(utils::Amount amount) {
  ...
  if (amount > remaining) {
    throw std::invalid_argument(
        "Slash amount exceeds remaining stake for validator " +
        m_validatorAddress);
  }
...
void StakeAccount::jail() { m_jailed = true; }
...
bool StakeAccount::isValid() const {
  return !m_validatorAddress.empty() && !m_bondedAmount.isNegative() &&
         !m_slashedAmount.isNegative() && m_slashedAmount <= m_bondedAmount;
}
