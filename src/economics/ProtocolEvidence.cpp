...
std::string protocolEvidenceTypeToString(ProtocolEvidenceType type) {
  switch (type) {
  case ProtocolEvidenceType::P2P_INVALID_MESSAGE:
    return "P2P_INVALID_MESSAGE";
  case ProtocolEvidenceType::P2P_RATE_LIMIT_EXCEEDED:
    return "P2P_RATE_LIMIT_EXCEEDED";
  case ProtocolEvidenceType::P2P_PEER_QUARANTINED:
    return "P2P_PEER_QUARANTINED";
  case ProtocolEvidenceType::P2P_PEER_BANNED:
    return "P2P_PEER_BANNED";
  case ProtocolEvidenceType::DATA_AVAILABILITY_FAILURE:
    return "DATA_AVAILABILITY_FAILURE";
  case ProtocolEvidenceType::DOUBLE_SIGN:
    return "DOUBLE_SIGN";
  case ProtocolEvidenceType::INVALID_BLOCK_VOTE:
    return "INVALID_BLOCK_VOTE";
  default:
    return "UNKNOWN";
  }
}

bool protocolEvidenceTypeFromString(const std::string &s,
                                    ProtocolEvidenceType &out) {
  if (s == "P2P_INVALID_MESSAGE") {
    out = ProtocolEvidenceType::P2P_INVALID_MESSAGE;
    return true;
  }
  if (s == "P2P_RATE_LIMIT_EXCEEDED") {
    out = ProtocolEvidenceType::P2P_RATE_LIMIT_EXCEEDED;
    return true;
  }
  if (s == "P2P_PEER_QUARANTINED") {
    out = ProtocolEvidenceType::P2P_PEER_QUARANTINED;
    return true;
  }
  if (s == "P2P_PEER_BANNED") {
    out = ProtocolEvidenceType::P2P_PEER_BANNED;
    return true;
  }
  if (s == "DATA_AVAILABILITY_FAILURE") {
    out = ProtocolEvidenceType::DATA_AVAILABILITY_FAILURE;
    return true;
  }
  if (s == "DOUBLE_SIGN") {
    out = ProtocolEvidenceType::DOUBLE_SIGN;
    return true;
  }
  if (s == "INVALID_BLOCK_VOTE") {
    out = ProtocolEvidenceType::INVALID_BLOCK_VOTE;
    return true;
  }
  return false;
}
...
ProtocolEvidence::ProtocolEvidence()
    : m_evidenceType(ProtocolEvidenceType::P2P_INVALID_MESSAGE),
      m_blockHeight(0), m_epoch(0), m_createdAt(0), m_valid(false),
      m_rejectionReason("ProtocolEvidence: default-constructed.") {}

ProtocolEvidence::ProtocolEvidence(std::string evidenceId,
                                   ProtocolEvidenceType evidenceType,
                                   std::string subjectId, std::string sourceId,
                                   std::uint64_t blockHeight,
                                   std::uint64_t epoch, std::string ruleId,
                                   std::string payloadDigest,
                                   std::string reason, std::int64_t createdAt)
    : m_evidenceId(std::move(evidenceId)), m_evidenceType(evidenceType),
      m_subjectId(std::move(subjectId)), m_sourceId(std::move(sourceId)),
      m_blockHeight(blockHeight), m_epoch(epoch), m_ruleId(std::move(ruleId)),
      m_payloadDigest(std::move(payloadDigest)), m_reason(std::move(reason)),
      m_createdAt(createdAt), m_valid(false), m_rejectionReason("") {
  ...
  if (!isSerializationSafeValue(m_evidenceId)) {
    m_rejectionReason = "ProtocolEvidence: evidenceId contains unsafe "
  ...
  if (!isSerializationSafeValue(m_subjectId)) {
    m_rejectionReason =
        "ProtocolEvidence: subjectId contains unsafe serialization characters.";
    return;
  }
  if (m_sourceId.empty()) {
  ...
  if (!isSerializationSafeValue(m_sourceId)) {
    m_rejectionReason =
        "ProtocolEvidence: sourceId contains unsafe serialization characters.";
    return;
  }
  if (m_ruleId.empty()) {
  ...
  if (!isSerializationSafeValue(m_ruleId)) {
    m_rejectionReason =
        "ProtocolEvidence: ruleId contains unsafe serialization characters.";
    return;
  }
  ...
  if (m_payloadDigest.size() > kMaxDigestLen) {
    m_rejectionReason =
        "ProtocolEvidence: payloadDigest exceeds maximum length.";
    return;
  }
  if (!isSerializationSafeValue(m_payloadDigest)) {
    m_rejectionReason = "ProtocolEvidence: payloadDigest contains unsafe "
                        "serialization characters.";
    return;
  }
  if (m_reason.empty()) {
  ...
  if (!isSerializationSafeValue(m_reason)) {
    m_rejectionReason =
        "ProtocolEvidence: reason contains unsafe serialization characters.";
    return;
  }
  m_valid = true;
}
...
ProtocolEvidenceType ProtocolEvidence::evidenceType() const {
...
const std::string &ProtocolEvidence::payloadDigest() const {
...
const std::string &ProtocolEvidence::rejectionReason() const {
...
std::string ProtocolEvidence::serialize() const {
  ...
  oss << "ProtocolEvidence{"
      ...
      << ";subjectId=" << m_subjectId << ";sourceId=" << m_sourceId
      << ";blockHeight=" << m_blockHeight << ";epoch=" << m_epoch
      << ";ruleId=" << m_ruleId << ";payloadDigest=" << m_payloadDigest
      << ";reason=" << m_reason << ";createdAt=" << m_createdAt << "}";
  return oss.str();
}
...
const std::string kKnownFields[] = {
    "evidenceId", "evidenceType", "subjectId",     "sourceId", "blockHeight",
    "epoch",      "ruleId",       "payloadDigest", "reason",   "createdAt"};

std::map<std::string, std::string>
parseProtocolEvidenceFields(const std::string &serialized) {
  const std::string prefix = "ProtocolEvidence{";
  if (serialized.rfind(prefix, 0) != 0 || serialized.size() <= prefix.size() ||
      serialized.back() != '}') {
...
ProtocolEvidence ProtocolEvidence::deserialize(const std::string &serialized) {
  ...
  try {
    ...
    throw std::invalid_argument(
        std::string("ProtocolEvidence::deserialize: invalid numeric field: ") +
        e.what());
  }

  ProtocolEvidence evidence(get("evidenceId"), evidenceType, get("subjectId"),
                            get("sourceId"), blockHeight, epoch, get("ruleId"),
                            get("payloadDigest"), get("reason"), createdAt);
