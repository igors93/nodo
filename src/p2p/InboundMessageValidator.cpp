...
InboundMessagePolicy::InboundMessagePolicy()
    : m_maxPayloadBytes(core::ProtocolLimits::MAX_NETWORK_PAYLOAD_BYTES),
      m_maxTtlSeconds(120), m_maxClockSkewSeconds(30),
      m_maxMessagesPerPeerWindow(1000), m_peerWindowSeconds(60) {}

InboundMessagePolicy::InboundMessagePolicy(std::size_t maxPayloadBytes,
                                           std::uint32_t maxTtlSeconds,
                                           std::int64_t maxClockSkewSeconds,
                                           std::size_t maxMessagesPerPeerWindow)
    : InboundMessagePolicy(maxPayloadBytes, maxTtlSeconds, maxClockSkewSeconds,
                           maxMessagesPerPeerWindow, maxTtlSeconds) {}
...
InboundMessagePolicy::InboundMessagePolicy(std::size_t maxPayloadBytes,
    ...
    : m_maxPayloadBytes(maxPayloadBytes), m_maxTtlSeconds(maxTtlSeconds),
...
std::size_t
InboundMessagePolicy::maxPayloadBytes(NetworkMessageType type) const {
  const auto it = m_maxPayloadBytesPerType.find(type);
  if (it != m_maxPayloadBytesPerType.end()) {
    return it->second;
  }
  return m_maxPayloadBytes;
}

void InboundMessagePolicy::setMaxPayloadBytes(NetworkMessageType type,
                                              std::size_t maxBytes) {
  m_maxPayloadBytesPerType[type] = maxBytes;
}
std::uint32_t InboundMessagePolicy::maxTtlSeconds() const {
  return m_maxTtlSeconds;
}
std::int64_t InboundMessagePolicy::maxClockSkewSeconds() const {
  return m_maxClockSkewSeconds;
}
std::size_t InboundMessagePolicy::maxMessagesPerPeerWindow() const {
  return m_maxMessagesPerPeerWindow;
}
std::uint32_t InboundMessagePolicy::peerWindowSeconds() const {
  return m_peerWindowSeconds;
}

bool InboundMessagePolicy::isValid() const {
  return m_maxPayloadBytes > 0 && m_maxTtlSeconds > 0 &&
         m_maxClockSkewSeconds >= 0 && m_maxMessagesPerPeerWindow > 0 &&
         m_peerWindowSeconds > 0;
}

std::string inboundMessageStatusToString(InboundMessageStatus status) {
  switch (status) {
  case InboundMessageStatus::ACCEPTED:
    return "ACCEPTED";
  case InboundMessageStatus::INVALID_POLICY:
    return "INVALID_POLICY";
  case InboundMessageStatus::INVALID_ENVELOPE:
    return "INVALID_ENVELOPE";
  case InboundMessageStatus::WRONG_NETWORK:
    return "WRONG_NETWORK";
  case InboundMessageStatus::WRONG_CHAIN:
    return "WRONG_CHAIN";
  case InboundMessageStatus::WRONG_PROTOCOL:
    return "WRONG_PROTOCOL";
  case InboundMessageStatus::EXPIRED:
    return "EXPIRED";
  case InboundMessageStatus::TTL_TOO_LARGE:
    return "TTL_TOO_LARGE";
  case InboundMessageStatus::CLOCK_SKEW:
    return "CLOCK_SKEW";
  case InboundMessageStatus::DUPLICATE_MESSAGE:
    return "DUPLICATE_MESSAGE";
  case InboundMessageStatus::RATE_LIMITED:
    return "RATE_LIMITED";
  default:
    return "INVALID_ENVELOPE";
  }
...
InboundMessageResult::InboundMessageResult(InboundMessageStatus status,
                                           std::string reason)
...
bool InboundMessageResult::accepted() const {
  return m_status == InboundMessageStatus::ACCEPTED;
}

InboundMessageValidator::InboundMessageValidator(InboundMessagePolicy policy)
    : m_policy(policy), m_seenMessageIds(), m_messagesByPeerInWindow(),
      m_lastPruneTime(0) {}

InboundMessageResult InboundMessageValidator::validate(
    const NetworkEnvelope &envelope, const std::string &expectedNetworkId,
    const std::string &expectedChainId,
    const std::string &expectedProtocolVersion, std::int64_t now) {
  if (!m_policy.isValid()) {
    return InboundMessageResult(InboundMessageStatus::INVALID_POLICY,
                                "Inbound message policy is invalid.");
  }

  if (!envelope.isStructurallyValid(
          m_policy.maxPayloadBytes(envelope.messageType()))) {
  ...
  if (envelope.networkId() != expectedNetworkId) {
    return InboundMessageResult(InboundMessageStatus::WRONG_NETWORK,
                                "Network id mismatch.");
  }

  if (envelope.chainId() != expectedChainId) {
    return InboundMessageResult(InboundMessageStatus::WRONG_CHAIN,
                                "Chain id mismatch.");
  }

  if (envelope.protocolVersion() != expectedProtocolVersion) {
    return InboundMessageResult(InboundMessageStatus::WRONG_PROTOCOL,
                                "Protocol version mismatch.");
  }

  if (envelope.ttlSeconds() > m_policy.maxTtlSeconds()) {
    return InboundMessageResult(InboundMessageStatus::TTL_TOO_LARGE,
                                "Message TTL exceeds local policy.");
  }

  if (envelope.expiredAt(now)) {
    return InboundMessageResult(InboundMessageStatus::EXPIRED,
                                "Message already expired.");
  }

  const std::int64_t delta = now - envelope.createdAt();
  if (delta < 0 && std::llabs(delta) > m_policy.maxClockSkewSeconds()) {
    return InboundMessageResult(InboundMessageStatus::CLOCK_SKEW,
  ...
  if (peerWindow.windowStartedAt <= 0 || now < peerWindow.windowStartedAt ||
  ...
  if (peerWindow.count >= m_policy.maxMessagesPerPeerWindow()) {
    return InboundMessageResult(InboundMessageStatus::RATE_LIMITED,
                                "Peer exceeded message window policy.");
  }
  ...
  return InboundMessageResult(InboundMessageStatus::ACCEPTED,
...
std::size_t InboundMessageValidator::peerWindowCount(
...
void InboundMessageValidator::pruneExpiredState(std::int64_t now) {
  ...
  for (auto iterator = m_messagesByPeerInWindow.begin();
    ...
    if (window.windowStartedAt <= 0 || now < window.windowStartedAt ||
