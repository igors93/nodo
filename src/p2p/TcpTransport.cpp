...
void cleanupSocketRuntime() { WSACleanup(); }

bool isInterruptedSocketError() { return WSAGetLastError() == WSAEINTR; }

bool isWouldBlockSocketError() { return WSAGetLastError() == WSAEWOULDBLOCK; }
...
#else
bool startupSocketRuntime() { return true; }

void cleanupSocketRuntime() {}

bool isInterruptedSocketError() { return errno == EINTR; }

bool isWouldBlockSocketError() {
  return errno == EAGAIN || errno == EWOULDBLOCK;
}

bool isConnectInProgressSocketError() { return errno == EINPROGRESS; }
...
bool waitForSocketReady(SocketHandle fd, bool writeReady, int timeoutMs) {
  while (true) {
    fd_set readSet;
    fd_set writeSet;
    FD_ZERO(&readSet);
    ...
    const int result = select(
        ...
        fd + 1,
#endif
        writeReady ? nullptr : &readSet, writeReady ? &writeSet : nullptr,
        nullptr, &timeout);
    ...
    return writeReady ? FD_ISSET(static_cast<NativeSocket>(fd), &writeSet)
...
bool writeAll(SocketHandle fd, const unsigned char *data, std::size_t size) {
  ...
  while (written < size) {
    const int result = ::send(static_cast<NativeSocket>(fd),
                              reinterpret_cast<const char *>(data + written),
                              static_cast<int>(size - written), SEND_FLAGS);
...
bool readAll(SocketHandle fd, unsigned char *data, std::size_t size) {
  ...
  while (read < size) {
    const int result = ::recv(static_cast<NativeSocket>(fd),
                              reinterpret_cast<char *>(data + read),
                              static_cast<int>(size - read), 0);
...
TcpIpRateLimitPolicy::TcpIpRateLimitPolicy(
    std::size_t bucketCapacity, std::size_t refillTokens,
    std::chrono::milliseconds refillInterval,
    std::chrono::milliseconds initialBackoff,
    std::chrono::milliseconds maxBackoff)
    : m_bucketCapacity(bucketCapacity), m_refillTokens(refillTokens),
      m_refillInterval(refillInterval), m_initialBackoff(initialBackoff),
...
bool TcpIpRateLimitPolicy::isValid() const {
  ...
  return m_bucketCapacity > 0 && m_bucketCapacity <= 1'000'000 &&
         m_refillTokens > 0 && m_refillTokens <= m_bucketCapacity &&
         m_refillInterval.count() > 0 && m_refillInterval <= maximumDuration &&
         m_initialBackoff.count() > 0 && m_initialBackoff <= maximumDuration &&
         m_maxBackoff >= m_initialBackoff && m_maxBackoff <= maximumDuration;
}

std::string tcpConnectionDirectionToString(TcpConnectionDirection direction) {
  switch (direction) {
  case TcpConnectionDirection::INBOUND:
    return "INBOUND";
  case TcpConnectionDirection::OUTBOUND:
    return "OUTBOUND";
  }
  return "INBOUND";
}
...
TcpConnectionSlotPolicy::TcpConnectionSlotPolicy()
    : m_maxTotal(DEFAULT_MAX_TOTAL), m_maxInbound(DEFAULT_MAX_INBOUND),
      m_maxOutbound(DEFAULT_MAX_OUTBOUND), m_maxPerIp(DEFAULT_MAX_PER_IP),
      m_maxPerSubnet(DEFAULT_MAX_PER_SUBNET) {}

TcpConnectionSlotPolicy::TcpConnectionSlotPolicy(std::size_t maxTotal,
                                                 std::size_t maxInbound,
                                                 std::size_t maxOutbound,
                                                 std::size_t maxPerIp,
                                                 std::size_t maxPerSubnet)
    : m_maxTotal(maxTotal), m_maxInbound(maxInbound),
      m_maxOutbound(maxOutbound), m_maxPerIp(maxPerIp),
...
std::size_t TcpConnectionSlotPolicy::maxOutbound() const {
  return m_maxOutbound;
}
std::size_t TcpConnectionSlotPolicy::maxPerIp() const { return m_maxPerIp; }
std::size_t TcpConnectionSlotPolicy::maxPerSubnet() const {
  return m_maxPerSubnet;
}

bool TcpConnectionSlotPolicy::isValid() const {
  return m_maxTotal > 0 && m_maxInbound > 0 && m_maxOutbound > 0 &&
         m_maxPerIp > 0 && m_maxPerSubnet > 0 && m_maxInbound <= m_maxTotal &&
         m_maxOutbound <= m_maxTotal && m_maxPerIp <= m_maxTotal &&
         m_maxPerSubnet <= m_maxTotal;
}

TcpCandidatePolicy::TcpCandidatePolicy()
    : m_maxTotal(DEFAULT_MAX_TOTAL), m_maxPerIp(DEFAULT_MAX_PER_IP),
      m_maxPerSubnet(DEFAULT_MAX_TOTAL),
      m_authenticationTimeout(DEFAULT_TIMEOUT_MILLISECONDS), m_ipRateLimit(),
      m_connectionSlots() {}

TcpCandidatePolicy::TcpCandidatePolicy(
    std::size_t maxTotal, std::size_t maxPerIp,
    std::chrono::milliseconds authenticationTimeout)
    : m_maxTotal(maxTotal), m_maxPerIp(maxPerIp), m_maxPerSubnet(maxTotal),
      m_authenticationTimeout(authenticationTimeout), m_ipRateLimit(),
      m_connectionSlots() {}
...
TcpCandidatePolicy::TcpCandidatePolicy(
    ...
    : m_maxTotal(maxTotal), m_maxPerIp(maxPerIp), m_maxPerSubnet(maxTotal),
      ...
      m_ipRateLimit(std::move(ipRateLimit)), m_connectionSlots() {}

TcpCandidatePolicy::TcpCandidatePolicy(
    std::size_t maxTotal, std::size_t maxPerIp,
    std::chrono::milliseconds authenticationTimeout,
    TcpIpRateLimitPolicy ipRateLimit, TcpConnectionSlotPolicy connectionSlots)
    : m_maxTotal(maxTotal), m_maxPerIp(maxPerIp), m_maxPerSubnet(maxTotal),
      m_authenticationTimeout(authenticationTimeout),
...
TcpCandidatePolicy::TcpCandidatePolicy(
    std::size_t maxTotal, std::size_t maxPerIp, std::size_t maxPerSubnet,
    std::chrono::milliseconds authenticationTimeout,
    TcpIpRateLimitPolicy ipRateLimit, TcpConnectionSlotPolicy connectionSlots)
    : m_maxTotal(maxTotal), m_maxPerIp(maxPerIp), m_maxPerSubnet(maxPerSubnet),
      m_authenticationTimeout(authenticationTimeout),
      m_ipRateLimit(std::move(ipRateLimit)),
      m_connectionSlots(std::move(connectionSlots)) {}
...
bool TcpCandidatePolicy::isValid() const {
  return m_maxTotal > 0 && m_maxPerIp > 0 && m_maxPerSubnet > 0 &&
         m_maxPerIp <= m_maxTotal && m_maxPerSubnet <= m_maxTotal &&
         m_authenticationTimeout.count() > 0 && m_ipRateLimit.isValid() &&
         m_connectionSlots.isValid();
}

TcpTransport::TcpTransport() : TcpTransport(TcpCandidatePolicy()) {}

TcpTransport::TcpTransport(TcpCandidatePolicy candidatePolicy)
    : m_listenFd(INVALID_FD), m_socketRuntimeReady(false), m_localNodeId(),
      m_localEndpoint(), m_peerEndpoints(), m_connectionsByPeer(),
      m_candidateInboundConnections(), m_candidateByPeer(),
      m_candidateCountByIp(), m_candidateCountBySubnet(),
      m_ipAdmissionByAddress(), m_subnetAdmissionByPrefix(),
      m_candidatePolicy(std::move(candidatePolicy)), m_expiredCandidateCount(0),
      m_rateLimitedCandidateCount(0), m_temporalRateLimitedConnectionCount(0),
      m_subnetRateLimitedConnectionCount(0), m_evictedConnectionCount(0),
      m_slotRejectedConnectionCount(0), m_nextConnectionId(1) {
...
TransportResult TcpTransport::bind(const std::string &localNodeId,
  ...
  int reuse = 1;
  (void)::setsockopt(static_cast<NativeSocket>(fd), SOL_SOCKET, SO_REUSEADDR,
  ...
  if (::bind(static_cast<NativeSocket>(fd),
  ...
  if (::getsockname(static_cast<NativeSocket>(fd),
  ...
  m_localEndpoint = PeerEndpoint(endpoint.host(), ntohs(boundAddress.sin_port));

  return TransportResult(TransportStatus::SENT,
...
bool TcpTransport::listening() const { return m_listenFd != INVALID_FD; }

const std::string &TcpTransport::localNodeId() const { return m_localNodeId; }

const PeerEndpoint &TcpTransport::localEndpoint() const {
  return m_localEndpoint;
}

void TcpTransport::setIpQuarantineCheck(
    std::function<bool(const std::string &)> check) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  m_ipQuarantineCheck = std::move(check);
}

std::uint16_t TcpTransport::localPort() const { return m_localEndpoint.port(); }

void TcpTransport::registerPeerEndpoint(const std::string &remoteNodeId,
                                        const PeerEndpoint &endpoint) {
...
bool TcpTransport::hasPeerEndpoint(const std::string &remoteNodeId) const {
...
std::size_t
TcpTransport::connectedCountForIp(const std::string &remoteIp) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  std::size_t count = 0;
  for (const auto &[peer, connection] : m_connectionsByPeer) {
    (void)peer;
...
std::size_t
TcpTransport::connectedCountForSubnet(const std::string &subnetPrefix) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  std::size_t count = 0;
  for (const auto &[peer, connection] : m_connectionsByPeer) {
    (void)peer;
...
std::size_t
TcpTransport::pendingCandidateCountForIp(const std::string &remoteIp) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
std::size_t TcpTransport::pendingCandidateCountForSubnet(
    const std::string &subnetPrefix) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
TcpTransport::ipBackoffRemaining(const std::string &remoteIp) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  const auto found = m_ipAdmissionByAddress.find(remoteIp);
...
std::chrono::milliseconds
TcpTransport::subnetBackoffRemaining(const std::string &subnetPrefix) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  const auto found = m_subnetAdmissionByPrefix.find(subnetPrefix);
  const auto now = std::chrono::steady_clock::now();
...
TransportResult TcpTransport::connect(const std::string &localNodeId,
  ...
  if (endpoint == m_peerEndpoints.end()) {
    return TransportResult(TransportStatus::NOT_CONNECTED,
  ...
  if (remoteSubnet.empty() ||
    ...
    return TransportResult(TransportStatus::REJECTED,
                           "Outbound TCP endpoint exceeded per-IP or "
                           "per-subnet connection rate limit.");
  }

  if (!ensureConnectionSlotFor(remoteNodeId, remoteIp,
  ...
  if (!setNonBlocking(fd)) {
    ...
    return TransportResult(
        ...
        lastSocketErrorMessage(
            "Unable to set outbound TCP socket nonblocking"));
  }
  ...
  if (::inet_pton(AF_INET, endpoint->second.host().c_str(),
                  &address.sin_addr) != 1) {
    SocketHandle closeFd = fd;
    closeSocket(closeFd);
    return TransportResult(TransportStatus::REJECTED,
  ...
  if (::connect(static_cast<NativeSocket>(fd),
                reinterpret_cast<sockaddr *>(&address),
                static_cast<SocketLength>(sizeof(address))) != 0) {
    ...
    if (socketError != 0) {
      ...
      reason << "Unable to connect TCP peer: socket error " << socketError;
      return TransportResult(TransportStatus::NOT_CONNECTED, reason.str());
    }
  }

  rememberConnection(remoteNodeId, fd, false, TcpConnectionDirection::OUTBOUND,
                     remoteIp);

  return TransportResult(TransportStatus::SENT, "TCP peer connected.");
}

TransportResult TcpTransport::disconnect(const std::string &localNodeId,
                                         const std::string &remoteNodeId) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
...
bool TcpTransport::connected(const std::string &localNodeId,
...
TransportResult TcpTransport::send(const TransportMessage &message) {
  ...
  if (!message.isValid()) {
    return TransportResult(TransportStatus::INVALID_MESSAGE,
  ...
  return TransportResult(TransportStatus::SENT, "TCP frame sent.");
}

std::optional<TransportMessage>
TcpTransport::poll(const std::string &localNodeId) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  ...
  for (auto iterator = m_connectionsByPeer.begin();
    ...
    if (result.status == PollFdResult::Status::MESSAGE) {
      ...
      return TransportMessage(message.fromNodeId(), message.toNodeId(),
  ...
  for (auto iterator = m_candidateInboundConnections.begin();
    ...
    if (result.status == PollFdResult::Status::MESSAGE) {
      ...
      if (!isSafeNodeId(fromId) || (!iterator->second.claimedNodeId.empty() &&
...
bool TcpTransport::authenticateConnection(TransportConnectionId connectionId,
                                          const std::string &remoteNodeId) {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  if (connectionId == 0 || !isSafeNodeId(remoteNodeId))
    return false;
  ...
  if (!ensureConnectionSlotFor(remoteNodeId, candidate->second.remoteIp,
  ...
  const ManagedConnection promoted{candidate->second.fd,
                                   candidate->second.id,
                                   true,
                                   TcpConnectionDirection::INBOUND,
...
bool TcpTransport::isConnectionAuthenticated(
    TransportConnectionId connectionId, const std::string &remoteNodeId) const {
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  const auto found = m_connectionsByPeer.find(remoteNodeId);
  return found != m_connectionsByPeer.end() &&
         found->second.id == connectionId && found->second.authenticated;
}
...
TransportResult TcpTransport::acceptAvailableConnections() {
  ...
  while (true) {
    ...
    SocketLength remoteLength =
        static_cast<SocketLength>(sizeof(remoteAddress));

    const SocketHandle fd = static_cast<SocketHandle>(
        ::accept(static_cast<NativeSocket>(m_listenFd),
    ...
    const char *convertedIp =
        ::inet_ntop(AF_INET, &remoteAddress.sin_addr, remoteIpBuffer.data(),
    ...
    if (m_ipQuarantineCheck && m_ipQuarantineCheck(remoteIp)) {
      SocketHandle closeFd = fd;
      closeSocket(closeFd);
      continue;
    }
    ...
    const std::string remoteSubnet = subnetPrefixForIp(remoteIp);
    const auto now = std::chrono::steady_clock::now();
    if (remoteSubnet.empty() || !consumeIpAdmissionToken(remoteIp, now) ||
    ...
    const std::size_t candidatesForIp = pendingCandidateCountForIp(remoteIp);
    const std::size_t candidatesForSubnet =
        pendingCandidateCountForSubnet(remoteSubnet);
    if (m_candidateInboundConnections.size() >= m_candidatePolicy.maxTotal() ||
    ...
    m_candidateInboundConnections.emplace(
        connectionId,
        CandidateConnection{fd, connectionId, "", remoteIp, remoteSubnet, now});
    ++m_candidateCountByIp[remoteIp];
    ++m_candidateCountBySubnet[remoteSubnet];
  }

  return TransportResult(TransportStatus::SENT, "TCP accept cycle completed.");
}
...
TcpTransport::PollFdResult
TcpTransport::PollFdResult::received(TransportMessage message) {
  PollFdResult result;
...
TcpTransport::PollFdResult TcpTransport::pollFd(SocketHandle fd,
                                                bool unidentified) {
  (void)unidentified;
  ...
  const int peeked =
      ::recv(static_cast<NativeSocket>(fd), reinterpret_cast<char *>(header),
             static_cast<int>(sizeof(header)), MSG_PEEK);
  if (peeked == 0) {
    return PollFdResult::closed();
  }
  ...
  if (availableBytes <
      static_cast<IoctlAvailableBytes>(sizeof(header) + frameSize)) {
    return PollFdResult::none();
  }
...
void TcpTransport::rememberConnection(const std::string &remoteNodeId,
                                      SocketHandle fd, bool authenticated,
...
SocketHandle
TcpTransport::socketForSend(const TransportMessage &message) const {
  ...
  const auto active = m_connectionsByPeer.find(message.toNodeId());
  return active == m_connectionsByPeer.end() ? INVALID_FD : active->second.fd;
}

void TcpTransport::closeCandidateConnection(
    TransportConnectionId connectionId) {
  const auto found = m_candidateInboundConnections.find(connectionId);
  if (found == m_candidateInboundConnections.end())
    return;
  if (!found->second.claimedNodeId.empty()) {
    const auto claimed = m_candidateByPeer.find(found->second.claimedNodeId);
    if (claimed != m_candidateByPeer.end() && claimed->second == connectionId) {
...
void TcpTransport::pruneExpiredCandidateConnections(
  ...
  for (const auto &[connectionId, candidate] : m_candidateInboundConnections) {
...
void TcpTransport::decrementCandidateIpCount(const std::string &remoteIp) {
  const auto found = m_candidateCountByIp.find(remoteIp);
  if (found == m_candidateCountByIp.end())
    return;
  if (found->second <= 1) {
...
void TcpTransport::decrementCandidateSubnetCount(
  ...
  const auto found = m_candidateCountBySubnet.find(subnetPrefix);
  if (found == m_candidateCountBySubnet.end())
    return;
  if (found->second <= 1) {
...
bool TcpTransport::consumeIpAdmissionToken(
    const std::string &remoteIp, std::chrono::steady_clock::time_point now) {
  const TcpIpRateLimitPolicy &policy = m_candidatePolicy.ipRateLimit();
  const auto inserted = m_ipAdmissionByAddress.emplace(
      remoteIp, IpAdmissionState{policy.bucketCapacity(), 0, now, now, now});
  ...
  if (refillIntervals > 0) {
    const std::size_t missingTokens = policy.bucketCapacity() - state.tokens;
    const std::uint64_t intervals = static_cast<std::uint64_t>(refillIntervals);
    const std::uint64_t intervalsToFill =
        (missingTokens + policy.refillTokens() - 1) / policy.refillTokens();
    if (intervals >= intervalsToFill) {
      state.tokens = policy.bucketCapacity();
    } else {
      state.tokens +=
          static_cast<std::size_t>(intervals) * policy.refillTokens();
    }
    state.lastRefillAt += policy.refillInterval() * refillIntervals;
  ...
  state.blockedUntil = now + backoffForLimitHits(state.consecutiveLimitHits);
  return false;
}
...
bool TcpTransport::consumeSubnetAdmissionToken(
  ...
  const auto inserted = m_subnetAdmissionByPrefix.emplace(
      subnetPrefix,
      IpAdmissionState{policy.bucketCapacity(), 0, now, now, now});
  IpAdmissionState &state = inserted.first->second;
  state.lastActivityAt = now;
...
void TcpTransport::pruneIdleIpAdmissionStates(
  ...
  for (auto iterator = m_ipAdmissionByAddress.begin();
    ...
    const bool hasCandidates = m_candidateCountByIp.find(iterator->first) !=
  ...
  for (auto iterator = m_subnetAdmissionByPrefix.begin();
    ...
    const bool hasCandidates = m_candidateCountBySubnet.find(iterator->first) !=
...
std::chrono::milliseconds
TcpTransport::backoffForLimitHits(std::size_t limitHits) const {
  const TcpIpRateLimitPolicy &policy = m_candidatePolicy.ipRateLimit();
  std::chrono::milliseconds backoff = policy.initialBackoff();
  for (std::size_t hit = 1; hit < limitHits && backoff < policy.maxBackoff();
       ++hit) {
...
bool TcpTransport::ensureConnectionSlotFor(
    const std::string &remoteNodeId, const std::string &remoteIp,
    TcpConnectionDirection direction,
    std::chrono::steady_clock::time_point now) {
  (void)now;
  ...
  const std::size_t totalAfter =
      m_connectionsByPeer.size() + (replacingExisting ? 0 : 1);
  const std::size_t directionAfter =
      connectedDirectionCount(direction) +
  ...
  const bool needsEviction = totalAfter > policy.maxTotal() ||
  ...
  const std::size_t finalTotal =
      m_connectionsByPeer.size() +
      (m_connectionsByPeer.find(remoteNodeId) == m_connectionsByPeer.end() ? 1
                                                                           : 0);
  const auto finalExisting = m_connectionsByPeer.find(remoteNodeId);
  const std::size_t finalDirection =
      connectedDirectionCount(direction) +
      ((finalExisting == m_connectionsByPeer.end() ||
        finalExisting->second.direction != direction)
           ? 1
           : 0);
...
TcpTransport::evictionCandidateFor(const std::string &remoteNodeId,
                                   const std::string &remoteIp,
                                   TcpConnectionDirection direction) const {
  ...
  const auto consider = [&](bool sameDirectionOnly, bool sameSubnetOnly,
                            bool unauthOnly) {
  ...
  if (consider(true, true, true))
    return bestPeer;
  if (consider(true, false, true))
    return bestPeer;
  if (consider(true, true, false))
    return bestPeer;
  if (consider(true, false, false))
    return bestPeer;
  if (consider(false, true, true))
    return bestPeer;
  if (consider(false, false, true))
    return bestPeer;
...
std::size_t
TcpTransport::connectedDirectionCount(TcpConnectionDirection direction) const {
  std::size_t count = 0;
  for (const auto &[peerId, connection] : m_connectionsByPeer) {
    (void)peerId;
...
void TcpTransport::closePeerConnection(const std::string &remoteNodeId) {
  const auto found = m_connectionsByPeer.find(remoteNodeId);
