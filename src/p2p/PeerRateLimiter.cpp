...
PeerRateLimiter::PeerRateLimiter(std::uint32_t maxMessagesPerWindow,
                                 std::uint64_t windowSeconds)
    : m_maxMessagesPerWindow(maxMessagesPerWindow),
      m_windowSeconds(windowSeconds) {}

void PeerRateLimiter::setLimitForType(NetworkMessageType type,
                                      std::uint32_t limit) {
  m_customLimits[type] = limit;
}

std::uint32_t PeerRateLimiter::limitForType(NetworkMessageType type) const {
  auto it = m_customLimits.find(type);
  if (it != m_customLimits.end()) {
    return it->second;
  }
  return m_maxMessagesPerWindow;
}

std::string PeerRateLimiter::windowKey(const std::string &nodeId,
...
void PeerRateLimiter::advanceWindowIfExpired(PeerWindow &window,
                                             std::int64_t now) const {
  const bool clockMovedBackwards = now < window.windowStart;
  const std::uint64_t elapsed =
      clockMovedBackwards ? 0
...
bool PeerRateLimiter::shouldAllow(const std::string &nodeId,
                                  NetworkMessageType messageType,
                                  std::int64_t now) {
  const std::uint32_t limit = limitForType(messageType);
  if (nodeId.empty() || nodeId.size() > 128 || now <= 0 || limit == 0 ||
      m_windowSeconds == 0) {
    return false;
  }
...
bool PeerRateLimiter::recordInvalidMessage(const std::string &nodeId,
                                           std::int64_t now) {
...
std::uint32_t PeerRateLimiter::messageCount(const std::string &nodeId,
                                            std::int64_t now) const {
...
std::uint32_t PeerRateLimiter::maxMessagesPerWindow() const {
  return m_maxMessagesPerWindow;
}
