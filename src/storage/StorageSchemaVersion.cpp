...
std::uint64_t parseU64Strict(
    ...
    if (found->second.empty()) {
        throw std::invalid_argument("Storage schema numeric field is empty: " + key);
    }

    for (const char current : found->second) {
        if (current < '0' || current > '9') {
            throw std::invalid_argument("Storage schema numeric field is malformed: " + key);
        }
    }
...
std::string parseStringStrict(
    ...
    if (!isSafeScalar(found->second)) {
        throw std::invalid_argument("Storage schema scalar field is unsafe: " + key);
    }
...
const std::string& StorageSchemaVersion::schemaId() const {
    return m_schemaId;
}

std::uint64_t StorageSchemaVersion::version() const {
    return m_version;
}
...
bool StorageSchemaVersion::isStructurallyValid() const {
    return isSafeScalar(m_schemaId) &&
           m_version > 0 &&
           m_minimumCompatibleVersion > 0 &&
...
bool StorageSchemaVersion::isSupportedNodeDataDirectoryVersion() const {
    return isStructurallyValid() &&
           m_schemaId == nodeDataDirectorySchemaId() &&
...
std::string StorageSchemaVersion::toFileContents() const {
    if (!isStructurallyValid()) {
        throw std::invalid_argument("Invalid storage schema version cannot be serialized.");
    }
    ...
    return serialization::KeyValueFileCodec::serialize(
        STORAGE_SCHEMA_FILE_VERSION,
        {
            ...
            {"minimumCompatibleVersion", std::to_string(m_minimumCompatibleVersion)}
        }
    );
}
...
StorageSchemaVersion StorageSchemaVersion::currentNodeDataDirectorySchema() {
    return StorageSchemaVersion(
        nodeDataDirectorySchemaId(),
        currentNodeDataDirectoryVersion(),
        minimumSupportedNodeDataDirectoryVersion()
    );
}

StorageSchemaVersion StorageSchemaVersion::fromFileContents(
    ...
    StorageSchemaVersion schema(
        parseStringStrict(fields, "schemaId"),
        parseU64Strict(fields, "version"),
    ...
    if (schema.toFileContents() != contents) {
        throw std::invalid_argument("Storage schema version file is not canonical.");
    }
...
void StorageSchemaVersion::writeCurrentNodeDataDirectoryVersionFile(
    ...
    AtomicFile::writeTextFile(
        StorageSchemaVersionFile::pathForRoot(rootPath),
...
StorageSchemaValidationResult StorageSchemaValidationResult::accepted(
    StorageSchemaVersion schema
) {
...
StorageSchemaValidationResult StorageSchemaValidationResult::rejected(
    StorageSchemaValidationStatus status,
    std::string reason
) {
    StorageSchemaValidationResult result;
...
const std::optional<StorageSchemaVersion>& StorageSchemaValidationResult::schema() const {
    return m_schema;
}
...
std::filesystem::path StorageSchemaVersionFile::pathForRoot(
    const std::filesystem::path& rootPath
) {
    if (rootPath.empty()) {
        throw std::invalid_argument("Storage schema root path cannot be empty.");
...
StorageSchemaValidationResult StorageSchemaVersionFile::validateNodeDataDirectoryRoot(
    const std::filesystem::path& rootPath
) {
    if (rootPath.empty()) {
    ...
    try {
        const StorageSchemaVersion schema =
            StorageSchemaVersion::fromFileContents(
                AtomicFile::readTextFile(schemaPath)
            );

        if (schema.schemaId() != StorageSchemaVersion::nodeDataDirectorySchemaId()) {
            return StorageSchemaValidationResult::rejected(
        ...
        if (schema.version() > StorageSchemaVersion::currentNodeDataDirectoryVersion() ||
            schema.minimumCompatibleVersion() > StorageSchemaVersion::currentNodeDataDirectoryVersion()) {
            return StorageSchemaValidationResult::rejected(
        ...
        if (schema.version() < StorageSchemaVersion::minimumSupportedNodeDataDirectoryVersion()) {
            return StorageSchemaValidationResult::rejected(
        ...
        return StorageSchemaValidationResult::rejected(
            StorageSchemaValidationStatus::NON_CANONICAL,
            error.what()
        );
