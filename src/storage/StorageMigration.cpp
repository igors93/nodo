...
bool migrateV1toV2(const std::filesystem::path &dataRoot) {
  ...
  const StorageSchemaVersion v2(
      StorageSchemaVersion::nodeDataDirectorySchemaId(), 2, 1);
  storage::AtomicFile::writeTextFile(
      StorageSchemaVersionFile::pathForRoot(dataRoot), v2.toFileContents());
...
std::string storageMigrationStatusToString(StorageMigrationStatus status) {
  switch (status) {
  case StorageMigrationStatus::OK:
    return "OK";
  case StorageMigrationStatus::ALREADY_CURRENT:
    return "ALREADY_CURRENT";
  case StorageMigrationStatus::UNSUPPORTED_DOWNGRADE:
    return "UNSUPPORTED_DOWNGRADE";
  case StorageMigrationStatus::MIGRATION_FAILED:
    return "MIGRATION_FAILED";
  case StorageMigrationStatus::INVALID_INPUT:
...
StorageMigrationResult::StorageMigrationResult()
    : m_status(StorageMigrationStatus::INVALID_INPUT),
      m_reason("Uninitialized."), m_finalVersion(0) {}
...
StorageMigrationResult::alreadyCurrent(std::uint64_t version) {
  StorageMigrationResult r;
...
StorageMigrationResult
StorageMigrationResult::rejected(StorageMigrationStatus status,
                                 std::string reason) {
  StorageMigrationResult r;
  r.m_status = status;
...
StorageMigrationStatus StorageMigrationResult::status() const {
  return m_status;
}
const std::string &StorageMigrationResult::reason() const { return m_reason; }
std::uint64_t StorageMigrationResult::finalVersion() const {
  return m_finalVersion;
}
...
std::string StorageMigrationResult::serialize() const {
  ...
  oss << "StorageMigrationResult{"
      ...
      << ";reason=" << m_reason << ";finalVersion=" << m_finalVersion << "}";
  return oss.str();
}
...
StorageMigrationResult
StorageMigrationRunner::migrate(const std::filesystem::path &dataRoot,
  ...
  if (currentVersion > targetVersion) {
    return StorageMigrationResult::rejected(
        ...
        "Downgrade from v" + std::to_string(currentVersion) + " to v" +
  ...
  for (const auto *step : applicable) {
    if (step->fromVersion != version) {
      return StorageMigrationResult::rejected(
          ...
          "Missing migration step from v" + std::to_string(version) + " to v" +
              std::to_string(step->fromVersion));
    }
    if (!step->apply(dataRoot)) {
      return StorageMigrationResult::rejected(
          StorageMigrationStatus::MIGRATION_FAILED,
          "Migration step v" + std::to_string(step->fromVersion) + "->v" +
              std::to_string(step->toVersion) + " (" + step->description +
              ") failed.");
    }
...
StorageMigrationRunner StorageMigrationRunner::defaultRunner() {
  ...
  runner.registerStep({1, 2, "Add peers sub-directory", migrateV1toV2});
  return runner;
}
