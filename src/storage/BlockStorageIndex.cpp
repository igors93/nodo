...
std::uint64_t BlockIndexEntry::blockIndex() const {
    return m_blockIndex;
}

const std::string& BlockIndexEntry::blockHash() const {
    return m_blockHash;
}

const std::string& BlockIndexEntry::fileName() const {
    return m_fileName;
}
...
bool BlockIndexEntry::matchesBlock(
    const core::Block& block
) const {
    if (!isValid()) {
        return false;
...
std::string BlockIndexEntry::expectedFileName(
    std::uint64_t blockIndex,
    const std::string& blockHash
) {
    if (!isSafeHash(blockHash)) {
        throw std::invalid_argument("Unsafe block hash rejected by BlockIndexEntry.");
    }
...
bool BlockIndexEntry::isSafeHash(
...
bool BlockIndexEntry::isSafeFileName(
...
BlockStorageIndex BlockStorageIndex::fromBlockchainAndManifest(
    ...
    if (createdAt <= 0) {
        throw std::invalid_argument("BlockStorageIndex timestamp must be positive.");
    }
...
std::string BlockStorageIndex::indexFileName() {
    return "block_index.nodo";
}

std::string BlockStorageIndex::indexFilePath(
    const std::string& rootDirectory
) {
    if (rootDirectory.empty()) {
        throw std::invalid_argument("BlockStorageIndex root directory cannot be empty.");
    }

    return (std::filesystem::path(rootDirectory) / indexFileName()).string();
}

BlockStorageIndex BlockStorageIndex::readFromStorageRoot(
    const std::string& rootDirectory
) {
...
BlockStorageIndex::BlockStorageIndex(
    ...
    : m_indexVersion(std::move(indexVersion)),
      ...
      m_indexHash(std::move(indexHash)) {}
...
std::size_t BlockStorageIndex::blockCount() const {
    return m_blockCount;
}
...
std::int64_t BlockStorageIndex::createdAt() const {
    return m_createdAt;
}

const std::string& BlockStorageIndex::indexHash() const {
    return m_indexHash;
}
...
bool BlockStorageIndex::isValid() const {
    ...
    const std::string expectedHash = computeIndexHash(
        m_indexVersion,
        m_chainManifestHash,
        m_blockCount,
        m_entries,
...
void BlockStorageIndex::writeToStorageRoot(
    ...
    if (!isValid()) {
        throw std::invalid_argument("Invalid BlockStorageIndex rejected by storage writer.");
    }
...
bool BlockStorageIndex::isSafeHash(
    const std::string& hash
) {
    if (hash.empty()) {
        return false;
...
std::string BlockStorageIndex::hashString(
