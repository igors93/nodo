...
bool isEvidenceFile(const std::filesystem::directory_entry &entry) {
...
std::size_t evidenceFileCount(const std::filesystem::path &evidenceDirectory) {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(evidenceDirectory)) {
...
void SlashingEvidenceStore::persist(
  ...
  if (!validation.accepted()) {
    throw std::invalid_argument("Cannot persist invalid double-vote evidence.");
  }
  ...
  AtomicFile::writeTextFile(pathForEvidenceId(evidenceId), serialized);
}

void SlashingEvidenceStore::persist(
    const consensus::ProposerEquivocationEvidence &evidence) {
  const consensus::SlashingEvidenceValidationResult validation = consensus::
...
bool SlashingEvidenceStore::contains(const std::string &evidenceId) const {
...
SlashingEvidenceStore::load(const std::string &evidenceId) const {
  if (!contains(evidenceId)) {
  ...
  if (sizeError || fileSize == 0 ||
      fileSize > core::ProtocolLimits::MAX_SLASHING_EVIDENCE_GOSSIP_BYTES) {
    throw std::runtime_error("Stored slashing evidence has an invalid size.");
  }
...
SlashingEvidenceStore::loadProposerEquivocation(
...
SlashingEvidenceStore::loadAll() const {
  ...
  for (const auto &entry :
    ...
    if (isDoubleVoteEncoding(serialized)) {
      ...
      throw std::runtime_error(
          "Stored slashing evidence has an unknown encoding.");
    }
  }
...
SlashingEvidenceStore::loadAllProposerEquivocation() const {
  ...
  for (const auto &entry :
    ...
    if (isProposerEquivocationEncoding(serialized)) {
      evidence.push_back(
          loadProposerEquivocation(entry.path().stem().string()));
...
bool SlashingEvidenceStore::erase(const std::string &evidenceId) {
  if (!isSafeEvidenceId(evidenceId)) {
    return false;
  }
...
std::filesystem::path
SlashingEvidenceStore::pathForEvidenceId(const std::string &evidenceId) const {
  if (!isSafeEvidenceId(evidenceId)) {
...
bool SlashingEvidenceStore::isSafeEvidenceId(const std::string &evidenceId) {
  if (evidenceId.empty() || evidenceId.size() > 160) {
    return false;
  }

  for (const char current : evidenceId) {
    const bool allowed = (current >= 'a' && current <= 'z') ||
                         (current >= 'A' && current <= 'Z') ||
