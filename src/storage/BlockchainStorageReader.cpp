...
StoredBlockSnapshot StoredBlockSnapshot::fromIndexEntry(
    ...
    if (!entry.isValid()) {
        throw std::invalid_argument("Invalid BlockIndexEntry rejected by storage reader.");
    }

    const std::string filePath = buildSnapshotPath(
        rootDirectory,
        entry.fileName()
    );
...
std::uint64_t StoredBlockSnapshot::blockIndex() const {
    return m_blockIndex;
}
...
const std::string& StoredBlockSnapshot::fileName() const {
    return m_fileName;
}

const std::string& StoredBlockSnapshot::filePath() const {
    return m_filePath;
}

std::size_t StoredBlockSnapshot::contentSize() const {
...
std::string StoredBlockSnapshot::buildSnapshotPath(
    const std::string& rootDirectory,
    ...
    return (std::filesystem::path(rootDirectory) /
            "blocks" /
            fileName).string();
}

std::string StoredBlockSnapshot::readFile(
    ...
    if (!input.is_open()) {
        throw std::runtime_error("Failed to open stored block snapshot for reading.");
    }
...
bool StoredBlockSnapshot::snapshotContentMatchesEntry(
    ...
    try {
        const std::uint64_t serializedIndex =
...
bool StoredBlockSnapshot::isSafeHash(
...
bool StoredBlockSnapshot::isSafeFileName(
    const std::string& fileName
) {
    if (fileName.empty()) {
        return false;
...
std::string StoredBlockSnapshot::hashString(
...
BlockchainStorageReadReport::BlockchainStorageReadReport()
    : m_success(true),
      m_failureReason(""),
      m_manifestValid(false),
      m_indexValid(false),
      m_indexMatchesManifest(false),
      m_manifestBlockCount(0),
      m_indexBlockCount(0),
      m_blockSnapshotCount(0),
      m_totalSnapshotBytes(0) {}

bool BlockchainStorageReadReport::success() const {
    return m_success;
}
...
bool BlockchainStorageReadReport::indexValid() const {
    return m_indexValid;
}
...
void BlockchainStorageReadReport::markFailure(
    std::string reason
) {
    m_success = false;
...
void BlockchainStorageReadReport::setManifestBlockCount(
    std::size_t value
) {
    m_manifestBlockCount = value;
}

void BlockchainStorageReadReport::setIndexBlockCount(
    std::size_t value
) {
    m_indexBlockCount = value;
}

void BlockchainStorageReadReport::setBlockSnapshotCount(
    std::size_t value
) {
    m_blockSnapshotCount = value;
}

void BlockchainStorageReadReport::setTotalSnapshotBytes(
    std::size_t value
) {
...
BlockchainStorageReadReport BlockchainStorageReader::auditStorageRoot(
    const std::string& rootDirectory
) {
...
std::vector<StoredBlockSnapshot> BlockchainStorageReader::readBlockSnapshots(
    const std::string& rootDirectory
) {
    if (rootDirectory.empty()) {
        throw std::invalid_argument("Storage root directory cannot be empty.");
    ...
    if (!index.isValid()) {
        throw std::logic_error("Cannot read snapshots from invalid BlockStorageIndex.");
    }
...
void BlockchainStorageReader::validateManifestAndIndexCompatibility(
    ...
    if (!index.isValid()) {
        throw std::logic_error("Invalid BlockStorageIndex rejected by storage reader.");
    }
    ...
    if (index.blockCount() != manifest.blockCount()) {
        throw std::logic_error("BlockStorageIndex block count does not match ChainManifest.");
    }
}
