...
ChainManifest ChainManifest::fromBlockchain(const core::Blockchain &blockchain,
  ...
  if (!blockchain.isValid(false)) {
    throw std::invalid_argument(
        "Invalid Blockchain rejected by ChainManifest.");
  }
  ...
  const std::string manifestHash = computeManifestHash(
      version, blockCount, genesisHash, latestHash, createdAt);

  ChainManifest manifest(version, blockCount, genesisHash, latestHash,
...
ChainManifest ChainManifest::deserialize(const std::string &serialized) {
  return serialization::ChainManifestCodec::deserialize(serialized);
}

std::string ChainManifest::manifestFileName() { return "chain_manifest.nodo"; }

std::string ChainManifest::manifestFilePath(const std::string &rootDirectory) {
  if (rootDirectory.empty()) {
    throw std::invalid_argument(
        "ChainManifest root directory cannot be empty.");
  }
...
ChainManifest
ChainManifest::readFromStorageRoot(const std::string &rootDirectory) {
...
ChainManifest::ChainManifest(std::string chainVersion, std::size_t blockCount,
                             std::string genesisHash, std::string latestHash,
                             std::int64_t createdAt, std::string manifestHash)
    : m_chainVersion(std::move(chainVersion)), m_blockCount(blockCount),
      m_genesisHash(std::move(genesisHash)),
      m_latestHash(std::move(latestHash)), m_createdAt(createdAt),
...
std::size_t ChainManifest::blockCount() const { return m_blockCount; }

const std::string &ChainManifest::genesisHash() const { return m_genesisHash; }

const std::string &ChainManifest::latestHash() const { return m_latestHash; }

std::int64_t ChainManifest::createdAt() const { return m_createdAt; }
...
bool ChainManifest::isValid() const {
  ...
  const std::string expectedHash = computeManifestHash(
      m_chainVersion, m_blockCount, m_genesisHash, m_latestHash, m_createdAt);
...
void ChainManifest::writeToStorageRoot(const std::string &rootDirectory) const {
  if (!isValid()) {
    throw std::invalid_argument(
        "Invalid ChainManifest rejected by storage writer.");
  }
...
std::string ChainManifest::serialize() const {
  ...
  oss << "ChainManifest{"
      << "chainVersion=" << m_chainVersion << ";blockCount=" << m_blockCount
      << ";genesisHash=" << m_genesisHash << ";latestHash=" << m_latestHash
      << ";createdAt=" << m_createdAt << ";manifestHash=" << m_manifestHash
      << "}";
...
std::string ChainManifest::computeManifestHash(const std::string &chainVersion,
  ...
  oss << "ChainManifestPayload{"
      << "chainVersion=" << chainVersion << ";blockCount=" << blockCount
      << ";genesisHash=" << genesisHash << ";latestHash=" << latestHash
      << ";createdAt=" << createdAt << "}";

  return hashString(oss.str());
}

bool ChainManifest::isSafeHash(const std::string &hash) {
  if (hash.empty()) {
    return false;
  }
...
std::string ChainManifest::hashString(const std::string &value) {
  char output[65] = {0};
