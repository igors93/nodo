...
const std::vector<std::string>& StorageRecoveryResult::quarantinedFiles() const {
...
std::string StorageRecoveryResult::serialize() const {
    ...
    output << "StorageRecoveryResult{"
           << "quarantinedFileCount=" << m_quarantinedFileCount
           << ";files=[";
...
StorageRecoveryResult StorageRecovery::quarantineTemporaryWrites(
    ...
    if (rootDirectory.empty() ||
        !std::filesystem::exists(rootDirectory)) {
        return StorageRecoveryResult();
    }

    const std::filesystem::path quarantine =
        quarantineDirectory(rootDirectory);
