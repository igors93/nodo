...
constexpr const char *TEMPORARY_WRITE_MARKER = ".tmp.";
...
bool hasTemporaryWriteMarker(const std::filesystem::path &path) {
...
#if defined(__unix__) || defined(__APPLE__)
void syncAndCloseOrThrow(int descriptor, const std::string &description) {
  if (descriptor < 0) {
    throw std::system_error(errno, std::generic_category(),
                            "Unable to open " + description +
                                " for durable sync");
  }
  ...
  if (::fsync(descriptor) != 0) {
    ...
    ::close(descriptor);
    throw std::system_error(syncError, std::generic_category(),
...
void AtomicFile::writeTextFile(const std::filesystem::path &path,
  ...
  if (std::filesystem::exists(temporaryPath)) {
    throw std::runtime_error("Atomic write temporary file already exists: " +
...
#if defined(__unix__) || defined(__APPLE__)
      const int fd = ::open(temporaryPath.c_str(), O_RDONLY);
...
#if defined(__unix__) || defined(__APPLE__)
    {
      const std::filesystem::path dirPath = path.parent_path().empty()
                                                ? std::filesystem::path(".")
                                                : path.parent_path();
...
std::string AtomicFile::readTextFile(const std::filesystem::path &path) {
...
bool AtomicFile::isTemporaryWriteFile(const std::filesystem::path &path) {
  return hasTemporaryWriteMarker(path);
}

std::vector<std::filesystem::path>
AtomicFile::listTemporaryWriteFiles(const std::filesystem::path &directory) {
...
std::size_t
AtomicFile::removeTemporaryWriteFiles(const std::filesystem::path &directory) {
...
std::filesystem::path
AtomicFile::makeTemporaryPath(const std::filesystem::path &path) {
  const std::filesystem::path parent = path.parent_path();

  const std::string filename =
      path.filename().string() + TEMPORARY_WRITE_MARKER + monotonicSuffix();
