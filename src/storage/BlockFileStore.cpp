...
BlockFileStore::BlockFileStore(std::string rootDirectory)
  ...
  if (m_rootDirectory.empty()) {
    throw std::invalid_argument(
        "BlockFileStore root directory cannot be empty.");
  }
}
...
void BlockFileStore::ensureStorageDirectory() const {
  ...
  if (errorCode) {
    throw std::runtime_error("Failed to create block storage directory: " +
                             errorCode.message());
  }
...
void BlockFileStore::clearBlockStorage() const {
  ...
  if (std::filesystem::exists(blockDirectory())) {
    ...
    if (errorCode) {
      throw std::runtime_error("Failed to clear block storage directory: " +
                               errorCode.message());
    }
  }
...
void BlockFileStore::writeBlockchain(const core::Blockchain &blockchain) const {
  if (blockchain.empty()) {
  ...
  if (!blockchain.isValid(false)) {
    throw std::invalid_argument(
        "Invalid Blockchain rejected by BlockFileStore.");
  }
...
bool BlockFileStore::hasStoredBlock(const core::Block &block) const {
  if (!block.isValid(false)) {
    return false;
  }
...
bool BlockFileStore::verifyStoredBlock(const core::Block &block) const {
  if (!block.isValid(false)) {
    return false;
  }
...
std::string BlockFileStore::readBlockSnapshot(const core::Block &block) const {
  if (!block.isValid(false)) {
    throw std::invalid_argument(
        "Invalid block rejected while reading snapshot.");
  }
...
std::size_t BlockFileStore::storedBlockFileCount() const {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(blockDirectory())) {
...
std::filesystem::path
BlockFileStore::blockFilePath(const core::Block &block) const {
  return blockDirectory() / blockFileName(block);
}

std::string BlockFileStore::blockFileName(const core::Block &block) {
  if (!block.isValid(false)) {
    throw std::invalid_argument(
        "Invalid block cannot be converted to file name.");
  }

  validateHashForFileName(block.hash());

  return "block_" + std::to_string(block.index()) + "_" + block.hash() +
         ".nodo";
}

void BlockFileStore::validateHashForFileName(const std::string &hash) {
  if (hash.empty()) {
  ...
  for (const char current : hash) {
    ...
    if (!isDigit && !isLowerHex && !isUpperHex) {
      throw std::invalid_argument(
          "Block hash contains unsafe file name characters.");
    }
  }
}
