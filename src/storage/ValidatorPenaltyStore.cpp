...
bool isPenaltyFile(const std::filesystem::directory_entry &entry) {
  return entry.is_regular_file() &&
         entry.path().extension() == PENALTY_FILE_EXTENSION;
}

std::size_t penaltyFileCount(const std::filesystem::path &penaltyDirectory) {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(penaltyDirectory)) {
...
ValidatorPenaltyStore::ValidatorPenaltyStore(
    std::filesystem::path penaltyDirectory)
...
void ValidatorPenaltyStore::save(
  ...
  if (!decision.isValid()) {
    throw std::invalid_argument(
        "Cannot store invalid validator penalty decision.");
  }

  std::filesystem::create_directories(m_penaltyDirectory);
  AtomicFile::writeTextFile(pathForPenaltyId(decision.penaltyId()),
                            decision.serialize());
}
...
ValidatorPenaltyStore::load(const std::string &penaltyId) const {
  if (!isSafePenaltyId(penaltyId)) {
...
std::vector<consensus::ValidatorPenaltyDecision>
ValidatorPenaltyStore::loadAll() const {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(m_penaltyDirectory)) {
    if (isPenaltyFile(entry)) {
...
std::filesystem::path
ValidatorPenaltyStore::pathForPenaltyId(const std::string &penaltyId) const {
  if (!isSafePenaltyId(penaltyId)) {
    throw std::invalid_argument("Validator penalty id is unsafe.");
  }
...
bool ValidatorPenaltyStore::isSafePenaltyId(const std::string &penaltyId) {
