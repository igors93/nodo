...
BlockchainLoadReport::BlockchainLoadReport()
    : m_success(true), m_failureReason(""), m_storageReaderValid(false),
      m_loadedBlockchainValid(false), m_loadedBlockchainMatchesManifest(false),
      m_loadedBlockchainMatchesIndex(false), m_loadedBlockCount(0) {}

bool BlockchainLoadReport::success() const { return m_success; }
...
void BlockchainLoadReport::markFailure(std::string reason) {
  m_success = false;
  m_failureReason = std::move(reason);
}

void BlockchainLoadReport::setStorageReaderValid(bool value) {
  m_storageReaderValid = value;
}

void BlockchainLoadReport::setLoadedBlockchainValid(bool value) {
  m_loadedBlockchainValid = value;
}

void BlockchainLoadReport::setLoadedBlockchainMatchesManifest(bool value) {
  m_loadedBlockchainMatchesManifest = value;
}

void BlockchainLoadReport::setLoadedBlockchainMatchesIndex(bool value) {
  m_loadedBlockchainMatchesIndex = value;
}

void BlockchainLoadReport::setLoadedBlockCount(std::size_t value) {
...
std::string BlockchainLoadReport::serialize() const {
  ...
  oss << "BlockchainLoadReport{"
      ...
      << ";loadedBlockchainValid="
      << (m_loadedBlockchainValid ? "true" : "false")
      << ";loadedBlockchainMatchesManifest="
      << (m_loadedBlockchainMatchesManifest ? "true" : "false")
      << ";loadedBlockchainMatchesIndex="
      << (m_loadedBlockchainMatchesIndex ? "true" : "false")
...
BlockchainLoader::auditStorageRoot(const std::string &rootDirectory) {
...
core::Blockchain
BlockchainLoader::loadFromStorageRoot(const std::string &rootDirectory) {
  if (rootDirectory.empty()) {
    throw std::invalid_argument(
        "BlockchainLoader root directory cannot be empty.");
  }
  ...
  if (!manifest.isValid()) {
    throw std::logic_error(
        "Cannot load Blockchain from invalid ChainManifest.");
  }
  ...
  if (!index.isValid()) {
    throw std::logic_error(
        "Cannot load Blockchain from invalid BlockStorageIndex.");
  }
  ...
  if (snapshots.size() != manifest.blockCount()) {
    throw std::logic_error(
        "Stored snapshot count does not match ChainManifest.");
  }
  ...
  for (const auto &snapshot : snapshots) {
    loadedBlocks.push_back(
        serialization::BlockCodec::deserializeFromFile(snapshot.filePath()));
  }

  core::Blockchain loadedBlockchain = buildBlockchainFromBlocks(loadedBlocks);
  ...
  if (!index.matchesBlockchainAndManifest(loadedBlockchain, manifest)) {
    throw std::logic_error(
        "Loaded Blockchain does not match BlockStorageIndex.");
  }
...
core::Blockchain BlockchainLoader::buildBlockchainFromBlocks(
  ...
  if (blocks.empty()) {
    throw std::invalid_argument(
        "Cannot build Blockchain from an empty block list.");
  }
...
BlockchainCommitmentVerificationReport::BlockchainCommitmentVerificationReport()
    : m_passed(false), m_firstFailedHeight(0), m_reason(""),
      m_verifiedBlockCount(0) {}

BlockchainCommitmentVerificationReport
BlockchainCommitmentVerificationReport::passed(std::size_t verifiedBlockCount) {
...
BlockchainCommitmentVerificationReport
BlockchainCommitmentVerificationReport::failed(std::uint64_t firstFailedHeight,
                                               std::string reason) {
  BlockchainCommitmentVerificationReport r;
  r.m_passed = false;
...
std::uint64_t
BlockchainCommitmentVerificationReport::firstFailedHeight() const {
...
std::string BlockchainCommitmentVerificationReport::serialize() const {
  ...
  oss << "BlockchainCommitmentVerificationReport{"
      ...
      << ";firstFailedHeight=" << m_firstFailedHeight << ";reason=" << m_reason
