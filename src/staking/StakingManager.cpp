...
uint64_t saturatingBlockAdd(uint64_t block, uint64_t delta) {
    return saturatingAdd(block, delta);
}
...
bool StakingManager::bondStake(const std::string& validatorAddress, uint64_t amount) {
    ...
    m_bondedStakes[validatorAddress] = saturatingAdd(m_bondedStakes[validatorAddress], amount);
    return true;
}
...
bool StakingManager::initiateUnbond(const std::string& validatorAddress, uint64_t amount, uint64_t currentBlock) {
    ...
    it->second -= amount;
    m_unbondingQueue.push_back({validatorAddress, amount, saturatingBlockAdd(currentBlock, m_unbondingPeriodBlocks)});
    return true;
}
    ...
    auto it = std::remove_if(m_unbondingQueue.begin(), m_unbondingQueue.end(),
        [this, currentBlock](const UnbondingRequest& req) {
            if (currentBlock >= req.unlockBlock) {
                m_unlockedBalances[req.validatorAddress] = saturatingAdd(m_unlockedBalances[req.validatorAddress], req.amount);
                return true;
            }
            return false;
        });
...
void StakingManager::jailValidator(const std::string& validatorAddress, uint64_t jailDurationBlocks, uint64_t currentBlock) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_jailedUntilBlock[validatorAddress] = saturatingBlockAdd(currentBlock, jailDurationBlocks);
}
...
void StakingManager::slashValidator(const std::string& validatorAddress, double slashFraction, uint64_t currentBlock) {
    ...
    m_jailedUntilBlock[validatorAddress] = saturatingBlockAdd(currentBlock, uint64_t{100});
}
