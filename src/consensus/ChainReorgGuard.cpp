...
ChainReorgGuardConfig ChainReorgGuardConfig::defaults() {
    return {6, 3};
}

bool ChainReorgGuardConfig::isValid() const {
    return maxReorgDepth > 0 &&
           alertThresholdDepth < maxReorgDepth;
}
...
std::string reorgCheckOutcomeToString(ReorgCheckOutcome outcome) {
    switch (outcome) {
        case ReorgCheckOutcome::ALLOWED:             return "ALLOWED";
        case ReorgCheckOutcome::ALLOWED_WITH_ALERT:  return "ALLOWED_WITH_ALERT";
        case ReorgCheckOutcome::REJECTED:            return "REJECTED";
...
ReorgEvent::ReorgEvent()
    : m_timestamp(0),
      m_fromHeight(0),
      m_localTipHeight(0),
      m_candidateTipHeight(0),
      m_depth(0),
      m_outcome(ReorgCheckOutcome::REJECTED),
      m_reason("") {}
...
ReorgEvent::ReorgEvent(
    ...
    : m_timestamp(timestamp),
      ...
      m_candidateTipHeight(candidateTipHeight),
      m_depth(depth),
      m_outcome(outcome),
      m_reason(std::move(reason)) {}

std::int64_t ReorgEvent::timestamp() const {
    return m_timestamp;
}

std::uint64_t ReorgEvent::fromHeight() const {
    return m_fromHeight;
}

std::uint64_t ReorgEvent::localTipHeight() const {
    return m_localTipHeight;
}
...
std::uint64_t ReorgEvent::depth() const {
    return m_depth;
}

ReorgCheckOutcome ReorgEvent::outcome() const {
    return m_outcome;
}

const std::string& ReorgEvent::reason() const {
    return m_reason;
}

bool ReorgEvent::isValid() const {
    return m_timestamp > 0 &&
           m_localTipHeight >= m_fromHeight &&
...
std::string ReorgEvent::serialize() const {
    ...
    oss << "ReorgEvent{"
        ...
        << ";candidateTipHeight=" << m_candidateTipHeight
        << ";depth=" << m_depth
...
ChainReorgCheckResult ChainReorgCheckResult::allowed(
    std::uint64_t depth,
    std::string reason
) {
...
ChainReorgCheckResult ChainReorgCheckResult::allowedWithAlert(
    std::uint64_t depth,
    std::string reason
) {
    ChainReorgCheckResult result;
...
ChainReorgCheckResult ChainReorgCheckResult::rejected(
    std::uint64_t depth,
    std::string reason
) {
    ChainReorgCheckResult result;
...
ReorgCheckOutcome ChainReorgCheckResult::outcome() const {
    return m_outcome;
}

std::uint64_t ChainReorgCheckResult::depth() const {
    return m_depth;
}

const std::string& ChainReorgCheckResult::reason() const {
    return m_reason;
}
...
ChainReorgCheckResult ChainReorgGuard::checkReorg(
    std::uint64_t localTipHeight,
    ...
    if (depth > m_config.maxReorgDepth) {
        return ChainReorgCheckResult::rejected(
            depth,
            "Reorg depth " + std::to_string(depth) +
    ...
    return ChainReorgCheckResult::allowed(
        depth,
        "Reorg depth " + std::to_string(depth) + " is within safe bounds."
    );
}
