...
bool NetworkProfileRegistry::isKnown(const std::string &networkName) {
  return networkName == "localnet" || networkName == "localnet-soak" ||
         networkName == "testnet-candidate" || networkName == "mainnet";
}

bool NetworkProfileRegistry::isOfficialNetwork(const std::string &networkName) {
  return networkName == "testnet-candidate" || networkName == "testnet" ||
         networkName == "mainnet";
}
...
std::vector<std::string> NetworkProfileRegistry::knownProfiles() {
  return {"localnet", "localnet-soak", "testnet-candidate", "mainnet"};
}
...
NetworkParameters NetworkProfileRegistry::get(const std::string &networkName) {
  ...
  if (networkName == "mainnet") {
    throw std::invalid_argument(
        ...
        "mainnet startup requires an audited genesis and production crypto "
        "providers.");
  }
