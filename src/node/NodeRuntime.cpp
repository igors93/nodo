...
void initializeConsensusRound(
  ...
  const std::uint64_t nextHeight = blockchain.latestBlock().index() + 1;

  constexpr std::uint64_t initialRound = 1;

  const std::string proposer = consensus::ProposerSchedule::selectProposer(
      validatorRegistry, genesisConfig.networkParameters().chainId(),
      nextHeight, initialRound);

  manager.advanceToHeight(
      nextHeight, initialRound, proposer, genesisConfig.genesisTimestamp(),
      genesisConfig.networkParameters().targetBlockTimeSeconds());
}

} // namespace

std::string peerUpdateStatusToString(PeerUpdateStatus status) {
  switch (status) {
...
PeerUpdateResult::PeerUpdateResult()
    : m_status(PeerUpdateStatus::REJECTED),
      m_reason("Uninitialized peer update result."), m_peer() {}

PeerUpdateResult PeerUpdateResult::accepted(p2p::PeerInfo peer) {
...
PeerUpdateResult PeerUpdateResult::updated(p2p::PeerInfo peer) {
  PeerUpdateResult result;
...
PeerUpdateResult PeerUpdateResult::duplicate(p2p::PeerInfo peer) {
  PeerUpdateResult result;
...
PeerUpdateResult PeerUpdateResult::rejected(PeerUpdateStatus status,
                                            std::string reason) {
  PeerUpdateResult result;
...
PeerUpdateStatus PeerUpdateResult::status() const { return m_status; }

const std::string &PeerUpdateResult::reason() const { return m_reason; }

const p2p::PeerInfo &PeerUpdateResult::peer() const { return m_peer; }
...
LocalPeerManager::LocalPeerManager(std::size_t maxPeers)
    : m_maxPeers(maxPeers), m_peersById() {}

PeerUpdateResult LocalPeerManager::addOrUpdatePeer(const p2p::PeerInfo &peer) {
  if (m_maxPeers == 0) {
    return PeerUpdateResult::rejected(PeerUpdateStatus::REJECTED,
  ...
  const auto existing = m_peersById.find(peer.peerId());
  ...
  if (m_peersById.size() >= m_maxPeers) {
    return PeerUpdateResult::rejected(PeerUpdateStatus::CAPACITY_REACHED,
...
bool LocalPeerManager::removePeer(const std::string &peerId) {
  return m_peersById.erase(peerId) > 0;
}

bool LocalPeerManager::hasPeer(const std::string &peerId) const {
  return m_peersById.find(peerId) != m_peersById.end();
}

const p2p::PeerInfo *LocalPeerManager::peer(const std::string &peerId) const {
  const auto found = m_peersById.find(peerId);
...
const p2p::PeerInfo *
LocalPeerManager::bestSyncPeer(std::uint64_t localHeight) const {
  const p2p::PeerInfo *bestPeer = nullptr;

  for (const auto &[_, peerInfo] : m_peersById) {
    if (!peerInfo.isValid() || peerInfo.latestKnownHeight() <= localHeight) {
      continue;
    }
...
std::size_t LocalPeerManager::size() const { return m_peersById.size(); }

std::size_t LocalPeerManager::maxPeers() const { return m_maxPeers; }

bool LocalPeerManager::isValid() const {
  if (m_maxPeers == 0 || m_peersById.size() > m_maxPeers) {
    return false;
  }

  for (const auto &[peerId, peerInfo] : m_peersById) {
    if (peerId != peerInfo.peerId() || !peerInfo.isValid()) {
      return false;
    }
  }
...
std::string LocalPeerManager::serialize() const {
  ...
  oss << "LocalPeerManager{"
      << "maxPeers=" << m_maxPeers << ";size=" << m_peersById.size()
      << ";peers=[";
...
std::string nodeRuntimeStatusToString(NodeRuntimeStatus status) {
  switch (status) {
...
NodeRuntimeConfig::NodeRuntimeConfig()
    : m_genesisConfig(), m_localPeer(), m_maxPeers(0) {}

NodeRuntimeConfig::NodeRuntimeConfig(config::GenesisConfig genesisConfig,
                                     p2p::PeerInfo localPeer,
                                     std::size_t maxPeers)
    : m_genesisConfig(std::move(genesisConfig)),
      m_localPeer(std::move(localPeer)), m_maxPeers(maxPeers) {}
...
std::size_t NodeRuntimeConfig::maxPeers() const { return m_maxPeers; }

bool NodeRuntimeConfig::isValid() const {
  return m_genesisConfig.isValid() && m_localPeer.isValid() && m_maxPeers > 0 &&
...
std::string NodeRuntimeConfig::serialize() const {
  ...
  oss << "NodeRuntimeConfig{"
      << "localPeer="
      << (m_localPeer.isValid() ? m_localPeer.serialize() : "INVALID")
      << ";maxPeers=" << m_maxPeers
...
NodeRuntime::NodeRuntime()
    : m_config(), m_status(NodeRuntimeStatus::STOPPED), m_blockchain(),
      m_validatorRegistry(), m_validatorSetHistory(), m_finalizationRegistry(),
      m_consensusRoundManager(), m_mempool(), m_peerManager(1),
      m_validatorPenaltyLedger() {}

NodeRuntime::NodeRuntime(NodeRuntimeConfig config, core::Blockchain blockchain,
                         core::ValidatorRegistry validatorRegistry)
    : m_config(std::move(config)), m_status(NodeRuntimeStatus::RUNNING),
      m_blockchain(std::move(blockchain)),
      m_validatorRegistry(std::move(validatorRegistry)),
      m_validatorSetHistory(), m_finalizationRegistry(),
      m_consensusRoundManager(), m_mempool(),
      m_peerManager(m_config.maxPeers()), m_validatorPenaltyLedger() {
  if (m_config.genesisConfig().isValid() && !m_blockchain.empty() &&
      m_validatorRegistry.isValid()) {
    for (const std::string &address :
         m_validatorRegistry.activeValidatorAddresses()) {
      ...
      if (entry != nullptr && entry->stakeAmount() > 0 &&
        ...
        m_stakingRegistry.setAccount(
            address,
            economics::StakeAccount(
                address, utils::Amount::fromRawUnits(
                             static_cast<std::int64_t>(entry->stakeAmount()))));
      }
    }
    initializeConsensusRound(m_consensusRoundManager, m_config.genesisConfig(),
                             m_blockchain, m_validatorRegistry);
    m_validatorSetHistory.recordSet(1, m_validatorRegistry);
  }
}

const NodeRuntimeConfig &NodeRuntime::config() const { return m_config; }

NodeRuntimeStatus NodeRuntime::status() const { return m_status; }

const core::Blockchain &NodeRuntime::blockchain() const { return m_blockchain; }

core::Blockchain &NodeRuntime::mutableBlockchain() { return m_blockchain; }
...
const consensus::BlockFinalizationRegistry &
NodeRuntime::finalizationRegistry() const {
  return m_finalizationRegistry;
}

consensus::BlockFinalizationRegistry &
NodeRuntime::mutableFinalizationRegistry() {
  return m_finalizationRegistry;
}

const consensus::ConsensusRoundManager &
NodeRuntime::consensusRoundManager() const {
...
const mempool::Mempool &NodeRuntime::mempool() const { return m_mempool; }

mempool::Mempool &NodeRuntime::mutableMempool() { return m_mempool; }
...
RuntimeSupplyState &NodeRuntime::mutableSupplyState() { return m_supplyState; }

const core::StatePruner &NodeRuntime::statePruner() const {
  return m_statePruner;
}

core::StatePruner &NodeRuntime::mutableStatePruner() { return m_statePruner; }
...
NodeRuntime::cachedAccountStateAtTip(std::int64_t minimumFeeRawUnits) const {
  ...
  m_accountStateCache = RuntimeAccountStateBuilder::accountStateViewAtTip(
      m_config.genesisConfig(), m_blockchain,
...
std::uint64_t NodeRuntime::effectiveMinimumFeeRawUnits() const {
  ...
  if (!governed.empty()) {
    try {
      ...
      throw std::runtime_error("Malformed MINIMUM_FEE_RAW from governance: " +
                               governed);
    }
  }
...
LocalPeerManager &NodeRuntime::mutablePeerManager() { return m_peerManager; }
...
void NodeRuntime::halt() { m_status = NodeRuntimeStatus::HALTED; }

consensus::ChainForkSummary NodeRuntime::chainSummary() const {
  return consensus::ForkChoicePolicy::summarizeChain(m_blockchain,
                                                     m_finalizationRegistry);
}

consensus::VoteCollectResult
NodeRuntime::submitConsensusVote(const consensus::ValidatorVoteRecord &vote) {
  ...
  return m_consensusRoundManager.submitVote(
      vote, cryptoContext.policy(), cryptoContext.validatorSignatureProvider());
}

bool NodeRuntime::advanceConsensusRoundIfTimedOut(std::int64_t now) {
  const auto &params = m_config.genesisConfig().networkParameters();
  const std::uint64_t phaseTimeoutSecs =
      (params.proposalTimeoutMs() + params.prevoteTimeoutMs() +
       params.precommitTimeoutMs()) /
      1000;

  const std::int64_t elapsedSecs =
      now - m_consensusRoundManager.currentState().roundStartedAt();

  if (elapsedSecs < static_cast<std::int64_t>(phaseTimeoutSecs)) {
    return false;
  }
  ...
  m_consensusRoundManager.advanceRound(newRound, proposer, now,
                                       phaseTimeoutSecs);
...
p2p::PeerMessage
NodeRuntime::localChainSummaryMessage(const std::string &toPeerId,
                                      std::int64_t createdAt) const {
  return p2p::PeerMessageFactory::chainSummary(m_config.localPeer(), toPeerId,
...
p2p::LocalSyncPlan NodeRuntime::evaluatePeerForSync(
  ...
  return p2p::LocalNodeSynchronizer::evaluatePeerSummary(
      m_config.localPeer(), remotePeer, m_blockchain, m_finalizationRegistry,
      remoteSummary, createdAt);
}
...
std::string NodeRuntime::serialize() const {
  ...
  oss << "NodeRuntime{"
      ...
      << ";consensusRound="
      << m_consensusRoundManager.currentState().serialize()
      << ";peerManager=" << m_peerManager.serialize() << "}";

  return oss.str();
}

std::string nodeRuntimeStartStatusToString(NodeRuntimeStartStatus status) {
  switch (status) {
...
NodeRuntimeStartResult::NodeRuntimeStartResult()
    : m_status(NodeRuntimeStartStatus::INVALID_CONFIG),
      m_reason("Uninitialized node runtime start result."), m_runtime() {}

NodeRuntimeStartResult NodeRuntimeStartResult::started(NodeRuntime runtime) {
...
NodeRuntimeStartResult::rejected(NodeRuntimeStartStatus status,
                                 std::string reason) {
  NodeRuntimeStartResult result;
  result.m_status = status;
  result.m_reason = std::move(reason);
...
const std::string &NodeRuntimeStartResult::reason() const { return m_reason; }

bool NodeRuntimeStartResult::started() const {
  return m_status == NodeRuntimeStartStatus::STARTED && m_runtime.isValid();
}

const NodeRuntime &NodeRuntimeStartResult::runtime() const { return m_runtime; }
...
NodeRuntimeStartResult
NodeRuntimeFactory::startFromGenesis(const NodeRuntimeConfig &config) {
  ...
  const config::GenesisBuildResult genesis =
      config::GenesisBuilder::build(config.genesisConfig());

  if (!genesis.built()) {
    return NodeRuntimeStartResult::rejected(
        NodeRuntimeStartStatus::GENESIS_BUILD_FAILED, genesis.reason());
  }

  NodeRuntime runtime(config, genesis.blockchain(),
