...
core::AccountStateView credit(core::AccountStateView accounts,
...
accountViewFromAccounts(const std::vector<core::AccountState> &accounts) {
...
initialValidatorSetHistory(const core::ValidatorRegistry &validators) {
  core::ValidatorSetHistory history;
  if (!history.recordSet(1, validators)) {
    throw std::logic_error(
        "Unable to record genesis validator set for replay.");
  }
  return history;
}

void seedGenesisStakeAccounts(StakingRegistry &staking,
...
core::AccountStateView ProtocolStateTransition::initialAccountStateView(
  ...
  if (!genesisConfig.isValid()) {
    throw std::invalid_argument(
        "Cannot build account state from invalid genesis config.");
  }
  ...
  for (const config::GenesisAccountConfig &account :
    ...
    if (!view.putAccount(core::AccountState(
      ...
      throw std::logic_error(
          "Genesis account allocation produced invalid account state.");
    }
  }
...
ProtocolExecutionState ProtocolStateTransition::initialExecutionState(
  ...
  return ProtocolExecutionState{GovernanceExecutor(),
...
ProtocolReplayState ProtocolStateTransition::initialReplayState(
  ...
  state.stateRoot = core::StateRootCalculator::calculateProtocolStateRoot(
      state.accounts, protocolExecutionDomains(state.execution));
  if (state.stateRoot.empty()) {
    throw std::logic_error(
        "Genesis protocol replay produced an empty state root.");
  }
  return state;
}

void ProtocolStateTransition::applyValidatorEpochTransition(
    core::ValidatorRegistry &validators, std::uint64_t blockHeight,
    std::int64_t blockTimestamp) {
  if (blockHeight == 0 || blockHeight % NODO_VALIDATOR_EPOCH_BLOCKS != 0)
    return;
  ...
  for (const std::string &address : validators.pendingValidatorAddresses()) {
    ...
    if (entry != nullptr &&
      ...
      const auto result =
          validators.activateValidator(address, effectiveEpoch, blockTimestamp);
      if (!result.success())
...
core::AccountStateView
ProtocolStateTransition::settleFees(const core::AccountStateView &accounts,
  ...
  return credit(std::move(settled), ProtectionTreasury::TREASURY_ADDRESS,
...
ProtocolStateTransition::accountSettlementForReplay(std::uint64_t blockHeight) {
  return [blockHeight](const core::AccountStateView &accounts,
                       utils::Amount totalFee,
...
ProtocolStateTransition::contextFromReplayState(
    const config::GenesisConfig &genesisConfig,
    const ProtocolReplayState &state, std::int64_t minimumFeeRawUnits,
    std::shared_ptr<ProtocolExecutionState> resultTracker,
    std::int64_t wallClockNow) {
  if (minimumFeeRawUnits < 0) {
    throw std::invalid_argument(
        "Minimum fee cannot be negative in protocol replay context.");
  }
  if (!genesisConfig.isValid()) {
    throw std::invalid_argument(
        "Cannot build protocol replay context from invalid genesis config.");
  }
  if (!state.accounts.isValid()) {
    throw std::invalid_argument(
        "Cannot build protocol replay context from invalid account state.");
  }
  ...
  if (!resultTracker) {
    throw std::invalid_argument(
        "Protocol replay context requires a result tracker.");
  }

  const auto &params = genesisConfig.networkParameters();
  return core::StateTransitionPreviewContext(
      minimumFeeRawUnits, state.accounts, false, true, "", wallClockNow,
      params.chainId(), params.networkName(),
      protocolExecutionDomains(state.execution), {},
      makeProtocolDomainExecutorFactory(state.execution,
                                        state.validatorSetHistory, params,
...
ProtocolReplayState ProtocolStateTransition::replayBlock(
    const config::GenesisConfig &genesisConfig,
    const ProtocolReplayState &previousState, const core::Block &block,
    std::int64_t minimumFeeRawUnits, std::int64_t wallClockNow) {
  if (block.isGenesisBlock()) {
    throw std::invalid_argument("Protocol replayBlock cannot replay genesis as "
                                "a finalized transition.");
  }

  auto tracker =
      std::make_shared<ProtocolExecutionState>(previousState.execution);
  const core::StateTransitionPreviewContext context = contextFromReplayState(
      genesisConfig, previousState, minimumFeeRawUnits, tracker, wallClockNow);

  const core::StateTransitionPreviewResult execution =
      core::StateTransitionEngine::executeBlock(block, context);
  if (!execution.accepted()) {
    throw std::logic_error("Canonical protocol replay rejected block " +
                           std::to_string(block.index()) + ": " +
                           execution.reason());
  }
  if (execution.stateRoot().empty() || execution.receiptsRoot().empty()) {
    throw std::logic_error(
        "Canonical protocol replay produced empty commitments.");
  }
...
ProtocolReplayState
ProtocolStateTransition::replayToTip(const config::GenesisConfig &genesisConfig,
                                     const core::Blockchain &blockchain,
                                     std::int64_t minimumFeeRawUnits) {
  if (minimumFeeRawUnits < 0) {
    throw std::invalid_argument(
        "Minimum fee cannot be negative when replaying protocol state.");
  }
  if (!genesisConfig.isValid()) {
    throw std::invalid_argument(
        "Cannot replay protocol state from invalid genesis config.");
  }
  if (blockchain.empty() || !blockchain.isValid(false)) {
    throw std::invalid_argument(
        "Cannot replay protocol state from invalid blockchain.");
  }

  const config::GenesisBuildResult genesis =
      config::GenesisBuilder::build(genesisConfig);
  if (!genesis.built()) {
    throw std::invalid_argument("Cannot verify replay genesis chain: " +
                                genesis.reason());
  }
  if (genesis.blockchain().latestBlock().hash() !=
      blockchain.blocks().front().hash()) {
    throw std::invalid_argument(
        "Blockchain genesis block does not match genesis config.");
  }
  ...
  for (const core::Block &block : blockchain.blocks()) {
    ...
    state = replayBlock(genesisConfig, state, block, minimumFeeRawUnits,
                        block.timestamp());
  }
  return state;
}
...
ProtocolReplayState ProtocolStateTransition::replayNextBlock(
    ...
    std::int64_t minimumFeeRawUnits, std::int64_t wallClockNow) {
  const ProtocolReplayState tip =
      replayToTip(runtime.config().genesisConfig(), runtime.blockchain(),
                  minimumFeeRawUnits);
  ...
  return replayBlock(runtime.config().genesisConfig(), tip, block,
                     minimumFeeRawUnits, wallClockNow);
}

std::pair<core::StateTransitionPreviewContext,
          std::shared_ptr<ProtocolExecutionState>>
ProtocolStateTransition::contextForNextBlockWithState(
    const NodeRuntime &runtime, std::int64_t minimumFeeRawUnits,
    std::int64_t wallClockNow) {
  if (!runtime.isValid()) {
    throw std::invalid_argument(
        "Cannot build protocol transition from invalid runtime.");
  }
  if (minimumFeeRawUnits < 0) {
    throw std::invalid_argument(
  ...
  core::StateTransitionPreviewContext context =
      contextFromReplayState(runtime.config().genesisConfig(), tip,
                             minimumFeeRawUnits, tracker, wallClockNow);
  return {std::move(context), std::move(tracker)};
}

core::StateTransitionPreviewContext
ProtocolStateTransition::contextForNextBlock(const NodeRuntime &runtime,
                                             std::int64_t minimumFeeRawUnits,
                                             std::int64_t wallClockNow) {
  return contextForNextBlockWithState(runtime, minimumFeeRawUnits, wallClockNow)
      .first;
}
