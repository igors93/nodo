...
bool governanceVoterAuthorized(const core::ValidatorRegistryEntry &entry,
...
std::uint64_t
governanceVotingWeight(const core::ValidatorRegistryEntry &entry) {
...
const mempool::Mempool &TransactionAdmissionContext::mempool() const {
  return *m_mempool;
}
const StakingRegistry &TransactionAdmissionContext::staking() const {
  return *m_staking;
}
const core::ValidatorRegistry &TransactionAdmissionContext::validators() const {
  return *m_validators;
}
const GovernanceExecutor &TransactionAdmissionContext::governance() const {
  return *m_governance;
}
...
bool TransactionAdmissionPolicy::validateTypeAndPayload(
    const core::Transaction &transaction, std::string &reason) {
  if (!core::TransactionTypePolicyRegistry::isMempoolType(transaction.type()) ||
      !core::TransactionTypePolicyRegistry::validateShape(transaction,
                                                          reason)) {
    if (reason.empty())
      reason = "Transaction type is not accepted by the user mempool.";
    return false;
  }
  try {
    if (transaction.type() == core::TransactionType::VALIDATOR_REGISTER) {
      ...
      (void)core::GovernanceVotePayload::deserialize(transaction.data());
               ...
               core::TransactionType::GOVERNANCE_PROPOSE) {
      std::string detail;
      if (!GovernanceExecutor::validateProposalPayload(transaction.data(),
                                                       detail)) {
        reason = detail;
        return false;
      }
...
bool TransactionAdmissionPolicy::validateDomain(
    ...
    const TransactionAdmissionContext &context, std::string &reason) {
  try {
    ...
    case core::TransactionType::STAKE_DEPOSIT:
      if (context.staking()
              .accountOrDefault(transaction.toAddress())
              .tombstoned()) {
    ...
    case core::TransactionType::STAKE_TOP_UP:
      if (!context.staking()
               .ownedStake(transaction.fromAddress(), transaction.toAddress())
               .isPositive() ||
          context.staking()
              .accountOrDefault(transaction.toAddress())
              .tombstoned()) {
    ...
    case core::TransactionType::STAKE_UNLOCK: {
      ...
      if (entry != nullptr && entry->eligibleForConsensus()) {
        const std::int64_t available =
            context.staking()
                .activeStakeFor(transaction.toAddress())
                .rawUnits();
        ...
        if (available < requested ||
          ...
          reason = "Active validator stake cannot be unlocked below the "
                   "protocol minimum.";
          return false;
        }
      }
    ...
    case core::TransactionType::STAKE_WITHDRAW: {
      ...
      if (withdrawable < reserved + transaction.amount()) {
        reason = "Stake withdrawal exceeds withdrawable unbonded stake or is "
                 "still cooling down.";
        return false;
      }
      const auto account =
    ...
    case core::TransactionType::VALIDATOR_REGISTER: {
      const auto payload =
          core::ValidatorRegistrationPayload::deserialize(transaction.data());
      const std::string address =
          crypto::AddressDerivation::deriveFromPublicKey(
              payload.validatorPublicKey())
              .value();
      if (transaction.toAddress() != address ||
          context.validators().hasValidator(address) ||
          transaction.amount().rawUnits() <
              static_cast<std::int64_t>(
                  core::ValidatorRegistry::MIN_VALIDATOR_STAKE_RAW_UNITS)) {
        reason = "Validator registration target, uniqueness, or minimum stake "
                 "is invalid.";
        return false;
      }
      for (const auto &queued : pending) {
        if (queued.type() == core::TransactionType::VALIDATOR_REGISTER &&
            queued.toAddress() == address &&
            queued.nonce() != transaction.nonce()) {
    ...
    case core::TransactionType::VALIDATOR_UNJAIL_REQUEST: {
      ...
      if (transaction.type() ==
              core::TransactionType::VALIDATOR_UNJAIL_REQUEST &&
          !entry->jailed()) {
    ...
    case core::TransactionType::GOVERNANCE_VOTE: {
      const auto vote =
          core::GovernanceVotePayload::deserialize(transaction.data());
      if (vote.proposalId() != transaction.toAddress()) {
        reason = "Governance vote target does not match payload proposal id.";
        return false;
      }
      const auto *entry =
          context.validators().entryForAddress(vote.validatorAddress());
      const std::uint64_t votingWeight =
          entry == nullptr ? 0 : governanceVotingWeight(*entry);
      if (!context.governance().hasProposal(transaction.toAddress()) ||
          entry == nullptr || !entry->eligibleForConsensus() ||
          !governanceVoterAuthorized(*entry, vote.validatorAddress(),
                                     transaction.fromAddress()) ||
          votingWeight == 0 ||
          context.governance().hasVote(transaction.toAddress(),
                                       vote.validatorAddress())) {
        reason = "Governance vote proposal, validator authorization, or "
                 "uniqueness is invalid.";
        return false;
      }
      if (!context.governance().proposalOpenForVoting(
              transaction.toAddress(), context.nextBlockHeight())) {
        reason = "Governance proposal is not open for voting at the next block "
                 "height.";
        return false;
      }
      for (const auto &queued : pending) {
        if (queued.type() != core::TransactionType::GOVERNANCE_VOTE ||
            ...
            queued.nonce() == transaction.nonce())
          continue;
        const auto queuedVote =
            core::GovernanceVotePayload::deserialize(queued.data());
    ...
    case core::TransactionType::GOVERNANCE_EXECUTE:
      if (!context.governance().proposalReadyForExplicitExecution(
              transaction.toAddress(), context.nextBlockHeight())) {
        reason = "Governance proposal is not an approved treasury spend "
                 "eligible for execution at the next block height.";
        return false;
      }
      for (const auto &queued : pending) {
        if (queued.type() == core::TransactionType::GOVERNANCE_EXECUTE &&
            queued.toAddress() == transaction.toAddress() &&
            queued.nonce() != transaction.nonce()) {
          reason = "This proposal already has a pending execution transaction.";
          return false;
        }
      }
      break;
