...
std::int64_t parseInt64(const std::string &value, const char *fieldName) {
  ...
  try {
    ...
    throw std::invalid_argument(std::string("Peer file ") + fieldName +
                                " is malformed.");
  }
}
...
const std::string &TcpTestnetNodeRuntimeConfig::networkId() const {
  return m_networkId;
}
const std::string &TcpTestnetNodeRuntimeConfig::chainId() const {
  return m_chainId;
}
const std::string &TcpTestnetNodeRuntimeConfig::protocolVersion() const {
  return m_protocolVersion;
}
const std::string &TcpTestnetNodeRuntimeConfig::genesisId() const {
  return m_genesisId;
}
const std::filesystem::path &
TcpTestnetNodeRuntimeConfig::dataDirectory() const {
  return m_dataDirectory;
}
std::uint32_t TcpTestnetNodeRuntimeConfig::defaultTtlSeconds() const {
...
bool TcpTestnetNodeRuntimeConfig::isValid() const {
  return isSafeScalar(m_nodeId) && isSafeScalar(m_host) &&
         isSafeScalar(m_networkId) && isSafeScalar(m_chainId) &&
         isSafeScalar(m_protocolVersion) && !m_genesisId.empty() &&
         !m_dataDirectory.empty() && m_defaultTtlSeconds > 0 &&
         m_invalidMessageQuarantineThreshold > 0;
}

TcpTestnetPeerFileEntry::TcpTestnetPeerFileEntry()
    : m_nodeId(), m_endpoint(), m_publicKeyFingerprint(),
      m_hasPersistentState(false), m_firstSeenAt(0), m_lastSeenAt(0),
      m_score(0), m_quarantined(false), m_invalidMessageCount(0),
      m_bannedUntil(0), m_banReason(""), m_escalationCount(0) {}

TcpTestnetPeerFileEntry::TcpTestnetPeerFileEntry(
    std::string nodeId, p2p::PeerEndpoint endpoint,
    std::string publicKeyFingerprint)
    : m_nodeId(std::move(nodeId)), m_endpoint(std::move(endpoint)),
...
TcpTestnetPeerFileEntry::TcpTestnetPeerFileEntry(
    std::string nodeId, p2p::PeerEndpoint endpoint,
    std::string publicKeyFingerprint, std::int64_t firstSeenAt,
    std::int64_t lastSeenAt, std::int32_t score, bool quarantined,
    std::size_t invalidMessageCount)
    : TcpTestnetPeerFileEntry(std::move(nodeId), std::move(endpoint),
                              std::move(publicKeyFingerprint), firstSeenAt,
                              lastSeenAt, score, quarantined,
                              invalidMessageCount, 0, "", 0) {}

TcpTestnetPeerFileEntry::TcpTestnetPeerFileEntry(
    std::string nodeId, p2p::PeerEndpoint endpoint,
    std::string publicKeyFingerprint, std::int64_t firstSeenAt,
    std::int64_t lastSeenAt, std::int32_t score, bool quarantined,
    std::size_t invalidMessageCount, std::int64_t bannedUntil,
    std::string banReason, std::size_t escalationCount)
    : m_nodeId(std::move(nodeId)), m_endpoint(std::move(endpoint)),
      m_publicKeyFingerprint(std::move(publicKeyFingerprint)),
      m_hasPersistentState(true), m_firstSeenAt(firstSeenAt),
      m_lastSeenAt(lastSeenAt), m_score(score), m_quarantined(quarantined),
      m_invalidMessageCount(invalidMessageCount), m_bannedUntil(bannedUntil),
      m_banReason(std::move(banReason)), m_escalationCount(escalationCount) {}

const std::string &TcpTestnetPeerFileEntry::nodeId() const { return m_nodeId; }
const p2p::PeerEndpoint &TcpTestnetPeerFileEntry::endpoint() const {
  return m_endpoint;
}
const std::string &TcpTestnetPeerFileEntry::publicKeyFingerprint() const {
  return m_publicKeyFingerprint;
}
bool TcpTestnetPeerFileEntry::hasPersistentState() const {
  return m_hasPersistentState;
}
std::int64_t TcpTestnetPeerFileEntry::firstSeenAt() const {
  return m_firstSeenAt;
}
std::int64_t TcpTestnetPeerFileEntry::lastSeenAt() const {
  return m_lastSeenAt;
}
...
std::size_t TcpTestnetPeerFileEntry::invalidMessageCount() const {
  return m_invalidMessageCount;
}
std::int64_t TcpTestnetPeerFileEntry::bannedUntil() const {
  return m_bannedUntil;
}
const std::string &TcpTestnetPeerFileEntry::banReason() const {
  return m_banReason;
}
std::size_t TcpTestnetPeerFileEntry::escalationCount() const {
  return m_escalationCount;
}

bool TcpTestnetPeerFileEntry::isValid() const {
  const bool identityValid = isSafeScalar(m_nodeId) && m_endpoint.isValid() &&
...
std::string TcpTestnetPeerFileEntry::serialize() const {
  ...
  output << m_nodeId << " " << m_endpoint.host() << " " << m_endpoint.port()
         << " " << m_publicKeyFingerprint;
  if (m_hasPersistentState) {
    output << " " << m_firstSeenAt << " " << m_lastSeenAt << " " << m_score
           << " " << (m_quarantined ? 1 : 0) << " " << m_invalidMessageCount
           << " " << m_bannedUntil << " "
...
TcpTestnetPeerFileEntry::parseLine(const std::string &line) {
  ...
  if (fields.size() == 4) {
    entry = TcpTestnetPeerFileEntry(
        fields[0], p2p::PeerEndpoint(fields[1], parsePort(fields[2])),
        fields[3]);
  } else if (fields.size() == 9) {
    entry = TcpTestnetPeerFileEntry(
        fields[0], p2p::PeerEndpoint(fields[1], parsePort(fields[2])),
        fields[3], parseInt64(fields[4], "firstSeenAt"),
        parseInt64(fields[5], "lastSeenAt"), parseInt32(fields[6], "score"),
        parseBool(fields[7]), parseSize(fields[8], "invalidMessageCount"));
  } else {
    const std::string banReason = fields[10] == "none" ? "" : fields[10];
    const std::size_t escalationCount =
        fields.size() == 12 ? parseSize(fields[11], "escalationCount") : 0;
    entry = TcpTestnetPeerFileEntry(
        fields[0], p2p::PeerEndpoint(fields[1], parsePort(fields[2])),
        fields[3], parseInt64(fields[4], "firstSeenAt"),
        parseInt64(fields[5], "lastSeenAt"), parseInt32(fields[6], "score"),
        parseBool(fields[7]), parseSize(fields[8], "invalidMessageCount"),
        parseInt64(fields[9], "bannedUntil"), banReason, escalationCount);
  }
...
std::vector<TcpTestnetPeerFileEntry>
TcpTestnetPeerStore::load(const std::filesystem::path &peersFile) {
...
void TcpTestnetPeerStore::save(
  ...
  output << "# nodeId host port publicKeyFingerprint firstSeenAt "
            "lastSeenAt score quarantined invalidMessageCount bannedUntil "
            "banReason escalationCount\n";
...
TcpTestnetNodeRuntime::TcpTestnetNodeRuntime(TcpTestnetNodeRuntimeConfig config)
    : m_config(std::move(config)), m_transport(),
...
p2p::TcpTransport &TcpTestnetNodeRuntime::transport() { return m_transport; }

const p2p::TcpTransport &TcpTestnetNodeRuntime::transport() const {
  return m_transport;
}

p2p::GossipMesh &TcpTestnetNodeRuntime::gossipMesh() { return m_gossipMesh; }

const p2p::GossipMesh &TcpTestnetNodeRuntime::gossipMesh() const {
  return m_gossipMesh;
}

bool TcpTestnetNodeRuntime::running() const { return m_running; }
...
bool TcpTestnetNodeRuntime::hasAuthenticatedSession(
  ...
  return m_transport.connected(m_config.nodeId(), remoteNodeId) &&
         m_authenticatedTransport.hasSession(m_config.nodeId(), remoteNodeId);
}

p2p::TransportResult TcpTestnetNodeRuntime::start() {
  if (!m_config.isValid()) {
    return p2p::TransportResult(p2p::TransportStatus::REJECTED,
  ...
  p2p::TransportResult result =
      m_transport.bind(m_config.nodeId(), m_config.host(), m_config.port());
...
p2p::PeerRegistryResult
TcpTestnetNodeRuntime::addPeer(const p2p::PeerMetadata &peer) {
  const p2p::PeerRegistryResult registered = m_gossipMesh.registerPeer(peer);
  if (!registered.success()) {
    return registered;
  }

  m_transport.registerPeerEndpoint(peer.nodeId(), peer.endpoint());
  return registered;
}

std::size_t TcpTestnetNodeRuntime::loadPeersFromDisk(std::int64_t now) {
  ...
  for (const auto &entry : entries) {
    ...
    p2p::PeerMetadata peer(
        entry.nodeId(), entry.endpoint(), entry.publicKeyFingerprint(),
        firstSeenAt, lastSeenAt, entry.hasPersistentState() ? entry.score() : 0,
    ...
    if (addPeer(peer).success()) {
      if (entry.hasPersistentState()) {
        m_gossipMesh.restorePeerPenaltyState(
            entry.nodeId(), entry.invalidMessageCount(),
            entry.escalationCount(), entry.bannedUntil(), entry.banReason(),
            now, entry.hasPersistentState() ? entry.score() : 100);
      }
      ++loaded;
    }
  }
...
void TcpTestnetNodeRuntime::savePeersToDisk() const {
  ...
  for (const auto &peer : m_gossipMesh.peerRegistry().allPeers()) {
    entries.emplace_back(
        peer.nodeId(), peer.endpoint(), peer.publicKeyFingerprint(),
        peer.firstSeenAt(), peer.lastSeenAt(), peer.score(), peer.quarantined(),
        m_gossipMesh.invalidMessageCountForPeer(peer.nodeId()),
        peer.bannedUntil(), peer.banReason(),
        m_gossipMesh.quarantineEscalationForPeer(peer.nodeId()));
  }

  TcpTestnetPeerStore::save(m_config.peersFilePath(), entries);
}

p2p::TransportResult
TcpTestnetNodeRuntime::connectPeer(const std::string &remoteNodeId) {
  return m_gossipMesh.connectPeer(remoteNodeId);
}

p2p::TransportResult
TcpTestnetNodeRuntime::connectUnverifiedPeer(const std::string &remoteNodeId,
  ...
  if (knownPeer != nullptr &&
      (knownPeer->quarantined() || knownPeer->bannedAt(now) ||
...
TcpTestnetNodeRuntime::broadcast(p2p::NetworkMessageType type,
...
p2p::GossipDeliveryReport TcpTestnetNodeRuntime::broadcastChainStatus(
    const ChainStatusMessage &chainStatus, std::int64_t now) {
  return broadcast(p2p::NetworkMessageType::CHAIN_STATUS,
                   ChainStatusGossipCodec::encode(chainStatus), now);
}

p2p::GossipDeliveryReport TcpTestnetNodeRuntime::tick(std::int64_t now) {
  m_gossipMesh.liftExpiredPeerPenalties(now);

  const p2p::GossipDeliveryReport outbound = m_gossipMesh.flushOutbound(now);

  const p2p::GossipDeliveryReport inbound = m_gossipMesh.receiveAvailable(now);
