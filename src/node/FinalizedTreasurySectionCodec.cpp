...
// Prefix used when treasury section is embedded inside a larger artifact
// document.
...
std::uint64_t parseU64(const std::string &value, const std::string &field) {
  try {
    ...
    throw std::runtime_error("FinalizedTreasurySectionCodec: field '" + field +
                             "' is not a valid uint64: " + value);
  }
}

std::size_t parseCount(const serialization::KeyValueFileDocument &document,
                       const std::string &field) {
  const std::uint64_t parsed = parseU64(document.requireField(field), field);
  if (parsed >
          static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max()) ||
      parsed > document.fields().size()) {
    throw std::runtime_error("FinalizedTreasurySectionCodec: declared count "
                             "exceeds document bounds: " +
                             field);
  }
...
economics::TreasurySpendRecord
decodeRecord(const serialization::KeyValueFileDocument &doc, std::size_t index,
  ...
  const std::int64_t amountRaw =
      parseI64(doc.requireField(p + "amountRawUnits"), p + "amountRawUnits");
  const std::string purpose = doc.requireField(p + "purpose");
  const std::uint64_t execBlock =
      parseU64(doc.requireField(p + "executedAtBlock"), p + "executedAtBlock");
  const std::uint64_t epoch =
      parseU64(doc.requireField(p + "epoch"), p + "epoch");
  const std::int64_t beforeRaw =
      parseI64(doc.requireField(p + "balanceBeforeRawUnits"),
               p + "balanceBeforeRawUnits");
  const std::int64_t afterRaw = parseI64(
      doc.requireField(p + "balanceAfterRawUnits"), p + "balanceAfterRawUnits");

  economics::TreasurySpendRecord rec(
      spendId, proposalId, recipient, utils::Amount::fromRawUnits(amountRaw),
      purpose, execBlock, epoch, utils::Amount::fromRawUnits(beforeRaw),
...
void addRecordFields(std::set<std::string> &allowed, std::size_t index,
                     const std::string &prefix) {
  const std::string p = prefix + std::to_string(index) + ".";
...
void appendRecordFields(
    const economics::TreasurySpendRecord &rec, std::size_t index,
    const std::string &prefix,
  ...
  fields.emplace_back(p + "amountRawUnits",
                      std::to_string(rec.amount().rawUnits()));
  fields.emplace_back(p + "purpose", rec.purpose());
  fields.emplace_back(p + "executedAtBlock",
                      std::to_string(rec.executedAtBlock()));
...
economics::TreasuryExecutionEvidence
decodeEvidence(const serialization::KeyValueFileDocument &doc,
  ...
  const std::int64_t epochSpentRaw =
      parseI64(doc.requireField(p + "epochSpentSoFarRawUnits"),
               p + "epochSpentSoFarRawUnits");
  const std::int64_t createdAt =
      parseI64(doc.requireField(p + "createdAt"), p + "createdAt");
  ...
  economics::TreasuryApproval approval(
      doc.requireField(ap + "approvalId"), doc.requireField(ap + "proposalId"),
      parseU64(doc.requireField(ap + "approvedAtBlock"),
               ap + "approvedAtBlock"),
  ...
  economics::TreasuryPolicy policy(
      ...
      utils::Amount::fromRawUnits(
          parseI64(doc.requireField(polp + "maxSpendPerEpochRawUnits"),
                   polp + "maxSpendPerEpochRawUnits")),
      utils::Amount::fromRawUnits(
          parseI64(doc.requireField(polp + "maxSpendPerProposalRawUnits"),
                   polp + "maxSpendPerProposalRawUnits")),
      parseU64(doc.requireField(polp + "timelockBlocks"),
               polp + "timelockBlocks"),
      parseBool(doc.requireField(polp + "requireApproval"),
                polp + "requireApproval"),
  ...
  economics::TreasurySpendRecord spend(
      doc.requireField(sp + "spendId"), doc.requireField(sp + "proposalId"),
      doc.requireField(sp + "recipientAddress"),
      utils::Amount::fromRawUnits(parseI64(
          doc.requireField(sp + "amountRawUnits"), sp + "amountRawUnits")),
      doc.requireField(sp + "purpose"),
      parseU64(doc.requireField(sp + "executedAtBlock"),
               sp + "executedAtBlock"),
      parseU64(doc.requireField(sp + "epoch"), sp + "epoch"),
      utils::Amount::fromRawUnits(
          parseI64(doc.requireField(sp + "balanceBeforeRawUnits"),
                   sp + "balanceBeforeRawUnits")),
      utils::Amount::fromRawUnits(
          parseI64(doc.requireField(sp + "balanceAfterRawUnits"),
                   sp + "balanceAfterRawUnits")));

  economics::GovernanceApprovalContext governanceContext;
  governanceContext.governanceLifecycle =
      GovernanceLifecycleCodec::decodeFromDocument(doc,
                                                   p + "governanceLifecycle.");

  economics::TreasuryExecutionEvidence evidence(
      evidenceId, std::move(proposal), std::move(approval), std::move(policy),
      std::move(treasuryAccountBefore), blockHeight,
      utils::Amount::fromRawUnits(epochSpentRaw), std::move(spend), createdAt,
...
void addEvidenceFields(const serialization::KeyValueFileDocument &doc,
                       std::set<std::string> &allowed, std::size_t index,
                       const std::string &prefix) {
  const std::string p = prefix + std::to_string(index) + ".";
  ...
  GovernanceLifecycleCodec::addAllowedFields(doc, p + "governanceLifecycle.",
                                             allowed);
}

void appendEvidenceFields(
    const economics::TreasuryExecutionEvidence &ev, std::size_t index,
    const std::string &prefix,
    std::vector<std::pair<std::string, std::string>> &fields) {
  const std::string p = prefix + std::to_string(index) + ".";
  ...
  fields.emplace_back(pp + "recipientAddress",
                      ev.proposal().recipientAddress());
  ...
  const std::string tap = p + "treasuryAccountBefore.";
  fields.emplace_back(tap + "treasuryId",
                      ev.treasuryAccountBefore().treasuryId());
  ...
  fields.emplace_back(sp + "recipientAddress",
                      ev.spendRecord().recipientAddress());
  ...
  if (!ev.hasGovernanceContext()) {
    throw std::invalid_argument("FinalizedTreasurySectionCodec: evidence must "
                                "carry governance lifecycle context.");
  }

  GovernanceLifecycleCodec::appendFields(
      ev.governanceContext().governanceLifecycle, p + "governanceLifecycle.",
      fields);
}
...
std::string
FinalizedTreasurySectionCodec::encode(const FinalizedTreasurySection &section) {
  if (!section.isValid()) {
  ...
  if (section.hasEvidence()) {
    ...
    for (std::size_t i = 0; i < section.executionEvidence().size(); ++i) {
      appendEvidenceFields(section.executionEvidence()[i], i, "evidence.",
                           fields);
    }
  } else {
...
FinalizedTreasurySectionCodec::decode(const std::string &contents) {
...
// ---- Embedded mode ----
//
// Mirrors the standalone mode's evidence-based encoding exactly (see encode()/
// decode() above): every finalized artifact's treasury section is built via
// FinalizedTreasurySection's canonical evidence constructor, including when
// no treasury spend executed in that block (an empty evidence vector), so
// there is no separate legacy/spend-only representation to support here.

std::size_t FinalizedTreasurySectionCodec::evidenceCountFromDocument(
    const serialization::KeyValueFileDocument &doc) {
  return parseCount(doc, kEmbedPrefix + "evidenceCount");
}

void FinalizedTreasurySectionCodec::addAllowedFields(
    const serialization::KeyValueFileDocument &doc,
    std::set<std::string> &allowed, std::size_t evidenceCount) {
  allowed.insert(kEmbedPrefix + "evidenceCount");
  for (std::size_t i = 0; i < evidenceCount; ++i) {
    addEvidenceFields(doc, allowed, i, kEmbedPrefix + "evidence.");
  }
}

FinalizedTreasurySection FinalizedTreasurySectionCodec::decodeFromDocument(
    const serialization::KeyValueFileDocument &doc, std::size_t evidenceCount) {
  std::vector<economics::TreasuryExecutionEvidence> evidence;
  evidence.reserve(evidenceCount);
  for (std::size_t i = 0; i < evidenceCount; ++i) {
    evidence.push_back(decodeEvidence(doc, i, kEmbedPrefix + "evidence."));
  }
  return FinalizedTreasurySection(std::move(evidence));
}

void FinalizedTreasurySectionCodec::appendFields(
    const FinalizedTreasurySection &section, FieldList &fields) {
  if (!section.isValid()) {
    throw std::invalid_argument(
        "FinalizedTreasurySectionCodec::appendFields: invalid section: " +
        section.rejectionReason());
  }
  fields.emplace_back(kEmbedPrefix + "evidenceCount",
                      std::to_string(section.evidenceCount()));
  for (std::size_t i = 0; i < section.executionEvidence().size(); ++i) {
    appendEvidenceFields(section.executionEvidence()[i], i,
                         kEmbedPrefix + "evidence.", fields);
  }
}
