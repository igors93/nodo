...
ChainAuditResult auditImpl(const RuntimeStateLoadResult &load,
  ...
  if (!cryptoContext.isValid()) {
    return ChainAuditResult::failed("invalid crypto context for network " +
                                    manifest.networkName() + ": " +
                                    cryptoContext.rejectionReason());
  }

  if (!runtime.mempool().isValid(cryptoContext.policy(),
  ...
  if (!finalizedDeltas.empty()) {
    try {
      ...
      return ChainAuditResult::failed(
          std::string("chain audit: cannot determine genesis supply: ") +
          e.what());
    }
    ...
    // Supply continuity audit: verify finalized SupplyDeltas form a valid
    // chain.
    ...
    if (!supplyAudit.passed()) {
      const MonetaryAuditDiagnostic diag =
          MonetaryAuditDiagnostic::supplyContinuityFailure(
              supplyAudit.reason(), supplyAudit.failedBlockHeight(),
              supplyAudit.expectedSupply(), supplyAudit.actualSupply(),
    ...
    const economics::EpochMonetaryReport rebuiltReport =
        economics::EpochMonetaryReport::fromDeltas(
            policy, 0, finalizedDeltas.front().blockHeight(),
            finalizedDeltas.back().blockHeight(), finalizedDeltas);
    ...
    if (monetaryReportPath.empty()) {
      if (requireReport) {
        const MonetaryAuditDiagnostic diag =
            MonetaryAuditDiagnostic::reportMissing(
                "monetary report verification is required but no report path "
                "was provided",
                0);
        return ChainAuditResult::failed("chain audit: monetary report path "
                                        "required when finalized deltas exist"
                                        " | " +
                                        diag.serialize());
      }
      ...
      try {
        persistedReport =
            EpochMonetaryReportStore::read(monetaryReportPath, policy);
      } catch (const std::exception &e) {
        const MonetaryAuditDiagnostic diag =
            MonetaryAuditDiagnostic::reportMissing(
                std::string("failed to decode persisted monetary report: ") +
                    e.what(),
                0);
        return ChainAuditResult::failed(
            std::string(
                "chain audit: persisted monetary report decode failed: ") +
      ...
      if (!verif.matched()) {
        return ChainAuditResult::failed(
            "chain audit: persisted monetary report does not match rebuilt "
            "report: " +
    ...
    // the current localnet configuration (no extraordinary minting path
    // exists).
    for (const auto &artifact : load.loadedArtifacts()) {
      ...
      if (!EpochRewardSettlementService::finalizedRecordsMatch(
              runtime, artifact.block(), artifact.supplyDelta().supplyBefore(),
              epochRewardReason)) {
        return ChainAuditResult::failed(
    ...
    if (!treasuryAudit.passed()) {
      return ChainAuditResult::failed(
          "chain audit: treasury section audit failed: " +
    ...
    // slash twice.
    {
      ...
      for (const auto &artifact : load.loadedArtifacts()) {
        for (const auto &record : artifact.slashingEvidenceRecords()) {
          const auto key =
              std::make_pair(record.validatorAddress(), record.blockHeight());
    ...
    for (const auto &artifact : load.loadedArtifacts()) {
      ...
      if (summary.evidenceCount() != actualEvidenceCount) {
        return ChainAuditResult::failed(
            ...
            std::to_string(artifact.block().index()) + ": summary declares " +
            std::to_string(summary.evidenceCount()) + " evidence records but " +
            std::to_string(actualEvidenceCount) +
    ...
    for (const auto &artifact : load.loadedArtifacts()) {
      ...
      const auto recordCount = static_cast<std::uint64_t>(
          artifact.validatorLifecycleRecords().size());

      if (summaryTotal != recordCount) {
        return ChainAuditResult::failed(
            "chain audit: validator lifecycle summary count mismatch at "
            "block " +
            std::to_string(artifact.block().index()) + ": active(" +
            ...
            ") = " + std::to_string(summaryTotal) + " but " +
            std::to_string(recordCount) + " lifecycle records are present.");
      }
      ...
      for (const auto &dist : artifact.rewardDistributions()) {
        if (!dist.isValid()) {
          return ChainAuditResult::failed(
              ...
              "for validator " +
              dist.validatorAddress() + " at block " +
              std::to_string(artifact.block().index()) +
      ...
      for (const auto &position : artifact.lockedStakePositions()) {
        if (!position.isValid()) {
          return ChainAuditResult::failed(
              ...
              "for owner " +
              position.ownerAddress() + " at block " +
              std::to_string(artifact.block().index()) +
    ...
    for (const auto &artifact : load.loadedArtifacts()) {
      for (const auto &record :
           artifact.cryptographicSlashingEvidenceRecords()) {
        if (!record.isValid()) {
          return ChainAuditResult::failed(
      ...
      if (cryptoSummary.slashableEvidenceCount() >
          cryptoSummary.evidenceCount()) {
        return ChainAuditResult::failed(
      ...
      for (const auto &penalty : artifact.stakePenaltyRecords()) {
        if (!penalty.isValid()) {
          return ChainAuditResult::failed(
              ...
              "record for validator " +
              penalty.validatorAddress() + " at block " +
              std::to_string(artifact.block().index()) + ".");
        }
      }
    }
    ...
    if (!treasuryReportPath.empty()) {
      ...
      try {
        persistedTreasuryReport =
            EpochTreasuryReportStore::read(treasuryReportPath);
      } catch (const std::exception &e) {
        return ChainAuditResult::failed(
            std::string("chain audit: treasury report decode failed: ") +
            e.what());
      }
      ...
      if (crypto::KeyEncryptionPolicy::isOfficialNetwork(
              manifest.networkName()) &&
      ...
      const EpochTreasuryVerificationResult treasuryVerif =
          EpochTreasuryReportVerifier::verify(persistedTreasuryReport,
                                              treasuryAudit.rebuiltReport());

      if (!treasuryVerif.matched()) {
        return ChainAuditResult::failed(
            "chain audit: persisted treasury report does not match rebuilt "
            "report: " +
  ...
  const FinalizedGovernanceAuditResult governanceAudit =
      FinalizedGovernanceAudit::auditRuntime(runtime.governanceExecutor(),
                                             load.loadedArtifacts(),
                                             manifest.latestBlockHeight());

  if (!governanceAudit.accepted()) {
    return ChainAuditResult::failed(
        "chain audit: governance lifecycle audit failed: " +
        governanceAudit.reason() + " | " + governanceAudit.serialize());
  }

  ChainAuditResult result = ChainAuditResult::passed(
      manifest.networkName(), cryptoContext.networkProfile(),
      manifest.latestBlockHeight(), manifest.latestBlockHash(),
      manifest.latestStateRoot(), load.loadedBlockCount(),
      load.loadedMempoolTransactionCount(), manifest.validatorCount());

  // Compute a deterministic audit assignment for the latest artifact.
  // Seed = previous block hash (already-finalized data) to prevent
  // manipulation. An empty or insufficient validator set produces no assignment
  // (not an error).
  ...
  if (!artifacts.empty()) {
    ...
    if (!seedDigest.empty() && !validatorAddresses.empty()) {
      const std::string assignmentId = "chain-audit-assignment:" + seedDigest;
      const AuditAssignment assignment =
          AuditAssignmentCalculator::buildAssignment(
              assignmentId, manifest.latestBlockHeight(), 0,
              AuditAssignmentTargetType::BLOCK_ARTIFACT,
              latest.artifactDigest(), seedDigest, validatorAddresses);
...
ChainAuditResult
ChainAuditor::auditLoadedRuntimeDevMode(const RuntimeStateLoadResult &load) {
