...
const std::string& OperatorDiagnosticsReport::registeredGenesisId() const { return m_registeredGenesisId; }
const std::string& OperatorDiagnosticsReport::manifestGenesisId() const { return m_manifestGenesisId; }
const std::string& OperatorDiagnosticsReport::genesisId() const { return m_registeredGenesisId; }
...
const std::string& OperatorDiagnosticsReport::latestFinalizedHash() const { return m_latestFinalizedHash; }
...
std::string OperatorDiagnosticsReport::serialize() const {
    ...
    oss << "OperatorDiagnostics{\n"
        ...
        << "  registeredGenesisId=" << (m_registeredGenesisId.empty() ? "(none)" : m_registeredGenesisId) << "\n"
        << "  manifestGenesisId=" << (m_manifestGenesisId.empty() ? "(none)" : m_manifestGenesisId) << "\n"
        << "  genesisCompatible=" << (m_genesisCompatible ? "yes" : "no") << "\n"
        << "  finalizedHeight=" << m_finalizedHeight << "\n"
        << "  latestFinalizedHash=" << (m_latestFinalizedHash.empty() ? "(none)" : m_latestFinalizedHash) << "\n"
...
OperatorDiagnosticsReport OperatorDiagnostics::collect(
    ...
    const bool ready = keyPolicyPassed &&
                       genesisVerified &&
                       genesisCompatible &&
                       connectedPeers > 0;
