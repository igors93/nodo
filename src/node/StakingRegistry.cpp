...
StakePositionStatus statusFor(const StakingRegistry::Position& position) {
    ...
    if ((position.activeAmount + position.pendingActivationAmount +
         position.pendingUnbondingAmount).isZero()) {
        return StakePositionStatus::WITHDRAWN;
    }
    if (position.activeAmount.isPositive() || position.pendingUnbondingAmount.isPositive()) {
        return position.pendingUnbondingAmount.isPositive() && position.activeAmount.isZero()
...
void requireValidOperationInput(
...
std::string stakePositionStatusToString(StakePositionStatus status) {
    switch (status) {
        case StakePositionStatus::PENDING_ACTIVATION: return "PENDING_ACTIVATION";
        case StakePositionStatus::ACTIVE:             return "ACTIVE";
        case StakePositionStatus::UNBONDING:          return "UNBONDING";
        case StakePositionStatus::WITHDRAWN:          return "WITHDRAWN";
        case StakePositionStatus::TOMBSTONED:         return "TOMBSTONED";
...
bool StakeLifecycleRecord::isValid() const {
    return !recordId.empty() &&
           ...
           isSafeScalar(validatorAddress) &&
           !amount.isNegative() &&
           !activeAfter.isNegative() &&
           !pendingActivationAfter.isNegative() &&
           !pendingUnbondingAfter.isNegative() &&
           !slashedAfter.isNegative() &&
           blockHeight > 0;
}
...
std::string StakingRegistry::stakePositionId(
    const std::string& ownerAddress,
    const std::string& validatorAddress
) {
...
void StakingRegistry::setAccount(
    ...
    if (positions.empty()) {
        ...
        if (available.isPositive() || stored.slashedAmount().isPositive()) {
            ...
            position.status = stored.tombstoned()
                ? StakePositionStatus::TOMBSTONED
                : (available.isPositive() ? StakePositionStatus::ACTIVE : StakePositionStatus::WITHDRAWN);
...
void StakingRegistry::deposit(
    ...
    requireValidOperationInput(ownerAddress, validatorAddress, amount, blockHeight);
    if (blockHeight > std::numeric_limits<std::uint64_t>::max() - ACTIVATION_DELAY_BLOCKS) {
    ...
    if (found != positions.end() &&
        ...
        throw std::invalid_argument("Stake top-up requires a non-unbonding live position.");
    }
    ...
    rewriteAccountFromPositions(validatorAddress, current.jailed(), current.tombstoned());
    appendLifecycleRecord(
        requireExistingPosition ? "TOP_UP" : "DEPOSIT",
        transactionId,
        ownerAddress,
        validatorAddress,
        amount,
        blockHeight,
        positions.at(ownerAddress),
        requireExistingPosition ? "stake top-up pending activation" : "stake deposit pending activation"
    );
}

void StakingRegistry::requestUnlock(
    const std::string& ownerAddress,
    const std::string& validatorAddress,
    utils::Amount amount,
    std::uint64_t blockHeight,
    ...
    if (current.jailed() || current.tombstoned()) {
        throw std::invalid_argument("Jailed or tombstoned validator stake cannot be unlocked.");
    }
    ...
    owner->second.pendingUnbondingAmount = owner->second.pendingUnbondingAmount + amount;
...
void StakingRegistry::withdraw(
    ...
    auto owner = validator->second.find(ownerAddress);
    if (owner == validator->second.end() || owner->second.pendingUnbondingAmount < amount) {
        throw std::invalid_argument("Stake withdrawal exceeds the owner's unbonding position.");
    }
    if (owner->second.withdrawableHeight == 0 || blockHeight < owner->second.withdrawableHeight) {
...
void StakingRegistry::requestValidatorExit(
    const std::string& ownerAddress,
    const std::string& validatorAddress,
    std::uint64_t blockHeight,
    const std::string& transactionId
) {
    if (!isSafeScalar(ownerAddress) || !isSafeScalar(validatorAddress) || blockHeight == 0) {
...
bool StakingRegistry::activatePending(std::uint64_t boundaryHeight) {
    ...
    for (auto& [validator, positions] : m_positionsByValidatorAndOwner) {
        ...
        if (current.tombstoned()) continue;
        for (auto& [owner, position] : positions) {
            ...
            if (position.pendingActivationAmount.isPositive() &&
                ...
                appendLifecycleRecord(
                    ...
                    boundaryHeight,
                    position,
                    "pending stake activated at deterministic height"
                );
                changed = true;
            }
        }
        if (changed) {
            rewriteAccountFromPositions(validator, current.jailed(), current.tombstoned());
        }
    }
    return changed;
}

void StakingRegistry::applyPenaltyState(
    const std::string& validatorAddress,
    ...
    if (validator != m_positionsByValidatorAndOwner.end()) {
        for (auto& [owner, position] : validator->second) {
            if (remainingSlash.isZero()) break;
            ...
            if (remainingSlash.isPositive()) {
                fromPendingActivation = position.pendingActivationAmount < remainingSlash
                    ? position.pendingActivationAmount
                    : remainingSlash;
                if (fromPendingActivation.isPositive()) {
                    position.pendingActivationAmount =
                        position.pendingActivationAmount - fromPendingActivation;
                    position.slashedAmount = position.slashedAmount + fromPendingActivation;
            ...
            const utils::Amount applied = fromActive + fromPendingActivation + fromUnbonding;
            if (applied.isPositive()) {
                appendLifecycleRecord(
                    ...
                    blockHeight,
                    position,
                    tombstoned ? "stake slashed and validator tombstoned" : "stake slashed"
                );
            }
        }
    }
    ...
    if (remainingSlash.isPositive()) {
        throw std::invalid_argument("Canonical slashing total exceeds remaining stake positions.");
    }

    rewriteAccountFromPositions(validatorAddress, jailed || tombstoned, tombstoned);
...
utils::Amount StakingRegistry::activeStake(
    const std::string& ownerAddress,
    const std::string& validatorAddress
) const {
    ...
    return owner == validator->second.end() ? utils::Amount() : owner->second.activeAmount;
}
...
utils::Amount StakingRegistry::pendingActivationStake(
    ...
    if (validator == m_positionsByValidatorAndOwner.end()) return utils::Amount();
    const auto owner = validator->second.find(ownerAddress);
    return owner == validator->second.end() ? utils::Amount() : owner->second.pendingActivationAmount;
}
...
utils::Amount StakingRegistry::pendingUnbondingStake(
    ...
    if (validator == m_positionsByValidatorAndOwner.end()) return utils::Amount();
    const auto owner = validator->second.find(ownerAddress);
    return owner == validator->second.end() ? utils::Amount() : owner->second.pendingUnbondingAmount;
}
...
utils::Amount StakingRegistry::activeStakeFor(const std::string& validatorAddress) const {
    ...
    const auto validator = m_positionsByValidatorAndOwner.find(validatorAddress);
    if (validator == m_positionsByValidatorAndOwner.end() || validator->second.empty()) {
        return account == m_accounts.end() ? utils::Amount() : safeAvailable(account->second);
    }
    utils::Amount total;
...
utils::Amount StakingRegistry::bondedStakeFor(const std::string& validatorAddress) const {
...
std::uint64_t StakingRegistry::unlockHeight(
    ...
    const auto owner = validator->second.find(ownerAddress);
    return owner == validator->second.end() ? 0 : owner->second.withdrawableHeight;
}
...
std::vector<StakePositionView> StakingRegistry::positions() const {
    ...
    for (const auto& [validator, ownerPositions] : m_positionsByValidatorAndOwner) {
...
std::vector<StakePositionView> StakingRegistry::positionsForOwner(
    const std::string& ownerAddress
) const {
    std::vector<StakePositionView> result;
    for (const auto& [validator, ownerPositions] : m_positionsByValidatorAndOwner) {
...
const std::vector<StakeLifecycleRecord>& StakingRegistry::lifecycleRecords() const {
...
bool StakingRegistry::isValid() const {
    ...
    for (const auto& [validator, positions] : m_positionsByValidatorAndOwner) {
        if (!isSafeScalar(validator)) return false;
        ...
        for (const auto& [owner, position] : positions) {
            if (!isSafeScalar(owner) ||
                ...
                position.lockHeight == 0 ||
                position.activationHeight == 0 ||
        ...
        if (account == m_accounts.end()) return false;
        if (account->second.slashedAmount() != slashedTotal) return false;
        if (account->second.bondedAmount() != availableTotal + slashedTotal) return false;
    }
...
std::string StakingRegistry::serialize() const {
    ...
    oss << "StakingRegistry{count=" << m_accounts.size()
        ...
        << ";lifecycleRecordCount=" << m_lifecycleRecords.size()
        << ";accounts=[";
    bool first = true;
    for (const auto& [addr, account] : m_accounts) {
        if (!first) oss << ",";
...
StakePositionView StakingRegistry::viewFor(
    const std::string& validatorAddress,
    const std::string& ownerAddress,
...
void StakingRegistry::rewriteAccountFromPositions(
    ...
    setAccount(validatorAddress, economics::StakeAccount(
        validatorAddress,
        availableTotal + slashedTotal,
        slashedTotal,
        preserveJailed || preserveTombstoned,
        preserveTombstoned
    ));
}
