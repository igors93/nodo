...
const NodeDataDirectoryConfig &NodeOrchestratorConfig::dataDirectory() const {
  return m_dataDirectory;
}
const p2p::PeerInfo &NodeOrchestratorConfig::localPeer() const {
  return m_localPeer;
}
const std::string &NodeOrchestratorConfig::localValidatorAddress() const {
  return m_localValidatorAddress;
}
std::uint16_t NodeOrchestratorConfig::rpcPort() const { return m_rpcPort; }
const std::string &NodeOrchestratorConfig::rpcBindAddr() const {
  return m_rpcBindAddr;
}
std::int64_t NodeOrchestratorConfig::consensusTickMs() const {
  return m_consensusTickMs;
}
...
bool NodeOrchestratorConfig::isValid() const {
  return m_genesisConfig.isValid() && m_localPeer.isValid() &&
         !m_rpcBindAddr.empty() && m_consensusTickMs > 0 &&
...
tryUnwrapCanonical(const std::string &payload) {
...
parseReconnectEndpoint(const std::string &serialized) {
  if (serialized.rfind("PeerEndpoint{", 0) == 0 && serialized.size() > 15 &&
      serialized.back() == '}') {
    ...
    if (hostStart == std::string::npos || portStart == std::string::npos ||
        portStart <= hostStart + hostPrefix.size()) {
      return std::nullopt;
    }
    const std::string host =
        serialized.substr(hostStart + hostPrefix.size(),
                          portStart - (hostStart + hostPrefix.size()));
...
std::filesystem::path
peerExchangeCandidatePath(const NodeDataDirectoryConfig &config) {
  return config.peersDirectoryPath() / "peer_exchange_candidates.conf";
}

std::vector<p2p::PeerExchangeEntry>
sortAndCapPeerExchangeCandidates(std::vector<p2p::PeerExchangeEntry> entries,
...
// Build a PersistentBlockSyncBatch from [fromHeight, fromHeight+maxItems) of
// the local chain. When a durable QC record exists for a block, it is embedded
// in the batch item so the receiver can perform fast-path QC-required sync
// without needing to recompute the state root from scratch.
PersistentBlockSyncBatch
buildSyncResponseBatch(const core::Blockchain &blockchain,
                       const NodeDataDirectoryConfig &directoryConfig,
                       const std::string &localPeerId, std::uint64_t fromHeight,
                       std::uint64_t maxItems, std::int64_t now) {
  ...
  for (const auto &block : blockchain.blocks()) {
    ...
    if (artifact.block().hash() != block.hash()) {
      throw std::runtime_error(
          "Canonical sync artifact does not match runtime chain.");
    }

    items.emplace_back(block.index(), block.hash(), block.previousHash(),
                       block.serialize(),
                       block.hasCanonicalStateRoot()
                           ? block.stateRoot()
                           : "height-" + std::to_string(block.index()),
                       now, artifact.finalizedRecord().serialize());
  }
  if (items.empty())
  ...
  return PersistentBlockSyncBatch(localPeerId, firstHeight, lastHeight,
                                  std::move(items), now);
}
...
nodeOrchestratorStartStatusToString(NodeOrchestratorStartStatus status) {
  switch (status) {
  case NodeOrchestratorStartStatus::RUNNING:
    return "RUNNING";
  case NodeOrchestratorStartStatus::ALREADY_RUNNING:
    return "ALREADY_RUNNING";
  case NodeOrchestratorStartStatus::INIT_FAILED:
    return "INIT_FAILED";
  case NodeOrchestratorStartStatus::STATE_LOAD_FAILED:
    return "STATE_LOAD_FAILED";
  case NodeOrchestratorStartStatus::TRANSPORT_FAILED:
    return "TRANSPORT_FAILED";
  case NodeOrchestratorStartStatus::CONSENSUS_FAILED:
...
NodeOrchestrator::NodeOrchestrator(NodeOrchestratorConfig config,
...
void NodeOrchestrator::tick(std::int64_t now) {
      ...
      try {
        ...
        if (existing != nullptr &&
          ...
          m_runtime->mutablePeerManager().addOrUpdatePeer(p2p::PeerInfo(
              existing->peerId(), existing->endpoint(),
              existing->protocolVersion(), status.latestHeight(), now));
        }
  ...
  // being added to the chain. Applied blocks are persisted immediately.
  {
    ...
    PersistentSyncCheckpoint currentCheckpoint =
        checkpointMatchesRuntime
            ? readCp.checkpoint()
            : PersistentSyncCheckpoint(PersistentSyncCheckpoint::SCHEMA_VERSION,
                                       localTip.index(), localTip.hash(),
                                       localTip.hasCanonicalStateRoot()
                                           ? localTip.stateRoot()
                                           : "genesis-state-root",
                                       PersistentSyncStatus::IDLE,
                                       m_config.localPeer().peerId(), now);
      ...
      try {
        ...
        const PersistentSyncApplyResult applyResult =
            PersistentBlockStateSyncApplier::importFinalizedBatch(
                currentCheckpoint, batch, *m_runtime, m_config.dataDirectory(),
                &cpStore, now, &m_evidencePool, m_slashingEvidenceStore.get());
...
const NodeRuntime &NodeOrchestrator::runtime() const {
  if (!m_runtime)
    throw std::runtime_error("NodeOrchestrator: runtime not started");
  return *m_runtime;
}
...
const TcpTestnetNodeRuntime &NodeOrchestrator::tcpRuntime() const {
  if (!m_tcpRuntime)
    throw std::logic_error("TCP runtime not started.");
  return *m_tcpRuntime;
}

TcpTestnetNodeRuntime &NodeOrchestrator::mutableTcpRuntime() {
  if (!m_tcpRuntime)
    throw std::logic_error("TCP runtime not started.");
  return *m_tcpRuntime;
}

const NodeOrchestratorConfig &NodeOrchestrator::config() const {
  return m_config;
}
const consensus::EvidencePool &NodeOrchestrator::evidencePool() const {
  return m_evidencePool;
}
const crypto::CryptoPolicy &NodeOrchestrator::cryptoPolicy() const {
  return m_policy;
}
const crypto::SignatureProvider &NodeOrchestrator::signatureProvider() const {
  return m_provider;
}
...
NodeOrchestrator::gossipBroadcast(p2p::NetworkMessageType type,
...
void NodeOrchestrator::gossipInjectLoopback(p2p::NetworkMessageType type,
                                            const std::string &payload,
                                            std::int64_t now) {
  if (!m_tcpRuntime)
    return;
  m_tcpRuntime->gossipMesh().injectLocalMessage(type, payload, now);
}

void NodeOrchestrator::registerBootstrapPeer(const p2p::PeerMetadata &peer,
                                             std::int64_t now) {
...
const p2p::PeerReconnectionPolicy &
NodeOrchestrator::reconnectionPolicy() const {
  return m_reconnectionPolicy;
}

void NodeOrchestrator::triggerSyncIfBehind(
    const ChainStatusMessage &remotePeerStatus, const std::string &remotePeerId,
    std::int64_t now) {
  if (!m_runtime || !m_tcpRuntime || remotePeerId.empty())
    return;

  PersistentSyncCheckpointStore store(m_config.dataDirectory().rootPath());
  ...
  if (readResult.loaded() &&
    ...
    checkpoint = PersistentSyncCheckpoint(
        ...
        PersistentSyncStatus::IDLE, m_config.localPeer().peerId(), now);
  }

  const PersistentSyncPlan plan =
      PersistentBlockStateSyncPlanner::planFromRemoteStatus(
          checkpoint, remotePeerStatus, m_config.localPeer().peerId(),
          remotePeerId, NODO_PERSISTENT_SYNC_MAX_BLOCK_BATCH, now);

  if (plan.requestSnapshot()) {
    // When the height gap is large, signal the peer that we need a snapshot.
    // The peer will respond with a SNAPSHOT_SYNC_RESPONSE (future message
    // type). For now we record the need and fall through — the peer may still
    // serve blocks if snapshot responses are not yet supported on its side.
    m_syncHealth.recordRequestFailure(
        "Snapshot requested but not yet supported; will retry with block sync.",
        now);
    return;
  }

  if (!plan.requestBlocks())
    return;
...
NodeOrchestratorStartResult NodeOrchestrator::initOrLoad() {
  ...
  if (!NodeDataDirectory::isInitialized(m_config.dataDirectory())) {
    ...
    const auto initResult = NodeDataDirectory::initialize(
        m_config.dataDirectory(), m_config.genesisConfig(),
        m_config.localPeer(), now);
    ...
    NodeRuntimeConfig runtimeConfig(m_config.genesisConfig(),
                                    m_config.localPeer(), 128);

    auto startResult = NodeRuntimeFactory::startFromGenesis(runtimeConfig);
    if (!startResult.started()) {
      return {NodeOrchestratorStartStatus::INIT_FAILED,
              "NodeRuntimeFactory::startFromGenesis failed: " +
                  startResult.reason()};
    }
  ...
  m_slashingEvidenceStore = std::make_unique<storage::SlashingEvidenceStore>(
  ...
  const PersistentSlashingEvidencePoolLoadResult evidenceRestore =
      PersistentSlashingEvidencePool::restore(
          m_evidencePool, *m_slashingEvidenceStore,
          ...
          m_config.genesisConfig().networkParameters().chainId(), m_policy,
          m_provider, m_runtime->validatorPenaltyLedger());
...
bool NodeOrchestrator::startTransport() {
  ...
  for (const p2p::PeerExchangeEntry &entry :
       loadPersistedPeerExchangeCandidates()) {
    const std::optional<p2p::PeerEndpoint> endpoint =
        parseReconnectEndpoint(entry.endpoint);
  ...
  std::vector<std::pair<std::string, std::pair<std::string, std::uint16_t>>>
      discoverySeeds;
  const auto entries =
      TcpTestnetPeerStore::load(m_tcpRuntime->config().peersFilePath());
  for (const auto &entry : entries) {
    if (entry.hasPersistentState() && entry.quarantined()) {
      continue;
    }
    trackPeerCandidate(entry.nodeId(), entry.endpoint(), now, true);
    if (entry.endpoint().port() == std::numeric_limits<std::uint16_t>::max()) {
      continue;
    }
    const std::uint16_t udpPort =
        static_cast<std::uint16_t>(entry.endpoint().port() + 1);
    m_discoveryService->addPeer(entry.nodeId(), entry.endpoint().host(),
                                entry.endpoint().port(), udpPort);
    discoverySeeds.push_back(
        {entry.nodeId(), {entry.endpoint().host(), udpPort}});
  }
...
bool NodeOrchestrator::startConsensus() {
  ...
  consensus::ConsensusRoundState recoveryState = storedRecoveryState.value();
  ...
  if (recoveryState.height() < currentState.height()) {
    if (!m_runtime->finalizationRegistry().hasFinalizedHeight(
        ...
        !consensus::ConsensusRecoveryStore::save(recoveryPath, currentState)) {
      return false;
    }
  ...
  const std::string expectedProposer =
      consensus::ProposerSchedule::selectProposer(
          ...
          m_config.genesisConfig().networkParameters().chainId(),
          recoveryState.height(), recoveryState.round());
  ...
  m_consensusLoop->setBlockProducerCallback(
      [this](std::uint64_t height, std::uint64_t round,
  ...
  m_consensusLoop->setFinalizedCallback([this](const consensus::
                                                   FinalizedBlockRecord &rec) {
    if (!m_tcpRuntime || m_runtime->blockchain().empty())
      return;

    const auto &blocks = m_runtime->blockchain().blocks();
    if (rec.blockIndex() >= blocks.size())
      return;

    const core::Block &block =
        blocks[static_cast<std::size_t>(rec.blockIndex())];
    const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                         std::chrono::system_clock::now().time_since_epoch())
                         .count();
    try {
      ...
      m_syncHealth.recordBatchFailure(
          std::string("Evidence pool cleanup error after finalization: ") +
              e.what(),
    ...
    if (block.index() > 0 && block.index() % kSnapshotInterval == 0) {
      try {
        const auto &genesisConfig = m_runtime->config().genesisConfig();
        ...
        snapshotStore.save(
            storage::AccountStateSnapshot(genesisConfig.deterministicId(),
                                          block.index(), block.hash(), view));
      } catch (...) {
...
void NodeOrchestrator::setLocalNodeIdentity(crypto::KeyPair nodeIdentityKey) {
  if (!nodeIdentityKey.isValid() ||
      nodeIdentityKey.algorithm() != crypto::CryptoAlgorithm::CLASSIC_ED25519) {
...
std::optional<core::Block> NodeOrchestrator::produceBlock(std::uint64_t height,
  ...
  const std::size_t minimumTransactions = pendingEvidence.empty() ? 1U : 0U;
...
ChainStatusMessage NodeOrchestrator::currentChainStatus() const {
  ...
  return ChainStatusMessage(networkName, chainId,
                            m_config.localPeer().protocolVersion(), height,
                            latestHash, finalizedHeight, finalizedHash);
}

std::optional<p2p::PeerMetadata>
NodeOrchestrator::localHandshakePeer(std::int64_t now) const {
  if (!m_tcpRuntime || !m_localNodeIdentity.has_value() || now <= 0) {
    return std::nullopt;
  }
  return p2p::PeerMetadata(
      m_config.localPeer().peerId(), m_tcpRuntime->transport().localEndpoint(),
      m_localNodeIdentity->publicKey().fingerprint(), now, now, 0, false);
}

void NodeOrchestrator::trackPeerCandidate(const std::string &nodeId,
  ...
  m_reconnectionPolicy.recordCandidate(nodeId, endpoint.serialize(), now,
                                       immediateRetry);
}

void NodeOrchestrator::seedDiscoveryPeer(const std::string &nodeId,
                                         const p2p::PeerEndpoint &endpoint) {
  if (!m_discoveryService || nodeId.empty() || !endpoint.isValid() ||
      endpoint.port() == std::numeric_limits<std::uint16_t>::max()) {
    return;
  }
  const std::uint16_t udpPort = static_cast<std::uint16_t>(endpoint.port() + 1);
  m_discoveryService->addPeer(nodeId, endpoint.host(), endpoint.port(),
                              udpPort);
...
void NodeOrchestrator::handleDiscoveredPeer(const std::string &peerId,
                                            const std::string &host,
...
std::vector<p2p::PeerSubnetInfo> NodeOrchestrator::activePeerSubnets() const {
  ...
  for (const p2p::PeerMetadata &peer :
    ...
    p2p::PeerSubnetInfo info{peer.nodeId(), peer.endpoint().host(), subnet,
                             peer.endpoint().port()};
    if (info.isValid()) {
...
void NodeOrchestrator::persistPeerExchangeCandidates(
  ...
  std::vector<p2p::PeerExchangeEntry> merged =
      loadPersistedPeerExchangeCandidates();
  merged.insert(merged.end(), acceptedEntries.begin(), acceptedEntries.end());
  merged = sortAndCapPeerExchangeCandidates(std::move(merged),
                                            m_config.localPeer().peerId());
  if (merged.empty()) {
    return;
  }
  ...
  for (const p2p::NetworkEnvelope &envelope : envelopes) {
    if (!m_tcpRuntime->hasAuthenticatedSession(envelope.senderNodeId())) {
      gossip.reportPeerMisbehavior(
          ...
          "Unauthenticated peer exchange message rejected.", now);
      continue;
    }
    ...
    if (entries.empty() && envelope.payload() != emptyPeerExchangePayload) {
      gossip.reportPeerMisbehavior(
          envelope, p2p::PeerMisbehaviorType::INVALID_MESSAGE,
          "Malformed peer exchange payload rejected.", now);
      continue;
    }
...
void NodeOrchestrator::broadcastPeerExchange(std::int64_t now) {
  ...
  const p2p::GossipDeliveryReport report = m_tcpRuntime->gossipMesh().broadcast(
      p2p::NetworkMessageType::PEER_EXCHANGE,
      p2p::PeerExchangeService::serializePayload(entries), now);
...
std::optional<p2p::PeerEndpoint>
NodeOrchestrator::endpointForReconnectCandidate(
    const p2p::PeerReconnectionState &state) const {
...
bool NodeOrchestrator::attemptReconnectCandidate(
    const p2p::PeerReconnectionState &state, std::int64_t now) {
  if (!m_tcpRuntime || !m_localNodeIdentity.has_value() || now <= 0) {
    return false;
  }
  ...
  if (registered != nullptr &&
    ...
    m_reconnectionPolicy.quarantine(
        state.nodeId,
        registered->bannedAt(now) ? "Peer registry has an active temporary ban."
  ...
  if (m_tcpRuntime->transport().connected(m_config.localPeer().peerId(),
                                          state.nodeId)) {
  ...
  const p2p::GossipDeliveryReport report =
      PeerHandshakeAutoRegistrar::initiateHandshake(m_tcpRuntime->gossipMesh(),
                                                    state.nodeId, now);
...
void NodeOrchestrator::driveNetworkPeerPolicy(std::int64_t now) {
  ...
  for (const p2p::PeerMetadata &peer : knownPeers) {
    ...
    if (peer.quarantined() || peer.bannedAt(now)) {
      m_reconnectionPolicy.quarantine(
          peer.nodeId(),
          peer.bannedAt(now) ? "Peer registry has an active temporary ban."
                             : "Peer registry is quarantined.",
          now);
      continue;
    }
    ...
    if (!connected && tracked != nullptr && tracked->attemptInFlight &&
        now >= tracked->lastAttemptAt +
                   p2p::PeerReconnectionPolicy::BASE_DELAY_SECONDS) {
    ...
    if (!connected && tracked == nullptr) {
      m_reconnectionPolicy.recordDisconnect(peer.nodeId(),
                                            peer.endpoint().serialize(), now);
    }
  }
...
TcpTestnetNodeRuntimeConfig NodeOrchestrator::buildTransportConfig() const {
  ...
  if (colon != std::string::npos) {
    ...
    try {
      ...
      throw std::invalid_argument("Invalid port for peer endpoint: " +
                                  endpoint);
    }
  }

  return TcpTestnetNodeRuntimeConfig(
      peer.peerId(), host, port, np.networkName(), np.chainId(),
