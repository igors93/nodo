...
std::string syncHealthStatusToString(SyncHealthStatus status) {
    switch (status) {
        case SyncHealthStatus::HEALTHY:        return "HEALTHY";
        case SyncHealthStatus::BATCH_FAILURE:  return "BATCH_FAILURE";
        case SyncHealthStatus::REQUEST_FAILURE: return "REQUEST_FAILURE";
        case SyncHealthStatus::SERVE_FAILURE:  return "SERVE_FAILURE";
        case SyncHealthStatus::DISABLED:       return "DISABLED";
...
const std::string& SyncHealth::lastFailureReason() const { return m_lastFailureReason; }
...
bool SyncHealth::hasRecentFailure(std::int64_t now, std::int64_t windowSeconds) const {
    if (m_lastFailureAt <= 0) return false;
...
void SyncHealth::recordSuccess() {
    m_status = SyncHealthStatus::HEALTHY;
    ++m_totalSynced;
}

void SyncHealth::recordBatchFailure(const std::string& reason, std::int64_t now) {
    recordFailure(SyncHealthStatus::BATCH_FAILURE, reason, now);
}

void SyncHealth::recordRequestFailure(const std::string& reason, std::int64_t now) {
    recordFailure(SyncHealthStatus::REQUEST_FAILURE, reason, now);
}

void SyncHealth::recordServeFailure(const std::string& reason, std::int64_t now) {
    recordFailure(SyncHealthStatus::SERVE_FAILURE, reason, now);
}

void SyncHealth::recordFailure(
    SyncHealthStatus kind,
