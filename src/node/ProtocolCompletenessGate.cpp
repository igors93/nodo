...
using RequirementList = std::vector<ProtocolCompletenessRequirement>;

void addRequirement(RequirementList &requirements, const std::string &id,
                    const std::string &description, bool satisfied,
                    const std::string &detail) {
  requirements.emplace_back(id, description,
                            satisfied ? ProtocolCompletenessStatus::SATISFIED
                                      : ProtocolCompletenessStatus::FAILED,
                            detail);
}

std::int64_t minimumFeeRawUnits(const config::NetworkParameters &parameters) {
  if (parameters.minimumFeeRawUnits() >
      static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max())) {
    throw std::invalid_argument(
        "Network minimum fee does not fit signed amount range.");
  }

  return static_cast<std::int64_t>(parameters.minimumFeeRawUnits());
}

bool hasEconomicSecurityRules(const config::NetworkParameters &parameters) {
  return parameters.isValid() && parameters.minimumValidatorCount() > 0 &&
...
std::string
protocolCompletenessStatusToString(ProtocolCompletenessStatus status) {
  switch (status) {
...
ProtocolCompletenessRequirement::ProtocolCompletenessRequirement()
    : m_id(""), m_description(""), m_status(ProtocolCompletenessStatus::FAILED),
      m_detail("Uninitialized protocol completeness requirement.") {}

ProtocolCompletenessRequirement::ProtocolCompletenessRequirement(
    std::string id, std::string description, ProtocolCompletenessStatus status,
    std::string detail)
    : m_id(std::move(id)), m_description(std::move(description)),
      m_status(status), m_detail(std::move(detail)) {}

const std::string &ProtocolCompletenessRequirement::id() const { return m_id; }
...
std::string ProtocolCompletenessRequirement::serialize() const {
  ...
  oss << "ProtocolCompletenessRequirement{"
      << "id=" << m_id << ";description=" << m_description
...
ProtocolCompletenessReport::ProtocolCompletenessReport() : m_requirements() {}
...
bool ProtocolCompletenessReport::complete() const {
  ...
  for (const ProtocolCompletenessRequirement &requirement : m_requirements) {
...
std::string ProtocolCompletenessReport::serialize() const {
  ...
  oss << "ProtocolCompletenessReport{"
      ...
      << ";failedCount=" << failedCount() << ";requirements=[";
...
ProtocolCompletenessGate::evaluateRuntime(const NodeRuntime &runtime) {
...
ProtocolCompletenessReport ProtocolCompletenessGate::evaluate(
  ...
  addRequirement(requirements, "consensus_fork_choice_finality",
                 "Runtime has a valid chain, validator set, fork summary and "
                 "finality registry.",
  ...
  if (directoryConfig != nullptr) {
    ...
    if (!manifestRead.loaded()) {
      persistenceDetail =
          "Runtime manifest could not be loaded: " + manifestRead.reason();
    } else {
      const ProtocolInvariantCheckResult manifestCheck =
          ProtocolInvariantChecker::checkRuntimeAgainstManifest(
              runtime, manifestRead.manifest());

      persistenceSatisfied = manifestCheck.passed();
      persistenceDetail = manifestCheck.passed()
                              ? "Storage manifest matches runtime chain, state "
                                "and peer summaries."
                              : manifestCheck.reason();
    }

    addRequirement(requirements, "persistent_chain_state",
                   "Durable node directory exists and its manifest matches the "
                   "runtime tip.",
                   persistenceSatisfied, persistenceDetail);
  }

  try {
    const ProtocolReplayState replayState =
        ProtocolStateTransition::replayToTip(runtime.config().genesisConfig(),
                                             runtime.blockchain(),
                                             minimumFeeRawUnits(parameters));

    addRequirement(requirements, "verifiable_state_root",
                   "Latest full protocol state rebuilds to a deterministic "
                   "non-empty root.",
                   !replayState.stateRoot.empty(),
                   !replayState.stateRoot.empty()
                       ? "Latest protocol state root rebuilt successfully from "
  ...
  try {
    ...
    addRequirement(
        requirements, "deterministic_state_transition",
        "Block execution has a deterministic state-transition context at the "
        "tip.",
        context.enforceAccountState(),
        context.enforceAccountState()
            ? "State-transition context enforces account state, nonce and fee "
              "rules."
            : "State-transition context does not enforce account state.");
  } catch (const std::exception &error) {
    addRequirement(requirements, "deterministic_state_transition",
                   "Block execution has a deterministic state-transition "
                   "context at the tip.",
                   false, error.what());
  }

  addRequirement(
      requirements, "economic_rules",
      "Network parameters define validator, quorum, fee, block and finality "
      "limits.",
      hasEconomicSecurityRules(parameters),
      hasEconomicSecurityRules(parameters)
          ? "Economic and consensus limits are present in network parameters."
          : "Network parameters are missing required economic or consensus "
            "limits.");

  const p2p::PeerMessage summaryMessage = runtime.localChainSummaryMessage(
      runtime.config().localPeer().peerId(),
      runtime.config().genesisConfig().genesisTimestamp());

  addRequirement(requirements, "sync_protocol",
                 "Runtime can emit a valid chain-summary message for peer "
                 "synchronization.",
  ...
  const crypto::ProtocolCryptoContext cryptoContext =
      crypto::ProtocolCryptoContext::fromNetworkName(parameters.networkName());

  addRequirement(
      requirements, "advanced_mempool_policy",
      ...
      cryptoContext.isValid() && runtime.mempool().replaceByHigherFee()
          ...
          : "Mempool policy, crypto context or higher-fee replacement is "
            "invalid.");

  const p2p::InboundMessagePolicy inboundPolicy;
  const p2p::GossipMeshConfig gossipConfig(
      runtime.config().localPeer().peerId(), parameters.networkName(),
      parameters.chainId(), parameters.protocolVersion(),
      runtime.config().genesisConfig().deterministicId(), 60, 2,
      parameters.maxGossipMessagesPerPeerWindow(),
      parameters.maxTransactionGossipPerPeerWindow());

  addRequirement(
      requirements, "p2p_identity_security",
      "P2P has bounded inbound-message validation and explicit handshake "
      "identity parameters.",
      ...
      inboundPolicy.isValid() && gossipConfig.isValid()
          ...
          : "Inbound-message policy or handshake identity parameters are "
            "invalid.");

  addRequirement(requirements, "complete_block_validation",
                 "Runtime block chain passes structural and protocol invariant "
                 "validation.",
