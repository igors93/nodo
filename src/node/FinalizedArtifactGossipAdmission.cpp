...
bool FinalizedArtifactGossipAdmissionResult::fatalConsistencyError() const {
    return m_status == FinalizedArtifactGossipAdmissionStatus::PERSISTENCE_FAILED ||
           m_status == FinalizedArtifactGossipAdmissionStatus::REGISTRATION_FAILED;
}
...
FinalizedArtifactGossipAdmissionResult FinalizedArtifactGossipAdmission::admit(
    ...
    if (envelope.networkId() != network.networkName() ||
        ...
        envelope.protocolVersion() != runtime.config().localPeer().protocolVersion()) {
        return FinalizedArtifactGossipAdmissionResult::rejected(
            FinalizedArtifactGossipAdmissionStatus::NETWORK_CONTEXT_MISMATCH,
            "Finalized artifact envelope does not match the local network context."
        );
    }
    ...
    if (blockIndex >= blocks.size()) {
        return FinalizedArtifactGossipAdmissionResult::rejected(
            ...
            "Finalized artifact block is not present in the local canonical chain."
        );
    }
