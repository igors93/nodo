...
const char *noSlashingPreparationDigest() {
  return "NO_SLASHING_PREPARATION_EVIDENCE";
}

const ValidatorNetworkPolicy *
findNetworkPolicy(const std::vector<ValidatorNetworkPolicy> &policies,
...
const ProtectionWorkRecord *
findWorkRecord(const std::vector<ProtectionWorkRecord> &records,
               const std::string &validatorAddress) {
...
utils::Amount
lockedStakeForValidator(const std::vector<LockedStakePosition> &positions,
                        const std::string &validatorAddress) {
  utils::Amount total;
...
std::uint16_t severityForRiskLevel(const std::string &riskLevel) {
  if (riskLevel == "HIGH") {
    return 900;
  }
...
bool shouldRecordRiskEvidence(const ValidatorRiskAssessment &risk,
...
std::string evidenceTypeFor(const ValidatorRiskAssessment &risk,
                            const ValidatorNetworkPolicy *policy) {
  if (policy != nullptr && policy->requiresManualReview()) {
...
std::string evidenceDigest(const ValidatorRiskAssessment &risk,
                           const ValidatorNetworkPolicy *policy,
                           const ProtectionWorkRecord *work) {
...
std::string preparationActionForSeverity(std::uint16_t maxSeverityScore,
...
utils::Amount preparedPenalty(utils::Amount lockedStake,
                              std::uint16_t maxSeverityScore,
                              std::uint64_t slashableEvidenceCount) {
  if (lockedStake.isNegative() || lockedStake.isZero() ||
      slashableEvidenceCount == 0 || maxSeverityScore < 850) {
    return utils::Amount();
  }

  return utils::Amount::fromRawUnits(lockedStake.rawUnits() / 10);
}
...
SlashingEvidenceRecord::SlashingEvidenceRecord(
    ...
    : m_validatorAddress(std::move(validatorAddress)),
      m_blockHeight(blockHeight), m_evidenceType(std::move(evidenceType)),
      m_severityScore(severityScore), m_slashable(slashable),
...
std::string SlashingEvidenceRecord::serialize() const {
  ...
  oss << "SlashingEvidenceRecord{"
      ...
      << ";blockHeight=" << m_blockHeight << ";evidenceType=" << m_evidenceType
      << ";severityScore=" << m_severityScore
      << ";slashable=" << (m_slashable ? "true" : "false")
      << ";recommendedAction=" << m_recommendedAction << ";reason=" << m_reason
      << ";sourceSecurityDigest=" << m_sourceSecurityDigest << "}";
  return oss.str();
}

SlashingPreparationRecord::SlashingPreparationRecord()
    : m_validatorAddress(""), m_blockHeight(0), m_evidenceCount(0),
      m_slashableEvidenceCount(0), m_maxSeverityScore(0),
      m_preparedPenaltyAmount(), m_enforcementAction(""), m_reason(""),
      m_sourceEvidenceDigest("") {}

SlashingPreparationRecord::SlashingPreparationRecord(
    std::string validatorAddress, std::uint64_t blockHeight,
    std::uint64_t evidenceCount, std::uint64_t slashableEvidenceCount,
    std::uint16_t maxSeverityScore, utils::Amount preparedPenaltyAmount,
    std::string enforcementAction, std::string reason,
    std::string sourceEvidenceDigest)
    : m_validatorAddress(std::move(validatorAddress)),
      m_blockHeight(blockHeight), m_evidenceCount(evidenceCount),
...
const std::string &SlashingPreparationRecord::validatorAddress() const {
  return m_validatorAddress;
}
std::uint64_t SlashingPreparationRecord::blockHeight() const {
  return m_blockHeight;
}
std::uint64_t SlashingPreparationRecord::evidenceCount() const {
  return m_evidenceCount;
}
std::uint64_t SlashingPreparationRecord::slashableEvidenceCount() const {
  return m_slashableEvidenceCount;
}
std::uint16_t SlashingPreparationRecord::maxSeverityScore() const {
  return m_maxSeverityScore;
}
utils::Amount SlashingPreparationRecord::preparedPenaltyAmount() const {
  return m_preparedPenaltyAmount;
}
const std::string &SlashingPreparationRecord::enforcementAction() const {
  return m_enforcementAction;
}
const std::string &SlashingPreparationRecord::reason() const {
  return m_reason;
}
const std::string &SlashingPreparationRecord::sourceEvidenceDigest() const {
  return m_sourceEvidenceDigest;
}

bool SlashingPreparationRecord::isValid() const {
  if (m_validatorAddress.empty() || m_blockHeight == 0 ||
      m_evidenceCount == 0 || m_slashableEvidenceCount > m_evidenceCount ||
      m_maxSeverityScore == 0 || m_maxSeverityScore > 1000 ||
      m_preparedPenaltyAmount.isNegative() || m_enforcementAction.empty() ||
...
std::string SlashingPreparationRecord::serialize() const {
  ...
  oss << "SlashingPreparationRecord{"
      ...
      << ";enforcementAction=" << m_enforcementAction << ";reason=" << m_reason
      << ";sourceEvidenceDigest=" << m_sourceEvidenceDigest << "}";
  return oss.str();
}

SlashingEvidenceSummary::SlashingEvidenceSummary()
    : m_status("NOT_EVALUATED"), m_blockHeight(0), m_evidenceCount(0),
      m_slashableEvidenceCount(0), m_maxSeverityScore(0),
...
SlashingEvidenceSummary::SlashingEvidenceSummary(
    std::string status, std::uint64_t blockHeight, std::uint64_t evidenceCount,
    std::uint64_t slashableEvidenceCount, std::uint16_t maxSeverityScore,
    utils::Amount preparedPenaltyTotal, std::string reason,
    std::string sourcePreparationDigest)
    : m_status(std::move(status)), m_blockHeight(blockHeight),
      m_evidenceCount(evidenceCount),
      m_slashableEvidenceCount(slashableEvidenceCount),
      m_maxSeverityScore(maxSeverityScore),
      m_preparedPenaltyTotal(preparedPenaltyTotal), m_reason(std::move(reason)),
...
std::uint64_t SlashingEvidenceSummary::blockHeight() const {
  return m_blockHeight;
}
std::uint64_t SlashingEvidenceSummary::evidenceCount() const {
  return m_evidenceCount;
}
std::uint64_t SlashingEvidenceSummary::slashableEvidenceCount() const {
  return m_slashableEvidenceCount;
}
std::uint16_t SlashingEvidenceSummary::maxSeverityScore() const {
  return m_maxSeverityScore;
}
utils::Amount SlashingEvidenceSummary::preparedPenaltyTotal() const {
  return m_preparedPenaltyTotal;
}
const std::string &SlashingEvidenceSummary::reason() const { return m_reason; }
const std::string &SlashingEvidenceSummary::sourcePreparationDigest() const {
  return m_sourcePreparationDigest;
}
...
bool SlashingEvidenceSummary::isValid() const {
  if (m_status == "NOT_EVALUATED") {
    return m_blockHeight == 0 && m_evidenceCount == 0 &&
           m_slashableEvidenceCount == 0 && m_maxSeverityScore == 0 &&
           m_preparedPenaltyTotal.isZero() &&
           m_reason == SlashingEvidence::NOT_EVALUATED_REASON;
  }

  if (m_status != "ACTIVE" || m_blockHeight == 0 ||
      m_slashableEvidenceCount > m_evidenceCount || m_maxSeverityScore > 1000 ||
  ...
  if (m_evidenceCount == 0) {
    return m_slashableEvidenceCount == 0 && m_maxSeverityScore == 0 &&
           m_preparedPenaltyTotal.isZero() &&
           m_sourcePreparationDigest == noSlashingPreparationDigest();
  }
...
std::string SlashingEvidenceSummary::serialize() const {
  ...
  oss << "SlashingEvidenceSummary{"
      << "status=" << m_status << ";blockHeight=" << m_blockHeight
      << ";evidenceCount=" << m_evidenceCount
      << ";slashableEvidenceCount=" << m_slashableEvidenceCount
      << ";maxSeverityScore=" << m_maxSeverityScore
  ...
  for (const ValidatorRiskAssessment &risk : riskAssessments) {
    if (!risk.isValid()) {
      throw std::invalid_argument(
          "Cannot build slashing evidence from invalid risk assessment.");
    }
    ...
    const std::uint16_t severity =
        std::max(severityForRiskLevel(risk.riskLevel()),
                 work != nullptr && work->totalWorkScore() < 500
                     ? static_cast<std::uint16_t>(500 - work->totalWorkScore())
                     : static_cast<std::uint16_t>(0));

    records.emplace_back(
        risk.validatorAddress(), risk.blockHeight(),
        evidenceTypeFor(risk, policy),
        severity == 0 ? static_cast<std::uint16_t>(1) : severity, false,
        risk.recommendedAction(), RISK_CONTAINMENT_EVIDENCE_REASON,
...
SlashingEvidence::buildPreparationRecords(
  ...
  for (const auto &entry : byValidator) {
    ...
    const utils::Amount penalty = preparedPenalty(
        lockedStakeForValidator(lockedStakePositions, validatorAddress),
        maxSeverity, slashableCount);

    preparations.emplace_back(
        validatorAddress, blockHeight,
        static_cast<std::uint64_t>(records.size()), slashableCount, maxSeverity,
        penalty, preparationActionForSeverity(maxSeverity, slashableCount),
        SLASHING_PREPARATION_REASON, digest.str());
  }
...
SlashingEvidenceSummary SlashingEvidence::buildSummary(
  ...
  if (blockHeight == 0) {
    throw std::invalid_argument(
        "Cannot build slashing evidence summary at genesis height.");
  }
  ...
  for (const SlashingPreparationRecord &record : preparationRecords) {
    if (!record.isValid()) {
      throw std::invalid_argument(
          "Cannot summarize invalid slashing preparation record.");
    }
    preparedPenaltyTotal =
        preparedPenaltyTotal + record.preparedPenaltyAmount();
    digest << record.serialize();
  }
  ...
  if (digest.str().empty()) {
    digest << noSlashingPreparationDigest();
  }

  return SlashingEvidenceSummary(
      "ACTIVE", blockHeight, static_cast<std::uint64_t>(evidenceRecords.size()),
      slashableCount, maxSeverity, preparedPenaltyTotal,
      SLASHING_SUMMARY_REASON, digest.str());
}
...
bool SlashingEvidence::sameSummary(const SlashingEvidenceSummary &left,
