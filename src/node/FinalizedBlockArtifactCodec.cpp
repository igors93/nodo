...
std::string decodeKeyValueSafeSerialization(const std::string &value,
  ...
  const std::string canonical =
      prefix +
      crypto::hexEncode(reinterpret_cast<const unsigned char *>(decoded.data()),
                        decoded.size());
  if (value != canonical) {
    throw std::invalid_argument(
...
std::uint64_t parseU64Strict(const std::string &value,
                             const std::string &fieldName) {
  if (value.empty()) {
  ...
  const std::uint64_t parsed =
      static_cast<std::uint64_t>(std::stoull(value, &parsedSize));
...
std::size_t
parseCollectionCount(const serialization::KeyValueFileDocument &document,
                     const std::string &fieldName) {
  const std::uint64_t parsed =
...
std::uint16_t parseU16Strict(const std::string &value,
                             const std::string &fieldName) {
  const std::uint64_t parsed = parseU64Strict(value, fieldName);

  if (parsed > std::numeric_limits<std::uint16_t>::max()) {
    throw std::invalid_argument("Numeric field exceeds uint16 range: " +
                                fieldName);
  }
...
std::uint32_t parseU32Strict(const std::string &value,
                             const std::string &fieldName) {
  const std::uint64_t parsed = parseU64Strict(value, fieldName);

  if (parsed > std::numeric_limits<std::uint32_t>::max()) {
    throw std::invalid_argument("Numeric field exceeds uint32 range: " +
                                fieldName);
  }
...
std::int64_t parseI64Strict(const std::string &value,
                            const std::string &fieldName) {
  if (value.empty()) {
    throw std::invalid_argument("Empty numeric field: " + fieldName);
  }
  ...
  const std::int64_t parsed =
      static_cast<std::int64_t>(std::stoll(value, &parsedSize));
...
utils::Amount parseAmountStrict(const std::string &value,
                                const std::string &fieldName) {
  return utils::Amount::fromRawUnits(parseI64Strict(value, fieldName));
}

bool parseBoolStrict(const std::string &value, const std::string &fieldName) {
  if (value == "true") {
    return true;
  }
...
RewardDistribution
parseRewardDistribution(const serialization::KeyValueFileDocument &document,
                        std::size_t index) {
  ...
  RewardDistribution distribution(
      ...
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseAmountStrict(document.requireField(prefix + "totalRewardRawUnits"),
                        prefix + "totalRewardRawUnits"),
      parseAmountStrict(document.requireField(prefix + "liquidRewardRawUnits"),
                        prefix + "liquidRewardRawUnits"),
      parseAmountStrict(document.requireField(prefix + "lockedRewardRawUnits"),
                        prefix + "lockedRewardRawUnits"),
      document.requireField(prefix + "reason"));

  if (!distribution.isValid()) {
    throw std::invalid_argument(
        "Finalized block reward distribution is invalid.");
  }
...
LockedStakePosition
parseLockedStakePosition(const serialization::KeyValueFileDocument &document,
                         std::size_t index) {
  ...
  LockedStakePosition position(
      ...
      parseAmountStrict(document.requireField(prefix + "amountRawUnits"),
                        prefix + "amountRawUnits"),
      parseU64Strict(document.requireField(prefix + "createdAtHeight"),
                     prefix + "createdAtHeight"),
      parseU64Strict(document.requireField(prefix + "unlockAtHeight"),
                     prefix + "unlockAtHeight"),
      parseBoolStrict(document.requireField(prefix + "slashable"),
                      prefix + "slashable"),
      document.requireField(prefix + "sourceRewardId"));

  if (!position.isValid()) {
    throw std::invalid_argument(
        "Finalized block locked stake position is invalid.");
  }
...
SecurityScoreRecord
parseSecurityScoreRecord(const serialization::KeyValueFileDocument &document,
                         std::size_t index) {
  ...
  SecurityScoreRecord record(
      ...
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseU16Strict(document.requireField(prefix + "score"), prefix + "score"),
      parseU16Strict(document.requireField(prefix + "lockedStakeScore"),
                     prefix + "lockedStakeScore"),
      parseU16Strict(document.requireField(prefix + "participationScore"),
                     prefix + "participationScore"),
      parseU16Strict(document.requireField(prefix + "maturityScore"),
                     prefix + "maturityScore"),
      parseU16Strict(document.requireField(prefix + "penaltyScore"),
                     prefix + "penaltyScore"),
      document.requireField(prefix + "reason"),
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block security score record is invalid.");
  }

  return record;
}
ValidatorSecurityCheckpoint
parseSecurityCheckpoint(const serialization::KeyValueFileDocument &document,
                        std::size_t index) {
  ...
  ValidatorSecurityCheckpoint checkpoint(
      ...
      parseAmountStrict(document.requireField(prefix + "lockedStakeRawUnits"),
                        prefix + "lockedStakeRawUnits"),
      parseU64Strict(document.requireField(prefix + "securityScoreRecordCount"),
  ...
  if (!checkpoint.isValid()) {
    throw std::invalid_argument(
        "Finalized block security checkpoint is invalid.");
  }

  return checkpoint;
}
ValidatorRiskAssessment parseValidatorRiskAssessment(
    const serialization::KeyValueFileDocument &document, std::size_t index) {
  ...
  ValidatorRiskAssessment assessment(
      ...
      parseAmountStrict(document.requireField(prefix + "lockedStakeRawUnits"),
                        prefix + "lockedStakeRawUnits"),
      parseU16Strict(document.requireField(prefix + "riskScore"),
                     prefix + "riskScore"),
  ...
  if (!assessment.isValid()) {
    throw std::invalid_argument(
        "Finalized block validator risk assessment is invalid.");
  }
...
ValidatorContainmentDecision parseValidatorContainmentDecision(
  ...
  if (!decision.isValid()) {
    throw std::invalid_argument(
        "Finalized block validator containment decision is invalid.");
  }

  return decision;
}
ValidatorNetworkPolicy
parseValidatorNetworkPolicy(const serialization::KeyValueFileDocument &document,
                            std::size_t index) {
  const std::string prefix =
  ...
  ValidatorNetworkPolicy policy(
      ...
      parseBoolStrict(document.requireField(prefix + "requiresManualReview"),
  ...
  if (!policy.isValid()) {
    throw std::invalid_argument(
        "Finalized block validator network policy is invalid.");
  }
...
MonetaryFirewallAudit parseMonetaryFirewallAudit(
    const serialization::KeyValueFileDocument &document) {
  const std::string status = document.requireField("monetaryFirewallStatus");

  const SupplyLedgerSnapshot ledger(
      parseU64Strict(document.requireField("monetary.blockHeight"),
                     "monetary.blockHeight"),
      parseAmountStrict(document.requireField("monetary.supplyBeforeRawUnits"),
                        "monetary.supplyBeforeRawUnits"),
      parseAmountStrict(document.requireField("monetary.mintedRawUnits"),
                        "monetary.mintedRawUnits"),
      parseAmountStrict(document.requireField("monetary.burnedRawUnits"),
                        "monetary.burnedRawUnits"),
      parseAmountStrict(document.requireField("monetary.treasuryDeltaRawUnits"),
                        "monetary.treasuryDeltaRawUnits"),
      parseAmountStrict(document.requireField("monetary.supplyAfterRawUnits"),
  ...
  if (!audit.isValid()) {
    throw std::invalid_argument(
        "Finalized block monetary firewall audit is invalid.");
  }
...
GenesisTreasurySnapshot parseGenesisTreasurySnapshot(
  ...
  GenesisTreasurySnapshot snapshot(
      ...
      parseU64Strict(document.requireField("treasury.blockHeight"),
  ...
  if (!snapshot.isValid()) {
    throw std::invalid_argument(
        "Finalized block genesis treasury snapshot is invalid.");
  }
...
ProtectionRewardBudget parseProtectionRewardBudget(
  ...
  ProtectionRewardBudget budget(
      ...
      parseU64Strict(document.requireField("protectionBudget.blockHeight"),
      ...
      parseU64Strict(document.requireField("protectionBudget.beneficiaryCount"),
  ...
  if (!budget.isValid()) {
    throw std::invalid_argument(
        "Finalized block protection reward budget is invalid.");
  }
...
parseProtectionRewardGrant(const serialization::KeyValueFileDocument &document,
                           std::size_t index) {
  ...
  ProtectionRewardGrant grant(
      ...
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseAmountStrict(document.requireField(prefix + "plannedRewardRawUnits"),
                        prefix + "plannedRewardRawUnits"),
      parseU16Strict(document.requireField(prefix + "securityScore"),
  ...
  if (!grant.isValid()) {
    throw std::invalid_argument(
        "Finalized block protection reward grant is invalid.");
  }
...
InflationEpochSnapshot parseInflationEpochSnapshot(
  ...
  InflationEpochSnapshot snapshot(
      ...
      parseU64Strict(document.requireField("inflationEpoch.blockHeight"),
                     "inflationEpoch.blockHeight"),
      parseU64Strict(document.requireField("inflationEpoch.epochStartBlock"),
                     "inflationEpoch.epochStartBlock"),
      parseU64Strict(document.requireField("inflationEpoch.epochEndBlock"),
  ...
  if (!snapshot.isValid()) {
    throw std::invalid_argument(
        "Finalized block inflation epoch snapshot is invalid.");
  }
...
MintAuthorizationRecord parseMintAuthorizationRecord(
  ...
  MintAuthorizationRecord authorization(
      ...
      parseU64Strict(document.requireField("mintAuthorization.blockHeight"),
      ...
      parseU64Strict(document.requireField("mintAuthorization.activationBlock"),
                     "mintAuthorization.activationBlock"),
      parseU64Strict(document.requireField("mintAuthorization.expirationBlock"),
                     "mintAuthorization.expirationBlock"),
      parseU32Strict(document.requireField(
                         "mintAuthorization.requiredApprovalBasisPoints"),
                     "mintAuthorization.requiredApprovalBasisPoints"),
      parseU64Strict(document.requireField("mintAuthorization.timelockBlocks"),
  ...
  if (!authorization.isValid()) {
    throw std::invalid_argument(
        "Finalized block mint authorization record is invalid.");
  }
...
SupplyExpansionRecord parseSupplyExpansionRecord(
  ...
  SupplyExpansionRecord expansion(
      ...
      parseU64Strict(document.requireField("supplyExpansion.blockHeight"),
  ...
  if (!expansion.isValid()) {
    throw std::invalid_argument(
        "Finalized block supply expansion record is invalid.");
  }
...
FeeEconomicBalance
parseFeeEconomicBalance(const serialization::KeyValueFileDocument &document) {
  FeeEconomicBalance balance(
      document.requireField("feeEconomicBalanceStatus"),
      parseU64Strict(document.requireField("feeBalance.blockHeight"),
                     "feeBalance.blockHeight"),
      parseAmountStrict(document.requireField("feeBalance.totalFeeRawUnits"),
                        "feeBalance.totalFeeRawUnits"),
      parseAmountStrict(
          document.requireField("feeBalance.validatorRewardRawUnits"),
          "feeBalance.validatorRewardRawUnits"),
      parseAmountStrict(document.requireField("feeBalance.treasuryRawUnits"),
                        "feeBalance.treasuryRawUnits"),
      parseAmountStrict(document.requireField("feeBalance.burnRawUnits"),
  ...
  if (!balance.isValid()) {
    throw std::invalid_argument(
        "Finalized block fee economic balance is invalid.");
  }
...
FeeBurnRecord
parseFeeBurnRecord(const serialization::KeyValueFileDocument &document) {
  FeeBurnRecord burn(
      document.requireField("feeBurnStatus"),
      parseU64Strict(document.requireField("feeBurn.blockHeight"),
                     "feeBurn.blockHeight"),
      parseAmountStrict(document.requireField("feeBurn.burnAmountRawUnits"),
                        "feeBurn.burnAmountRawUnits"),
      parseAmountStrict(document.requireField("feeBurn.supplyBeforeRawUnits"),
                        "feeBurn.supplyBeforeRawUnits"),
      parseAmountStrict(document.requireField("feeBurn.supplyAfterRawUnits"),
...
TreasuryFeeRecord
parseTreasuryFeeRecord(const serialization::KeyValueFileDocument &document) {
  TreasuryFeeRecord treasuryFee(
      document.requireField("treasuryFeeStatus"),
      parseU64Strict(document.requireField("treasuryFee.blockHeight"),
  ...
  if (!treasuryFee.isValid()) {
    throw std::invalid_argument(
        "Finalized block treasury fee record is invalid.");
  }
...
parseProtectionWorkRecord(const serialization::KeyValueFileDocument &document,
                          std::size_t index) {
  ...
  ProtectionWorkRecord record(
      ...
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseU16Strict(document.requireField(prefix + "uptimeScore"),
                     prefix + "uptimeScore"),
      parseU16Strict(document.requireField(prefix + "correctVoteScore"),
                     prefix + "correctVoteScore"),
      parseU16Strict(document.requireField(prefix + "attackDetectionScore"),
                     prefix + "attackDetectionScore"),
      parseU16Strict(document.requireField(prefix + "auditContributionScore"),
                     prefix + "auditContributionScore"),
      parseU16Strict(document.requireField(prefix + "securityScore"),
                     prefix + "securityScore"),
      parseU16Strict(document.requireField(prefix + "riskPenaltyScore"),
                     prefix + "riskPenaltyScore"),
      parseU16Strict(document.requireField(prefix + "totalWorkScore"),
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block protection work record is invalid.");
  }
...
ProtectionRewardSummary parseProtectionRewardSummary(
  ...
  ProtectionRewardSummary summary(
      ...
      parseU64Strict(document.requireField("protectionSummary.blockHeight"),
  ...
  if (!summary.isValid()) {
    throw std::invalid_argument(
        "Finalized block protection reward summary is invalid.");
  }
...
ProtectionRewardSettlement parseProtectionRewardSettlement(
  ...
  ProtectionRewardSettlement settlement(
      ...
      parseAmountStrict(document.requireField(prefix + "plannedRewardRawUnits"),
                        prefix + "plannedRewardRawUnits"),
      parseAmountStrict(document.requireField(prefix + "earnedRewardRawUnits"),
                        prefix + "earnedRewardRawUnits"),
      parseAmountStrict(
          document.requireField(prefix + "deferredRewardRawUnits"),
          prefix + "deferredRewardRawUnits"),
      parseU16Strict(document.requireField(prefix + "workScore"),
                     prefix + "workScore"),
      parseU16Strict(document.requireField(prefix + "securityScore"),
                     prefix + "securityScore"),
      document.requireField(prefix + "reason"),
  ...
  if (!settlement.isValid()) {
    throw std::invalid_argument(
        "Finalized block protection reward settlement is invalid.");
  }
...
parseSlashingEvidenceRecord(const serialization::KeyValueFileDocument &document,
                            std::size_t index) {
  ...
  SlashingEvidenceRecord record(
      ...
      parseU16Strict(document.requireField(prefix + "severityScore"),
                     prefix + "severityScore"),
      parseBoolStrict(document.requireField(prefix + "slashable"),
                      prefix + "slashable"),
      document.requireField(prefix + "recommendedAction"),
      document.requireField(prefix + "reason"),
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block slashing evidence record is invalid.");
  }
...
SlashingPreparationRecord parseSlashingPreparationRecord(
    const serialization::KeyValueFileDocument &document, std::size_t index) {
  const std::string prefix =
      "slashingPreparation." + std::to_string(index) + ".";

  SlashingPreparationRecord record(
      document.requireField(prefix + "validatorAddress"),
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseU64Strict(document.requireField(prefix + "evidenceCount"),
                     prefix + "evidenceCount"),
      parseU64Strict(document.requireField(prefix + "slashableEvidenceCount"),
                     prefix + "slashableEvidenceCount"),
      parseU16Strict(document.requireField(prefix + "maxSeverityScore"),
                     prefix + "maxSeverityScore"),
      parseAmountStrict(
          document.requireField(prefix + "preparedPenaltyRawUnits"),
          prefix + "preparedPenaltyRawUnits"),
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block slashing preparation record is invalid.");
  }
...
SlashingEvidenceSummary parseSlashingEvidenceSummary(
  ...
  SlashingEvidenceSummary summary(
      ...
      parseU64Strict(document.requireField("slashingSummary.blockHeight"),
                     "slashingSummary.blockHeight"),
      parseU64Strict(document.requireField("slashingSummary.evidenceCount"),
                     "slashingSummary.evidenceCount"),
      parseU64Strict(
          document.requireField("slashingSummary.slashableEvidenceCount"),
          "slashingSummary.slashableEvidenceCount"),
      parseU16Strict(document.requireField("slashingSummary.maxSeverityScore"),
                     "slashingSummary.maxSeverityScore"),
      parseAmountStrict(
          document.requireField("slashingSummary.preparedPenaltyTotalRawUnits"),
  ...
  if (!summary.isValid()) {
    throw std::invalid_argument(
        "Finalized block slashing evidence summary is invalid.");
  }
...
CryptographicSlashingEvidenceRecord parseCryptographicSlashingEvidenceRecord(
    const serialization::KeyValueFileDocument &document, std::size_t index) {
  const std::string prefix =
      "cryptographicSlashingEvidence." + std::to_string(index) + ".";
  ...
  CryptographicSlashingEvidenceRecord record(
      ...
      parseU16Strict(document.requireField(prefix + "severityScore"),
                     prefix + "severityScore"),
      parseU32Strict(document.requireField(prefix + "penaltyBasisPoints"),
                     prefix + "penaltyBasisPoints"),
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block cryptographic slashing evidence is invalid.");
  }
...
StakePenaltyRecord
parseStakePenaltyRecord(const serialization::KeyValueFileDocument &document,
                        std::size_t index) {
  ...
  StakePenaltyRecord record(
      ...
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseAmountStrict(
          document.requireField(prefix + "lockedStakeBeforeRawUnits"),
          prefix + "lockedStakeBeforeRawUnits"),
      parseAmountStrict(document.requireField(prefix + "penaltyAmountRawUnits"),
                        prefix + "penaltyAmountRawUnits"),
      parseAmountStrict(
          document.requireField(prefix + "lockedStakeAfterRawUnits"),
          prefix + "lockedStakeAfterRawUnits"),
      parseU64Strict(document.requireField(prefix + "evidenceCount"),
                     prefix + "evidenceCount"),
      document.requireField(prefix + "reason"),
      document.requireField(prefix + "sourceEvidenceDigest"));
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block stake penalty record is invalid.");
  }
...
CryptographicSlashingSummary parseCryptographicSlashingSummary(
  ...
  CryptographicSlashingSummary summary(
      ...
      parseU64Strict(
          document.requireField("cryptographicSlashingSummary.blockHeight"),
          "cryptographicSlashingSummary.blockHeight"),
      parseU64Strict(
          document.requireField("cryptographicSlashingSummary.evidenceCount"),
          "cryptographicSlashingSummary.evidenceCount"),
      parseU64Strict(document.requireField(
                         "cryptographicSlashingSummary.slashableEvidenceCount"),
                     "cryptographicSlashingSummary.slashableEvidenceCount"),
      parseU16Strict(document.requireField(
  ...
  if (!summary.isValid()) {
    throw std::invalid_argument(
        "Finalized block cryptographic slashing summary is invalid.");
  }
...
GovernancePolicySnapshot parseGovernancePolicySnapshot(
  ...
  GovernancePolicySnapshot policy(
      ...
      parseU64Strict(document.requireField("governancePolicy.blockHeight"),
                     "governancePolicy.blockHeight"),
      parseU32Strict(
          document.requireField("governancePolicy.requiredApprovalBasisPoints"),
          "governancePolicy.requiredApprovalBasisPoints"),
      parseU64Strict(document.requireField("governancePolicy.timelockBlocks"),
                     "governancePolicy.timelockBlocks"),
      parseU64Strict(
          document.requireField("governancePolicy.activationDelayBlocks"),
  ...
  if (!policy.isValid()) {
    throw std::invalid_argument(
        "Finalized block governance policy is invalid.");
  }
...
parseGovernanceActionGuard(const serialization::KeyValueFileDocument &document,
                           std::size_t index) {
  ...
  GovernanceActionGuard guard(
      ...
      parseU32Strict(
          document.requireField(prefix + "requiredApprovalBasisPoints"),
          prefix + "requiredApprovalBasisPoints"),
      parseU64Strict(document.requireField(prefix + "timelockBlocks"),
                     prefix + "timelockBlocks"),
      document.requireField(prefix + "reason"),
      document.requireField(prefix + "sourcePolicyDigest"));

  if (!guard.isValid()) {
    throw std::invalid_argument(
        "Finalized block governance action guard is invalid.");
  }
...
GovernanceSummary
parseGovernanceSummary(const serialization::KeyValueFileDocument &document) {
  GovernanceSummary summary(
      document.requireField("governanceSummaryStatus"),
      parseU64Strict(document.requireField("governanceSummary.blockHeight"),
                     "governanceSummary.blockHeight"),
      parseU64Strict(document.requireField("governanceSummary.guardCount"),
                     "governanceSummary.guardCount"),
      parseU64Strict(
          document.requireField("governanceSummary.activeProposalCount"),
          "governanceSummary.activeProposalCount"),
      parseU64Strict(
          document.requireField("governanceSummary.approvedProposalCount"),
          "governanceSummary.approvedProposalCount"),
      parseU64Strict(
          document.requireField("governanceSummary.executableProposalCount"),
          "governanceSummary.executableProposalCount"),
      parseU64Strict(
          document.requireField("governanceSummary.executedProposalCount"),
  ...
  if (!summary.isValid()) {
    throw std::invalid_argument(
        "Finalized block governance summary is invalid.");
  }
...
ValidatorLifecycleRecord parseValidatorLifecycleRecord(
    const serialization::KeyValueFileDocument &document, std::size_t index) {
  const std::string prefix =
      "validatorLifecycle." + std::to_string(index) + ".";

  ValidatorLifecycleRecord record(
      document.requireField(prefix + "validatorAddress"),
      parseU64Strict(document.requireField(prefix + "blockHeight"),
                     prefix + "blockHeight"),
      parseU64Strict(document.requireField(prefix + "epochIndex"),
                     prefix + "epochIndex"),
      ...
      parseAmountStrict(document.requireField(prefix + "earnedRewardRawUnits"),
                        prefix + "earnedRewardRawUnits"),
      parseAmountStrict(
          document.requireField(prefix + "slashingPenaltyRawUnits"),
          prefix + "slashingPenaltyRawUnits"),
      parseU16Strict(document.requireField(prefix + "securityScore"),
                     prefix + "securityScore"),
      document.requireField(prefix + "reason"),
      document.requireField(prefix + "sourceDigest"));
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block validator lifecycle record is invalid.");
  }
...
EpochAccountingRecord parseEpochAccountingRecord(
  ...
  EpochAccountingRecord record(
      ...
      parseU64Strict(document.requireField("epochAccounting.blockHeight"),
                     "epochAccounting.blockHeight"),
      parseU64Strict(document.requireField("epochAccounting.epochIndex"),
                     "epochAccounting.epochIndex"),
      parseU64Strict(document.requireField("epochAccounting.epochStartBlock"),
                     "epochAccounting.epochStartBlock"),
      parseU64Strict(document.requireField("epochAccounting.epochEndBlock"),
                     "epochAccounting.epochEndBlock"),
      parseU64Strict(document.requireField("epochAccounting.validatorCount"),
                     "epochAccounting.validatorCount"),
      parseU64Strict(
          document.requireField("epochAccounting.activeValidatorCount"),
          "epochAccounting.activeValidatorCount"),
      parseAmountStrict(
          document.requireField("epochAccounting.totalLockedStakeRawUnits"),
          "epochAccounting.totalLockedStakeRawUnits"),
      parseAmountStrict(
          document.requireField("epochAccounting.totalEarnedRewardsRawUnits"),
          "epochAccounting.totalEarnedRewardsRawUnits"),
      parseAmountStrict(document.requireField(
  ...
  if (!record.isValid()) {
    throw std::invalid_argument(
        "Finalized block epoch accounting record is invalid.");
  }
...
ValidatorLifecycleSummary parseValidatorLifecycleSummary(
  ...
  ValidatorLifecycleSummary summary(
      ...
      parseU64Strict(
          document.requireField("validatorLifecycleSummary.blockHeight"),
          "validatorLifecycleSummary.blockHeight"),
      parseU64Strict(
          document.requireField("validatorLifecycleSummary.epochIndex"),
  ...
  if (!summary.isValid()) {
    throw std::invalid_argument(
        "Finalized block validator lifecycle summary is invalid.");
  }
...
core::Block
FinalizedBlockArtifactCodec::readBlockFile(const std::filesystem::path &path) {
...
FinalizedBlockArtifact FinalizedBlockArtifactCodec::readBlockArtifactFile(
  ...
  return decodeBlockArtifactFileContents(readTextFile(path));
}

FinalizedBlockArtifact
FinalizedBlockArtifactCodec::decodeBlockArtifactFileContents(
    const std::string &contents) {
  const serialization::KeyValueFileDocument document =
      serialization::KeyValueFileCodec::parse(
          contents, FinalizedArtifactSchema::currentSchemaId());

  const std::size_t recordCount = parseCollectionCount(document, "recordCount");
  const std::size_t rewardDistributionCount =
      parseCollectionCount(document, "rewardDistributionCount");
  const std::size_t lockedStakePositionCount =
      parseCollectionCount(document, "lockedStakePositionCount");
  const std::size_t securityScoreRecordCount =
      parseCollectionCount(document, "securityScoreRecordCount");
  const std::size_t securityCheckpointCount =
      parseCollectionCount(document, "securityCheckpointCount");
  const std::size_t validatorRiskAssessmentCount =
      parseCollectionCount(document, "validatorRiskAssessmentCount");
  const std::size_t validatorContainmentDecisionCount =
      parseCollectionCount(document, "validatorContainmentDecisionCount");
  const std::size_t validatorNetworkPolicyCount =
      parseCollectionCount(document, "validatorNetworkPolicyCount");
  const std::size_t protectionRewardGrantCount =
      parseCollectionCount(document, "protectionRewardGrantCount");
  const std::size_t protectionWorkRecordCount =
      parseCollectionCount(document, "protectionWorkRecordCount");
  const std::size_t protectionRewardSettlementCount =
      parseCollectionCount(document, "protectionRewardSettlementCount");
  const std::size_t slashingEvidenceRecordCount =
      parseCollectionCount(document, "slashingEvidenceRecordCount");
  const std::size_t slashingPreparationRecordCount =
      parseCollectionCount(document, "slashingPreparationRecordCount");
  const std::size_t cryptographicSlashingEvidenceCount =
      parseCollectionCount(document, "cryptographicSlashingEvidenceCount");
  const std::size_t stakePenaltyRecordCount =
      parseCollectionCount(document, "stakePenaltyRecordCount");
  const std::size_t governanceActionGuardCount =
      parseCollectionCount(document, "governanceActionGuardCount");
  const std::size_t validatorLifecycleRecordCount =
      parseCollectionCount(document, "validatorLifecycleRecordCount");
  ...
  const std::size_t treasurySpendCount =
      FinalizedTreasurySectionCodec::evidenceCountFromDocument(document);

  if (recordCount > core::Block::MAX_RECORDS) {
    throw std::invalid_argument(
        "Artifact record count exceeds block protocol limit.");
  }
  validateTotalCollectionCount(
      document, std::array<std::size_t, 20>{recordCount,
  ...
  FinalizedMonetarySectionCodec::addAllowedFields(
      allowedFields, supplyDeltaMintRecordCount, supplyDeltaBurnRecordCount);

  FinalizedTreasurySectionCodec::addAllowedFields(document, allowedFields,
                                                  treasurySpendCount);
  ...
  for (std::size_t index = 0; index < validatorContainmentDecisionCount;
       ++index) {
    const std::string prefix =
        "validatorContainment." + std::to_string(index) + ".";
    allowedFields.insert(prefix + "validatorAddress");
  ...
  for (std::size_t index = 0; index < protectionRewardSettlementCount;
       ++index) {
    const std::string prefix =
        "protectionSettlement." + std::to_string(index) + ".";
    allowedFields.insert(prefix + "validatorAddress");
  ...
  for (std::size_t index = 0; index < slashingEvidenceRecordCount; ++index) {
    const std::string prefix =
        "slashingEvidence." + std::to_string(index) + ".";
    allowedFields.insert(prefix + "validatorAddress");
    allowedFields.insert(prefix + "blockHeight");
  ...
  for (std::size_t index = 0; index < cryptographicSlashingEvidenceCount;
       ++index) {
    const std::string prefix =
        "cryptographicSlashingEvidence." + std::to_string(index) + ".";
    allowedFields.insert(prefix + "validatorAddress");
  ...
  const std::uint64_t index = static_cast<std::uint64_t>(
      parseU64Strict(document.requireField("blockIndex"), "blockIndex"));
  ...
  const utils::Amount totalFee = parseAmountStrict(
      document.requireField("totalFeeRawUnits"), "totalFeeRawUnits");

  std::vector<RewardDistribution> rewardDistributions;
  rewardDistributions.reserve(rewardDistributionCount);
  for (std::size_t rewardIndex = 0; rewardIndex < rewardDistributionCount;
       ++rewardIndex) {
    rewardDistributions.push_back(
        parseRewardDistribution(document, rewardIndex));
  }

  std::vector<LockedStakePosition> lockedStakePositions;
  lockedStakePositions.reserve(lockedStakePositionCount);
  for (std::size_t positionIndex = 0; positionIndex < lockedStakePositionCount;
       ++positionIndex) {
    lockedStakePositions.push_back(
        parseLockedStakePosition(document, positionIndex));
  }

  std::vector<SecurityScoreRecord> securityScoreRecords;
  securityScoreRecords.reserve(securityScoreRecordCount);
  for (std::size_t scoreIndex = 0; scoreIndex < securityScoreRecordCount;
       ++scoreIndex) {
    securityScoreRecords.push_back(
        parseSecurityScoreRecord(document, scoreIndex));
  }

  std::vector<ValidatorSecurityCheckpoint> securityCheckpoints;
  securityCheckpoints.reserve(securityCheckpointCount);
  for (std::size_t checkpointIndex = 0;
       checkpointIndex < securityCheckpointCount; ++checkpointIndex) {
    securityCheckpoints.push_back(
        parseSecurityCheckpoint(document, checkpointIndex));
  }

  std::vector<ValidatorRiskAssessment> validatorRiskAssessments;
  validatorRiskAssessments.reserve(validatorRiskAssessmentCount);
  for (std::size_t assessmentIndex = 0;
       assessmentIndex < validatorRiskAssessmentCount; ++assessmentIndex) {
    validatorRiskAssessments.push_back(
        parseValidatorRiskAssessment(document, assessmentIndex));
  }

  std::vector<ValidatorContainmentDecision> validatorContainmentDecisions;
  validatorContainmentDecisions.reserve(validatorContainmentDecisionCount);
  for (std::size_t decisionIndex = 0;
       decisionIndex < validatorContainmentDecisionCount; ++decisionIndex) {
    validatorContainmentDecisions.push_back(
        parseValidatorContainmentDecision(document, decisionIndex));
  }

  std::vector<ValidatorNetworkPolicy> validatorNetworkPolicies;
  validatorNetworkPolicies.reserve(validatorNetworkPolicyCount);
  for (std::size_t policyIndex = 0; policyIndex < validatorNetworkPolicyCount;
       ++policyIndex) {
    validatorNetworkPolicies.push_back(
        parseValidatorNetworkPolicy(document, policyIndex));
  }
  ...
  for (std::size_t grantIndex = 0; grantIndex < protectionRewardGrantCount;
       ++grantIndex) {
    protectionRewardGrants.push_back(
        parseProtectionRewardGrant(document, grantIndex));
  }

  std::vector<ProtectionWorkRecord> protectionWorkRecords;
  protectionWorkRecords.reserve(protectionWorkRecordCount);
  for (std::size_t workIndex = 0; workIndex < protectionWorkRecordCount;
       ++workIndex) {
    protectionWorkRecords.push_back(
        parseProtectionWorkRecord(document, workIndex));
  }
  ...
  for (std::size_t settlementIndex = 0;
       settlementIndex < protectionRewardSettlementCount; ++settlementIndex) {
    protectionRewardSettlements.push_back(
        parseProtectionRewardSettlement(document, settlementIndex));
  }
  ...
  const FeeBurnRecord feeBurnRecord = parseFeeBurnRecord(document);

  const TreasuryFeeRecord treasuryFeeRecord = parseTreasuryFeeRecord(document);

  std::vector<SlashingEvidenceRecord> slashingEvidenceRecords;
  slashingEvidenceRecords.reserve(slashingEvidenceRecordCount);
  for (std::size_t evidenceIndex = 0;
       evidenceIndex < slashingEvidenceRecordCount; ++evidenceIndex) {
    slashingEvidenceRecords.push_back(
        parseSlashingEvidenceRecord(document, evidenceIndex));
  }

  std::vector<SlashingPreparationRecord> slashingPreparationRecords;
  slashingPreparationRecords.reserve(slashingPreparationRecordCount);
  for (std::size_t preparationIndex = 0;
       preparationIndex < slashingPreparationRecordCount; ++preparationIndex) {
    slashingPreparationRecords.push_back(
        parseSlashingPreparationRecord(document, preparationIndex));
  }
  ...
  std::vector<CryptographicSlashingEvidenceRecord>
      cryptographicSlashingEvidenceRecords;
  cryptographicSlashingEvidenceRecords.reserve(
      cryptographicSlashingEvidenceCount);
  for (std::size_t evidenceIndex = 0;
       evidenceIndex < cryptographicSlashingEvidenceCount; ++evidenceIndex) {
    cryptographicSlashingEvidenceRecords.push_back(
        parseCryptographicSlashingEvidenceRecord(document, evidenceIndex));
  }

  std::vector<StakePenaltyRecord> stakePenaltyRecords;
  stakePenaltyRecords.reserve(stakePenaltyRecordCount);
  for (std::size_t penaltyIndex = 0; penaltyIndex < stakePenaltyRecordCount;
       ++penaltyIndex) {
    stakePenaltyRecords.push_back(
        parseStakePenaltyRecord(document, penaltyIndex));
  }
  ...
  for (std::size_t guardIndex = 0; guardIndex < governanceActionGuardCount;
       ++guardIndex) {
    governanceActionGuards.push_back(
        parseGovernanceActionGuard(document, guardIndex));
  }

  const GovernanceSummary governanceSummary = parseGovernanceSummary(document);

  std::vector<ValidatorLifecycleRecord> validatorLifecycleRecords;
  validatorLifecycleRecords.reserve(validatorLifecycleRecordCount);
  for (std::size_t lifecycleIndex = 0;
       lifecycleIndex < validatorLifecycleRecordCount; ++lifecycleIndex) {
    validatorLifecycleRecords.push_back(
        parseValidatorLifecycleRecord(document, lifecycleIndex));
  }
  ...
  const std::int64_t timestamp =
      parseI64Strict(document.requireField("timestamp"), "timestamp");
  ...
  economics::SupplyDelta parsedSupplyDelta =
      FinalizedMonetarySectionCodec::decodeSupplyDelta(document, block.index(),
                                                       block.hash());

  FinalizedTreasurySection parsedTreasurySection =
      FinalizedTreasurySectionCodec::decodeFromDocument(document,
                                                        treasurySpendCount);
  ...
  if (block.index() != index || block.hash() != blockHash ||
      block.previousHash() != previousHash || block.timestamp() != timestamp ||
      block.records().size() != recordCount) {
    throw std::invalid_argument(
        "Finalized block file header does not match block payload.");
  }
  ...
  for (std::size_t recordIndex = 0; recordIndex < recordCount; ++recordIndex) {
    ...
    if (block.records()[recordIndex].serialize() !=
      ...
      throw std::invalid_argument(
          "Finalized block record line does not match block payload.");
    }
  }

  if (!consensus::BlockFinalizer::certificateMatchesBlock(block,
                                                          quorumCertificate)) {
    throw std::invalid_argument(
        "Finalized block quorum certificate does not match block payload.");
  }

  if (!finalizedRecord.matchesBlock(block) ||
      finalizedRecord.quorumCertificate().serialize() !=
          quorumCertificate.serialize()) {
    throw std::invalid_argument(
        "Finalized block record does not match block or quorum certificate.");
  }

  if (RewardDistributionCalculator::totalReward(rewardDistributions) !=
      feeEconomicBalance.validatorRewardAmount()) {
    throw std::invalid_argument("Finalized block reward distributions do not "
                                "match validator fee allocation.");
  }
  ...
  if (!LockedStakePositionBuilder::samePositions(
    ...
    throw std::invalid_argument("Finalized block locked stake positions do not "
                                "match reward distributions.");
  }
  ...
  if (!SecurityScoreCalculator::sameRecords(
    ...
    throw std::invalid_argument("Finalized block security score records do not "
                                "match locked stake positions.");
  }
  ...
  if (!ValidatorSecurityCheckpointBuilder::sameCheckpoints(
          ...
          securityCheckpoints)) {
    throw std::invalid_argument("Finalized block security checkpoints do not "
                                "match security score records.");
  }
  ...
  if (!ValidatorRiskAssessmentBuilder::sameAssessments(
    ...
    throw std::invalid_argument("Finalized block validator risk assessments do "
                                "not match security checkpoints.");
  }
  ...
  if (!ValidatorContainmentDecisionBuilder::sameDecisions(
    ...
    throw std::invalid_argument("Finalized block validator containment "
                                "decisions do not match risk assessments.");
  }
  ...
  if (!ValidatorNetworkPolicyBuilder::samePolicies(
    ...
    throw std::invalid_argument("Finalized block validator network policies do "
                                "not match containment decisions.");
  }

  if (!monetaryFirewallAudit.passed()) {
    throw std::invalid_argument(
        "Finalized block monetary firewall audit did not pass.");
  }

  if (!genesisTreasurySnapshot.active() || !protectionRewardBudget.active()) {
    throw std::invalid_argument(
        "Finalized block protection treasury plan is inactive.");
  }
  ...
  if (!ProtectionTreasury::sameBudget(
    ...
    throw std::invalid_argument("Finalized block protection reward budget does "
                                "not match treasury snapshot.");
  }

  if (!ProtectionTreasury::sameGrants(
          ProtectionTreasury::buildProtectionRewardGrants(
              protectionRewardBudget, rewardDistributions,
              securityScoreRecords),
          protectionRewardGrants)) {
    throw std::invalid_argument("Finalized block protection reward grants do "
                                "not match protection budget.");
  }

  if (!ProtectionRewards::sameWorkRecords(
          ProtectionRewards::buildWorkRecords(
              protectionRewardGrants, securityScoreRecords,
              validatorRiskAssessments, validatorNetworkPolicies),
          protectionWorkRecords)) {
    throw std::invalid_argument("Finalized block protection work records do "
                                "not match security context.");
  }

  if (!ProtectionRewards::sameSettlements(
          ProtectionRewards::buildSettlements(protectionRewardGrants,
                                              protectionWorkRecords),
          protectionRewardSettlements)) {
    throw std::invalid_argument("Finalized block protection reward settlements "
                                "do not match work records.");
  }

  if (!ProtectionRewards::sameSummary(
          ProtectionRewards::buildSummary(protectionRewardBudget,
                                          protectionRewardSettlements),
          protectionRewardSummary)) {
    throw std::invalid_argument("Finalized block protection reward summary "
                                "does not match settlements.");
  }

  const bool issuanceMatches =
      mintAuthorizationRecord.status() == "ACTIVE"
          ? ControlledIssuance::sameAuthorization(
                ControlledIssuance::buildEpochRewardAuthorization(
  ...
  if (!inflationEpochSnapshot.active() || !mintAuthorizationRecord.isValid() ||
    ...
    throw std::invalid_argument(
        "Finalized block controlled issuance records are invalid.");
  }

  if (!FeeEconomics::sameBalance(
          FeeEconomics::buildFeeEconomicBalance(
              feeEconomicBalance.blockHeight(), totalFee),
          feeEconomicBalance)) {
    throw std::invalid_argument(
        "Finalized block fee economic balance does not match total fee.");
  }

  if (RewardDistributionCalculator::totalReward(rewardDistributions) !=
      feeEconomicBalance.validatorRewardAmount()) {
    throw std::invalid_argument(
        "Finalized block validator rewards do not match fee split.");
  }

  if (!FeeEconomics::sameBurn(
          FeeEconomics::buildFeeBurnRecord(feeEconomicBalance,
                                           feeBurnRecord.supplyBefore()),
          feeBurnRecord)) {
    throw std::invalid_argument(
        "Finalized block fee burn record does not match fee split.");
  }
  ...
  if (!FeeEconomics::sameTreasuryFee(
          ...
          treasuryFeeRecord)) {
    throw std::invalid_argument(
        "Finalized block treasury fee record does not match fee split.");
  }

  if (feeBurnRecord.burnAmount() >
          monetaryFirewallAudit.supplyLedger().burned() ||
      feeBurnRecord.supplyBefore() !=
          monetaryFirewallAudit.supplyLedger().supplyBefore() ||
      treasuryFeeRecord.treasuryAmount() !=
          monetaryFirewallAudit.supplyLedger().treasuryDelta()) {
    throw std::invalid_argument(
        "Finalized block fee records do not match monetary firewall audit.");
  }

  if (parsedSupplyDelta.blockHeight() != block.index() ||
      parsedSupplyDelta.blockHash() != block.hash() ||
      parsedSupplyDelta.supplyBefore() !=
          monetaryFirewallAudit.supplyLedger().supplyBefore() ||
      parsedSupplyDelta.mintedAmount() !=
          monetaryFirewallAudit.supplyLedger().minted() ||
      parsedSupplyDelta.burnedAmount() !=
          monetaryFirewallAudit.supplyLedger().burned() ||
      parsedSupplyDelta.supplyAfter() !=
          monetaryFirewallAudit.supplyLedger().supplyAfter()) {
    throw std::invalid_argument(
        "Finalized block SupplyDelta does not match monetary firewall audit.");
  }

  if (parsedSupplyDelta.burnedAmount() < feeBurnRecord.burnAmount() ||
      parsedSupplyDelta.mintedAmount() !=
          supplyExpansionRecord.mintedAmount()) {
    throw std::invalid_argument(
        "Finalized block SupplyDelta does not match legacy monetary records.");
  }

  const std::vector<SlashingEvidenceRecord> expectedSlashingEvidence =
      SlashingEvidence::buildEvidenceRecords(validatorRiskAssessments,
                                             validatorNetworkPolicies,
                                             protectionWorkRecords);

  if (!SlashingEvidence::sameEvidenceRecords(expectedSlashingEvidence,
                                             slashingEvidenceRecords)) {
    throw std::invalid_argument("Finalized block slashing evidence records do "
                                "not match rebuilt security evidence.");
  }

  const std::vector<SlashingPreparationRecord> expectedSlashingPreparation =
      SlashingEvidence::buildPreparationRecords(expectedSlashingEvidence,
                                                lockedStakePositions);

  if (!SlashingEvidence::samePreparationRecords(expectedSlashingPreparation,
                                                slashingPreparationRecords)) {
    throw std::invalid_argument("Finalized block slashing preparation records "
                                "do not match rebuilt evidence.");
  }

  const SlashingEvidenceSummary expectedSlashingSummary =
      SlashingEvidence::buildSummary(block.index(), expectedSlashingEvidence,
                                     expectedSlashingPreparation);

  if (!SlashingEvidence::sameSummary(expectedSlashingSummary,
                                     slashingEvidenceSummary)) {
    throw std::invalid_argument("Finalized block slashing evidence summary "
                                "does not match rebuilt evidence.");
  }
  ...
  if (!CryptographicSlashing::sameEvidenceRecords(
          expectedCryptographicEvidence,
          cryptographicSlashingEvidenceRecords)) {
    throw std::invalid_argument(
        "Finalized block cryptographic slashing evidence does not match "
        "rebuilt vote evidence.");
  }

  const std::vector<StakePenaltyRecord> expectedStakePenalties =
      CryptographicSlashing::buildStakePenaltyRecords(
          expectedCryptographicEvidence, lockedStakePositions);

  if (!CryptographicSlashing::sameStakePenaltyRecords(expectedStakePenalties,
                                                      stakePenaltyRecords)) {
    throw std::invalid_argument("Finalized block stake penalty records do not "
                                "match rebuilt cryptographic evidence.");
  }

  const CryptographicSlashingSummary expectedCryptographicSlashingSummary =
      CryptographicSlashing::buildSummary(
          block.index(), expectedCryptographicEvidence, expectedStakePenalties);

  if (!CryptographicSlashing::sameSummary(expectedCryptographicSlashingSummary,
                                          cryptographicSlashingSummary)) {
    throw std::invalid_argument("Finalized block cryptographic slashing "
                                "summary does not match rebuilt evidence.");
  }
  ...
  std::vector<std::pair<std::string, std::string>> canonicalFields = {
      ...
      {"validatorRiskAssessmentCount",
       std::to_string(validatorRiskAssessments.size())},
      {"validatorContainmentDecisionCount",
       std::to_string(validatorContainmentDecisions.size())},
      {"validatorNetworkPolicyCount",
       std::to_string(validatorNetworkPolicies.size())},
      ...
      {"protectionRewardGrantCount",
       std::to_string(protectionRewardGrants.size())},
      {"protectionWorkRecordCount",
       std::to_string(protectionWorkRecords.size())},
      {"protectionRewardSummaryStatus", protectionRewardSummary.status()},
      {"protectionRewardSettlementCount",
       std::to_string(protectionRewardSettlements.size())},
      ...
      {"slashingEvidenceRecordCount",
       std::to_string(slashingEvidenceRecords.size())},
      {"slashingPreparationRecordCount",
       std::to_string(slashingPreparationRecords.size())},
      {"slashingEvidenceSummaryStatus", slashingEvidenceSummary.status()},
      {"cryptographicSlashingEvidenceCount",
       std::to_string(cryptographicSlashingEvidenceRecords.size())},
      {"stakePenaltyRecordCount", std::to_string(stakePenaltyRecords.size())},
      {"cryptographicSlashingSummaryStatus",
       cryptographicSlashingSummary.status()},
      {"governancePolicyStatus", governancePolicySnapshot.status()},
      {"governanceActionGuardCount",
       std::to_string(governanceActionGuards.size())},
      {"governanceSummaryStatus", governanceSummary.status()},
      {"validatorLifecycleRecordCount",
       std::to_string(validatorLifecycleRecords.size())},
  ...
  for (std::size_t rewardIndex = 0; rewardIndex < rewardDistributions.size();
       ++rewardIndex) {
    const std::string prefix = "reward." + std::to_string(rewardIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        rewardDistributions[rewardIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(rewardDistributions[rewardIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "totalRewardRawUnits",
        std::to_string(
            rewardDistributions[rewardIndex].totalReward().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "liquidRewardRawUnits",
        std::to_string(
            rewardDistributions[rewardIndex].liquidReward().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "lockedRewardRawUnits",
        std::to_string(
            rewardDistributions[rewardIndex].lockedReward().rawUnits()));
    canonicalFields.emplace_back(prefix + "reason",
                                 rewardDistributions[rewardIndex].reason());
  }

  for (std::size_t positionIndex = 0;
       positionIndex < lockedStakePositions.size(); ++positionIndex) {
    const std::string prefix =
        "lockedStake." + std::to_string(positionIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "ownerAddress",
        lockedStakePositions[positionIndex].ownerAddress());
    canonicalFields.emplace_back(
        prefix + "amountRawUnits",
        std::to_string(
            lockedStakePositions[positionIndex].amount().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "createdAtHeight",
        std::to_string(lockedStakePositions[positionIndex].createdAtHeight()));
    canonicalFields.emplace_back(
        prefix + "unlockAtHeight",
        std::to_string(lockedStakePositions[positionIndex].unlockAtHeight()));
    canonicalFields.emplace_back(
        prefix + "slashable",
        lockedStakePositions[positionIndex].slashable() ? "true" : "false");
    canonicalFields.emplace_back(
        prefix + "sourceRewardId",
        lockedStakePositions[positionIndex].sourceRewardId());
  }

  for (std::size_t scoreIndex = 0; scoreIndex < securityScoreRecords.size();
       ++scoreIndex) {
    const std::string prefix =
        "securityScore." + std::to_string(scoreIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        securityScoreRecords[scoreIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(securityScoreRecords[scoreIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "score",
        std::to_string(securityScoreRecords[scoreIndex].score()));
    canonicalFields.emplace_back(
        prefix + "lockedStakeScore",
        std::to_string(securityScoreRecords[scoreIndex].lockedStakeScore()));
    canonicalFields.emplace_back(
        prefix + "participationScore",
        std::to_string(securityScoreRecords[scoreIndex].participationScore()));
    canonicalFields.emplace_back(
        prefix + "maturityScore",
        std::to_string(securityScoreRecords[scoreIndex].maturityScore()));
    canonicalFields.emplace_back(
        prefix + "penaltyScore",
        std::to_string(securityScoreRecords[scoreIndex].penaltyScore()));
    canonicalFields.emplace_back(prefix + "reason",
                                 securityScoreRecords[scoreIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceLockedStakeId",
        securityScoreRecords[scoreIndex].sourceLockedStakeId());
  }

  for (std::size_t checkpointIndex = 0;
       checkpointIndex < securityCheckpoints.size(); ++checkpointIndex) {
    const std::string prefix =
        "securityCheckpoint." + std::to_string(checkpointIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        securityCheckpoints[checkpointIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(securityCheckpoints[checkpointIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "score",
        std::to_string(securityCheckpoints[checkpointIndex].score()));
    canonicalFields.emplace_back(prefix + "band",
                                 securityCheckpoints[checkpointIndex].band());
    canonicalFields.emplace_back(
        prefix + "lockedStakeRawUnits",
        std::to_string(
            securityCheckpoints[checkpointIndex].lockedStake().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "securityScoreRecordCount",
        std::to_string(
            securityCheckpoints[checkpointIndex].securityScoreRecordCount()));
    canonicalFields.emplace_back(prefix + "reason",
                                 securityCheckpoints[checkpointIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceDigest",
        securityCheckpoints[checkpointIndex].sourceDigest());
  }

  for (std::size_t assessmentIndex = 0;
       assessmentIndex < validatorRiskAssessments.size(); ++assessmentIndex) {
    const std::string prefix =
        "validatorRisk." + std::to_string(assessmentIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        validatorRiskAssessments[assessmentIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            validatorRiskAssessments[assessmentIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "score",
        std::to_string(validatorRiskAssessments[assessmentIndex].score()));
    canonicalFields.emplace_back(
        prefix + "band", validatorRiskAssessments[assessmentIndex].band());
    canonicalFields.emplace_back(
        prefix + "lockedStakeRawUnits",
        std::to_string(validatorRiskAssessments[assessmentIndex]
                           .lockedStake()
                           .rawUnits()));
    canonicalFields.emplace_back(
        prefix + "riskScore",
        std::to_string(validatorRiskAssessments[assessmentIndex].riskScore()));
    canonicalFields.emplace_back(
        prefix + "riskLevel",
        validatorRiskAssessments[assessmentIndex].riskLevel());
    canonicalFields.emplace_back(
        prefix + "recommendedAction",
        validatorRiskAssessments[assessmentIndex].recommendedAction());
    canonicalFields.emplace_back(
        prefix + "reason", validatorRiskAssessments[assessmentIndex].reason());
    canonicalFields.emplace_back(
        prefix + "checkpointDigest",
        validatorRiskAssessments[assessmentIndex].checkpointDigest());
  }

  for (std::size_t decisionIndex = 0;
       decisionIndex < validatorContainmentDecisions.size(); ++decisionIndex) {
    const std::string prefix =
        "validatorContainment." + std::to_string(decisionIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        validatorContainmentDecisions[decisionIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            validatorContainmentDecisions[decisionIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "riskLevel",
        validatorContainmentDecisions[decisionIndex].riskLevel());
    canonicalFields.emplace_back(
        prefix + "recommendedAction",
        validatorContainmentDecisions[decisionIndex].recommendedAction());
    canonicalFields.emplace_back(
        prefix + "containmentMode",
        validatorContainmentDecisions[decisionIndex].containmentMode());
    canonicalFields.emplace_back(
        prefix + "peerTrustState",
        validatorContainmentDecisions[decisionIndex].peerTrustState());
    canonicalFields.emplace_back(
        prefix + "networkAdmissionState",
        validatorContainmentDecisions[decisionIndex].networkAdmissionState());
    canonicalFields.emplace_back(
        prefix + "reason",
        validatorContainmentDecisions[decisionIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceRiskDigest",
        validatorContainmentDecisions[decisionIndex].sourceRiskDigest());
  }

  for (std::size_t policyIndex = 0;
       policyIndex < validatorNetworkPolicies.size(); ++policyIndex) {
    const std::string prefix =
        "validatorNetworkPolicy." + std::to_string(policyIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        validatorNetworkPolicies[policyIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(validatorNetworkPolicies[policyIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "containmentMode",
        validatorNetworkPolicies[policyIndex].containmentMode());
    canonicalFields.emplace_back(
        prefix + "peerTrustState",
        validatorNetworkPolicies[policyIndex].peerTrustState());
    canonicalFields.emplace_back(
        prefix + "networkAdmissionState",
        validatorNetworkPolicies[policyIndex].networkAdmissionState());
    canonicalFields.emplace_back(
        prefix + "connectionPolicy",
        validatorNetworkPolicies[policyIndex].connectionPolicy());
    canonicalFields.emplace_back(
        prefix + "messagePolicy",
        validatorNetworkPolicies[policyIndex].messagePolicy());
    canonicalFields.emplace_back(
        prefix + "consensusPolicy",
        validatorNetworkPolicies[policyIndex].consensusPolicy());
    canonicalFields.emplace_back(
        prefix + "requiresManualReview",
        validatorNetworkPolicies[policyIndex].requiresManualReview() ? "true"
                                                                     : "false");
    canonicalFields.emplace_back(
        prefix + "reason", validatorNetworkPolicies[policyIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceContainmentDigest",
        validatorNetworkPolicies[policyIndex].sourceContainmentDigest());
  }

  canonicalFields.emplace_back(
      "monetary.blockHeight",
      std::to_string(monetaryFirewallAudit.supplyLedger().blockHeight()));
  canonicalFields.emplace_back(
      "monetary.supplyBeforeRawUnits",
      std::to_string(
          monetaryFirewallAudit.supplyLedger().supplyBefore().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.mintedRawUnits",
      std::to_string(monetaryFirewallAudit.supplyLedger().minted().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.burnedRawUnits",
      std::to_string(monetaryFirewallAudit.supplyLedger().burned().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.treasuryDeltaRawUnits",
      std::to_string(
          monetaryFirewallAudit.supplyLedger().treasuryDelta().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.supplyAfterRawUnits",
      std::to_string(
          monetaryFirewallAudit.supplyLedger().supplyAfter().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.annualMintLimitRawUnits",
      std::to_string(monetaryFirewallAudit.annualMintLimit().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.annualMintUsedBeforeRawUnits",
      std::to_string(monetaryFirewallAudit.annualMintUsedBefore().rawUnits()));
  canonicalFields.emplace_back(
      "monetary.annualMintUsedAfterRawUnits",
      std::to_string(monetaryFirewallAudit.annualMintUsedAfter().rawUnits()));
  canonicalFields.emplace_back("monetary.policyId",
                               monetaryFirewallAudit.policyId());
  canonicalFields.emplace_back("monetary.reason",
                               monetaryFirewallAudit.reason());

  canonicalFields.emplace_back("treasury.treasuryAddress",
                               genesisTreasurySnapshot.treasuryAddress());
  canonicalFields.emplace_back(
      "treasury.blockHeight",
      std::to_string(genesisTreasurySnapshot.blockHeight()));
  canonicalFields.emplace_back(
      "treasury.genesisTreasuryBalanceRawUnits",
      std::to_string(
          genesisTreasurySnapshot.genesisTreasuryBalance().rawUnits()));
  canonicalFields.emplace_back(
      "treasury.protectedReserveRawUnits",
      std::to_string(genesisTreasurySnapshot.protectedReserve().rawUnits()));
  canonicalFields.emplace_back(
      "treasury.protectionBudgetRawUnits",
      std::to_string(genesisTreasurySnapshot.protectionBudget().rawUnits()));
  canonicalFields.emplace_back(
      "treasury.availableBalanceRawUnits",
      std::to_string(genesisTreasurySnapshot.availableBalance().rawUnits()));
  canonicalFields.emplace_back("treasury.reason",
                               genesisTreasurySnapshot.reason());

  canonicalFields.emplace_back(
      "protectionBudget.blockHeight",
      std::to_string(protectionRewardBudget.blockHeight()));
  canonicalFields.emplace_back("protectionBudget.treasuryAddress",
                               protectionRewardBudget.treasuryAddress());
  canonicalFields.emplace_back(
      "protectionBudget.availableBudgetRawUnits",
      std::to_string(protectionRewardBudget.availableBudget().rawUnits()));
  canonicalFields.emplace_back(
      "protectionBudget.plannedTotalRawUnits",
      std::to_string(protectionRewardBudget.plannedTotal().rawUnits()));
  canonicalFields.emplace_back(
      "protectionBudget.remainingBudgetRawUnits",
      std::to_string(protectionRewardBudget.remainingBudget().rawUnits()));
  canonicalFields.emplace_back(
      "protectionBudget.beneficiaryCount",
      std::to_string(protectionRewardBudget.beneficiaryCount()));
  canonicalFields.emplace_back("protectionBudget.reason",
                               protectionRewardBudget.reason());
  canonicalFields.emplace_back("protectionBudget.sourceTreasuryDigest",
                               protectionRewardBudget.sourceTreasuryDigest());

  for (std::size_t grantIndex = 0; grantIndex < protectionRewardGrants.size();
       ++grantIndex) {
    const std::string prefix =
        "protectionGrant." + std::to_string(grantIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        protectionRewardGrants[grantIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(protectionRewardGrants[grantIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "plannedRewardRawUnits",
        std::to_string(
            protectionRewardGrants[grantIndex].plannedReward().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "securityScore",
        std::to_string(protectionRewardGrants[grantIndex].securityScore()));
    canonicalFields.emplace_back(prefix + "reason",
                                 protectionRewardGrants[grantIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceBudgetDigest",
        protectionRewardGrants[grantIndex].sourceBudgetDigest());
  }

  for (std::size_t workIndex = 0; workIndex < protectionWorkRecords.size();
       ++workIndex) {
    const std::string prefix =
        "protectionWork." + std::to_string(workIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        protectionWorkRecords[workIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(protectionWorkRecords[workIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "uptimeScore",
        std::to_string(protectionWorkRecords[workIndex].uptimeScore()));
    canonicalFields.emplace_back(
        prefix + "correctVoteScore",
        std::to_string(protectionWorkRecords[workIndex].correctVoteScore()));
    canonicalFields.emplace_back(
        prefix + "attackDetectionScore",
        std::to_string(
            protectionWorkRecords[workIndex].attackDetectionScore()));
    canonicalFields.emplace_back(
        prefix + "auditContributionScore",
        std::to_string(
            protectionWorkRecords[workIndex].auditContributionScore()));
    canonicalFields.emplace_back(
        prefix + "securityScore",
        std::to_string(protectionWorkRecords[workIndex].securityScore()));
    canonicalFields.emplace_back(
        prefix + "riskPenaltyScore",
        std::to_string(protectionWorkRecords[workIndex].riskPenaltyScore()));
    canonicalFields.emplace_back(
        prefix + "totalWorkScore",
        std::to_string(protectionWorkRecords[workIndex].totalWorkScore()));
    canonicalFields.emplace_back(prefix + "reason",
                                 protectionWorkRecords[workIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceSecurityDigest",
        protectionWorkRecords[workIndex].sourceSecurityDigest());
  }

  canonicalFields.emplace_back(
      "protectionSummary.blockHeight",
      std::to_string(protectionRewardSummary.blockHeight()));
  canonicalFields.emplace_back(
      "protectionSummary.plannedTotalRawUnits",
      std::to_string(protectionRewardSummary.plannedTotal().rawUnits()));
  canonicalFields.emplace_back(
      "protectionSummary.earnedTotalRawUnits",
      std::to_string(protectionRewardSummary.earnedTotal().rawUnits()));
  canonicalFields.emplace_back(
      "protectionSummary.deferredTotalRawUnits",
      std::to_string(protectionRewardSummary.deferredTotal().rawUnits()));
  canonicalFields.emplace_back(
      "protectionSummary.beneficiaryCount",
      std::to_string(protectionRewardSummary.beneficiaryCount()));
  canonicalFields.emplace_back("protectionSummary.reason",
                               protectionRewardSummary.reason());
  canonicalFields.emplace_back("protectionSummary.sourceBudgetDigest",
                               protectionRewardSummary.sourceBudgetDigest());

  for (std::size_t settlementIndex = 0;
       settlementIndex < protectionRewardSettlements.size();
       ++settlementIndex) {
    const std::string prefix =
        "protectionSettlement." + std::to_string(settlementIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        protectionRewardSettlements[settlementIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            protectionRewardSettlements[settlementIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "plannedRewardRawUnits",
        std::to_string(protectionRewardSettlements[settlementIndex]
                           .plannedReward()
                           .rawUnits()));
    canonicalFields.emplace_back(
        prefix + "earnedRewardRawUnits",
    ...
    canonicalFields.emplace_back(
        prefix + "workScore",
        std::to_string(
            protectionRewardSettlements[settlementIndex].workScore()));
    canonicalFields.emplace_back(
        prefix + "securityScore",
        std::to_string(
            protectionRewardSettlements[settlementIndex].securityScore()));
    canonicalFields.emplace_back(
        prefix + "reason",
        protectionRewardSettlements[settlementIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceGrantDigest",
        protectionRewardSettlements[settlementIndex].sourceGrantDigest());
    canonicalFields.emplace_back(
        prefix + "sourceWorkDigest",
        protectionRewardSettlements[settlementIndex].sourceWorkDigest());
  }

  canonicalFields.emplace_back(
      "inflationEpoch.blockHeight",
      std::to_string(inflationEpochSnapshot.blockHeight()));
  canonicalFields.emplace_back(
      "inflationEpoch.epochStartBlock",
      std::to_string(inflationEpochSnapshot.epochStartBlock()));
  canonicalFields.emplace_back(
      "inflationEpoch.epochEndBlock",
      std::to_string(inflationEpochSnapshot.epochEndBlock()));
  canonicalFields.emplace_back(
      "inflationEpoch.maxAnnualInflationBasisPoints",
      std::to_string(inflationEpochSnapshot.maxAnnualInflationBasisPoints()));
  canonicalFields.emplace_back(
      "inflationEpoch.baseSupplyRawUnits",
      std::to_string(inflationEpochSnapshot.baseSupply().rawUnits()));
  canonicalFields.emplace_back(
      "inflationEpoch.annualMintLimitRawUnits",
      std::to_string(inflationEpochSnapshot.annualMintLimit().rawUnits()));
  canonicalFields.emplace_back(
      "inflationEpoch.mintedThisEpochRawUnits",
      std::to_string(inflationEpochSnapshot.mintedThisEpoch().rawUnits()));
  canonicalFields.emplace_back(
      "inflationEpoch.remainingMintCapacityRawUnits",
      std::to_string(
          inflationEpochSnapshot.remainingMintCapacity().rawUnits()));
  canonicalFields.emplace_back("inflationEpoch.policyId",
                               inflationEpochSnapshot.policyId());
  canonicalFields.emplace_back("inflationEpoch.reason",
                               inflationEpochSnapshot.reason());

  canonicalFields.emplace_back(
      "mintAuthorization.blockHeight",
      std::to_string(mintAuthorizationRecord.blockHeight()));
  canonicalFields.emplace_back("mintAuthorization.authorizationId",
                               mintAuthorizationRecord.authorizationId());
  canonicalFields.emplace_back(
      "mintAuthorization.authorizedAmountRawUnits",
      std::to_string(mintAuthorizationRecord.authorizedAmount().rawUnits()));
  canonicalFields.emplace_back(
      "mintAuthorization.activationBlock",
      std::to_string(mintAuthorizationRecord.activationBlock()));
  canonicalFields.emplace_back(
      "mintAuthorization.expirationBlock",
      std::to_string(mintAuthorizationRecord.expirationBlock()));
  canonicalFields.emplace_back(
      "mintAuthorization.requiredApprovalBasisPoints",
      std::to_string(mintAuthorizationRecord.requiredApprovalBasisPoints()));
  canonicalFields.emplace_back(
      "mintAuthorization.timelockBlocks",
      std::to_string(mintAuthorizationRecord.timelockBlocks()));
  canonicalFields.emplace_back("mintAuthorization.governanceDigest",
                               mintAuthorizationRecord.governanceDigest());
  canonicalFields.emplace_back("mintAuthorization.reason",
                               mintAuthorizationRecord.reason());
  canonicalFields.emplace_back("mintAuthorization.sourceEpochDigest",
                               mintAuthorizationRecord.sourceEpochDigest());

  canonicalFields.emplace_back(
      "supplyExpansion.blockHeight",
      std::to_string(supplyExpansionRecord.blockHeight()));
  canonicalFields.emplace_back(
      "supplyExpansion.mintedAmountRawUnits",
      std::to_string(supplyExpansionRecord.mintedAmount().rawUnits()));
  canonicalFields.emplace_back("supplyExpansion.recipientAddress",
                               supplyExpansionRecord.recipientAddress());
  canonicalFields.emplace_back("supplyExpansion.authorizationId",
                               supplyExpansionRecord.authorizationId());
  canonicalFields.emplace_back("supplyExpansion.policyId",
                               supplyExpansionRecord.policyId());
  canonicalFields.emplace_back("supplyExpansion.reason",
                               supplyExpansionRecord.reason());
  canonicalFields.emplace_back(
      "supplyExpansion.sourceAuthorizationDigest",
      supplyExpansionRecord.sourceAuthorizationDigest());

  canonicalFields.emplace_back(
      "feeBalance.blockHeight",
      std::to_string(feeEconomicBalance.blockHeight()));
  canonicalFields.emplace_back(
      "feeBalance.totalFeeRawUnits",
      std::to_string(feeEconomicBalance.totalFee().rawUnits()));
  canonicalFields.emplace_back(
      "feeBalance.validatorRewardRawUnits",
      std::to_string(feeEconomicBalance.validatorRewardAmount().rawUnits()));
  canonicalFields.emplace_back(
      "feeBalance.treasuryRawUnits",
      std::to_string(feeEconomicBalance.treasuryAmount().rawUnits()));
  canonicalFields.emplace_back(
      "feeBalance.burnRawUnits",
      std::to_string(feeEconomicBalance.burnAmount().rawUnits()));
  canonicalFields.emplace_back("feeBalance.policyId",
                               feeEconomicBalance.policyId());
  canonicalFields.emplace_back("feeBalance.reason",
                               feeEconomicBalance.reason());

  canonicalFields.emplace_back("feeBurn.blockHeight",
                               std::to_string(feeBurnRecord.blockHeight()));
  canonicalFields.emplace_back(
      "feeBurn.burnAmountRawUnits",
      std::to_string(feeBurnRecord.burnAmount().rawUnits()));
  canonicalFields.emplace_back(
      "feeBurn.supplyBeforeRawUnits",
      std::to_string(feeBurnRecord.supplyBefore().rawUnits()));
  canonicalFields.emplace_back(
      "feeBurn.supplyAfterRawUnits",
      std::to_string(feeBurnRecord.supplyAfter().rawUnits()));
  canonicalFields.emplace_back("feeBurn.reason", feeBurnRecord.reason());
  canonicalFields.emplace_back("feeBurn.sourceFeeBalanceDigest",
                               feeBurnRecord.sourceFeeBalanceDigest());

  canonicalFields.emplace_back("treasuryFee.blockHeight",
                               std::to_string(treasuryFeeRecord.blockHeight()));
  canonicalFields.emplace_back("treasuryFee.treasuryAddress",
                               treasuryFeeRecord.treasuryAddress());
  canonicalFields.emplace_back(
      "treasuryFee.treasuryAmountRawUnits",
      std::to_string(treasuryFeeRecord.treasuryAmount().rawUnits()));
  canonicalFields.emplace_back("treasuryFee.reason",
                               treasuryFeeRecord.reason());
  canonicalFields.emplace_back("treasuryFee.sourceFeeBalanceDigest",
                               treasuryFeeRecord.sourceFeeBalanceDigest());

  for (std::size_t evidenceIndex = 0;
       evidenceIndex < slashingEvidenceRecords.size(); ++evidenceIndex) {
    const std::string prefix =
        "slashingEvidence." + std::to_string(evidenceIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        slashingEvidenceRecords[evidenceIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(slashingEvidenceRecords[evidenceIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "evidenceType",
        slashingEvidenceRecords[evidenceIndex].evidenceType());
    canonicalFields.emplace_back(
        prefix + "severityScore",
        std::to_string(slashingEvidenceRecords[evidenceIndex].severityScore()));
    canonicalFields.emplace_back(
        prefix + "slashable",
        slashingEvidenceRecords[evidenceIndex].slashable() ? "true" : "false");
    canonicalFields.emplace_back(
        prefix + "recommendedAction",
        slashingEvidenceRecords[evidenceIndex].recommendedAction());
    canonicalFields.emplace_back(
        prefix + "reason", slashingEvidenceRecords[evidenceIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceSecurityDigest",
        slashingEvidenceRecords[evidenceIndex].sourceSecurityDigest());
  }

  for (std::size_t preparationIndex = 0;
       preparationIndex < slashingPreparationRecords.size();
       ++preparationIndex) {
    const std::string prefix =
        "slashingPreparation." + std::to_string(preparationIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        slashingPreparationRecords[preparationIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            slashingPreparationRecords[preparationIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "evidenceCount",
        std::to_string(
            slashingPreparationRecords[preparationIndex].evidenceCount()));
    canonicalFields.emplace_back(
        prefix + "slashableEvidenceCount",
        std::to_string(slashingPreparationRecords[preparationIndex]
                           .slashableEvidenceCount()));
    canonicalFields.emplace_back(
        prefix + "maxSeverityScore",
        std::to_string(
            slashingPreparationRecords[preparationIndex].maxSeverityScore()));
    canonicalFields.emplace_back(
        prefix + "preparedPenaltyRawUnits",
        std::to_string(slashingPreparationRecords[preparationIndex]
                           .preparedPenaltyAmount()
                           .rawUnits()));
    canonicalFields.emplace_back(
        prefix + "enforcementAction",
        slashingPreparationRecords[preparationIndex].enforcementAction());
    canonicalFields.emplace_back(
        prefix + "reason",
        slashingPreparationRecords[preparationIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceEvidenceDigest",
        slashingPreparationRecords[preparationIndex].sourceEvidenceDigest());
  }

  canonicalFields.emplace_back(
      "slashingSummary.blockHeight",
      std::to_string(slashingEvidenceSummary.blockHeight()));
  canonicalFields.emplace_back(
      "slashingSummary.evidenceCount",
      std::to_string(slashingEvidenceSummary.evidenceCount()));
  canonicalFields.emplace_back(
      "slashingSummary.slashableEvidenceCount",
      std::to_string(slashingEvidenceSummary.slashableEvidenceCount()));
  canonicalFields.emplace_back(
      "slashingSummary.maxSeverityScore",
      std::to_string(slashingEvidenceSummary.maxSeverityScore()));
  canonicalFields.emplace_back(
      "slashingSummary.preparedPenaltyTotalRawUnits",
      std::to_string(
          slashingEvidenceSummary.preparedPenaltyTotal().rawUnits()));
  canonicalFields.emplace_back("slashingSummary.reason",
                               slashingEvidenceSummary.reason());
  canonicalFields.emplace_back(
      "slashingSummary.sourcePreparationDigest",
      slashingEvidenceSummary.sourcePreparationDigest());

  for (std::size_t evidenceIndex = 0;
       evidenceIndex < cryptographicSlashingEvidenceRecords.size();
       ++evidenceIndex) {
    const std::string prefix =
        "cryptographicSlashingEvidence." + std::to_string(evidenceIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        cryptographicSlashingEvidenceRecords[evidenceIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            cryptographicSlashingEvidenceRecords[evidenceIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "round",
        std::to_string(
            cryptographicSlashingEvidenceRecords[evidenceIndex].round()));
    canonicalFields.emplace_back(
        prefix + "evidenceType",
        cryptographicSlashingEvidenceRecords[evidenceIndex].evidenceType());
    canonicalFields.emplace_back(
        prefix + "severityScore",
        std::to_string(cryptographicSlashingEvidenceRecords[evidenceIndex]
                           .severityScore()));
    canonicalFields.emplace_back(
        prefix + "penaltyBasisPoints",
        std::to_string(cryptographicSlashingEvidenceRecords[evidenceIndex]
                           .penaltyBasisPoints()));
    canonicalFields.emplace_back(
        prefix + "firstVoteDigest",
        cryptographicSlashingEvidenceRecords[evidenceIndex].firstVoteDigest());
    canonicalFields.emplace_back(
        prefix + "secondVoteDigest",
        cryptographicSlashingEvidenceRecords[evidenceIndex].secondVoteDigest());
    canonicalFields.emplace_back(
        prefix + "reason",
        cryptographicSlashingEvidenceRecords[evidenceIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceEvidenceDigest",
        cryptographicSlashingEvidenceRecords[evidenceIndex]
            .sourceEvidenceDigest());
  }

  for (std::size_t penaltyIndex = 0; penaltyIndex < stakePenaltyRecords.size();
       ++penaltyIndex) {
    const std::string prefix =
        "stakePenalty." + std::to_string(penaltyIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        stakePenaltyRecords[penaltyIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(stakePenaltyRecords[penaltyIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "lockedStakeBeforeRawUnits",
        std::to_string(
            stakePenaltyRecords[penaltyIndex].lockedStakeBefore().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "penaltyAmountRawUnits",
        std::to_string(
            stakePenaltyRecords[penaltyIndex].penaltyAmount().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "lockedStakeAfterRawUnits",
        std::to_string(
            stakePenaltyRecords[penaltyIndex].lockedStakeAfter().rawUnits()));
    canonicalFields.emplace_back(
        prefix + "evidenceCount",
        std::to_string(stakePenaltyRecords[penaltyIndex].evidenceCount()));
    canonicalFields.emplace_back(prefix + "reason",
                                 stakePenaltyRecords[penaltyIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceEvidenceDigest",
        stakePenaltyRecords[penaltyIndex].sourceEvidenceDigest());
  }

  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.blockHeight",
      std::to_string(cryptographicSlashingSummary.blockHeight()));
  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.evidenceCount",
      std::to_string(cryptographicSlashingSummary.evidenceCount()));
  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.slashableEvidenceCount",
      std::to_string(cryptographicSlashingSummary.slashableEvidenceCount()));
  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.maxSeverityScore",
      std::to_string(cryptographicSlashingSummary.maxSeverityScore()));
  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.penaltyTotalRawUnits",
      std::to_string(cryptographicSlashingSummary.penaltyTotal().rawUnits()));
  canonicalFields.emplace_back("cryptographicSlashingSummary.reason",
                               cryptographicSlashingSummary.reason());
  canonicalFields.emplace_back(
      "cryptographicSlashingSummary.sourcePenaltyDigest",
      cryptographicSlashingSummary.sourcePenaltyDigest());
  ...
  canonicalFields.emplace_back(
      "governancePolicy.blockHeight",
      std::to_string(governancePolicySnapshot.blockHeight()));
  canonicalFields.emplace_back(
      "governancePolicy.requiredApprovalBasisPoints",
      std::to_string(governancePolicySnapshot.requiredApprovalBasisPoints()));
  canonicalFields.emplace_back(
      "governancePolicy.timelockBlocks",
      std::to_string(governancePolicySnapshot.timelockBlocks()));
  canonicalFields.emplace_back(
      "governancePolicy.activationDelayBlocks",
      std::to_string(governancePolicySnapshot.activationDelayBlocks()));
  canonicalFields.emplace_back("governancePolicy.policyId",
                               governancePolicySnapshot.policyId());
  canonicalFields.emplace_back("governancePolicy.reason",
                               governancePolicySnapshot.reason());

  for (std::size_t guardIndex = 0; guardIndex < governanceActionGuards.size();
       ++guardIndex) {
    const std::string prefix =
        "governanceGuard." + std::to_string(guardIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "actionType", governanceActionGuards[guardIndex].actionType());
    canonicalFields.emplace_back(prefix + "status",
                                 governanceActionGuards[guardIndex].status());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(governanceActionGuards[guardIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "protectedResource",
        governanceActionGuards[guardIndex].protectedResource());
    canonicalFields.emplace_back(
        prefix + "requiredApprovalBasisPoints",
        std::to_string(
            governanceActionGuards[guardIndex].requiredApprovalBasisPoints()));
    canonicalFields.emplace_back(
        prefix + "timelockBlocks",
        std::to_string(governanceActionGuards[guardIndex].timelockBlocks()));
    canonicalFields.emplace_back(prefix + "reason",
                                 governanceActionGuards[guardIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourcePolicyDigest",
        governanceActionGuards[guardIndex].sourcePolicyDigest());
  }

  canonicalFields.emplace_back("governanceSummary.blockHeight",
                               std::to_string(governanceSummary.blockHeight()));
  canonicalFields.emplace_back("governanceSummary.guardCount",
                               std::to_string(governanceSummary.guardCount()));
  canonicalFields.emplace_back(
      "governanceSummary.activeProposalCount",
      std::to_string(governanceSummary.activeProposalCount()));
  canonicalFields.emplace_back(
      "governanceSummary.approvedProposalCount",
      std::to_string(governanceSummary.approvedProposalCount()));
  canonicalFields.emplace_back(
      "governanceSummary.executableProposalCount",
      std::to_string(governanceSummary.executableProposalCount()));
  canonicalFields.emplace_back(
      "governanceSummary.executedProposalCount",
      std::to_string(governanceSummary.executedProposalCount()));
  canonicalFields.emplace_back("governanceSummary.reason",
                               governanceSummary.reason());
  canonicalFields.emplace_back("governanceSummary.sourceGuardDigest",
                               governanceSummary.sourceGuardDigest());

  for (std::size_t lifecycleIndex = 0;
       lifecycleIndex < validatorLifecycleRecords.size(); ++lifecycleIndex) {
    const std::string prefix =
        "validatorLifecycle." + std::to_string(lifecycleIndex) + ".";
    canonicalFields.emplace_back(
        prefix + "validatorAddress",
        validatorLifecycleRecords[lifecycleIndex].validatorAddress());
    canonicalFields.emplace_back(
        prefix + "blockHeight",
        std::to_string(
            validatorLifecycleRecords[lifecycleIndex].blockHeight()));
    canonicalFields.emplace_back(
        prefix + "epochIndex",
        std::to_string(validatorLifecycleRecords[lifecycleIndex].epochIndex()));
    canonicalFields.emplace_back(
        prefix + "lifecycleStatus",
        validatorLifecycleRecords[lifecycleIndex].lifecycleStatus());
    canonicalFields.emplace_back(
        prefix + "lockedStakeRawUnits",
        std::to_string(validatorLifecycleRecords[lifecycleIndex]
                           .lockedStake()
                           .rawUnits()));
    canonicalFields.emplace_back(
        prefix + "earnedRewardRawUnits",
    ...
    canonicalFields.emplace_back(
        prefix + "securityScore",
        std::to_string(
            validatorLifecycleRecords[lifecycleIndex].securityScore()));
    canonicalFields.emplace_back(
        prefix + "reason", validatorLifecycleRecords[lifecycleIndex].reason());
    canonicalFields.emplace_back(
        prefix + "sourceDigest",
        validatorLifecycleRecords[lifecycleIndex].sourceDigest());
  }

  canonicalFields.emplace_back(
      "epochAccounting.blockHeight",
      std::to_string(epochAccountingRecord.blockHeight()));
  canonicalFields.emplace_back(
      "epochAccounting.epochIndex",
      std::to_string(epochAccountingRecord.epochIndex()));
  canonicalFields.emplace_back(
      "epochAccounting.epochStartBlock",
      std::to_string(epochAccountingRecord.epochStartBlock()));
  canonicalFields.emplace_back(
      "epochAccounting.epochEndBlock",
      std::to_string(epochAccountingRecord.epochEndBlock()));
  canonicalFields.emplace_back(
      "epochAccounting.validatorCount",
      std::to_string(epochAccountingRecord.validatorCount()));
  canonicalFields.emplace_back(
      "epochAccounting.activeValidatorCount",
      std::to_string(epochAccountingRecord.activeValidatorCount()));
  canonicalFields.emplace_back(
      "epochAccounting.totalLockedStakeRawUnits",
      std::to_string(epochAccountingRecord.totalLockedStake().rawUnits()));
  canonicalFields.emplace_back(
      "epochAccounting.totalEarnedRewardsRawUnits",
      std::to_string(epochAccountingRecord.totalEarnedRewards().rawUnits()));
  canonicalFields.emplace_back(
      "epochAccounting.totalSlashingPenaltiesRawUnits",
      std::to_string(
          epochAccountingRecord.totalSlashingPenalties().rawUnits()));
  canonicalFields.emplace_back("epochAccounting.reason",
                               epochAccountingRecord.reason());
  canonicalFields.emplace_back("epochAccounting.sourceLifecycleDigest",
                               epochAccountingRecord.sourceLifecycleDigest());

  canonicalFields.emplace_back(
      "validatorLifecycleSummary.blockHeight",
      std::to_string(validatorLifecycleSummary.blockHeight()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.epochIndex",
      std::to_string(validatorLifecycleSummary.epochIndex()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.activeValidatorCount",
      std::to_string(validatorLifecycleSummary.activeValidatorCount()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.jailedValidatorCount",
      std::to_string(validatorLifecycleSummary.jailedValidatorCount()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.slashedValidatorCount",
      std::to_string(validatorLifecycleSummary.slashedValidatorCount()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.totalLockedStakeRawUnits",
      std::to_string(validatorLifecycleSummary.totalLockedStake().rawUnits()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.totalEarnedRewardsRawUnits",
      std::to_string(
          validatorLifecycleSummary.totalEarnedRewards().rawUnits()));
  canonicalFields.emplace_back(
      "validatorLifecycleSummary.totalSlashingPenaltiesRawUnits",
      std::to_string(
          validatorLifecycleSummary.totalSlashingPenalties().rawUnits()));
  canonicalFields.emplace_back("validatorLifecycleSummary.reason",
                               validatorLifecycleSummary.reason());
  canonicalFields.emplace_back("validatorLifecycleSummary.sourceEpochDigest",
                               validatorLifecycleSummary.sourceEpochDigest());

  canonicalFields.emplace_back("block", storedBlock);
  canonicalFields.emplace_back("quorumCertificate",
                               quorumCertificate.serialize());
  canonicalFields.emplace_back("finalizedRecord", finalizedRecord.serialize());

  FinalizedMonetarySectionCodec::appendSupplyDeltaFields(parsedSupplyDelta,
                                                         canonicalFields);

  FinalizedTreasurySectionCodec::appendFields(parsedTreasurySection,
                                              canonicalFields);

  const std::string canonicalContents =
      serialization::KeyValueFileCodec::serialize(
          FinalizedArtifactSchema::currentSchemaId(), canonicalFields);
  ...
  FinalizedBlockArtifact artifact(
      block, postStateRoot, totalFee, std::move(parsedSupplyDelta),
      ...
      monetaryFirewallAudit, genesisTreasurySnapshot, protectionRewardBudget,
      protectionRewardGrants, protectionWorkRecords, protectionRewardSummary,
      protectionRewardSettlements, inflationEpochSnapshot,
      mintAuthorizationRecord, supplyExpansionRecord, feeEconomicBalance,
      feeBurnRecord, treasuryFeeRecord, slashingEvidenceRecords,
      ...
      governanceActionGuards, governanceSummary, validatorLifecycleRecords,
      epochAccountingRecord, validatorLifecycleSummary, quorumCertificate,
      finalizedRecord, std::move(parsedTreasurySection));
  return artifact;
}
