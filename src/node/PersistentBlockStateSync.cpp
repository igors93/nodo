...
constexpr const char *CODEC_VERSION =
...
parseCheckpointFields(const std::string &serialized) {
...
finalizedSlashingEvidenceIds(const core::Block &block) {
  ...
  for (const consensus::ProposerEquivocationEvidence &evidence :
       CanonicalSlashingTransition::proposerEquivocationEvidenceFromBlock(
           block)) {
    ids.push_back(evidence.evidenceId());
  }
...
bool removeFinalizedSlashingEvidenceFromPendingStores(
    ...
    storage::SlashingEvidenceStore *pendingEvidenceStore, std::string &reason) {
  try {
    for (const core::Block &block : finalizedBlocks) {
      for (const std::string &evidenceId :
           finalizedSlashingEvidenceIds(block)) {
        ...
        if (!removedFromPool) {
          reason = "Finalized slashing evidence could not be removed from "
                   "pending pool: " +
                   evidenceId;
          return false;
        }
    ...
    reason = std::string("Finalized slashing evidence cleanup failed: ") +
             error.what();
    return false;
  }
  return true;
...
PersistentSyncStatus persistentSyncStatusFromString(const std::string &value) {
  if (value == "IDLE")
    return PersistentSyncStatus::IDLE;
  if (value == "SYNCING")
    return PersistentSyncStatus::SYNCING;
  if (value == "COMPLETE")
    return PersistentSyncStatus::COMPLETE;
  if (value == "FAILED")
    return PersistentSyncStatus::FAILED;
  return PersistentSyncStatus::UNKNOWN;
}

PersistentSyncCheckpoint::PersistentSyncCheckpoint()
    : m_schemaVersion(), m_finalizedHeight(0), m_finalizedBlockHash(),
      m_finalizedStateRoot(), m_status(PersistentSyncStatus::UNKNOWN),
      m_sourcePeerId(), m_updatedAt(0) {}

PersistentSyncCheckpoint::PersistentSyncCheckpoint(
    std::string schemaVersion, std::uint64_t finalizedHeight,
    std::string finalizedBlockHash, std::string finalizedStateRoot,
    PersistentSyncStatus status, std::string sourcePeerId,
    ...
    : m_schemaVersion(std::move(schemaVersion)),
      ...
      m_finalizedStateRoot(std::move(finalizedStateRoot)), m_status(status),
      m_sourcePeerId(std::move(sourcePeerId)), m_updatedAt(updatedAt) {}

const std::string &PersistentSyncCheckpoint::schemaVersion() const {
  return m_schemaVersion;
}
std::uint64_t PersistentSyncCheckpoint::finalizedHeight() const {
  return m_finalizedHeight;
}
const std::string &PersistentSyncCheckpoint::finalizedBlockHash() const {
  return m_finalizedBlockHash;
}
const std::string &PersistentSyncCheckpoint::finalizedStateRoot() const {
  return m_finalizedStateRoot;
}
PersistentSyncStatus PersistentSyncCheckpoint::status() const {
  return m_status;
}
const std::string &PersistentSyncCheckpoint::sourcePeerId() const {
...
bool PersistentSyncCheckpoint::isValid() const {
  return m_schemaVersion == SCHEMA_VERSION &&
         ...
         isSafeScalar(m_finalizedStateRoot) && isSafeScalar(m_sourcePeerId) &&
         m_status != PersistentSyncStatus::UNKNOWN && m_updatedAt > 0;
}
...
PersistentSyncCheckpoint::genesis(const std::string &genesisBlockHash,
                                  const std::string &genesisStateRoot,
                                  std::int64_t now) {
  return PersistentSyncCheckpoint(
      SCHEMA_VERSION, 0, genesisBlockHash, genesisStateRoot,
      PersistentSyncStatus::COMPLETE, "LOCAL_GENESIS", now);
}

PersistentSyncCheckpoint
PersistentSyncCheckpoint::deserialize(const std::string &serialized) {
  const auto fields = parseCheckpointFields(serialized);

  PersistentSyncCheckpoint checkpoint(
      requireField(fields, "schemaVersion"),
      parseU64Strict(requireField(fields, "finalizedHeight"),
                     "finalizedHeight"),
...
PersistentSyncCheckpointStore::dataDirectory() const {
  return m_dataDirectory;
}

std::filesystem::path
PersistentSyncCheckpointStore::checkpointFilePath() const {
...
std::string persistentSyncCheckpointReadStatusToString(
  ...
  switch (status) {
  case PersistentSyncCheckpointReadStatus::LOADED:
    return "LOADED";
  case PersistentSyncCheckpointReadStatus::MISSING:
    return "MISSING";
  case PersistentSyncCheckpointReadStatus::MALFORMED:
    return "MALFORMED";
  case PersistentSyncCheckpointReadStatus::INVALID:
    return "INVALID";
  case PersistentSyncCheckpointReadStatus::IO_FAILURE:
    return "IO_FAILURE";
  default:
    return "IO_FAILURE";
  }
...
PersistentSyncCheckpointReadResult::PersistentSyncCheckpointReadResult()
    : m_status(PersistentSyncCheckpointReadStatus::IO_FAILURE),
      m_reason("Uninitialized."), m_checkpoint() {}
...
PersistentSyncCheckpointReadResult::missing() {
  PersistentSyncCheckpointReadResult result;
...
// static
PersistentSyncCheckpointReadResult
PersistentSyncCheckpointReadResult::malformed(std::string reason) {
  PersistentSyncCheckpointReadResult result;
...
// static
PersistentSyncCheckpointReadResult
PersistentSyncCheckpointReadResult::invalid(std::string reason) {
  PersistentSyncCheckpointReadResult result;
...
// static
PersistentSyncCheckpointReadResult
PersistentSyncCheckpointReadResult::ioFailure(std::string reason) {
  PersistentSyncCheckpointReadResult result;
...
PersistentSyncCheckpointReadStatus
PersistentSyncCheckpointReadResult::status() const {
  return m_status;
}
...
const PersistentSyncCheckpoint &
PersistentSyncCheckpointReadResult::checkpoint() const {
  return m_checkpoint;
}
...
std::string persistentSyncCheckpointWriteStatusToString(
  ...
  switch (status) {
  case PersistentSyncCheckpointWriteStatus::SAVED:
    return "SAVED";
  case PersistentSyncCheckpointWriteStatus::INVALID_CHECKPOINT:
    return "INVALID_CHECKPOINT";
  case PersistentSyncCheckpointWriteStatus::IO_FAILURE:
    return "IO_FAILURE";
  default:
    return "IO_FAILURE";
  }
...
PersistentSyncCheckpointWriteResult::saved() {
...
// static
PersistentSyncCheckpointWriteResult
PersistentSyncCheckpointWriteResult::invalidCheckpoint(std::string reason) {
  PersistentSyncCheckpointWriteResult result;
...
// static
PersistentSyncCheckpointWriteResult
PersistentSyncCheckpointWriteResult::ioFailure(std::string reason) {
  PersistentSyncCheckpointWriteResult result;
...
PersistentSyncCheckpointWriteStatus
PersistentSyncCheckpointWriteResult::status() const {
  return m_status;
}
...
PersistentBlockSyncItem::PersistentBlockSyncItem()
    : m_height(0), m_blockHash(), m_previousBlockHash(), m_serializedBlock(),
      m_finalizedStateRoot(), m_createdAt(0), m_serializedFinalizedRecord() {}

PersistentBlockSyncItem::PersistentBlockSyncItem(
    std::uint64_t height, std::string blockHash, std::string previousBlockHash,
    std::string serializedBlock, std::string finalizedStateRoot,
    std::int64_t createdAt, std::string serializedFinalizedRecord)
    : m_height(height), m_blockHash(std::move(blockHash)),
...
const std::string &PersistentBlockSyncItem::blockHash() const {
  return m_blockHash;
}
const std::string &PersistentBlockSyncItem::previousBlockHash() const {
  return m_previousBlockHash;
}
const std::string &PersistentBlockSyncItem::serializedBlock() const {
  return m_serializedBlock;
}
const std::string &PersistentBlockSyncItem::finalizedStateRoot() const {
  return m_finalizedStateRoot;
}
std::int64_t PersistentBlockSyncItem::createdAt() const { return m_createdAt; }
const std::string &PersistentBlockSyncItem::serializedFinalizedRecord() const {
  return m_serializedFinalizedRecord;
}

bool PersistentBlockSyncItem::isValid() const {
  return m_height > 0 && core::Block::isCanonicalCommitmentRoot(m_blockHash) &&
...
std::string PersistentBlockSyncItem::serialize() const {
  ...
  output << "PersistentBlockSyncItem{height=" << m_height
         ...
         << ";finalizedStateRoot=" << m_finalizedStateRoot
         << ";createdAt=" << m_createdAt << ";hasFinalizedRecord="
         << (!m_serializedFinalizedRecord.empty() ? "true" : "false") << "}";
  return output.str();
}

PersistentBlockSyncBatch::PersistentBlockSyncBatch()
    : m_sourcePeerId(), m_fromHeight(0), m_toHeight(0), m_items(),
      m_createdAt(0) {}

PersistentBlockSyncBatch::PersistentBlockSyncBatch(
    std::string sourcePeerId, std::uint64_t fromHeight, std::uint64_t toHeight,
    std::vector<PersistentBlockSyncItem> items, std::int64_t createdAt)
    : m_sourcePeerId(std::move(sourcePeerId)), m_fromHeight(fromHeight),
      m_toHeight(toHeight), m_items(std::move(items)), m_createdAt(createdAt) {}

const std::string &PersistentBlockSyncBatch::sourcePeerId() const {
  return m_sourcePeerId;
}
std::uint64_t PersistentBlockSyncBatch::fromHeight() const {
  return m_fromHeight;
}
std::uint64_t PersistentBlockSyncBatch::toHeight() const { return m_toHeight; }
const std::vector<PersistentBlockSyncItem> &
PersistentBlockSyncBatch::items() const {
  return m_items;
}
...
bool PersistentBlockSyncBatch::isValid() const {
  if (!isSafeScalar(m_sourcePeerId) || m_fromHeight == 0 ||
      m_toHeight < m_fromHeight || m_items.empty() ||
  ...
  for (std::size_t index = 0; index < m_items.size(); ++index) {
    ...
    if (index > 0 &&
        item.previousBlockHash() != m_items[index - 1].blockHash()) {
      return false;
    }
  }
...
std::string PersistentBlockSyncBatch::serialize() const {
  ...
  output << "PersistentBlockSyncBatch{sourcePeerId=" << m_sourcePeerId
         << ";fromHeight=" << m_fromHeight << ";toHeight=" << m_toHeight
         << ";blockCount=" << m_items.size() << ";createdAt=" << m_createdAt
         << ";items=[";
...
PersistentSnapshotSyncManifest::PersistentSnapshotSyncManifest()
    : m_sourcePeerId(), m_snapshotHeight(0), m_snapshotBlockHash(),
      m_snapshotStateRoot(), m_snapshotDigest(), m_createdAt(0) {}

PersistentSnapshotSyncManifest::PersistentSnapshotSyncManifest(
    std::string sourcePeerId, std::uint64_t snapshotHeight,
    std::string snapshotBlockHash, std::string snapshotStateRoot,
    std::string snapshotDigest, std::int64_t createdAt)
    : m_sourcePeerId(std::move(sourcePeerId)), m_snapshotHeight(snapshotHeight),
      m_snapshotBlockHash(std::move(snapshotBlockHash)),
      m_snapshotStateRoot(std::move(snapshotStateRoot)),
      m_snapshotDigest(std::move(snapshotDigest)), m_createdAt(createdAt) {}

const std::string &PersistentSnapshotSyncManifest::sourcePeerId() const {
  return m_sourcePeerId;
}
std::uint64_t PersistentSnapshotSyncManifest::snapshotHeight() const {
  return m_snapshotHeight;
}
const std::string &PersistentSnapshotSyncManifest::snapshotBlockHash() const {
  return m_snapshotBlockHash;
}
const std::string &PersistentSnapshotSyncManifest::snapshotStateRoot() const {
  return m_snapshotStateRoot;
}
const std::string &PersistentSnapshotSyncManifest::snapshotDigest() const {
  return m_snapshotDigest;
}
std::int64_t PersistentSnapshotSyncManifest::createdAt() const {
  return m_createdAt;
}

bool PersistentSnapshotSyncManifest::isValid() const {
  return isSafeScalar(m_sourcePeerId) && m_snapshotHeight > 0 &&
         isSafeScalar(m_snapshotBlockHash) &&
         isSafeScalar(m_snapshotStateRoot) && isSafeScalar(m_snapshotDigest) &&
         m_createdAt > 0;
}
...
PersistentSnapshotSyncManifest::deserialize(const std::string &serialized) {
  ...
  if (serialized.size() < prefix.size() + 1 ||
    ...
    throw std::invalid_argument(
        "Invalid PersistentSnapshotSyncManifest format.");
  }
  ...
  const std::string snapshotBlockHash =
      requireField(fields, "snapshotBlockHash");
  const std::string snapshotStateRoot =
      requireField(fields, "snapshotStateRoot");
  const std::string snapshotDigest = requireField(fields, "snapshotDigest");
  const std::uint64_t snapshotHeight =
      parseU64Strict(requireField(fields, "snapshotHeight"), "snapshotHeight");
  const std::int64_t createdAt =
      parseI64Strict(requireField(fields, "createdAt"), "createdAt");

  return PersistentSnapshotSyncManifest(sourcePeerId, snapshotHeight,
                                        snapshotBlockHash, snapshotStateRoot,
                                        snapshotDigest, createdAt);
}
...
PersistentSyncPlan::PersistentSyncPlan()
    : m_status(PersistentSyncPlanStatus::REJECTED),
      ...
      m_blockRequest(std::nullopt), m_snapshotRequest(std::nullopt) {}

PersistentSyncPlan::PersistentSyncPlan(
    PersistentSyncPlanStatus status, std::string reason,
...
const std::optional<NetworkBlockSyncRequest> &
PersistentSyncPlan::blockRequest() const {
  return m_blockRequest;
}
const std::optional<PersistentSnapshotSyncManifest> &
PersistentSyncPlan::snapshotRequest() const {
  return m_snapshotRequest;
}
bool PersistentSyncPlan::requestBlocks() const {
  return m_status == PersistentSyncPlanStatus::REQUEST_BLOCKS &&
         m_blockRequest.has_value();
}
bool PersistentSyncPlan::requestSnapshot() const {
  return m_status == PersistentSyncPlanStatus::REQUEST_SNAPSHOT;
}
bool PersistentSyncPlan::notRequired() const {
  return m_status == PersistentSyncPlanStatus::NOT_REQUIRED;
}
bool PersistentSyncPlan::rejected() const {
  return m_status == PersistentSyncPlanStatus::REJECTED;
}

std::string PersistentSyncPlan::serialize() const {
  std::ostringstream output;
  output << "PersistentSyncPlan{status="
         << persistentSyncPlanStatusToString(m_status) << ";reason=" << m_reason
         << ";hasBlockRequest="
         << (m_blockRequest.has_value() ? "true" : "false")
         << ";hasSnapshotRequest="
         << (m_snapshotRequest.has_value() ? "true" : "false") << "}";
  return output.str();
}

PersistentSyncPlan PersistentBlockStateSyncPlanner::planFromRemoteStatus(
    const PersistentSyncCheckpoint &localCheckpoint,
    const ChainStatusMessage &remoteStatus, const std::string &localNodeId,
    const std::string &sourcePeerId, std::uint64_t maxBlocksPerRequest,
    std::int64_t now) {
  if (!localCheckpoint.isValid()) {
    return PersistentSyncPlan(PersistentSyncPlanStatus::REJECTED,
  ...
  NetworkBlockSyncRequest request(
      localNodeId,
      BlockLocator(fromHeight, requestedCount,
...
std::string
persistentSyncApplyStatusToString(PersistentSyncApplyStatus status) {
  switch (status) {
...
PersistentSyncApplyResult::PersistentSyncApplyResult(
    PersistentSyncApplyStatus status, std::string reason,
    std::optional<PersistentSyncCheckpoint> checkpoint)
    : m_status(status), m_reason(std::move(reason)),
      m_checkpoint(std::move(checkpoint)) {}

PersistentSyncApplyStatus PersistentSyncApplyResult::status() const {
  return m_status;
}
const std::string &PersistentSyncApplyResult::reason() const {
  return m_reason;
}
const std::optional<PersistentSyncCheckpoint> &
PersistentSyncApplyResult::checkpoint() const {
  return m_checkpoint;
}
bool PersistentSyncApplyResult::applied() const {
  return m_status == PersistentSyncApplyStatus::APPLIED &&
         m_checkpoint.has_value();
}
bool PersistentSyncApplyResult::stale() const {
  return m_status == PersistentSyncApplyStatus::STALE;
}

std::string PersistentSyncApplyResult::serialize() const {
  std::ostringstream output;
  output << "PersistentSyncApplyResult{status="
         << persistentSyncApplyStatusToString(m_status)
         << ";reason=" << m_reason
...
PersistentSyncApplyResult PersistentBlockStateSyncApplier::applyValidatedBatch(
    ...
    const PersistentBlockSyncBatch &batch, core::Blockchain &blockchain,
    const core::ValidatorRegistry &validatorRegistry,
    const crypto::CryptoPolicy &policy,
    const crypto::SignatureProvider &provider,
    std::function<core::StateTransitionPreviewContext(const core::Blockchain &)>
        contextBuilder,
    std::int64_t now, const core::ValidatorSetHistory *validatorSetHistory) {
  ...
  // alone.
  if (!checkpoint.isValid()) {
    return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
  ...
  if (!batch.isValid() || now <= 0) {
    return PersistentSyncApplyResult(
        ...
        "Block sync batch or timestamp is invalid.", std::nullopt);
  }

  if (batch.fromHeight() != checkpoint.finalizedHeight() + 1) {
    return PersistentSyncApplyResult(
        PersistentSyncApplyStatus::REJECTED,
        "Batch does not start at the next expected height.", std::nullopt);
  }

  if (batch.items().front().previousBlockHash() !=
    ...
    for (const auto &item : batch.items()) {
      if (!stagedValidatorSetHistory.hasSet(item.height())) {
        ...
        if (!stagedValidatorSetHistory.hasSet(previousHeight) ||
          ...
          return PersistentSyncApplyResult(
              PersistentSyncApplyStatus::REJECTED,
              "Historical validator-set continuity is missing at block "
              "height " +
  ...
  for (const auto &item : batch.items()) {
    ...
    if (record.blockHash() != item.blockHash()) {
      return PersistentSyncApplyResult(
          PersistentSyncApplyStatus::REJECTED,
          "FinalizedBlockRecord blockHash does not match sync item blockHash "
          "at height " +
              std::to_string(item.height()) + ".",
          std::nullopt);
    }
  ...
  for (const auto &item : batch.items()) {
    ...
    if (block.index() != item.height() || block.hash() != item.blockHash() ||
        block.previousHash() != item.previousBlockHash() ||
        block.stateRoot() != item.finalizedStateRoot()) {
      return PersistentSyncApplyResult(
          PersistentSyncApplyStatus::REJECTED,
          "Serialized block identity or state commitment does not match sync "
          "metadata at height " +
              std::to_string(item.height()) + ".",
          std::nullopt);
    }
    ...
    if (record.blockIndex() != block.index() ||
      ...
      return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
                                       "FinalizedBlockRecord is not bound to "
                                       "the serialized block at height " +
                                           std::to_string(item.height()) + ".",
                                       std::nullopt);
    }
    ...
    const core::BlockValidationResult validation =
        core::BlockStateTransitionValidator::validateCandidateBlock(
            stagedBlockchain, blockOpt.value(), context
  ...
  if (last == nullptr) {
    return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
                                     "Batch has no final item.", std::nullopt);
  }

  PersistentSyncCheckpoint updatedCheckpoint(
      PersistentSyncCheckpoint::SCHEMA_VERSION, last->height(),
      last->blockHash(), last->finalizedStateRoot(),
...
PersistentSyncApplyResult PersistentBlockStateSyncApplier::importFinalizedBatch(
    const PersistentSyncCheckpoint &checkpoint,
    const PersistentBlockSyncBatch &batch, NodeRuntime &runtime,
    const NodeDataDirectoryConfig &directoryConfig,
    PersistentSyncCheckpointStore *checkpointStore, std::int64_t now,
    consensus::EvidencePool *pendingEvidencePool,
    storage::SlashingEvidenceStore *pendingEvidenceStore) {
  if (!checkpoint.isValid()) {
    return PersistentSyncApplyResult(
        PersistentSyncApplyStatus::REJECTED,
        "Sync checkpoint is invalid for finalized sync import.", std::nullopt);
  }
  if (!batch.isValid()) {
    return PersistentSyncApplyResult(
        PersistentSyncApplyStatus::REJECTED,
        "Finalized sync batch is structurally invalid.", std::nullopt);
  }
  if (!runtime.isValid()) {
    return PersistentSyncApplyResult(
        PersistentSyncApplyStatus::REJECTED,
        "Runtime is invalid for finalized sync import.", std::nullopt);
  }
  ...
  for (std::size_t itemIndex = firstNewItemIndex;
    ...
    try {
      ...
      if (item.createdAt() > now + 300 ||
        ...
        return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
                                         "Finalized sync item timestamp is too "
                                         "far in the future at height " +
                                             std::to_string(item.height()) +
                                             ".",
                                         std::nullopt);
      }
      ...
      RuntimeBlockPipelineResult result =
          RuntimeBlockPipeline::commitCertifiedBlock(
              stagedRuntime, block, remoteRecord.quorumCertificate(),
              remoteRecord.finalizedAt(), nullptr);
      if (!result.finalized() ||
      ...
      const FinalizedSlashingEvidenceAuditResult slashingAudit =
          FinalizedSlashingEvidenceAudit::auditBlockEffects(
              block, stagedRuntime.validatorPenaltyLedger(),
      ...
      if (!slashingAudit.passed()) {
        return PersistentSyncApplyResult(
            ...
            "Finalized sync slashing audit failed at height " +
                std::to_string(item.height()) + ": " + slashingAudit.reason(),
            std::nullopt);
      }
  ...
  PersistentSyncCheckpoint updated(
      PersistentSyncCheckpoint::SCHEMA_VERSION, last.height(), last.blockHash(),
      last.finalizedStateRoot(), PersistentSyncStatus::COMPLETE,
      batch.sourcePeerId(), now);
  if (!updated.isValid()) {
    return PersistentSyncApplyResult(
        PersistentSyncApplyStatus::REJECTED,
        "Finalized sync batch produced an invalid checkpoint.", std::nullopt);
  }

  std::string evidenceCleanupReason;
  if (!removeFinalizedSlashingEvidenceFromPendingStores(
          stagedImportedBlocks, pendingEvidencePool, pendingEvidenceStore,
          evidenceCleanupReason)) {
    return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
                                     evidenceCleanupReason, std::nullopt);
  }
  ...
  if (checkpointStore != nullptr) {
    ...
    if (!writeResult.isSaved()) {
      return PersistentSyncApplyResult(
          PersistentSyncApplyStatus::REJECTED,
          "Checkpoint persistence failed before runtime commit: " +
              writeResult.reason(),
          std::nullopt);
    }
  }
  ...
  return PersistentSyncApplyResult(
      PersistentSyncApplyStatus::APPLIED,
      "Finalized sync batch was atomically imported and published.", updated);
}

PersistentSyncApplyResult PersistentBlockStateSyncApplier::importSnapshot(
    const PersistentSyncCheckpoint &checkpoint,
    const PersistentSnapshotSyncManifest &manifest, NodeRuntime &runtime,
    const NodeDataDirectoryConfig &directoryConfig,
    PersistentSyncCheckpointStore *checkpointStore, std::int64_t now) {
  if (!checkpoint.isValid()) {
    return PersistentSyncApplyResult(
  ...
  if (!snapshot.has_value()) {
    return PersistentSyncApplyResult(PersistentSyncApplyStatus::REJECTED,
                                     "Snapshot payload is not available "
                                     "locally for the advertised manifest.",
                                     std::nullopt);
  }

  const FastSyncSnapshotImportResult verification =
      FastSyncSnapshotService::verifyAndCheckpoint(
          *snapshot, runtime.config().genesisConfig(), manifest, nullptr, now);
  ...
  if (checkpointStore != nullptr) {
    ...
    if (!write.isSaved()) {
      return PersistentSyncApplyResult(
          PersistentSyncApplyStatus::REJECTED,
          "Checkpoint persistence failed after snapshot verification: " +
              write.reason(),
          std::nullopt);
    }
  }
...
PersistentSyncCheckpoint PersistentBlockStateSyncCodec::decodeCheckpoint(
  ...
  PersistentSyncCheckpoint checkpoint(schemaVersion, finalizedHeight,
                                      finalizedBlockHash, finalizedStateRoot,
                                      status, sourcePeerId, updatedAt);
...
std::string PersistentBlockStateSyncCodec::hashCheckpoint(
  ...
  return serialization::CanonicalHash::hashBytes(
      encodeCheckpoint(checkpoint), "NODO_PERSISTENT_SYNC_CHECKPOINT_HASH_V1");
}
...
PersistentBlockSyncItem PersistentBlockStateSyncCodec::decodeBlockSyncItem(
  ...
  PersistentBlockSyncItem item(height, blockHash, previousBlockHash,
                               serializedBlock, finalizedStateRoot, createdAt,
...
PersistentBlockSyncBatch PersistentBlockStateSyncCodec::decodeBlockSyncBatch(
  ...
  if (itemCount > NODO_PERSISTENT_SYNC_MAX_BLOCK_BATCH) {
    throw std::runtime_error(
        "Decoded persistent block sync batch is too large.");
  }
  ...
  PersistentBlockSyncBatch batch(sourcePeerId, fromHeight, toHeight, items,
                                 createdAt);
...
std::string PersistentBlockStateSyncCodec::hashBlockSyncBatch(
    const PersistentBlockSyncBatch &batch) {
  return serialization::CanonicalHash::hashBytes(
      encodeBlockSyncBatch(batch), "NODO_PERSISTENT_BLOCK_SYNC_BATCH_HASH_V1");
}

std::vector<unsigned char>
PersistentBlockStateSyncCodec::encodeSnapshotManifest(
...
PersistentSnapshotSyncManifest
PersistentBlockStateSyncCodec::decodeSnapshotManifest(
    const std::vector<unsigned char> &bytes) {
  serialization::CanonicalReader reader(bytes, MAX_SYNC_CODEC_FIELD_BYTES);
  ...
  PersistentSnapshotSyncManifest manifest(sourcePeerId, snapshotHeight,
                                          snapshotBlockHash, snapshotStateRoot,
                                          snapshotDigest, createdAt);
