...
std::vector<std::string> splitTopLevel(const std::string &body, char sep) {
  ...
  for (std::size_t i = 0; i < body.size(); ++i) {
    ...
    if (depth < 0)
      throw std::invalid_argument(
          "Unbalanced braces in SignedBlockProposalMessage.");
    if (depth == 0 && c == sep) {
      if (i > start)
  ...
  if (depth != 0)
    throw std::invalid_argument("Unbalanced braces in message body.");
  if (start < body.size())
...
std::uint64_t parseU64(const std::string &v) {
  ...
  if (n != v.size())
    throw std::invalid_argument("Non-numeric uint64: " + v);
  return static_cast<std::uint64_t>(parsed);
}

std::int64_t parseI64(const std::string &v) {
  if (v.empty())
    throw std::invalid_argument("Empty int64 field.");
  std::size_t n = 0;
  const long long parsed = std::stoll(v, &n);
  if (n != v.size())
    throw std::invalid_argument("Non-numeric int64: " + v);
...
SignedBlockProposalMessage::SignedBlockProposalMessage()
    : m_proposerAddress(), m_proposerPublicKey(), m_blockIndex(0), m_round(0),
      m_blockHash(), m_serializedBlock(), m_proposedAt(0), m_signatureBundle(),
      m_justification() {}
...
SignedBlockProposalMessage::SignedBlockProposalMessage(
    ...
    : m_proposerAddress(std::move(proposerAddress)),
      ...
      m_serializedBlock(std::move(serializedBlock)), m_proposedAt(proposedAt),
      m_signatureBundle(std::move(signatureBundle)),
      m_justification(std::move(justification)) {}

const std::string &SignedBlockProposalMessage::proposerAddress() const {
  return m_proposerAddress;
}
const crypto::PublicKey &SignedBlockProposalMessage::proposerPublicKey() const {
  return m_proposerPublicKey;
}
std::uint64_t SignedBlockProposalMessage::blockIndex() const {
  return m_blockIndex;
}
std::uint64_t SignedBlockProposalMessage::round() const { return m_round; }
const std::string &SignedBlockProposalMessage::blockHash() const {
  return m_blockHash;
}
const std::string &SignedBlockProposalMessage::serializedBlock() const {
  return m_serializedBlock;
}
std::int64_t SignedBlockProposalMessage::proposedAt() const {
  return m_proposedAt;
}
const crypto::SignatureBundle &
...
bool SignedBlockProposalMessage::isValid() const {
  return isSafeScalar(m_proposerAddress) && m_proposerPublicKey.isValid() &&
         m_blockIndex > 0 && m_round > 0 && isSafeScalar(m_blockHash) &&
         !m_serializedBlock.empty() &&
         m_serializedBlock.size() <= kMaxSerializedBlockBytes &&
         m_proposedAt > 0 && !m_signatureBundle.empty();
}
...
bool SignedBlockProposalMessage::verify(
  ...
  if (m_proposerAddress != expectedProposer)
    return false;
  ...
  const std::string payload =
      buildSigningPayload(m_proposerAddress, m_proposerPublicKey, m_blockHash,
                          m_blockIndex, m_round, m_proposedAt, m_justification);

  return m_signatureBundle.verifyForPolicy(
      payload, policy, crypto::SecurityContext::VALIDATOR_OPERATION, provider);
}

std::string SignedBlockProposalMessage::serialize() const {
  std::ostringstream oss;
  oss << "SignedBlockProposalMessage{"
      << "schema=" << SCHEMA_ID << ";proposerAddress=" << m_proposerAddress
...
SignedBlockProposalMessage::deserialize(const std::string &text) {
  const auto newlinePos = text.find('\n');
  if (newlinePos == std::string::npos || newlinePos + 1 >= text.size()) {
    throw std::invalid_argument(
        "SignedBlockProposalMessage: missing block payload separator.");
  }
  ...
  if (header.size() <= prefix.size() ||
      header.compare(0, prefix.size(), prefix) != 0 || header.back() != '}') {
    throw std::invalid_argument(
        "SignedBlockProposalMessage: malformed header.");
  }

  const std::string body =
      header.substr(prefix.size(), header.size() - prefix.size() - 1);
  const auto fields = parseFields(body);
  ...
  if (requireField(fields, "proposerPublicKeyFingerprint") !=
      proposerPublicKey.fingerprint()) {
    throw std::invalid_argument(
        "SignedBlockProposalMessage: public key fingerprint mismatch.");
  }

  const std::uint64_t declaredBlockBytes =
      parseU64(requireField(fields, "serializedBlockBytes"));
  if (blockPayload.size() != declaredBlockBytes) {
    throw std::invalid_argument(
        "SignedBlockProposalMessage: block payload length mismatch.");
  }
  ...
  for (const crypto::Signature &signature : bundle.signatures()) {
    if (signature.publicKey().serialize() != proposerPublicKey.serialize()) {
      throw std::invalid_argument("SignedBlockProposalMessage: signature key "
                                  "does not match proposer key.");
    }
  }

  return SignedBlockProposalMessage(
      requireField(fields, "proposerAddress"), proposerPublicKey,
...
// static
SignedBlockProposalMessage SignedBlockProposalMessage::fromSignatureBundle(
  ...
  if (!block.isValid() || proposerAddress.empty() ||
    ...
    throw std::invalid_argument(
        "SignedBlockProposalMessage::fromSignatureBundle: invalid inputs.");
  }

  return SignedBlockProposalMessage(
      proposerAddress, proposerPublicKey, block.index(), round, block.hash(),
      block.serialize(), proposedAt, std::move(signatureBundle),
...
SignedBlockProposalMessage SignedBlockProposalMessage::sign(
  ...
  if (!block.isValid() || proposerAddress.empty() ||
      !proposerPublicKey.isValid() || round == 0 || proposedAt <= 0) {
    throw std::invalid_argument(
        "SignedBlockProposalMessage::sign: invalid inputs.");
  }
  ...
  const std::string payload =
      buildSigningPayload(proposerAddress, proposerPublicKey, blockHash,
                          block.index(), round, proposedAt, justification);

  const crypto::SignatureBundle bundle =
      crypto::SignatureBundle::createSignature(
          payload, proposerPublicKey, proposerPrivateKey, proposedAt, provider,
          crypto::SigningDomain::VALIDATOR_BLOCK_PROPOSAL);

  return SignedBlockProposalMessage(
      proposerAddress, proposerPublicKey, block.index(), round, blockHash,
      std::move(serializedBlock), proposedAt, std::move(bundle), justification);
}
