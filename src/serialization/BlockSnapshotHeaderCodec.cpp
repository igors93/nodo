...
BlockSnapshotHeaderCodec::deserializeFromSerializedBlock(
  ...
  const std::uint64_t blockIndex = parseUnsigned64(
      FieldCodec::extractField(serializedBlock, "index"), "index");
  ...
  const std::int64_t timestamp = parseSigned64(
      FieldCodec::extractField(serializedBlock, "timestamp"), "timestamp");
  ...
  if (recordCount != parsedRecordCount) {
    throw std::logic_error(
        "Block snapshot record count does not match payload.");
  }

  assertSafePreviousHash(previousHash, "previousHash");
  assertSafeHashLikeField(blockHash, "hash");

  storage::BlockSnapshotHeader header(blockIndex, previousHash, blockHash,
                                      timestamp, recordCount, headerPayload,
...
bool BlockSnapshotHeaderCodec::headerPayloadMatchesMetadata(
    const std::string &headerPayload, std::uint64_t blockIndex,
    const std::string &previousHash, std::int64_t timestamp,
    std::size_t recordCount) {
  try {
    const std::uint64_t payloadIndex = parseUnsigned64(
        FieldCodec::extractField(headerPayload, "index"), "payload.index");

    const std::string payloadPreviousHash =
        FieldCodec::extractField(headerPayload, "previousHash");

    const std::int64_t payloadTimestamp =
        parseSigned64(FieldCodec::extractField(headerPayload, "timestamp"),
                      "payload.timestamp");
...
std::uint64_t
BlockSnapshotHeaderCodec::parseUnsigned64(const std::string &value,
...
std::size_t BlockSnapshotHeaderCodec::parseSize(const std::string &value,
                                                const std::string &fieldName) {
  try {
    if (!value.empty() && value.front() == '-') {
...
std::int64_t
BlockSnapshotHeaderCodec::parseSigned64(const std::string &value,
                                        const std::string &fieldName) {
  try {
    std::size_t parsedCharacters = 0;
...
std::string BlockSnapshotHeaderCodec::hashString(const std::string &value) {
  char output[65] = {0};
