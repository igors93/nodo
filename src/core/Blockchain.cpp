...
bool Blockchain::empty() const { return m_blocks.empty(); }

std::size_t Blockchain::size() const { return m_blocks.size(); }
...
const std::vector<Block> &Blockchain::blocks() const { return m_blocks; }
...
bool Blockchain::isValid() const { return isValid(true); }
...
std::string Blockchain::serialize() const {
  ...
  oss << "Blockchain{"
      << "blockCount=" << m_blocks.size() << ";blocks=[";
...
bool Blockchain::isValidNextBlock(
    const Block &previousBlock, const Block &currentBlock,
    bool requireProtocolCommitmentsForCurrentBlock) const {
