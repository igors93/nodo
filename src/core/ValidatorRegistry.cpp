...
validatorRegistrationStatusFromString(const std::string &value) {
  if (value == "PENDING_ACTIVATION")
    return ValidatorRegistrationStatus::PENDING_ACTIVATION;
  if (value == "ACTIVE")
    return ValidatorRegistrationStatus::ACTIVE;
  if (value == "JAILED")
    return ValidatorRegistrationStatus::JAILED;
  if (value == "EXIT_REQUESTED")
    return ValidatorRegistrationStatus::EXIT_REQUESTED;
  if (value == "EXITED")
    return ValidatorRegistrationStatus::EXITED;
  if (value == "DEACTIVATED")
    return ValidatorRegistrationStatus::DEACTIVATED;
  return ValidatorRegistrationStatus::UNKNOWN;
}

ValidatorRegistrationRecord::ValidatorRegistrationRecord()
    : m_validatorAddress(""), m_validatorPublicKey(), m_activationEpoch(0),
      m_metadataHash(""), m_registeredAt(0) {}

ValidatorRegistrationRecord::ValidatorRegistrationRecord(
    std::string validatorAddress, crypto::PublicKey validatorPublicKey,
    std::uint64_t activationEpoch, std::string metadataHash,
    ...
    : m_validatorAddress(std::move(validatorAddress)),
      ...
      m_metadataHash(std::move(metadataHash)), m_registeredAt(registeredAt) {}
...
ValidatorRegistrationRecord::validatorPublicKey() const {
...
bool ValidatorRegistrationRecord::isValid() const {
  if (!isSafeScalar(m_validatorAddress) || !isSafeScalar(m_metadataHash)) {
    return false;
  }
  ...
  if (m_activationEpoch == 0 || m_registeredAt <= 0) {
    return false;
  }
...
ValidatorRegistryEntry::ValidatorRegistryEntry()
    : m_registrationRecord(), m_status(ValidatorRegistrationStatus::UNKNOWN),
      m_lastUpdatedAt(0), m_stakeAmount(0), m_jailUntilEpoch(0),
      m_exitRequestHeight(0), m_ownerAddress("") {}

ValidatorRegistryEntry::ValidatorRegistryEntry(
    ValidatorRegistrationRecord registrationRecord,
    ValidatorRegistrationStatus status, std::int64_t lastUpdatedAt)
    : m_registrationRecord(std::move(registrationRecord)), m_status(status),
      m_lastUpdatedAt(lastUpdatedAt), m_stakeAmount(0), m_jailUntilEpoch(0),
      m_exitRequestHeight(0), m_ownerAddress("") {}

ValidatorRegistryEntry::ValidatorRegistryEntry(
    ValidatorRegistrationRecord registrationRecord,
    ValidatorRegistrationStatus status, std::int64_t lastUpdatedAt,
    std::uint64_t stakeAmount, std::uint64_t jailUntilEpoch,
    std::uint64_t exitRequestHeight, std::string ownerAddress)
    : m_registrationRecord(std::move(registrationRecord)), m_status(status),
      m_lastUpdatedAt(lastUpdatedAt), m_stakeAmount(stakeAmount),
      m_jailUntilEpoch(jailUntilEpoch), m_exitRequestHeight(exitRequestHeight),
      m_ownerAddress(std::move(ownerAddress)) {}

const ValidatorRegistrationRecord &
ValidatorRegistryEntry::registrationRecord() const {
...
std::uint64_t ValidatorRegistryEntry::consensusWeight() const {
  return nodo::consensus::ConsensusWeight::weightFromStake(m_stakeAmount);
}
...
validatorRegistryUpdateStatusToString(ValidatorRegistryUpdateStatus status) {
  ...
  case ValidatorRegistryUpdateStatus::DEACTIVATED:
    return "DEACTIVATED";
  case ValidatorRegistryUpdateStatus::JAILED:
    return "JAILED";
  case ValidatorRegistryUpdateStatus::UNJAILED:
    return "UNJAILED";
  case ValidatorRegistryUpdateStatus::EXIT_REQUESTED:
    return "EXIT_REQUESTED";
  ...
  case ValidatorRegistryUpdateStatus::ALREADY_ACTIVE:
    return "ALREADY_ACTIVE";
  case ValidatorRegistryUpdateStatus::ALREADY_JAILED:
    return "ALREADY_JAILED";
  case ValidatorRegistryUpdateStatus::INVALID_STATUS_TRANSITION:
    return "INVALID_STATUS_TRANSITION";
  case ValidatorRegistryUpdateStatus::INSUFFICIENT_STAKE:
...
ValidatorRegistryUpdateResult::ValidatorRegistryUpdateResult()
    : m_status(ValidatorRegistryUpdateStatus::INVALID_REGISTRY),
      m_reason("Uninitialized registry update result."), m_entry() {}

ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::accepted(ValidatorRegistryEntry entry) {
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::duplicate(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::deactivated(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::jailed(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::unjailed(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::exitRequested(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult
ValidatorRegistryUpdateResult::activated(ValidatorRegistryEntry entry) {
  ValidatorRegistryUpdateResult result;
...
ValidatorRegistryUpdateResult ValidatorRegistry::registerValidator(
  ...
  if (existing != m_entries.end()) {
    ...
    if (existing->second.registrationRecord().publicKeyFingerprint() ==
      ...
      return ValidatorRegistryUpdateResult::duplicate(existing->second);
    }
  ...
  ValidatorRegistryEntry entry(
      registrationRecord, ValidatorRegistrationStatus::ACTIVE,
      registrationRecord.registeredAt(), stakeAmount, 0, 0, ownerAddress);
...
ValidatorRegistryUpdateResult ValidatorRegistry::registerPendingValidator(
  ...
  ValidatorRegistryEntry entry(
      registrationRecord, ValidatorRegistrationStatus::PENDING_ACTIVATION,
      registrationRecord.registeredAt(), stakeAmount, 0, 0, ownerAddress);
  if (!entry.isValid()) {
    return ValidatorRegistryUpdateResult::rejected(
        ValidatorRegistryUpdateStatus::INVALID_RECORD,
...
ValidatorRegistryUpdateResult
ValidatorRegistry::deactivateValidator(const std::string &validatorAddress,
  ...
  auto existing = m_entries.find(validatorAddress);
  ...
  ValidatorRegistryEntry updatedEntry(
      ...
      ValidatorRegistrationStatus::DEACTIVATED, timestamp,
      existing->second.stakeAmount(), existing->second.jailUntilEpoch(),
      existing->second.exitRequestHeight(), existing->second.ownerAddress());
...
bool ValidatorRegistry::isActiveValidator(
    const std::string &validatorAddress) const {
  const auto entry = m_entries.find(validatorAddress);
...
bool ValidatorRegistry::verifyValidatorIdentity(
  ...
  const crypto::Address address = crypto::Address::fromString(validatorAddress);
...
ValidatorRegistry::entryForAddress(const std::string &validatorAddress) const {
  const auto entry = m_entries.find(validatorAddress);
...
ValidatorRegistry::exitRequestedValidatorAddresses() const {
  std::vector<std::string> addresses;
...
ValidatorRegistryUpdateResult
ValidatorRegistry::activateValidator(const std::string &validatorAddress,
  ...
  ValidatorRegistryEntry updated(
      existing.registrationRecord(), ValidatorRegistrationStatus::ACTIVE,
      timestamp, existing.stakeAmount(), existing.jailUntilEpoch(),
...
ValidatorRegistryUpdateResult
ValidatorRegistry::jailValidator(const std::string &validatorAddress,
  ...
  ValidatorRegistryEntry updated(
      existing.registrationRecord(), ValidatorRegistrationStatus::JAILED,
      timestamp, existing.stakeAmount(), jailUntilEpoch,
      existing.exitRequestHeight(), existing.ownerAddress());
...
ValidatorRegistryUpdateResult
ValidatorRegistry::unjailValidator(const std::string &validatorAddress,
                                   std::uint64_t currentEpoch,
                                   std::int64_t timestamp) {
  auto it = m_entries.find(validatorAddress);
  if (it == m_entries.end()) {
  ...
  ValidatorRegistryEntry updated(
      existing.registrationRecord(), ValidatorRegistrationStatus::ACTIVE,
      timestamp, existing.stakeAmount(), 0, existing.exitRequestHeight(),
...
ValidatorRegistryUpdateResult
ValidatorRegistry::requestExit(const std::string &validatorAddress,
  ...
  ValidatorRegistryEntry updated(existing.registrationRecord(),
                                 ValidatorRegistrationStatus::EXIT_REQUESTED,
                                 timestamp, existing.stakeAmount(),
                                 existing.jailUntilEpoch(), requestHeight,
                                 existing.ownerAddress());
...
ValidatorRegistryUpdateResult
ValidatorRegistry::completeExit(const std::string &validatorAddress,
                                std::int64_t timestamp) {
  auto it = m_entries.find(validatorAddress);
  if (it == m_entries.end()) {
    return ValidatorRegistryUpdateResult::rejected(
  ...
  ValidatorRegistryEntry updated(
      existing.registrationRecord(), ValidatorRegistrationStatus::EXITED,
      timestamp, existing.stakeAmount(), existing.jailUntilEpoch(),
      existing.exitRequestHeight(), existing.ownerAddress());
...
ValidatorRegistryUpdateResult
ValidatorRegistry::updateStake(const std::string &validatorAddress,
  ...
  ValidatorRegistryEntry updated(
      ...
      newStakeAmount, existing.jailUntilEpoch(), existing.exitRequestHeight(),
      existing.ownerAddress());
...
std::uint64_t
ValidatorRegistry::consensusWeightFromStake(std::uint64_t stakeAmount) {
  return nodo::consensus::ConsensusWeight::weightFromStake(stakeAmount);
}

std::uint64_t ValidatorRegistry::consensusWeightFor(
    const std::string &validatorAddress) const {
  const auto it = m_entries.find(validatorAddress);
  if (it == m_entries.end() || !it->second.eligibleForConsensus()) {
    return 0;
  }
...
std::string ValidatorRegistry::serialize() const {
  ...
  oss << "ValidatorRegistry{"
      << "size=" << m_entries.size() << ";activeCount=" << activeCount()
      << ";totalConsensusWeight=" << totalConsensusWeight() << ";entries=[";
