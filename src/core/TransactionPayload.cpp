...
GovernanceProposalType governanceProposalTypeFromString(const std::string& value) {
    if (value == "PARAMETER_CHANGE") return GovernanceProposalType::PARAMETER_CHANGE;
    if (value == "TREASURY_SPEND") return GovernanceProposalType::TREASURY_SPEND;
    if (value == "TEXT") return GovernanceProposalType::TEXT;
    throw std::invalid_argument("Unknown governance proposal type.");
}

GovernanceProposalPayload::GovernanceProposalPayload()
    : m_type(GovernanceProposalType::TEXT),
      m_votingStartDelayBlocks(0),
      m_votingPeriodBlocks(0),
      m_quorumNumerator(0),
      m_quorumDenominator(0),
      m_approvalNumerator(0),
      m_approvalDenominator(0),
...
GovernanceProposalPayload GovernanceProposalPayload::treasurySpend(
    ...
    return GovernanceProposalPayload(
        GovernanceProposalType::TREASURY_SPEND,
        std::move(title),
        std::move(description),
        votingStartDelayBlocks,
        votingPeriodBlocks,
...
GovernanceProposalType GovernanceProposalPayload::type() const { return m_type; }
const std::string& GovernanceProposalPayload::title() const { return m_title; }
const std::string& GovernanceProposalPayload::description() const { return m_description; }
std::uint64_t GovernanceProposalPayload::votingStartDelayBlocks() const { return m_votingStartDelayBlocks; }
std::uint64_t GovernanceProposalPayload::votingPeriodBlocks() const { return m_votingPeriodBlocks; }
std::uint64_t GovernanceProposalPayload::quorumNumerator() const { return m_quorumNumerator; }
std::uint64_t GovernanceProposalPayload::quorumDenominator() const { return m_quorumDenominator; }
std::uint64_t GovernanceProposalPayload::approvalNumerator() const { return m_approvalNumerator; }
std::uint64_t GovernanceProposalPayload::approvalDenominator() const { return m_approvalDenominator; }
const std::string& GovernanceProposalPayload::parameterTarget() const { return m_parameterTarget; }
const std::string& GovernanceProposalPayload::parameterValue() const { return m_parameterValue; }
std::uint64_t GovernanceProposalPayload::parameterEffectiveHeight() const { return m_parameterEffectiveHeight; }
const std::string& GovernanceProposalPayload::treasuryRecipient() const { return m_treasuryRecipient; }
std::int64_t GovernanceProposalPayload::treasuryAmountRaw() const { return m_treasuryAmountRaw; }
    ...
    switch (m_type) {
        case GovernanceProposalType::PARAMETER_CHANGE:
            return safeScalar(m_parameterTarget, MAX_GOVERNANCE_SCALAR_BYTES) &&
                   ...
                   m_parameterEffectiveHeight > 0 &&
                   m_treasuryRecipient.empty() &&
                   m_treasuryAmountRaw == 0;
        case GovernanceProposalType::TREASURY_SPEND:
            return m_parameterTarget.empty() &&
                   m_parameterValue.empty() &&
                   m_parameterEffectiveHeight == 0 &&
...
std::string GovernanceProposalPayload::serialize() const {
    if (!isValid()) throw std::invalid_argument("Invalid governance proposal payload.");
    serialization::CanonicalWriter writer;
...
GovernanceProposalPayload GovernanceProposalPayload::deserialize(
    const std::string& serialized
) {
    serialization::CanonicalReader reader(serialized, 4096);
    ...
    const GovernanceProposalType type = governanceProposalTypeFromString(reader.readString());
...
GovernanceVoteChoice governanceVoteChoiceFromString(const std::string& value) {
    if (value == "YES") return GovernanceVoteChoice::YES;
    if (value == "NO") return GovernanceVoteChoice::NO;
    if (value == "ABSTAIN") return GovernanceVoteChoice::ABSTAIN;
...
const std::string& GovernanceVotePayload::proposalId() const { return m_proposalId; }
