...
Block::Block(std::uint64_t index, std::string previousHash,
             std::vector<LedgerRecord> records, std::int64_t timestamp,
             std::string stateRoot, std::string receiptsRoot)
    : m_index(index), m_previousHash(std::move(previousHash)), m_hash(""),
      m_records(std::move(records)), m_timestamp(timestamp),
  ...
  if (m_records.empty() || m_records.size() > MAX_RECORDS) {
    if (m_previousHash != "SNAPSHOT") {
      throw std::invalid_argument(
          "Block must contain at least one LedgerRecord.");
    }
  }
  ...
  for (const auto &record : m_records) {
    if (!record.isValid() ||
        record.payload().size() > MAX_RECORD_PAYLOAD_BYTES) {
  ...
  if (!isWithinResourceLimits() && m_previousHash != "SNAPSHOT") {
    throw std::invalid_argument(
        "Block exceeds canonical protocol resource limits.");
  }
}

Block Block::createGenesisBlock(std::vector<LedgerRecord> records,
                                std::int64_t timestamp, std::string stateRoot) {
  return Block(0, "GENESIS", std::move(records), timestamp,
               std::move(stateRoot));
}

Block Block::createSnapshotDummy(std::uint64_t height, const std::string &hash,
                                 const std::string &stateRoot) {
  Block dummy(height, "SNAPSHOT", {}, 1, stateRoot, "");
  dummy.m_hash = hash;
  return dummy;
}

std::uint64_t Block::index() const { return m_index; }

const std::string &Block::previousHash() const { return m_previousHash; }

const std::string &Block::hash() const { return m_hash; }

std::int64_t Block::timestamp() const { return m_timestamp; }

const std::vector<LedgerRecord> &Block::records() const { return m_records; }

const std::string &Block::stateRoot() const { return m_stateRoot; }
...
bool Block::isValid(bool requireProtocolCommitments) const {
  ...
  if (m_previousHash != "SNAPSHOT" && m_hash != calculateHash()) {
    return false;
  }
...
std::string Block::calculateHash() const { return hashString(headerPayload()); }
...
std::optional<Block> Block::deserialize(const std::string &text) {
  ...
  if (indexStr.empty() || prevHash.empty() || storedHash.empty() ||
      tsStr.empty() || countStr.empty() || payload.empty()) {
    return std::nullopt;
  }
  ...
  try {
    ...
    parsed = 0;
    const unsigned long long parsedRecordCount = std::stoull(countStr, &parsed);
  ...
  while (i < arrayContent.size()) {
    ...
    if (i >= arrayContent.size())
      break;
    if (arrayContent.substr(i, 13) != "LedgerRecord{")
      return std::nullopt;
