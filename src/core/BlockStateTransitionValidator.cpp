...
BlockValidationResult::BlockValidationResult()
    : m_status(BlockValidationStatus::INVALID_BLOCK),
      m_reason("Uninitialized block validation result."), m_stateRoot(""),
      m_totalFee(), m_receiptsRoot("") {}

BlockValidationResult BlockValidationResult::valid() {
  return valid("", utils::Amount());
}

BlockValidationResult BlockValidationResult::valid(std::string stateRoot) {
  return valid(std::move(stateRoot), utils::Amount());
}

BlockValidationResult
BlockValidationResult::valid(std::string stateRoot, utils::Amount totalFee,
...
BlockValidationResult
BlockValidationResult::rejected(BlockValidationStatus status,
                                std::string reason) {
  BlockValidationResult result;
...
BlockValidationStatus BlockValidationResult::status() const { return m_status; }

const std::string &BlockValidationResult::reason() const { return m_reason; }

const std::string &BlockValidationResult::stateRoot() const {
  return m_stateRoot;
}

utils::Amount BlockValidationResult::totalFee() const { return m_totalFee; }

const std::string &BlockValidationResult::receiptsRoot() const {
  return m_receiptsRoot;
}

const std::vector<AccountState> &
BlockValidationResult::resultingAccounts() const {
...
std::string BlockValidationResult::serialize() const {
  ...
  oss << "BlockValidationResult{"
      ...
      << ";reason=" << m_reason << ";stateRoot=" << m_stateRoot
...
BlockValidationResult BlockStateTransitionValidator::validateCandidateBlock(
  ...
  return validateCandidateBlock(blockchain, candidateBlock, 0, mode);
}

BlockValidationResult BlockStateTransitionValidator::validateCandidateBlock(
    const Blockchain &blockchain, const Block &candidateBlock,
    std::int64_t minimumFeeRawUnits, BlockValidationMode mode) {
  return validateCandidateBlock(
      blockchain, candidateBlock,
      StateTransitionPreviewContext::structuralOnly(minimumFeeRawUnits), mode);
}

BlockValidationResult BlockStateTransitionValidator::validateCandidateBlock(
    const Blockchain &blockchain, const Block &candidateBlock,
    const StateTransitionPreviewContext &context, BlockValidationMode mode) {
  ...
  // receive a specific, actionable rejection reason instead of
  // INVALID_PREVIOUS_HASH.
  if (!candidateBlock.isGenesisBlock()) {
    if (candidateBlock.timestamp() <= blockchain.latestBlock().timestamp()) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::INVALID_BLOCK,
          "Candidate block timestamp must be strictly greater than parent "
          "block timestamp.");
    }
  }
  ...
  if (!preview.accepted()) {
    if (preview.status() == StateTransitionPreviewStatus::INVALID_CONTEXT) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::STATE_PREVIEW_FAILED, preview.reason());
    }

    if (preview.status() ==
        StateTransitionPreviewStatus::DUPLICATE_TRANSACTION) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::DUPLICATE_LEDGER_SOURCE, preview.reason());
    }

    if (preview.status() ==
        StateTransitionPreviewStatus::INVALID_LEDGER_RECORD) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::INVALID_LEDGER_RECORD, preview.reason());
    }

    if (preview.status() == StateTransitionPreviewStatus::INVALID_TRANSACTION) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::INVALID_TRANSACTION, preview.reason());
    }
  ...
  if (!candidateBlock.isGenesisBlock() &&
      mode == BlockValidationMode::ProtocolCommitment) {
    if (candidateBlock.stateRoot() != preview.stateRoot()) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::INVALID_BLOCK,
          "State root mismatch between block header and computed state "
          "transition.");
    }
    ...
    if (preview.receiptsRoot() != candidateBlock.receiptsRoot()) {
      return BlockValidationResult::rejected(
          BlockValidationStatus::INVALID_BLOCK,
          "Receipts root mismatch between block header and computed state "
          "transition.");
    }
  }

  return BlockValidationResult::valid(preview.stateRoot(), preview.totalFee(),
                                      preview.receiptsRoot(),
                                      preview.resultingAccounts());
}
