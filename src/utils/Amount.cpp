...
Amount::Amount(std::int64_t rawUnits) : m_rawUnits(rawUnits) {
  if (rawUnits < 0) {
    throw std::invalid_argument(
        "Amount cannot be created from negative raw units.");
  }
}
...
Amount Amount::fromNodo(std::int64_t wholeNodo) {
  if (wholeNodo < 0) {
    throw std::invalid_argument(
        "Amount cannot be created from negative NODO value.");
  }
...
std::int64_t Amount::rawUnits() const { return m_rawUnits; }

bool Amount::isNegative() const { return m_rawUnits < 0; }

bool Amount::isZero() const { return m_rawUnits == 0; }

bool Amount::isPositive() const { return m_rawUnits > 0; }
...
std::string Amount::toString() const {
  ...
  oss << whole << "." << std::setw(8) << std::setfill('0') << fraction
      << " NODO";
...
Amount Amount::operator-(const Amount &other) const {
  if (other.m_rawUnits > m_rawUnits) {
    throw std::underflow_error(
        "Amount subtraction would create negative value.");
  }
...
bool Amount::operator!=(const Amount &other) const { return !(*this == other); }
